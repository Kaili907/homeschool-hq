...
function buildLesson(course, unit, unitNumber, dayInUnit, courseDay) {
  ...
  const objectives =
    cycle === 1
      ...
      : [
          `Sustain ${focus} ${transfer}, holding the critical features when conditions stop being ideal.${transferEvidence ? ` ${transferEvidence}` : ''}`,
  ...
  const criteria =
    cycle === 1
      ...
      : [
          `The learner holds ${focus} ${transfer}, in whatever version their body and setting allow.${transferEvidence ? ` ${transferEvidence}` : ''}`,
  ...
  const lesson = {
    ...
    student_activity:
      cycle === 1
        ...
        : `Learner works ${focus} ${transfer} at a self-set intensity, then records what held, what degraded first, and how this occasion differed from the earlier one.${transferEvidence ? ` ${transferEvidence}` : ''}`,
    formative_check:
      cycle === 1
    ...
    answer_or_scoring_guidance:
      `Score safe participation, skill cues, tactical reasoning, self-management, and revision. Accept any version of the task the learner’s body and setting allow as full credit${transferEvidence ? ' when it supplies the stated transfer evidence' : ''}. Never score body size, weight, body composition, appearance, speed alone, or comparison with another person.${transferEvidence ? ` ${transferEvidence}` : ''}`,
    adaptive_tutor_routes: [
      ...
      {
        signal: 'learner declines a task',
        action: `Offer the adapted, seated, solo, or described version of ${focus}; the alternate route is full credit${transferEvidence ? ' when it supplies the stated transfer evidence' : ''} and is not logged as a refusal.${transferEvidence ? ` ${transferEvidence}` : ''}`,
      },
      ...(cycle === 2
        ? [{
            signal: 'transfer condition is too much today',
            action: `Drop back to the first-pass version of ${focus} for this session and re-attempt the added demand on a later occasion. Reducing the demand is the correct response and is scored as full participation${transferEvidence ? ', but it does not itself satisfy the transfer evidence requirement' : ''}.`,
          }]
        : []),
