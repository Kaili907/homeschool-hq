...
- Both hashes matched the supplied expected values.
...
- Reduced-motion styling and native keyboard controls remain in the unchanged
  standalone demo.
...
- Demo JavaScript syntax and static safety/accessibility inspection: PASS.
- Actual browser interaction: not completed because the controlled browser
  runtime reported no available browser backend. This is an environmental
  freeze-gate hold, not a proven source defect.
