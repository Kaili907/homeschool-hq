...
describe('hosted-applied migrations are never executable work', () => {
  ...
  // by any five executables, including a hosted-applied member demoted back out of the
  ...
  // depends on it, the actor-bound session verification overload above both, the
  // read-only academic readiness contract above that, and the learner runtime
  // operations contract above that. This list grows by one on each forward append and
  // must be extended deliberately; that it has to be edited is the point, not friction
  // to design away.
