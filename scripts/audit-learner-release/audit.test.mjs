...
import { inspectAssessment, inspectLesson, resolveProductionRef, syntheticMaterial } from './lib.mjs'
import { EXPECTED } from './rules.mjs'
...
test('full population gate inspects exactly 8,292 lessons and 699 assessments and passes the convergence release', () => {
  ...
  assert.equal(report.evidenceBaseSha, '3b89a20234d2e8a2ddfa11f9de27bd8d10a82fa4')
  ...
  assert.deepEqual([...new Set(report.matrix.map((cell) => cell.grade))], EXPECTED.grades)
  assert.deepEqual([...new Set(report.matrix.map((cell) => cell.subject))].sort(), EXPECTED.subjects)
  assert.equal(report.matrix.some((cell) => cell.grade === 6), false)
  assert.equal(report.matrix.every((cell) => cell.status === 'READY'), true)
})
...
test('production refs resolve within the repository on the host path separator', () => {
  const root = process.cwd()
  const ref = 'git+15ca63353e533db49374f5a2bc54e9b488750cca:curriculum-production/example.json'
  assert.equal(resolveProductionRef(root, ref), resolve(root, 'curriculum-production/example.json'))
})

test('production refs cannot escape the repository', () => {
  const ref = 'git+15ca63353e533db49374f5a2bc54e9b488750cca:../outside.json'
  assert.throws(() => resolveProductionRef(process.cwd(), ref), /escapes repository/)
})

test('Social Studies tasks remain actionable after a Windows CRLF checkout', () => {
  const result = inspect('social-studies', {
    binding: {
      sourceReadinessKind: 'STATIC_VERIFIED_SOURCE',
      sourceRuntimeState: 'READY',
