...
function sourceItems(subject, value, markdown) {
  ...
  if (subject === 'social-studies') {
    const body = String(markdown).match(/## 3\. Independent response\n\n([\s\S]*?)(?:\n\n## 4\.|$)/)?.[1] ?? ''
...
function elaRules(value, material, findings) {
  ...
  if (value.lessonRef?.grade && [5, 7, 8].includes(value.lessonRef.grade) &&
...
export function resolveProductionRef(root, ref) {
  ...
  if (!path.startsWith(`${resolve(root)}/`)) throw new Error(`Production ref escapes repository: ${ref}`)
  return path
}
