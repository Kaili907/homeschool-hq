...
test('markdown projection removes adult-only sections while preserving learner rubric and CRLF response structure', () => {
  ...
  assert.deepEqual(
    material.sections.find((section) => section.sectionKind === '3-independent-response').items,
    [{
      itemRef: 'social-1#independent-response',
...
test('all 8,292 production packages build into the audited lazy structured projection', async () => {
  ...
  assert.deepEqual(manifest.structuredProjection, {
    ...
    itemsProjected: 35975,
    choiceItemsProjected: 13777,
    choicesPreserved: 54089,
    sourceMetadataPreserved: 5412,
    dynamicSourceContractsPreserved: 12,
    taskGroupsProjected: 3039,
    taskStepsProjected: 7254,
  ...
  assert.equal(items, 35975)
  assert.equal(projectedItemRefs.size, 35975)
  assert.equal(choices, 54089)
