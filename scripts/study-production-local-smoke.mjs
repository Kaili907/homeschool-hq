...
export async function runStudyProductionLocalSmoke() {
  ...
  try {
    ...
    const localDateResult = await database.query(`
      select to_char(clock_timestamp() at time zone 'America/New_York', 'YYYY-MM-DD') as local_date
    `)
    const scenarioDate = localDateResult.rows[0]?.local_date
    invariant(/^\d{4}-\d{2}-\d{2}$/.test(scenarioDate), 'fixture-local-date-unavailable')
    const requestForBegin = (overrides = {}) => beginRequest({
      intendedLocalDate: scenarioDate,
      ...overrides,
    })
    ...
    await check(steps, 'enrollment', 'enrollment', 'active-date-effective-enrollment', async () => {
      ...
      `, [STUDENT_A, RELEASE_VERSION, scenarioDate])
    ...
    const startRequest = requestForBegin()
    ...
    await check(negativeCases, 'settings-unavailable', 'settings unavailable',
      'safety-constraints-unavailable', async () => {
        ...
        try {
          const result = responseJson(await academic('session:begin', requestForBegin({
    ...
    await check(negativeCases, 'forged-authority', 'forged authority',
      'caller-authority-rejected-before-rpc', async () => {
        ...
        const response = await academic('session:begin', {
          ...requestForBegin({ idempotencyKey: 'begin-smoke-forged-authority' }),
          studentId: STUDENT_A,
          role: 'guardian',
        })
