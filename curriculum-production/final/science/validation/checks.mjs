...
const HS_H4_COMMIT = 'a86780a315b5a6ba4f134f35b7033f35707b0e52'
/** H3 and H2 are superseded. A High School sheet built from either is stale. */
const HS_SUPERSEDED_H3_COMMIT = 'e7551b959112fcd9e6868d614799028cf22acf5d'
...
export const CHECKS = [
  ...
  {
    id: 'hs-safety-from-h4-only',
    description:
      'Every High School package is built at the required H4 SHA and carries its safety-review text verbatim; H3, H2, and the failed base candidate are rejected.',
    run({ packages, sheets, sources }) {
      const problems = []
      ...
      for (const pkg of hsPackages) {
        if (pkg.source.commit !== HS_H4_COMMIT) {
          fail(problems, pkg, `built from ${pkg.source.commit.slice(0, 7)}, not required H4`)
        }
        if (pkg.source.commit === HS_FAILED_BASE_COMMIT) {
          fail(problems, pkg, 'built from the failed base High School candidate')
        }
        if (pkg.source.commit === HS_SUPERSEDED_H3_COMMIT) {
          fail(problems, pkg, 'built from the superseded H3 source, not H4')
        }
        if (pkg.source.commit === HS_SUPERSEDED_H2_COMMIT) {
          fail(problems, pkg, 'built from the superseded H2 safety fix, not H4')
        }
        const source = sources.get(pkg.lesson_id)
        ...
        if (!segment) continue
        carried += 1
        const brief = pkg.safety_brief.h4_safety_segment_verbatim ?? ''
        if (brief !== segment.teacher_or_tutor_action) {
          fail(problems, pkg, 'H4 safety-review segment is not carried verbatim')
        }
        // The safe order and stop conditions H4 carries must reach the sheet.
      ...
      if (carried === 0) {
        problems.push('no High School lesson carried an H4 safety-review segment')
      }
      return problems
    },
  },
  {
    ...
    description:
      'None of the defects removed before H4 — flame demonstration, water on a fire, uncapped hot water, sealing a reacting mixture — reappears in any student sheet.',
    run({ packages, sheets }) {
      const problems = []
      const defects = [
  ...
  },
  {
    ...
    description:
      'Every non-disableable prohibition the pinned H4 policy set declares appears on every student sheet, in the wording for that grade band.',
  ...
  },
  {
    ...
    description:
      'Any lesson declaring a chemical hazard declares direct-adult supervision, as the H4 framework requires.',
    run({ packages }) {
      const problems = []
      for (const pkg of packages) {
  ...
  },
  {
    ...
    run({ floor }) {
      const problems = []
      const hsCommit = floor.attribution['hs-h4'].commit
      ...
      for (const item of [
        ...
        if (!declared.has(item.text)) {
          problems.push(`floor clause not in the H4 policy set: ${item.text.slice(0, 60)}`)
        }
      }
      ...
      for (const item of floor.global_stop_conditions) {
        if (!hsLessons.includes(JSON.stringify(item.text).slice(1, -1))) {
          problems.push(`stop condition not in the H4 lessons: ${item.text.slice(0, 60)}`)
        }
      }
      return problems
    },
  ...
  },
  {
    ...
    description:
      'Every unit phenomenon that reaches a learner surface — the printed sheet or the instruction the app renders — and names a hazard-bearing material carries the H4 student-visible SAFETY rule with it.',
    run({ packages, sheets }) {
      const problems = []
  ...
  },
  {
    ...
    description:
      'Any lesson whose pinned source declares a non-food-grade material carries the H4 never-return-to-food-use rule to the learner in its hazard, its safe order, and its disposal.',
    run({ packages, sheets }) {
      const problems = []
  ...
  },
  {
    ...
    description:
      'Protective equipment a mitigation tells the learner to wear — eye protection, gloves, a waterproof dressing, apron, or dust mask — is resolved onto that lesson’s materials list.',
    run({ packages }) {
      const problems = []
  ...
  },
  {
    ...
    description:
      'On every hazard-bearing lesson the adult record states the safe order, protective equipment, and disposal, resolved exactly as the learner reads them.',
  ...
  {
    id: 'h4-b1-b2-b3-closed-on-rendered-sheets',
    description:
      'The exact prior B1/B2/B3 findings are closed in H4-derived package data and in both rendered learner and adult sheets.',
    run({ packages, sheets, scoring }) {
      const problems = []
      const byId = new Map(packages.map((pkg) => [pkg.lesson_id, pkg]))
      const requireText = (lessonId, label, text, patterns) => {
        for (const pattern of patterns) {
          if (!pattern.test(text)) problems.push(`${lessonId}: ${label} misses ${pattern}`)
        }
      }
      ...
      for (const lessonId of [
        ...
        const pkg = byId.get(lessonId)
        if (!pkg) {
          problems.push(`${lessonId}: B1 closure lesson missing`)
          continue
        }
        requireText(lessonId, 'declared materials', (pkg.materials ?? []).join(' '), [
          /\bapron\b/i,
          /\btray\b/i,
          /\bdropper\b/i,
        ])
        requireText(lessonId, 'learner sheet', sheets.get(lessonId) ?? '', [
          /\bapron\b/i,
          /\btray\b/i,
          /\bdropper\b/i,
        ])
        requireText(lessonId, 'adult sheet', scoring.get(lessonId) ?? '', [
          /\bapron\b/i,
          /\btray\b/i,
          /\bdropper\b/i,
        ])
      ...
      for (const lessonId of [
        'ma-hs10-chemistry-u06-l07',
        'ma-hs10-chemistry-u06-l09',
      ]) {
        const pkg = byId.get(lessonId)
        if (!pkg) {
          problems.push(`${lessonId}: B2/B3 closure lesson missing`)
          continue
        }
        const alternative = pkg.equal_credit_safe_alternative?.text ?? ''
        requireText(lessonId, 'package alternative', alternative, [
          /no direction or size of change is supplied in advance/i,
        ])
        for (const [label, text] of [
          ['package alternative', alternative],
          ['learner sheet', sheets.get(lessonId) ?? ''],
          ['adult sheet', scoring.get(lessonId) ?? ''],
        ]) {
          if (/observe cooling|observe warming|mild warming/i.test(text)) {
            problems.push(`${lessonId}: ${label} still contains the fabricated B2 result`)
          }
        }
        for (const [label, text] of [
          ['learner sheet', sheets.get(lessonId) ?? ''],
          ...
          requireText(lessonId, label, text, [
            /no direction or size of change is supplied in advance/i,
            /calcium chloride goes ONLY in the DISPOSABLE double cup/i,
            /insulated drinking cup holds ONLY the Epsom-salt, baking-soda, and vinegar trials/i,
          ])
        }
      }
      return problems
