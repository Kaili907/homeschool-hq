# Unit assessment: computational prediction of orbits

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 4: Orbital Motion and the Solar System · Day 11 of the unit (course day 47) · about 55–75 minutes

**Today's focus:** computational prediction of orbits
...
- Demonstrate independent mastery evidence for computational prediction of orbits on a fresh task.
- Explain computational prediction of orbits using evidence, a model, or a documented process rather than an unsupported answer.
- Apply computational prediction of orbits to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on computational prediction of orbits against the stated success criteria and name the next step.
...
- Science brief: an orbital simulation advances position and velocity in steps using the gravitational force, and its accuracy depends on the step size and method.
- Science brief: simulation output is a prediction conditional on the initial conditions and assumptions supplied, and both are stated.
- Science brief: a simulation that does not conserve energy over long runs is exhibiting numerical error, not physics.

### Evidence check: computational prediction of orbits
...
| --- | --- | --- |
| E1 | established science reference | Science brief: an orbital simulation advances position and velocity in steps using the gravitational force, and its accuracy depends on the step size and method. |
| E2 | established science reference | Science brief: simulation output is a prediction conditional on the initial conditions and assumptions supplied, and both are stated. |
| E3 | established science reference | Science brief: a simulation that does not conserve energy over long runs is exhibiting numerical error, not physics. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs12-earth-space-environmental-u04::computational prediction of orbits, derived from the reviewed correctness source policy/correctness/ma-hs12-earth-space-environmental.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about computational prediction of orbits, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about computational prediction of orbits, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about computational prediction of orbits, then name one check that would catch an error or a weak claim.**
...
**Q1.** Without copying a sentence from the brief, evaluate the printed candidate case about computational prediction of orbits. Cite E1-E3, write the reasoning in your own words, and correct the case claim so it fits exactly what the record establishes.

>

**Q2.** Explain computational prediction of orbits in your own words using at least two specifically named rows from E1-E3. Then explain why the candidate claim does or does not follow.

>

**Q3.** Treat the printed candidate claim as the fresh case for computational prediction of orbits. Decide whether E1-E3 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q4.** Check the conclusion about computational prediction of orbits, revise its weakest claim with a named row from E1-E3, and name the next observation or model input that would resolve the remaining limitation.

>

**Q5.** Answer the printed unit question using computational prediction of orbits as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q6.** Which one row in E1-E3 is strongest for evaluating the candidate claim about computational prediction of orbits? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q7.** Using the supplied case and E1-E3, state one conclusion about computational prediction of orbits that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q8.** Audit the inputs used in your response about computational prediction of orbits. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about computational prediction of orbits.
...
- Apply computational prediction of orbits under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect computational prediction of orbits to gravitational orbits from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs12-earth-space-environmental-u04-l11` · lesson `ma-hs12-earth-space-environmental-u04-l11` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
