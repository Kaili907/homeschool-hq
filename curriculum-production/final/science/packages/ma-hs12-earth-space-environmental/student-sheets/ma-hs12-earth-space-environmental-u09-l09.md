# Performance task build: computational simulation of coupled systems

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 9: Sustainability Engineering and the Grade 12 Capstone · Day 9 of the unit (course day 105) · about 55–75 minutes

**Today's focus:** computational simulation of coupled systems
...
- Produce performance-task evidence for computational simulation of coupled systems that a reader could check independently.
- Explain computational simulation of coupled systems using evidence, a model, or a documented process rather than an unsupported answer.
- Apply computational simulation of coupled systems to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on computational simulation of coupled systems against the stated success criteria and name the next step.
...
**Protective equipment.** No protective equipment is required for this lesson's desk work.

### What could go wrong, and what stops it

- **Physical hazard.** Desk-based work only; no chemicals, heat, electricity, or tools are used in this lesson.
  - What stops it: Materials are notebook, writing tool, and printed or on-screen text. Any hands-on extension defers to the unit investigation's safety review.

### Order of work

This lesson is desk-based and prescribes no handling sequence.
...
**Clearing up.** Nothing in this lesson produces waste beyond ordinary paper. File or recycle it.
...
- Science brief: a coupled-system simulation represents the interactions between components, and the coupling is often what produces the interesting behaviour.
- Science brief: output depends on assumptions, parameters, and initial conditions, all of which are stated with the result.
- Science brief: a simulation is compared against observation where possible, and its limits are reported alongside its output.

### Evidence check: computational simulation of coupled systems

**Candidate claim:** A case report presents this as acceptable science: “presenting simulation output as observational data.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: a coupled-system simulation represents the interactions between components, and the coupling is often what produces the interesting behaviour. |
| E2 | established science reference | Science brief: output depends on assumptions, parameters, and initial conditions, all of which are stated with the result. |
| E3 | established science reference | Science brief: a simulation is compared against observation where possible, and its limits are reported alongside its output. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs12-earth-space-environmental-u09::computational simulation of coupled systems, derived from the reviewed correctness source policy/correctness/ma-hs12-earth-space-environmental.correctness.json.

**Supplied unit connections:** criteria, constraints, and prioritisation; cost-benefit analysis of energy and mineral resources; computational simulation of coupled systems; refining technological solutions; sustainability and human population; defending a design under uncertainty

**Supplied unit performance task:** Complete the Grade 12 capstone: analyse a global challenge, build a computational simulation of coupled Earth and human systems, evaluate competing solutions on prioritised criteria and trade-offs, refine your solution, and defend it with stated limitations.

### Bound task

**Question:** What does evidence E1 through E3 justify about computational simulation of coupled systems, and does it support the candidate claim?
...
**Model/source rule:** Population(next)=round(1.10×population); stock(next)=stock+renewal-population×use per person, using the current step population.
...
3. Verify each transition, extend three steps, test one policy change, and defend a design while stating assumptions, uncertainty, and the result that would change the recommendation.
...
**Pinned H4 alternative statement (lineage evidence):** Day 7 investigation: Computational simulation of a coupled resource and sustainability system | No-special-equipment alternative: Build the simulation as an iterated spreadsheet or as a hand-computed table over time steps on paper. No programming environment and no downloads required.
...
**My question.** Write the question you are testing about computational simulation of coupled systems. It has to be a question your evidence could actually answer.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about computational simulation of coupled systems, then name one check that would catch an error or a weak claim.**
...
**Q1.** Build a checkable contribution to the supplied unit performance task — Complete the Grade 12 capstone: analyse a global challenge, build a computational simulation of coupled Earth and human systems, evaluate competing solutions on prioritised criteria and trade-offs, refine your solution, and defend it with stated limitations. — using the case and E1-E3. Include claim, evidence, reasoning, limitation, and a recorded revision.

>

**Q2.** Explain computational simulation of coupled systems in your own words using at least two specifically named rows from E1-E3. Then explain why the candidate claim does or does not follow.

>

**Q3.** Treat the printed candidate claim as the fresh case for computational simulation of coupled systems. Decide whether E1-E3 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q4.** Check the conclusion about computational simulation of coupled systems, revise its weakest claim with a named row from E1-E3, and name the next observation or model input that would resolve the remaining limitation.

>

**Q5.** Answer the printed unit question using computational simulation of coupled systems as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q6.** Which one row in E1-E3 is strongest for evaluating the candidate claim about computational simulation of coupled systems? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q7.** Using the supplied case and E1-E3, state one conclusion about computational simulation of coupled systems that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q8.** Audit the inputs used in your response about computational simulation of coupled systems. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about computational simulation of coupled systems.
...
- Apply computational simulation of coupled systems under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect computational simulation of coupled systems to criteria, constraints, and prioritisation from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
- Take the unit's performance task — Complete the Grade 12 capstone: analyse a global challenge, build a computational simulation of coupled Earth and human systems, evaluate competing solutions on prioritised criteria and trade-offs, refine your solution, and defend it with stated limitations. — and write the one measurement or piece of evidence that would most change its outcome, then say why.
...
_Package `swp-ma-hs12-earth-space-environmental-u09-l09` · lesson `ma-hs12-earth-space-environmental-u09-l09` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
