# Reteach and varied practice: spatial and temporal scale

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 1: Earth and Space Systems, Scale, and Data Practice · Day 8 of the unit (course day 8) · about 55–75 minutes

**Today's focus:** spatial and temporal scale
...
- Apply spatial and temporal scale to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on spatial and temporal scale against the stated success criteria and name the next step.
...
- Science brief: earth and space processes span timescales from seconds to billions of years and spatial scales from grains to galaxies.
- Science brief: a process observable at one scale may be invisible at another, so the scale of observation determines what can be concluded.
- Science brief: slow processes accumulate large effects over long times, which is why geologic change is substantial despite being imperceptible year to year.
- Science brief: extrapolating a rate measured over a short interval to a much longer one requires evidence that the rate is constant.

### Evidence check: spatial and temporal scale

**Candidate claim:** A case report presents this as acceptable science: “extrapolating a short-term rate across geologic time with no evidence that the rate is constant.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: earth and space processes span timescales from seconds to billions of years and spatial scales from grains to galaxies. |
| E2 | established science reference | Science brief: a process observable at one scale may be invisible at another, so the scale of observation determines what can be concluded. |
| E3 | established science reference | Science brief: slow processes accumulate large effects over long times, which is why geologic change is substantial despite being imperceptible year to year. |
| E4 | established science reference | Science brief: extrapolating a rate measured over a short interval to a much longer one requires evidence that the rate is constant. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs12-earth-space-environmental-u01::spatial and temporal scale, derived from the reviewed correctness source policy/correctness/ma-hs12-earth-space-environmental.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about spatial and temporal scale, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about spatial and temporal scale, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about spatial and temporal scale, then name one check that would catch an error or a weak claim.**
...
**Q1.** Explain spatial and temporal scale in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q2.** Treat the printed candidate claim as the fresh case for spatial and temporal scale. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q3.** Check the conclusion about spatial and temporal scale, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q4.** Answer the printed unit question using spatial and temporal scale as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E4 is strongest for evaluating the candidate claim about spatial and temporal scale? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E4, state one conclusion about spatial and temporal scale that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about spatial and temporal scale. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about spatial and temporal scale.
...
- Apply spatial and temporal scale under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect spatial and temporal scale to systems and system models from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs12-earth-space-environmental-u01-l08` · lesson `ma-hs12-earth-space-environmental-u01-l08` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
