# Guided practice A: computational simulation of coupled systems

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 9: Sustainability Engineering and the Grade 12 Capstone · Day 3 of the unit (course day 99) · about 55–75 minutes
...
**The task.** What does evidence E1 through E3 justify about computational simulation of coupled systems, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**Q1.** Explain computational simulation of coupled systems in your own words using at least two specifically named rows from E1-E3. Then explain why the candidate claim does or does not follow.

>

**Q2.** Treat the printed candidate claim as the fresh case for computational simulation of coupled systems. Decide whether E1-E3 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q3.** Check the conclusion about computational simulation of coupled systems, revise its weakest claim with a named row from E1-E3, and name the next observation or model input that would resolve the remaining limitation.

>

**Q4.** Answer the printed unit question using computational simulation of coupled systems as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about computational simulation of coupled systems? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about computational simulation of coupled systems that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about computational simulation of coupled systems. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
_Package `swp-ma-hs12-earth-space-environmental-u09-l03` · lesson `ma-hs12-earth-space-environmental-u09-l03` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
