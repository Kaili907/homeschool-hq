# Scoring authority — Unit assessment: computational prediction of orbits

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 4: Orbital Motion and the Solar System · Day 11 · lesson `ma-hs12-earth-space-environmental-u04-l11`
...
_Topic `ma-hs12-earth-space-environmental-u04::computational prediction of orbits` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- An orbital simulation advances position and velocity in steps using the gravitational force, and its accuracy depends on the step size and method.
- Simulation output is a prediction conditional on the initial conditions and assumptions supplied, and both are stated.
- A simulation that does not conserve energy over long runs is exhibiting numerical error, not physics.
...
- Presenting simulation output as observational data.
- Attributing a simulated orbit's drift to a physical effect without checking the numerical method.
- Reporting simulation results without stating initial conditions or assumptions.

**Grade boundary.** Numerical integrator theory and error analysis are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for computational prediction of orbits on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain computational prediction of orbits using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply computational prediction of orbits to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on computational prediction of orbits against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply computational prediction of orbits to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain computational prediction of orbits using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply computational prediction of orbits to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for computational prediction of orbits on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for computational prediction of orbits (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for computational prediction of orbits (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for computational prediction of orbits (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for computational prediction of orbits (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for computational prediction of orbits (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u04-l11` · built from `hs912-science-h4` at `a86780a`._
