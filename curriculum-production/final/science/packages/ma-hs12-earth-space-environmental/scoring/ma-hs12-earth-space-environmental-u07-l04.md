# Scoring authority — Independent application A: climate versus weather

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 7: Climate, Energy Flow, and the Carbon Cycle · Day 4 · lesson `ma-hs12-earth-space-environmental-u07-l04`
...
_Topic `ma-hs12-earth-space-environmental-u07::climate versus weather` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Weather is the atmospheric state over hours to days; climate is the statistical description of weather over decades.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Climate is described by averages, ranges, and the frequency of extremes, over periods conventionally of thirty years or more.
- An individual weather event neither confirms nor refutes a claim about climate, though a change in the frequency of such events is climate evidence.
- Climate predictability and weather predictability are different problems: the long-term statistics can be projected even where individual days cannot.
...
- Using an individual cold or hot event as evidence for or against long-term climate change.
- Claiming that because weather cannot be forecast beyond about two weeks, climate cannot be projected at all.
- Treating climate and weather as the same quantity measured over different periods with no other distinction.

**Grade boundary.** Attribution science methodology and extreme-value statistics are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain climate versus weather using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply climate versus weather to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on climate versus weather against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply climate versus weather to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain climate versus weather using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply climate versus weather to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain climate versus weather using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for climate versus weather (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for climate versus weather (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for climate versus weather (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for climate versus weather (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for climate versus weather (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u07-l04` · built from `hs912-science-h4` at `a86780a`._
