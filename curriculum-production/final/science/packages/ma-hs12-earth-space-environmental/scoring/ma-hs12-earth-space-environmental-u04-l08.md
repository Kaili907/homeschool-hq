# Scoring authority — Reteach and varied practice: Kepler's relationships

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 4: Orbital Motion and the Solar System · Day 8 · lesson `ma-hs12-earth-space-environmental-u04-l08`
...
_Topic `ma-hs12-earth-space-environmental-u04::Kepler's relationships` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._
...
- A planet moves faster when nearer the Sun and slower when farther away, sweeping equal areas in equal times.
- The period-radius relationship holds for any set of bodies orbiting the same central mass, which is why it applies to moons and satellites too.
- Kepler's relationships describe the motion; Newton's law of gravitation explains why it takes that form.

**Also correct.** Accept any of these framings — do not require the wording above:

- Naming the laws by number, or describing them, are equally correct.
...
- Claiming the Sun sits at the centre of a planet's elliptical orbit rather than at a focus.
- Claiming orbital period is directly proportional to orbital radius.
- Claiming a planet moves at constant speed throughout an elliptical orbit.

**Grade boundary.** Derivation of Kepler's laws from Newtonian gravitation is acceptable but not required.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain Kepler's relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply Kepler's relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on Kepler's relationships against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply Kepler's relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain Kepler's relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply Kepler's relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain Kepler's relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for Kepler's relationships (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for Kepler's relationships (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for Kepler's relationships (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for Kepler's relationships (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for Kepler's relationships (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u04-l08` · built from `hs912-science-h4` at `a86780a`._
