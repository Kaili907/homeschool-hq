# Scoring authority — Performance task build: convection in Earth's interior

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 5: Plate Tectonics and the History of Earth · Day 9 · lesson `ma-hs12-earth-space-environmental-u05-l09`
...
_Topic `ma-hs12-earth-space-environmental-u05::convection in Earth's interior` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Earth's mantle is solid rock that deforms and flows very slowly; it is not a liquid layer.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Heat from Earth's interior, from residual formation heat and radioactive decay, drives slow convective flow in the mantle.
- Mantle flow, together with the pull of dense subducting slabs and the push at ridges, drives plate motion.
- The mantle behaves as a solid on short timescales and flows on geological ones, which is why seismic shear waves pass through it.

**Also correct.** Accept any of these framings — do not require the wording above:

- Attributing plate motion to mantle convection, or to a combination including slab pull and ridge push, are both acceptable at this level.
...
- Claiming the plates float on a layer of liquid magma.
- Claiming Earth's internal heat comes principally from the Sun.
- Claiming the entire outer core and mantle are molten; only the outer core is liquid.

**Grade boundary.** Mantle rheology and geodynamic modelling are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for convection in Earth's interior that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain convection in Earth's interior using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply convection in Earth's interior to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on convection in Earth's interior against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply convection in Earth's interior to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain convection in Earth's interior using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply convection in Earth's interior to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for convection in Earth's interior that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for convection in Earth's interior (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for convection in Earth's interior (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for convection in Earth's interior (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for convection in Earth's interior (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for convection in Earth's interior (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Warm water used for the convection model can scald, and glass can crack from thermal shock. — mitigation: Keep water below 50 degrees Celsius, use a plastic or heatproof container, and let an adult pour.
- physical: Food colouring stains surfaces and clothing. — mitigation: Work over a tray with old clothing or an apron.
...
_Package `swp-ma-hs12-earth-space-environmental-u05-l09` · built from `hs912-science-h4` at `a86780a`._
