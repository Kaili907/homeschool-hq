# Scoring authority — Unit assessment: climate models and their uncertainty

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 7: Climate, Energy Flow, and the Carbon Cycle · Day 11 · lesson `ma-hs12-earth-space-environmental-u07-l11`
...
_Topic `ma-hs12-earth-space-environmental-u07::climate models and their uncertainty` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Climate models are built from physical laws and are tested against past and present observations before being used for projection.
- Projections are given as ranges under stated scenarios, because future emissions are a choice and the physics carries its own uncertainty.
- Model uncertainty is quantified; it is not a statement that the projections carry no information.

**Also correct.** Accept any of these framings — do not require the wording above:

- Describing projections by scenario, by range, or by ensemble spread are all acceptable.
...
- Treating a projection range as evidence that models are uninformative.
- Presenting a single model run as a prediction rather than as one member of a range.
- Claiming a scenario-based projection failed because emissions followed a different scenario.

**Grade boundary.** Model intercomparison methodology and parameterisation schemes are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for climate models and their uncertainty on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain climate models and their uncertainty using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply climate models and their uncertainty to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on climate models and their uncertainty against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply climate models and their uncertainty to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain climate models and their uncertainty using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply climate models and their uncertainty to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for climate models and their uncertainty on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for climate models and their uncertainty (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for climate models and their uncertainty (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for climate models and their uncertainty (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for climate models and their uncertainty (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for climate models and their uncertainty (evidence type: application, require explanation: True, review timing: same-day).
...
**Mastery rule:** Do not mark mastery from one answer. This course requires independent evidence on at least 2 occasions across at least 2 distinct dates, using explanation, application, error-analysis, with transfer to a novel context.
...
_Package `swp-ma-hs12-earth-space-environmental-u07-l11` · built from `hs912-science-h4` at `a86780a`._
