# Scoring authority — Investigation: nuclear fusion in stellar cores

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 2: The Sun, Stars, and the Origin of the Elements · Day 7 · lesson `ma-hs12-earth-space-environmental-u02-l07`
...
_Topic `ma-hs12-earth-space-environmental-u02::nuclear fusion in stellar cores` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Stars release energy by fusing lighter nuclei into heavier ones, principally hydrogen into helium during the main sequence.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Fusion requires the extreme temperature and pressure found in stellar cores, because positively charged nuclei repel one another.
- The energy released corresponds to the small mass difference between the reactants and the products.
- The outward pressure from fusion balances the inward pull of gravity, which is what keeps a main-sequence star stable.

**Also correct.** Accept any of these framings — do not require the wording above:

- Naming the proton-proton chain or the CNO cycle is correct but not required.
...
- Claiming stars are powered by chemical combustion.
- Claiming stellar energy comes from fission of heavy nuclei.
- Claiming fusion occurs throughout a star rather than in its core region.

**Grade boundary.** Reaction rate calculations and stellar structure equations are beyond this course.
...
> Published solar constant, solar mass, and stellar spectra, plus the learner's own warming measurements if the outdoor route is used. Fusion rates are computed by the learner.
...
**Q1 — investigate.** _Serves objective 1:_ Plan and carry out a safe investigation of nuclear fusion in stellar cores, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain nuclear fusion in stellar cores using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply nuclear fusion in stellar cores to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on nuclear fusion in stellar cores against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply nuclear fusion in stellar cores to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain nuclear fusion in stellar cores using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 1:_ Plan and carry out a safe investigation of nuclear fusion in stellar cores, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q8 — provenance.** _Serves objective 1:_ Plan and carry out a safe investigation of nuclear fusion in stellar cores, or its stated alternative, and record the learner's own data with its uncertainty.
...
- **prerequisite-gap** — Run the prerequisite reteach route for nuclear fusion in stellar cores (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for nuclear fusion in stellar cores (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for nuclear fusion in stellar cores (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for nuclear fusion in stellar cores (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for nuclear fusion in stellar cores (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Looking at the sun causes permanent eye damage. — mitigation: NEVER look at the sun directly, through any lens, grating, CD, camera, or filter. All solar work is done by measuring warming water in shade-versus-sun, with the learner's back to the sun.
- physical: Working outdoors in sun risks heat and sunburn, and a sealed dark container left in full sun pressurises. — mitigation: Leave the water container OPEN - never capped or sealed - in the sun. Limit exposure to 30 minutes at a time, wear a hat and sunscreen, keep drinking water available, and work in the cooler part of the day.
...
**Disposal and clearing up:** Pour the water down the drain and dry the container, and return the CD or grating to its sleeve. Nothing hazardous is produced, and no image of the sun is captured by any means.
...
_Package `swp-ma-hs12-earth-space-environmental-u02-l07` · built from `hs912-science-h4` at `a86780a`._
