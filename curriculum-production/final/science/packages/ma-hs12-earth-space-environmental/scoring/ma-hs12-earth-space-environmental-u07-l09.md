# Scoring authority — Performance task build: the global carbon cycle

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 7: Climate, Energy Flow, and the Carbon Cycle · Day 9 · lesson `ma-hs12-earth-space-environmental-u07-l09`
...
_Topic `ma-hs12-earth-space-environmental-u07::the global carbon cycle` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Burning fossil fuels transfers carbon from a long-term geological reservoir into the atmosphere far faster than natural processes return it.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Carbon moves between atmosphere, ocean, land biosphere, soils, and rock, on timescales ranging from years to millions of years.
- The ocean has absorbed a large fraction of the carbon dioxide added by human activity, which has also made surface waters more acidic.
- A carbon budget accounts for all sources and sinks; the atmospheric increase is the difference between what is added and what is taken up.
...
- Claiming carbon added to the atmosphere is removed on a timescale of a few years.
- Claiming the fast biological cycle and the slow geological cycle operate at comparable rates.
- Claiming ocean uptake of carbon dioxide has no chemical consequences for seawater.

**Grade boundary.** Quantitative reservoir and flux figures, and carbonate chemistry calculations, are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for the global carbon cycle that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain the global carbon cycle using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply the global carbon cycle to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on the global carbon cycle against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply the global carbon cycle to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain the global carbon cycle using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply the global carbon cycle to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for the global carbon cycle that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for the global carbon cycle (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for the global carbon cycle (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for the global carbon cycle (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for the global carbon cycle (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for the global carbon cycle (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u07-l09` · built from `hs912-science-h4` at `a86780a`._
