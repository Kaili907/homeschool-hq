...
_Topic `ma-hs12-earth-space-environmental-u09::refining technological solutions` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Refinement proceeds by testing against criteria, identifying a specific shortfall, and making a change aimed at it.
- Changing one thing at a time keeps the effect of each change identifiable.
- Unintended consequences are looked for deliberately, because a solution that shifts a problem elsewhere has not solved it.
...
- Changing several features at once and attributing the improvement to one.
- Declaring a refinement successful without retesting against the criteria.
- Reporting a solution as effective while omitting an effect it displaced elsewhere.

**Grade boundary.** Formal design-of-experiments methodology is beyond this course.
...
_Package `swp-ma-hs12-earth-space-environmental-u09-l04` · built from `hs912-science-h4` at `a86780a`._
