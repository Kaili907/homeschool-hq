# Scoring authority — Unit assessment: spectra as compositional evidence

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 2: The Sun, Stars, and the Origin of the Elements · Day 11 · lesson `ma-hs12-earth-space-environmental-u02-l11`
...
_Topic `ma-hs12-earth-space-environmental-u02::spectra as compositional evidence` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Each element produces a characteristic pattern of spectral lines, which is how the composition of distant objects is determined.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Emission lines appear where atoms release specific energies; absorption lines appear where atoms remove those same energies from light passing through.
- Because the line pattern is characteristic of the element, a spectrum identifies which elements are present without any sample being collected.
- Line positions also carry information about motion, through Doppler shift, and about conditions such as temperature and pressure.
...
- Claiming spectral line patterns are the same for different elements.
- Claiming composition of distant stars is determined by collecting material from them.
- Confusing an absorption spectrum with an emission spectrum in interpreting which material lies where.

**Grade boundary.** Quantitative abundance determination and line-profile analysis are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for spectra as compositional evidence on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain spectra as compositional evidence using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply spectra as compositional evidence to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on spectra as compositional evidence against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply spectra as compositional evidence to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain spectra as compositional evidence using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply spectra as compositional evidence to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for spectra as compositional evidence on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for spectra as compositional evidence (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for spectra as compositional evidence (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for spectra as compositional evidence (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for spectra as compositional evidence (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for spectra as compositional evidence (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u02-l11` · built from `hs912-science-h4` at `a86780a`._
