# Scoring authority — Performance task build: stellar life cycles

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 2: The Sun, Stars, and the Origin of the Elements · Day 9 · lesson `ma-hs12-earth-space-environmental-u02-l09`
...
_Topic `ma-hs12-earth-space-environmental-u02::stellar life cycles` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- A star's mass determines its life cycle and its end state: white dwarf for low and intermediate masses, neutron star or black hole for high masses.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Stars form from collapsing clouds of gas and dust, spend most of their lives fusing hydrogen on the main sequence, and then evolve as their core fuel is exhausted.
- Low and intermediate mass stars expand to red giants, shed their outer layers, and leave a white dwarf.
- High-mass stars end in supernova explosions, leaving a neutron star or a black hole.

**Also correct.** Accept any of these framings — do not require the wording above:

- Hertzsprung-Russell diagram reasoning is correct but not required.
...
- Claiming all stars end their lives as supernovae.
- Claiming a star's life cycle depends principally on its brightness rather than on its mass.
- Claiming stars burn fuel chemically.

**Grade boundary.** Detailed stellar evolution tracks and mass-loss modelling are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for stellar life cycles that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain stellar life cycles using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply stellar life cycles to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on stellar life cycles against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply stellar life cycles to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain stellar life cycles using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply stellar life cycles to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for stellar life cycles that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for stellar life cycles (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for stellar life cycles (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for stellar life cycles (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for stellar life cycles (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for stellar life cycles (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u02-l09` · built from `hs912-science-h4` at `a86780a`._
