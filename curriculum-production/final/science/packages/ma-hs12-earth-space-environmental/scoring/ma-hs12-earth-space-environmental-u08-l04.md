# Scoring authority — Independent application A: natural hazards and their causes

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 8: Coevolution of Earth and Life, Resources, and Natural Hazards · Day 4 · lesson `ma-hs12-earth-space-environmental-u08-l04`
...
_Topic `ma-hs12-earth-space-environmental-u08::natural hazards and their causes` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Earthquakes and volcanoes occur predominantly at plate boundaries, and their global distribution follows those boundaries.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Each hazard has a physical cause, and understanding the cause is what makes the hazard mappable and, in some cases, forecastable.
- Hazard forecasting is generally probabilistic: the likelihood over a period can be estimated where the timing of a specific event cannot.
- A hazard becomes a disaster through exposure and vulnerability, so risk depends on people and infrastructure as well as on the physical event.
...
- Claiming individual earthquakes can be predicted for a specific day and place with current science.
- Claiming hazards occur at random with no relation to physical processes.
- Treating hazard and risk as the same quantity.

**Grade boundary.** Probabilistic seismic hazard analysis and eruption forecasting methods are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain natural hazards and their causes using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply natural hazards and their causes to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on natural hazards and their causes against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply natural hazards and their causes to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain natural hazards and their causes using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply natural hazards and their causes to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain natural hazards and their causes using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for natural hazards and their causes (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for natural hazards and their causes (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for natural hazards and their causes (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for natural hazards and their causes (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for natural hazards and their causes (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u08-l04` · built from `hs912-science-h4` at `a86780a`._
