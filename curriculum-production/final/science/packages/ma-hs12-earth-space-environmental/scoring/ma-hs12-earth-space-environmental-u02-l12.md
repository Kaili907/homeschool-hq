# Scoring authority — Correction and reflection: communicating stellar science

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 2: The Sun, Stars, and the Origin of the Elements · Day 12 · lesson `ma-hs12-earth-space-environmental-u02-l12`
...
_Topic `ma-hs12-earth-space-environmental-u02::communicating stellar science` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Communicating astronomical results means distinguishing what was measured, what was inferred from a model, and how confident the conclusion is.
- Astronomical images are frequently processed and colour-mapped, and a communication says so rather than presenting them as direct views.
- Scale statements need explicit units and comparisons, because astronomical magnitudes are outside everyday experience.
...
- Presenting a false-colour or composite astronomical image as a direct visual observation.
- Reporting an inferred property, such as a star's mass, as though it had been measured directly.
- Stating an astronomical distance or age with no units or uncertainty.

**Grade boundary.** Science communication theory and image processing methods are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain communicating stellar science using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply communicating stellar science to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on communicating stellar science against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply communicating stellar science to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain communicating stellar science using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply communicating stellar science to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain communicating stellar science using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for communicating stellar science (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for communicating stellar science (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for communicating stellar science (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for communicating stellar science (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for communicating stellar science (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u02-l12` · built from `hs912-science-h4` at `a86780a`._
