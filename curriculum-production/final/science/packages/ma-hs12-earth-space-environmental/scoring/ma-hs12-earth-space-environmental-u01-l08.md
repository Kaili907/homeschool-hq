# Scoring authority — Reteach and varied practice: spatial and temporal scale

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 1: Earth and Space Systems, Scale, and Data Practice · Day 8 · lesson `ma-hs12-earth-space-environmental-u01-l08`
...
_Topic `ma-hs12-earth-space-environmental-u01::spatial and temporal scale` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Earth and space processes span timescales from seconds to billions of years and spatial scales from grains to galaxies.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A process observable at one scale may be invisible at another, so the scale of observation determines what can be concluded.
- Slow processes accumulate large effects over long times, which is why geologic change is substantial despite being imperceptible year to year.
- Extrapolating a rate measured over a short interval to a much longer one requires evidence that the rate is constant.
...
- Extrapolating a short-term rate across geologic time with no evidence that the rate is constant.
- Concluding a process does not occur because it is not observable within a human lifetime.
- Comparing measurements made at different spatial scales as though they were equivalent.

**Grade boundary.** Formal scaling theory and fractal analysis are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain spatial and temporal scale using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply spatial and temporal scale to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on spatial and temporal scale against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply spatial and temporal scale to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain spatial and temporal scale using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply spatial and temporal scale to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain spatial and temporal scale using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for spatial and temporal scale (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for spatial and temporal scale (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for spatial and temporal scale (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for spatial and temporal scale (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for spatial and temporal scale (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u01-l08` · built from `hs912-science-h4` at `a86780a`._
