...
_Topic `ma-hs12-earth-space-environmental-u09::computational simulation of coupled systems` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- A coupled-system simulation represents the interactions between components, and the coupling is often what produces the interesting behaviour.
- Output depends on assumptions, parameters, and initial conditions, all of which are stated with the result.
- A simulation is compared against observation where possible, and its limits are reported alongside its output.
...
- Reporting a simulation result without its assumptions or parameter choices.
- Claiming a simulation is validated with no comparison against observation.

**Grade boundary.** Numerical methods and formal model validation frameworks are beyond this course.
...
_Package `swp-ma-hs12-earth-space-environmental-u09-l09` · built from `hs912-science-h4` at `a86780a`._
