# Scoring authority — Launch and diagnostic: gravitational orbits

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 4: Orbital Motion and the Solar System · Day 1 · lesson `ma-hs12-earth-space-environmental-u04-l01`
...
_Topic `ma-hs12-earth-space-environmental-u04::gravitational orbits` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Gravity provides the force that keeps a body in orbit, and an orbiting body is continuously accelerating toward the body it orbits.

**Accepted relationships and models.** What this lesson's learning target asserts:

- An orbit is the result of a body's sideways motion combined with continuous gravitational acceleration toward the central body.
- An orbiting object has weight; it appears weightless because it and everything with it are in free fall together.
- The two bodies attract each other equally and both orbit their common centre of mass, which lies inside the more massive body when the mass difference is large.
...
- Claiming there is no gravity where satellites orbit.
- Claiming an outward force balances gravity to hold an object in orbit.
- Claiming an orbiting object is not accelerating because its speed is nearly constant.

**Grade boundary.** Orbital perturbation theory and n-body dynamics are beyond this course.
...
> Published planetary and satellite orbital periods and radii. The learner derives and tests the period-radius relationship from that data.


## What a complete response contains, question by question

**Q1 — phenomenon question.** _Serves objective 1:_ Observe the anchoring phenomenon, surface initial thinking about gravitational orbits without penalty, and pose a testable question.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain gravitational orbits using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply gravitational orbits to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on gravitational orbits against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply gravitational orbits to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain gravitational orbits using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply gravitational orbits to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Observe the anchoring phenomenon, surface initial thinking about gravitational orbits without penalty, and pose a testable question.
...
- **prerequisite-gap** — Run the prerequisite reteach route for gravitational orbits (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for gravitational orbits (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for gravitational orbits (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for gravitational orbits (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for gravitational orbits (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u04-l01` · built from `hs912-science-h4` at `a86780a`._
