# Concept model A: how we see

**Grade 4 Science** · Unit 6: Light, Vision, and the Senses · Day 2 of the unit (course day 62) · about 40–55 minutes

**Today's focus:** how we see
...
- Build a clear mental model of how we see through an explicit worked example, demonstration, text, or phenomenon.
- Use subject-appropriate evidence or representation to communicate understanding of how we see.
- Check, revise, or improve work about how we see using stated success criteria.
...
- Science brief: seeing requires light travelling from a source, reflecting off the object, and entering the eye.
- Science brief: light from a source reflects off an object, enters the eye, and is detected there.
- Science brief: the eye detects the light; the brain interprets the signal as an image.
- Science brief: without light entering the eye, nothing is seen, however close or large the object is.

### Evidence check: how we see

**Candidate claim:** A case report presents this as acceptable science: “vision works by the eye reaching out to the object.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: seeing requires light travelling from a source, reflecting off the object, and entering the eye. |
| E2 | established science reference | Science brief: light from a source reflects off an object, enters the eye, and is detected there. |
| E3 | established science reference | Science brief: the eye detects the light; the brain interprets the signal as an image. |
| E4 | established science reference | Science brief: without light entering the eye, nothing is seen, however close or large the object is. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g4-science-u06::how we see, derived from the reviewed correctness source policy/correctness/ma-g4-science.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about how we see, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about how we see, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about how we see; then identify one check that would catch an error or weak claim.**
...
**Q1.** Build a labelled model of how we see that represents every supplied row in E1-E4. Annotate each model feature with its evidence ID, then show where the candidate claim fails or remains uncertain.

>

**Q2.** Communicate what the supplied record establishes about how we see in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E4 and mark where the representation stops being supported.

>

**Q3.** Check your response about how we see against each success criterion. Revise one sentence by adding or changing a citation to E1-E4; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using how we see as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E4 is strongest for evaluating the candidate claim about how we see? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E4, state one conclusion about how we see that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about how we see. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about how we see.
...
- Apply how we see under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect how we see to light reflection from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g4-science-u06-l02` · lesson `ma-g4-science-u06-l02` · built from `g34-committed` at `4c6ca4e`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
