# Concept model A: design criteria

**Grade 4 Science** · Unit 4: Designing an Energy Converter · Day 2 of the unit (course day 38) · about 40–55 minutes

**Today's focus:** design criteria
...
- Build a clear mental model of design criteria through an explicit worked example, demonstration, text, or phenomenon.
- Use subject-appropriate evidence or representation to communicate understanding of design criteria.
- Check, revise, or improve work about design criteria using stated success criteria.
...
- Science brief: design criteria state what the converter has to achieve, in terms that can actually be checked.
- Science brief: a criterion that cannot be tested cannot be used to judge the design.
- Science brief: safety limits are constraints, and they are not traded away for better performance.

### Evidence check: design criteria

**Candidate claim:** A case report presents this as acceptable science: “writing a criterion so vague that no test could settle whether it was met.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: design criteria state what the converter has to achieve, in terms that can actually be checked. |
| E2 | established science reference | Science brief: a criterion that cannot be tested cannot be used to judge the design. |
| E3 | established science reference | Science brief: safety limits are constraints, and they are not traded away for better performance. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g4-science-u04::design criteria, derived from the reviewed correctness source policy/correctness/ma-g4-science.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about design criteria, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about design criteria, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about design criteria; then identify one check that would catch an error or weak claim.**
...
**Q1.** Build a labelled model of design criteria that represents every supplied row in E1-E3. Annotate each model feature with its evidence ID, then show where the candidate claim fails or remains uncertain.

>

**Q2.** Communicate what the supplied record establishes about design criteria in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E3 and mark where the representation stops being supported.

>

**Q3.** Check your response about design criteria against each success criterion. Revise one sentence by adding or changing a citation to E1-E3; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using design criteria as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about design criteria? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about design criteria that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about design criteria. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about design criteria.
...
- Apply design criteria under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect design criteria to energy conversion from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g4-science-u04-l02` · lesson `ma-g4-science-u04-l02` · built from `g34-committed` at `4c6ca4e`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
