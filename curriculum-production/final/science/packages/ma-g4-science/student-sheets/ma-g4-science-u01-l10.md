# Synthesis and review: fair tests

**Grade 4 Science** · Unit 1: Science and Engineering Practices · Day 10 of the unit (course day 10) · about 40–55 minutes

**Today's focus:** fair tests
...
- Connect fair tests to the unit’s other ideas and select evidence that shows readiness for assessment.
- Use subject-appropriate evidence or representation to communicate understanding of fair tests.
- Check, revise, or improve work about fair tests using stated success criteria.
...
- Science brief: in a fair test only one thing is changed on purpose; everything else that could affect the result is held the same.
- Science brief: in a fair test one thing is changed on purpose and everything else that could affect the result is held the same.
- Science brief: if two things change at once, the result cannot be attributed to either one.
- Science brief: repeating a test and getting a similar result gives more confidence than a single trial does.

### Evidence check: fair tests

**Candidate claim:** A case report presents this as acceptable science: “calling a test fair when more than one condition was changed.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: in a fair test only one thing is changed on purpose; everything else that could affect the result is held the same. |
| E2 | established science reference | Science brief: in a fair test one thing is changed on purpose and everything else that could affect the result is held the same. |
| E3 | established science reference | Science brief: if two things change at once, the result cannot be attributed to either one. |
| E4 | established science reference | Science brief: repeating a test and getting a similar result gives more confidence than a single trial does. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g4-science-u01::fair tests, derived from the reviewed correctness source policy/correctness/ma-g4-science.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about fair tests, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about fair tests, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about fair tests; then identify one check that would catch an error or weak claim.**
...
**Q1.** The supplied unit connections are: careful observations, testable questions, models and diagrams, fair tests, measuring and recording data, criteria and constraints. Choose one idea other than fair tests, connect it to one row in E1-E4, and explain one way treating the two ideas as identical would be misleading.

>

**Q2.** Communicate what the supplied record establishes about fair tests in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E4 and mark where the representation stops being supported.

>

**Q3.** Check your response about fair tests against each success criterion. Revise one sentence by adding or changing a citation to E1-E4; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using fair tests as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E4 is strongest for evaluating the candidate claim about fair tests? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E4, state one conclusion about fair tests that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about fair tests. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about fair tests.
...
- Apply fair tests under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect fair tests to careful observations from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g4-science-u01-l10` · lesson `ma-g4-science-u01-l10` · built from `g34-committed` at `4c6ca4e`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
