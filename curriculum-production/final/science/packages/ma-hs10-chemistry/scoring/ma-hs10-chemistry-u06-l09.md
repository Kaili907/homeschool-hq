# Scoring authority — Performance task build: total bond energy changes

**High School Chemistry (Grade 10)** · Unit 6: Energy in Chemical Systems · Day 9 · lesson `ma-hs10-chemistry-u06-l09`
...
_Topic `ma-hs10-chemistry-u06::total bond energy changes` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- The net energy change is the total energy needed to break all bonds in the reactants minus the total released in forming all bonds in the products.
- If more energy is released in forming bonds than was required to break them, the reaction is exothermic; otherwise it is endothermic.
- Bond energy values are averages across compounds, so calculations from them give estimates rather than exact results.
...
- Computing an energy change from bond breaking alone, ignoring bond formation.
- Reversing the sign convention so that bond breaking is treated as releasing energy.
- Presenting an estimate from average bond energies as an exact measured value.

**Grade boundary.** Hess's law cycles and calorimetric enthalpy determination are acceptable but not required.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for total bond energy changes that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain total bond energy changes using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply total bond energy changes to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on total bond energy changes against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply total bond energy changes to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain total bond energy changes using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply total bond energy changes to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for total bond energy changes that a reader could check independently.
...
- The learner completes the central task about total bond energy changes.
...
- **prerequisite-gap** — Run the prerequisite reteach route for total bond energy changes (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for total bond energy changes (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for total bond energy changes (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for total bond energy changes (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for total bond energy changes (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u06-l09` · built from `hs912-science-h4` at `a86780a`._
