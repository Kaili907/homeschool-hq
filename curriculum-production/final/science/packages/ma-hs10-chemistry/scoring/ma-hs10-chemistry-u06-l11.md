# Scoring authority — Unit assessment: computational energy models

**High School Chemistry (Grade 10)** · Unit 6: Energy in Chemical Systems · Day 11 · lesson `ma-hs10-chemistry-u06-l11`
...
_Topic `ma-hs10-chemistry-u06::computational energy models` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- A computational energy model represents the system's energy inputs, outputs, and stores, and it must balance.
- The model's output depends on the assumptions and parameters entered, and both are stated with the result.
- A model result is a prediction, and it is compared against measurement rather than substituted for it.

**Also correct.** Accept any of these framings — do not require the wording above:

- Spreadsheet, diagram, and equation-based models are all acceptable forms here.
...
- Presenting model output as though it were experimental data.
- Building an energy model whose inputs and outputs do not balance.
- Reporting a model result without stating its assumptions.

**Grade boundary.** Numerical methods and molecular simulation are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for computational energy models on a fresh task.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for computational energy models on a fresh task.
...
_Package `swp-ma-hs10-chemistry-u06-l11` · built from `hs912-science-h4` at `a86780a`._
