# Scoring authority — Performance task build: fission

**High School Chemistry (Grade 10)** · Unit 9: The Nucleus, Nuclear Energy, and a Chemistry Engineering Capstone · Day 9 · lesson `ma-hs10-chemistry-u09-l09`
...
_Topic `ma-hs10-chemistry-u09::fission` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Fission is the splitting of a heavy nucleus into lighter nuclei, releasing energy and typically neutrons.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Fission of a heavy nucleus releases energy because the products are more tightly bound per nucleon than the original nucleus.
- Released neutrons can induce further fissions, which is the basis of a chain reaction.
- Control of a fission reactor works by controlling the number of neutrons available to induce further fission.
...
- Claiming fission is the combining of nuclei.
- Claiming fission energy comes from breaking chemical bonds.
- Claiming a reactor's fuel can undergo a nuclear explosion in the manner of a weapon.

**Grade boundary.** Reactor engineering, cross sections, and criticality calculations are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for fission that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain fission using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply fission to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on fission against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply fission to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain fission using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply fission to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for fission that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for fission (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for fission (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for fission (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for fission (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for fission (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u09-l09` · built from `hs912-science-h4` at `a86780a`._
