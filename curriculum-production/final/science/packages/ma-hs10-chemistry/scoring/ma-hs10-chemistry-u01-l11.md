# Scoring authority — Unit assessment: separation of mixtures

**High School Chemistry (Grade 10)** · Unit 1: Matter, Measurement, and Laboratory Practice · Day 11 · lesson `ma-hs10-chemistry-u01-l11`
...
_Topic `ma-hs10-chemistry-u01::separation of mixtures` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Mixtures are separated by physical means that exploit differences in properties; the components of a compound are separated only by chemical change.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Filtration exploits particle size, distillation exploits boiling point, chromatography exploits differing affinity, and evaporation exploits volatility.
- Because a mixture's components keep their own properties, separation does not change the substances involved.
- The choice of technique follows from which property differs most between the components.
...
- Claiming a compound can be separated into its elements by filtration or distillation.
- Claiming a homogeneous mixture is a pure substance because it looks uniform.
- Claiming dissolving destroys the solute.

**Grade boundary.** Chromatographic theory, fractional distillation column design, and separation efficiency are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for separation of mixtures on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain separation of mixtures using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply separation of mixtures to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on separation of mixtures against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply separation of mixtures to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain separation of mixtures using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply separation of mixtures to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for separation of mixtures on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for separation of mixtures (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for separation of mixtures (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for separation of mixtures (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for separation of mixtures (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for separation of mixtures (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u01-l11` · built from `hs912-science-h4` at `a86780a`._
