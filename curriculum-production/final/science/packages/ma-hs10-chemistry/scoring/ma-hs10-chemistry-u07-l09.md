# Scoring authority — Performance task build: concentration and reaction rate

**High School Chemistry (Grade 10)** · Unit 7: Reaction Rates and the Conditions That Change Them · Day 9 · lesson `ma-hs10-chemistry-u07-l09`
...
_Topic `ma-hs10-chemistry-u07::concentration and reaction rate` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Increasing the concentration of a reactant generally increases the reaction rate.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Higher concentration means more reactant particles in a given volume, so collisions between them are more frequent.
- Concentration affects collision frequency but not the energy of individual collisions.
- For gases, increasing pressure by reducing volume has the same effect as increasing concentration.
...
- Claiming higher concentration increases the energy of each collision.
- Claiming concentration changes the activation energy.
- Claiming rate depends on the total amount of reactant rather than on its concentration.

**Grade boundary.** Reaction order and rate law determination are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for concentration and reaction rate that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain concentration and reaction rate using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply concentration and reaction rate to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on concentration and reaction rate against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply concentration and reaction rate to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain concentration and reaction rate using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply concentration and reaction rate to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for concentration and reaction rate that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for concentration and reaction rate (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for concentration and reaction rate (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for concentration and reaction rate (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for concentration and reaction rate (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for concentration and reaction rate (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u07-l09` · built from `hs912-science-h4` at `a86780a`._
