# Scoring authority — Independent application A: thermal energy transfer and equilibration

**High School Chemistry (Grade 10)** · Unit 6: Energy in Chemical Systems · Day 4 · lesson `ma-hs10-chemistry-u06-l04`
...
_Topic `ma-hs10-chemistry-u06::thermal energy transfer and equilibration` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Thermal energy transfers spontaneously from the higher-temperature body to the lower-temperature one until they reach the same temperature.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Two bodies in thermal contact exchange energy until their temperatures are equal, which is thermal equilibrium.
- Temperature is a measure of average particle kinetic energy; total thermal energy also depends on the amount and the material.
- The energy lost by one body equals the energy gained by the other and its surroundings; energy is conserved.
...
- Claiming cold flows from the cooler body into the warmer one.
- Treating temperature and total thermal energy as the same quantity.
- Claiming thermal energy moves spontaneously from a cooler body to a warmer one.

**Grade boundary.** Specific heat calculations are acceptable but not required; entropy is treated qualitatively in this unit.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain thermal energy transfer and equilibration using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply thermal energy transfer and equilibration to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on thermal energy transfer and equilibration against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply thermal energy transfer and equilibration to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain thermal energy transfer and equilibration using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply thermal energy transfer and equilibration to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain thermal energy transfer and equilibration using evidence, a model, or a documented process rather than an unsupported answer.
...
- The learner completes the central task about thermal energy transfer and equilibration.
...
- **prerequisite-gap** — Run the prerequisite reteach route for thermal energy transfer and equilibration (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for thermal energy transfer and equilibration (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for thermal energy transfer and equilibration (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for thermal energy transfer and equilibration (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for thermal energy transfer and equilibration (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u06-l04` · built from `hs912-science-h4` at `a86780a`._
