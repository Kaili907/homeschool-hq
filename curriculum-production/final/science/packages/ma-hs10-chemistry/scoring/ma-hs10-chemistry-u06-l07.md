# Scoring authority — Investigation: bond breaking and bond making

**High School Chemistry (Grade 10)** · Unit 6: Energy in Chemical Systems · Day 7 · lesson `ma-hs10-chemistry-u06-l07`
...
_Topic `ma-hs10-chemistry-u06::bond breaking and bond making` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Breaking a chemical bond requires an input of energy; forming a chemical bond releases energy.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Energy must be supplied to separate bonded atoms, because the bond exists precisely because the bonded state is lower in energy.
- Energy is released when atoms come together to form a bond.
- The overall energy change of a reaction is the difference between the energy required to break bonds and the energy released in forming them.
...
- Claiming breaking bonds releases energy.
- Claiming forming bonds requires a net energy input.
- Claiming a molecule such as ATP or glucose releases energy by having its bonds broken alone, with no account of the bonds formed.

**Grade boundary.** Bond enthalpy tables and quantitative enthalpy calculations are acceptable but not required.
...
> Learner's own temperature-versus-time measurements, plus published bond-energy tables for the model. No temperature change is predicted numerically before measurement.
...
**Q1 — investigate.** _Serves objective 1:_ Plan and carry out a safe investigation of bond breaking and bond making, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain bond breaking and bond making using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply bond breaking and bond making to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on bond breaking and bond making against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply bond breaking and bond making to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain bond breaking and bond making using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 1:_ Plan and carry out a safe investigation of bond breaking and bond making, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q8 — provenance.** _Serves objective 1:_ Plan and carry out a safe investigation of bond breaking and bond making, or its stated alternative, and record the learner's own data with its uncertainty.
...
- **prerequisite-gap** — Run the prerequisite reteach route for bond breaking and bond making (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for bond breaking and bond making (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for bond breaking and bond making (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for bond breaking and bond making (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for bond breaking and bond making (evidence type: application, require explanation: True, review timing: same-day).
...
- chemical: A cold pack holds a salt that irritates skin and eyes and is harmful if swallowed; a torn hand warmer releases self-heating iron powder reaching about 65 degrees Celsius, an inhalation and ignition hazard. — mitigation: OBSERVED FROM THE OUTSIDE ONLY: never cut, torn, punctured, or opened, and the contents never touched or tasted. If one leaks, an adult bags and bins it and contact is rinsed 15 minutes. Not disableable.
- chemical: Calcium chloride releases substantial heat when dissolving and can burn skin and eyes, and pouring water onto the solid spatters hot solution. — mitigation: OPTIONAL. An adult handles the solid from its original labelled container - never bulk road salt. Eye protection and gloves; under one teaspoon; SOLID IS ALWAYS ADDED TO THE WATER, never the reverse; never touched warm.
- chemical: Ice-melt calcium chloride is not a food grade and leaves residue on whatever holds, stirs, or measures it. — mitigation: The calcium chloride route runs in disposable cups with a disposable stirrer, binned after one use. No mug, tumbler, flask, bottle, or drinking cup holds it, and nothing that touches it returns to food or drinking use.
- physical: Solutions can become hot enough to burn. — mitigation: Each route has its OWN vessel: calcium chloride only in the DISPOSABLE double cup, NEVER the insulated drinking cup. Stand it on the work tray, stir with the thermometer, judge temperature only from it, and stop above 50 degrees Celsius.
...
_Package `swp-ma-hs10-chemistry-u06-l07` · built from `hs912-science-h4` at `a86780a`._
