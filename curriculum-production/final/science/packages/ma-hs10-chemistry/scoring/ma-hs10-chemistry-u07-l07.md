# Scoring authority — Investigation: collision model of reactions

**High School Chemistry (Grade 10)** · Unit 7: Reaction Rates and the Conditions That Change Them · Day 7 · lesson `ma-hs10-chemistry-u07-l07`
...
_Topic `ma-hs10-chemistry-u07::collision model of reactions` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- For a reaction to occur, particles must collide with sufficient energy and in a suitable orientation.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Reaction rate depends on how often effective collisions occur, where effective means both energetic enough and correctly oriented.
- Activation energy is the minimum energy a collision must have to lead to reaction.
- Anything that raises the frequency or the effectiveness of collisions raises the rate.
...
- Claiming every collision between reactant particles produces a reaction.
- Claiming orientation is irrelevant provided the collision is energetic enough.
- Confusing activation energy with the overall energy change of the reaction.

**Grade boundary.** Rate laws, rate constants, and the Arrhenius equation are beyond this course.
...
**Q1 — investigate.** _Serves objective 1:_ Plan and carry out a safe investigation of collision model of reactions, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain collision model of reactions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply collision model of reactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on collision model of reactions against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply collision model of reactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain collision model of reactions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 1:_ Plan and carry out a safe investigation of collision model of reactions, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q8 — provenance.** _Serves objective 1:_ Plan and carry out a safe investigation of collision model of reactions, or its stated alternative, and record the learner's own data with its uncertainty.
...
- **prerequisite-gap** — Run the prerequisite reteach route for collision model of reactions (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for collision model of reactions (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for collision model of reactions (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for collision model of reactions (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for collision model of reactions (evidence type: application, require explanation: True, review timing: same-day).
...
- chemical: A glow stick holds an internal glass ampoule, irritant esters, and hydrogen peroxide. — mitigation: Bend to activate; never cut, bite, open, or mouth a glow stick. If one splits, nobody touches it: rinse any contact 15 minutes and an adult bags and bins it. This prohibition is not disableable.
- chemical: Vinegar and effervescent tablets irritate eyes. — mitigation: Wear eye protection, work over a tray, and rinse splashes with water.
- physical: Warm water can scald and glass can break with thermal shock. — mitigation: An adult pours every warm sample and confirms with the thermometer that it reads below 50 degrees Celsius. Use plastic or unbreakable cups only.
...
_Package `swp-ma-hs10-chemistry-u07-l07` · built from `hs912-science-h4` at `a86780a`._
