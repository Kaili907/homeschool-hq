# Scoring authority — Independent application A: density and derived quantities

**High School Chemistry (Grade 10)** · Unit 1: Matter, Measurement, and Laboratory Practice · Day 4 · lesson `ma-hs10-chemistry-u01-l04`
...
_Topic `ma-hs10-chemistry-u01::density and derived quantities` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Density is mass divided by volume, and it is an intensive property: it does not change with the size of the sample.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Density is a derived quantity, calculated from two measurements, so its uncertainty depends on both.
- Because density is intensive, a small piece and a large piece of the same pure substance have the same density.
- Density varies with temperature, because volume does, so a density value is quoted with the temperature it applies at.

**Also correct.** Accept any of these framings — do not require the wording above:

- Any consistent unit set is acceptable, provided mass and volume units are stated.
...
- Claiming a larger sample of the same substance has a greater density.
- Confusing density with mass, or treating a heavier object as necessarily denser.
- Reporting a density with no units, or with units that are not mass over volume.

**Grade boundary.** Thermal expansion coefficients and buoyancy calculations are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain density and derived quantities using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply density and derived quantities to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on density and derived quantities against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply density and derived quantities to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain density and derived quantities using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply density and derived quantities to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain density and derived quantities using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for density and derived quantities (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for density and derived quantities (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for density and derived quantities (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for density and derived quantities (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for density and derived quantities (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u01-l04` · built from `hs912-science-h4` at `a86780a`._
