# Scoring authority — Unit assessment: reactivity patterns by group

**High School Chemistry (Grade 10)** · Unit 2: Atomic Structure and the Periodic Table as a Model · Day 11 · lesson `ma-hs10-chemistry-u02-l11`
...
_Topic `ma-hs10-chemistry-u02::reactivity patterns by group` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._
...
- Noble gases are very unreactive because their outer energy level is already full.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Group 1 metals lose their single valence electron more readily further down the group, because that electron is held less tightly, so reactivity increases downward.
- Group 17 elements gain an electron less readily further down the group, for the same reason, so reactivity decreases downward.
- Similar valence electron arrangements are what make the elements of a group behave similarly.

**Also correct.** Accept any of these framings — do not require the wording above:

- Naming the groups alkali metals, halogens, and noble gases, or by number, are equally correct.
...
- Claiming group 1 reactivity decreases down the group.
- Claiming halogen reactivity increases down the group.
- Claiming noble gases are unreactive because they have no electrons available.

**Grade boundary.** Standard electrode potentials and transition metal reactivity series are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for reactivity patterns by group on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain reactivity patterns by group using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply reactivity patterns by group to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on reactivity patterns by group against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply reactivity patterns by group to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain reactivity patterns by group using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply reactivity patterns by group to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for reactivity patterns by group on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for reactivity patterns by group (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for reactivity patterns by group (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for reactivity patterns by group (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for reactivity patterns by group (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for reactivity patterns by group (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u02-l11` · built from `hs912-science-h4` at `a86780a`._
