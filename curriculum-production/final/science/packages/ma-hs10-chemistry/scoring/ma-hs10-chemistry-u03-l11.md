# Scoring authority — Unit assessment: polymers and designed materials

**High School Chemistry (Grade 10)** · Unit 3: Bonding, Structure, and the Properties of Designed Materials · Day 11 · lesson `ma-hs10-chemistry-u03-l11`
...
_Topic `ma-hs10-chemistry-u03::polymers and designed materials` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- A polymer is a large molecule built from many repeating monomer units joined by covalent bonds.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A polymer's bulk properties follow from its monomer, its chain length, its branching, and the forces between chains.
- Cross-linking between chains increases rigidity and reduces the ability to melt and flow.
- Materials are designed by choosing structure to obtain properties, which is why structure-property reasoning is the core of materials design.

**Also correct.** Accept any of these framings — do not require the wording above:

- Addition and condensation polymerisation are both correct classifications where the lesson calls for them.
...
- Claiming a polymer is a mixture of its monomers rather than a molecule built from them.
- Claiming a polymer's properties depend only on which monomer it is made from.
- Claiming synthetic materials are not chemically related to the natural resources they are made from.

**Grade boundary.** Polymerisation kinetics, molecular weight distributions, and copolymer theory are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for polymers and designed materials on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain polymers and designed materials using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply polymers and designed materials to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on polymers and designed materials against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply polymers and designed materials to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain polymers and designed materials using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply polymers and designed materials to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for polymers and designed materials on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for polymers and designed materials (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for polymers and designed materials (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for polymers and designed materials (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for polymers and designed materials (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for polymers and designed materials (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u03-l11` · built from `hs912-science-h4` at `a86780a`._
