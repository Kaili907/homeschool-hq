# Scoring authority — Reteach and varied practice: dynamic equilibrium

**High School Chemistry (Grade 10)** · Unit 8: Equilibrium and the Design of Chemical Systems · Day 8 · lesson `ma-hs10-chemistry-u08-l08`
...
_Topic `ma-hs10-chemistry-u08::dynamic equilibrium` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- At equilibrium the forward and reverse reaction rates are equal, so concentrations remain constant while both reactions continue.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Equilibrium is dynamic: both reactions continue at equal rates, and the system is not static.
- Equal rates do not mean equal concentrations; the concentrations at equilibrium depend on the reaction.
- Equilibrium is reached only in a closed system, since an open one loses products or reactants.
...
- Claiming equilibrium means equal concentrations of reactants and products.
- Claiming an open system with material escaping can reach equilibrium.

**Grade boundary.** Equilibrium constants, reaction quotients, and equilibrium calculations are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain dynamic equilibrium using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply dynamic equilibrium to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on dynamic equilibrium against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply dynamic equilibrium to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain dynamic equilibrium using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply dynamic equilibrium to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain dynamic equilibrium using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for dynamic equilibrium (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for dynamic equilibrium (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for dynamic equilibrium (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for dynamic equilibrium (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for dynamic equilibrium (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u08-l08` · built from `hs912-science-h4` at `a86780a`._
