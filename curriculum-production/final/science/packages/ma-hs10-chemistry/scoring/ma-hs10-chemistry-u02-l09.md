# Scoring authority — Performance task build: valence electrons

**High School Chemistry (Grade 10)** · Unit 2: Atomic Structure and the Periodic Table as a Model · Day 9 · lesson `ma-hs10-chemistry-u02-l09`
...
_Topic `ma-hs10-chemistry-u02::valence electrons` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Valence electrons are the electrons in the outermost energy level, and they are the electrons involved in bonding.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The number of valence electrons largely determines an element's chemical behaviour, which is why elements in the same main group behave similarly.
- Atoms tend toward outer-electron arrangements resembling those of the noble gases, by transferring, sharing, or delocalising electrons.
- For main-group elements the valence electron count follows from the group: groups 1 and 2 have one and two, and groups 13 to 18 have the group number minus ten, with helium the exception at two.

**Also correct.** Accept any of these framings — do not require the wording above:

- The octet rule and the noble-gas configuration description are equivalent framings; both have known exceptions.
...
- Counting all of an atom's electrons as valence electrons.
- Claiming the octet rule holds without exception.
- Claiming atoms bond in order to satisfy a preference or a need of their own.

**Grade boundary.** Transition metal electron configurations and hypervalency are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for valence electrons that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain valence electrons using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply valence electrons to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on valence electrons against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply valence electrons to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain valence electrons using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply valence electrons to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for valence electrons that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for valence electrons (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for valence electrons (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for valence electrons (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for valence electrons (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for valence electrons (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u02-l09` · built from `hs912-science-h4` at `a86780a`._
