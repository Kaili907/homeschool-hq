# Scoring authority — Performance task build: the mole as a counting unit

**High School Chemistry (Grade 10)** · Unit 5: Conservation of Mass and Quantitative Reasoning in Reactions · Day 9 · lesson `ma-hs10-chemistry-u05-l09`
...
_Topic `ma-hs10-chemistry-u05::the mole as a counting unit` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- One mole contains Avogadro's number of particles, approximately 6.022 times ten to the twenty-third.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The mole is a counting unit for particles, in the same way that a dozen is, chosen so that molar mass in grams matches atomic and formula mass.
- A mole of any substance contains the same number of particles, but moles of different substances have different masses.
- The particle counted must be specified, since a mole of oxygen atoms and a mole of oxygen molecules are different quantities.
...
- Claiming a mole is a unit of mass.
- Claiming a mole of every substance has the same mass.
- Using a mole quantity without specifying whether atoms, molecules, or formula units are counted.

**Grade boundary.** The current SI definition of the mole in terms of a fixed Avogadro constant is acceptable to state but not required.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for the mole as a counting unit that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain the mole as a counting unit using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply the mole as a counting unit to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on the mole as a counting unit against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply the mole as a counting unit to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain the mole as a counting unit using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply the mole as a counting unit to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for the mole as a counting unit that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for the mole as a counting unit (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for the mole as a counting unit (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for the mole as a counting unit (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for the mole as a counting unit (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for the mole as a counting unit (evidence type: application, require explanation: True, review timing: same-day).
...
- chemical: Vinegar stings eyes, and the reaction produces carbon dioxide. — mitigation: Wear eye protection and work in a ventilated room. At the quantities fixed below the carbon dioxide is harmless; the quantities are what keep it that way, so they are part of the procedure and not a suggestion.
- physical: A closed bag holding a gas-producing reaction can burst and spray liquid. — mitigation: The closed trial is SEALED COMPLETELY - a part-sealed bag is not a closed system - and is made safe by volume: half a teaspoon of baking soda and 20 mL of vinegar in a one-litre bag. Flat on a tray, pointed away, never held.
...
**Disposal and clearing up:** Open the bag by one corner only, over the sink, away from every face; pour the contents down the drain with the tap running and rinse the bag before recycling or binning it. Wipe the tray and wash hands. Never re-seal a bag that still holds gas, and never store a sealed bag from this investigation.
...
_Package `swp-ma-hs10-chemistry-u05-l09` · built from `hs912-science-h4` at `a86780a`._
