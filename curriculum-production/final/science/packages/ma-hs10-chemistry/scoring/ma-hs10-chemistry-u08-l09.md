# Scoring authority — Performance task build: stress and system response

**High School Chemistry (Grade 10)** · Unit 8: Equilibrium and the Design of Chemical Systems · Day 9 · lesson `ma-hs10-chemistry-u08-l09`
...
_Topic `ma-hs10-chemistry-u08::stress and system response` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- A system at equilibrium responds to a change in conditions by shifting in the direction that partially counteracts that change.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Changing concentration, pressure, or temperature disturbs the balance of forward and reverse rates, and the system shifts until they are equal again.
- The shift partially offsets the change; it does not restore the original conditions.
- Temperature is the only one of these changes that alters the equilibrium position by changing the balance of the reaction's own energetics rather than by changing amounts.

**Also correct.** Accept any of these framings — do not require the wording above:

- Naming this Le Chatelier's principle, or describing the response without the name, are equally correct.
...
- Claiming the system returns exactly to its original conditions after a disturbance.
- Claiming a catalyst shifts the position of equilibrium.
- Claiming adding an inert gas at constant volume shifts a gas-phase equilibrium.

**Grade boundary.** Quantitative prediction of the new equilibrium position is beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for stress and system response that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain stress and system response using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply stress and system response to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on stress and system response against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply stress and system response to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain stress and system response using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply stress and system response to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for stress and system response that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for stress and system response (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for stress and system response (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for stress and system response (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for stress and system response (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for stress and system response (evidence type: application, require explanation: True, review timing: same-day).
...
- chemical: Acidic and basic solutions sting eyes and irritate skin. — mitigation: Wear eye protection, work over a tray, use small volumes with droppers, and rinse any splash with water for 15 minutes.
- chemical: Household chemical mixing beyond the listed materials can produce toxic gas. — mitigation: Use ONLY cabbage indicator, vinegar, baking soda, and carbonated water. Never introduce a cleaning product. This prohibition is not disableable.
- physical: A sealed carbonated bottle is a pressure vessel, and warming or shaking it turns the cap into a projectile. — mitigation: A carbonated bottle is never warmed above 40 degrees Celsius, never shaken while closed, and is always opened slowly with the cap pointed away from every face.
- physical: Warm water can scald. — mitigation: An adult pours the warm bath and confirms with the thermometer that it reads below 50 degrees Celsius. Temperature is judged with the thermometer, never with a hand.
...
**Disposal and clearing up:** Pour all indicator, vinegar, baking-soda, and carbonated solutions down the drain with the tap running, rinse the cups and droppers, and wipe the tray. Open and empty any carbonated bottle before recycling it. No solution is stored or reused.
...
_Package `swp-ma-hs10-chemistry-u08-l09` · built from `hs912-science-h4` at `a86780a`._
