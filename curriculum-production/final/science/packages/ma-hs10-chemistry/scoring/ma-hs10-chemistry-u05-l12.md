# Scoring authority — Correction and reflection: mathematical representations of reactions

**High School Chemistry (Grade 10)** · Unit 5: Conservation of Mass and Quantitative Reasoning in Reactions · Day 12 · lesson `ma-hs10-chemistry-u05-l12`
...
_Topic `ma-hs10-chemistry-u05::mathematical representations of reactions` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- A balanced equation is a mathematical statement about relative numbers of particles, and mole ratios are read directly from its coefficients.
- Mass, moles, and particle count are related through molar mass and Avogadro's number, and calculations move between them in a stated order.
- A calculated result carries the units and the precision of the measurements it came from.

**Also correct.** Accept any of these framings — do not require the wording above:

- Dimensional analysis and proportional reasoning both yield correct results and are equally acceptable.
...
- Reading mole ratios from an unbalanced equation.
- Applying coefficients as mass ratios rather than mole ratios.
- Reporting a calculated result with more significant figures than the input measurements support.

**Grade boundary.** Solution stoichiometry with concentration and gas law stoichiometry are acceptable but not required here.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain mathematical representations of reactions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply mathematical representations of reactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on mathematical representations of reactions against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply mathematical representations of reactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain mathematical representations of reactions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply mathematical representations of reactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain mathematical representations of reactions using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for mathematical representations of reactions (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for mathematical representations of reactions (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for mathematical representations of reactions (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for mathematical representations of reactions (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for mathematical representations of reactions (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u05-l12` · built from `hs912-science-h4` at `a86780a`._
