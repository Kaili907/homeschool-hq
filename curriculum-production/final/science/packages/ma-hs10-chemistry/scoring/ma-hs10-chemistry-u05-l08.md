# Scoring authority — Reteach and varied practice: balancing equations as bookkeeping

**High School Chemistry (Grade 10)** · Unit 5: Conservation of Mass and Quantitative Reasoning in Reactions · Day 8 · lesson `ma-hs10-chemistry-u05-l08`
...
_Topic `ma-hs10-chemistry-u05::balancing equations as bookkeeping` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Balancing an equation is done by changing coefficients only; subscripts in a formula are never changed to make an equation balance.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A balanced equation has the same number of atoms of each element on both sides, and the same total charge.
- Coefficients represent relative numbers of particles or moles, and they scale the whole formula unit they precede.
- Changing a subscript changes the substance, which is why balancing never alters subscripts.

**Also correct.** Accept any of these framings — do not require the wording above:

- Any set of coefficients in the correct ratio balances the equation; the lowest whole-number set is conventional but not the only correct answer.
...
- Altering a subscript in a chemical formula in order to balance an equation.
- Presenting an equation as balanced when the atom counts differ between sides.
- Claiming coefficients represent masses rather than numbers of particles or moles.

**Grade boundary.** Balancing redox half-reactions in solution and matrix methods are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain balancing equations as bookkeeping using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply balancing equations as bookkeeping to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on balancing equations as bookkeeping against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply balancing equations as bookkeeping to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain balancing equations as bookkeeping using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply balancing equations as bookkeeping to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain balancing equations as bookkeeping using evidence, a model, or a documented process rather than an unsupported answer.
...
- The learner completes the central task about balancing equations as bookkeeping.
...
- **prerequisite-gap** — Run the prerequisite reteach route for balancing equations as bookkeeping (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for balancing equations as bookkeeping (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for balancing equations as bookkeeping (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for balancing equations as bookkeeping (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for balancing equations as bookkeeping (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u05-l08` · built from `hs912-science-h4` at `a86780a`._
