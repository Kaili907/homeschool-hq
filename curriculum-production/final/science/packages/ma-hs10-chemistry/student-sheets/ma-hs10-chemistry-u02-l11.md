# Unit assessment: reactivity patterns by group

**High School Chemistry (Grade 10)** · Unit 2: Atomic Structure and the Periodic Table as a Model · Day 11 of the unit (course day 23) · about 55–75 minutes

**Today's focus:** reactivity patterns by group
...
- Demonstrate independent mastery evidence for reactivity patterns by group on a fresh task.
- Explain reactivity patterns by group using evidence, a model, or a documented process rather than an unsupported answer.
- Apply reactivity patterns by group to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on reactivity patterns by group against the stated success criteria and name the next step.
...
- Science brief: reactivity increases down group 1 and decreases down group 17.
- Science brief: noble gases are very unreactive because their outer energy level is already full.
- Science brief: group 1 metals lose their single valence electron more readily further down the group, because that electron is held less tightly, so reactivity increases downward.
- Science brief: group 17 elements gain an electron less readily further down the group, for the same reason, so reactivity decreases downward.

### Evidence check: reactivity patterns by group

**Candidate claim:** A case report presents this as acceptable science: “group 1 reactivity decreases down the group.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: reactivity increases down group 1 and decreases down group 17. |
| E2 | established science reference | Science brief: noble gases are very unreactive because their outer energy level is already full. |
| E3 | established science reference | Science brief: group 1 metals lose their single valence electron more readily further down the group, because that electron is held less tightly, so reactivity increases downward. |
| E4 | established science reference | Science brief: group 17 elements gain an electron less readily further down the group, for the same reason, so reactivity decreases downward. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs10-chemistry-u02::reactivity patterns by group, derived from the reviewed correctness source policy/correctness/ma-hs10-chemistry.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about reactivity patterns by group, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about reactivity patterns by group, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about reactivity patterns by group, then name one check that would catch an error or a weak claim.**
...
**Q1.** Without copying a sentence from the brief, evaluate the printed candidate case about reactivity patterns by group. Cite E1-E4, write the reasoning in your own words, and correct the case claim so it fits exactly what the record establishes.

>

**Q2.** Explain reactivity patterns by group in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q3.** Treat the printed candidate claim as the fresh case for reactivity patterns by group. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q4.** Check the conclusion about reactivity patterns by group, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q5.** Answer the printed unit question using reactivity patterns by group as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q6.** Which one row in E1-E4 is strongest for evaluating the candidate claim about reactivity patterns by group? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q7.** Using the supplied case and E1-E4, state one conclusion about reactivity patterns by group that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q8.** Audit the inputs used in your response about reactivity patterns by group. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about reactivity patterns by group.
...
- Apply reactivity patterns by group under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect reactivity patterns by group to subatomic structure and isotopes from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs10-chemistry-u02-l11` · lesson `ma-hs10-chemistry-u02-l11` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
