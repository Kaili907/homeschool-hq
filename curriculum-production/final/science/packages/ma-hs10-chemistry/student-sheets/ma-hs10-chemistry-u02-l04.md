# Independent application A: periodic trends in atomic radius and ionization

**High School Chemistry (Grade 10)** · Unit 2: Atomic Structure and the Periodic Table as a Model · Day 4 of the unit (course day 16) · about 55–75 minutes

**Today's focus:** periodic trends in atomic radius and ionization
...
- Explain periodic trends in atomic radius and ionization using evidence, a model, or a documented process rather than an unsupported answer.
- Apply periodic trends in atomic radius and ionization to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on periodic trends in atomic radius and ionization against the stated success criteria and name the next step.
...
- Science brief: atomic radius generally decreases across a period from left to right and increases down a group.
- Science brief: first ionisation energy generally increases across a period from left to right and decreases down a group.
- Science brief: across a period, nuclear charge rises while electrons are added to the same energy level, so the outer electrons are pulled in more strongly.
- Science brief: down a group, outer electrons occupy higher energy levels and are shielded by more inner electrons, so they are held less tightly.

### Evidence check: periodic trends in atomic radius and ionization

**Candidate claim:** A case report presents this as acceptable science: “atomic radius increases across a period from left to right.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: atomic radius generally decreases across a period from left to right and increases down a group. |
| E2 | established science reference | Science brief: first ionisation energy generally increases across a period from left to right and decreases down a group. |
| E3 | established science reference | Science brief: across a period, nuclear charge rises while electrons are added to the same energy level, so the outer electrons are pulled in more strongly. |
| E4 | established science reference | Science brief: down a group, outer electrons occupy higher energy levels and are shielded by more inner electrons, so they are held less tightly. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs10-chemistry-u02::periodic trends in atomic radius and ionization, derived from the reviewed correctness source policy/correctness/ma-hs10-chemistry.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about periodic trends in atomic radius and ionization, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about periodic trends in atomic radius and ionization, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about periodic trends in atomic radius and ionization, then name one check that would catch an error or a weak claim.**
...
**Q1.** Explain periodic trends in atomic radius and ionization in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q2.** Treat the printed candidate claim as the fresh case for periodic trends in atomic radius and ionization. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q3.** Check the conclusion about periodic trends in atomic radius and ionization, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q4.** Answer the printed unit question using periodic trends in atomic radius and ionization as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E4 is strongest for evaluating the candidate claim about periodic trends in atomic radius and ionization? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E4, state one conclusion about periodic trends in atomic radius and ionization that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about periodic trends in atomic radius and ionization. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about periodic trends in atomic radius and ionization.
...
- Apply periodic trends in atomic radius and ionization under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect periodic trends in atomic radius and ionization to subatomic structure and isotopes from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs10-chemistry-u02-l04` · lesson `ma-hs10-chemistry-u02-l04` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
