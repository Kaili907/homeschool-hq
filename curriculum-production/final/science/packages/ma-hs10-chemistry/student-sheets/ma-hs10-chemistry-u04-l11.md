# Unit assessment: predicting products from periodic trends

**High School Chemistry (Grade 10)** · Unit 4: Chemical Reactions and Electron Rearrangement · Day 11 of the unit (course day 47) · about 55–75 minutes

**Today's focus:** predicting products from periodic trends
...
- Demonstrate independent mastery evidence for predicting products from periodic trends on a fresh task.
- Explain predicting products from periodic trends using evidence, a model, or a documented process rather than an unsupported answer.
- Apply predicting products from periodic trends to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on predicting products from periodic trends against the stated success criteria and name the next step.
...
- Science brief: position in the periodic table indicates likely charges for main-group ions, and therefore likely formulas for the compounds formed.
- Science brief: a predicted product is a hypothesis to be checked against conservation of atoms, charge balance, and experimental evidence.
- Science brief: trends give likely outcomes, not guarantees; exceptions exist and are part of the chemistry.

### Evidence check: predicting products from periodic trends

**Candidate claim:** A case report presents this as acceptable science: “predicting a compound whose formula does not balance charge.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: position in the periodic table indicates likely charges for main-group ions, and therefore likely formulas for the compounds formed. |
| E2 | established science reference | Science brief: a predicted product is a hypothesis to be checked against conservation of atoms, charge balance, and experimental evidence. |
| E3 | established science reference | Science brief: trends give likely outcomes, not guarantees; exceptions exist and are part of the chemistry. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs10-chemistry-u04::predicting products from periodic trends, derived from the reviewed correctness source policy/correctness/ma-hs10-chemistry.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about predicting products from periodic trends, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about predicting products from periodic trends, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about predicting products from periodic trends, then name one check that would catch an error or a weak claim.**
...
**Q1.** Without copying a sentence from the brief, evaluate the printed candidate case about predicting products from periodic trends. Cite E1-E3, write the reasoning in your own words, and correct the case claim so it fits exactly what the record establishes.

>

**Q2.** Explain predicting products from periodic trends in your own words using at least two specifically named rows from E1-E3. Then explain why the candidate claim does or does not follow.

>

**Q3.** Treat the printed candidate claim as the fresh case for predicting products from periodic trends. Decide whether E1-E3 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q4.** Check the conclusion about predicting products from periodic trends, revise its weakest claim with a named row from E1-E3, and name the next observation or model input that would resolve the remaining limitation.

>

**Q5.** Answer the printed unit question using predicting products from periodic trends as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q6.** Which one row in E1-E3 is strongest for evaluating the candidate claim about predicting products from periodic trends? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q7.** Using the supplied case and E1-E3, state one conclusion about predicting products from periodic trends that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q8.** Audit the inputs used in your response about predicting products from periodic trends. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about predicting products from periodic trends.
...
- Apply predicting products from periodic trends under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect predicting products from periodic trends to evidence of chemical change from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs10-chemistry-u04-l11` · lesson `ma-hs10-chemistry-u04-l11` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
