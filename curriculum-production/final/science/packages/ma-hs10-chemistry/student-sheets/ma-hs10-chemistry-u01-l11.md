# Unit assessment: separation of mixtures

**High School Chemistry (Grade 10)** · Unit 1: Matter, Measurement, and Laboratory Practice · Day 11 of the unit (course day 11) · about 55–75 minutes

**Today's focus:** separation of mixtures
...
- Demonstrate independent mastery evidence for separation of mixtures on a fresh task.
- Explain separation of mixtures using evidence, a model, or a documented process rather than an unsupported answer.
- Apply separation of mixtures to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on separation of mixtures against the stated success criteria and name the next step.
...
- Science brief: mixtures are separated by physical means that exploit differences in properties; the components of a compound are separated only by chemical change.
- Science brief: filtration exploits particle size, distillation exploits boiling point, chromatography exploits differing affinity, and evaporation exploits volatility.
- Science brief: because a mixture's components keep their own properties, separation does not change the substances involved.
- Science brief: the choice of technique follows from which property differs most between the components.

### Evidence check: separation of mixtures

**Candidate claim:** A case report presents this as acceptable science: “a compound can be separated into its elements by filtration or distillation.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: mixtures are separated by physical means that exploit differences in properties; the components of a compound are separated only by chemical change. |
| E2 | established science reference | Science brief: filtration exploits particle size, distillation exploits boiling point, chromatography exploits differing affinity, and evaporation exploits volatility. |
| E3 | established science reference | Science brief: because a mixture's components keep their own properties, separation does not change the substances involved. |
| E4 | established science reference | Science brief: the choice of technique follows from which property differs most between the components. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs10-chemistry-u01::separation of mixtures, derived from the reviewed correctness source policy/correctness/ma-hs10-chemistry.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about separation of mixtures, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about separation of mixtures, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about separation of mixtures, then name one check that would catch an error or a weak claim.**
...
**Q1.** Without copying a sentence from the brief, evaluate the printed candidate case about separation of mixtures. Cite E1-E4, write the reasoning in your own words, and correct the case claim so it fits exactly what the record establishes.

>

**Q2.** Explain separation of mixtures in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q3.** Treat the printed candidate claim as the fresh case for separation of mixtures. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q4.** Check the conclusion about separation of mixtures, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q5.** Answer the printed unit question using separation of mixtures as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q6.** Which one row in E1-E4 is strongest for evaluating the candidate claim about separation of mixtures? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q7.** Using the supplied case and E1-E4, state one conclusion about separation of mixtures that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q8.** Audit the inputs used in your response about separation of mixtures. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about separation of mixtures.
...
- Apply separation of mixtures under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect separation of mixtures to physical and chemical properties from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs10-chemistry-u01-l11` · lesson `ma-hs10-chemistry-u01-l11` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
