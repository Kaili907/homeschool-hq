# Unit assessment: limiting reactants

**High School Chemistry (Grade 10)** · Unit 5: Conservation of Mass and Quantitative Reasoning in Reactions · Day 11 of the unit (course day 59) · about 55–75 minutes

**Today's focus:** limiting reactants
...
- Demonstrate independent mastery evidence for limiting reactants on a fresh task.
- Explain limiting reactants using evidence, a model, or a documented process rather than an unsupported answer.
- Apply limiting reactants to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on limiting reactants against the stated success criteria and name the next step.
...
- Science brief: the limiting reactant is the one that runs out first and therefore determines how much product can form.
- Science brief: the limiting reactant is identified by comparing available amounts in moles against the ratio the balanced equation requires, not by comparing masses.
- Science brief: reactant left over when the reaction stops is in excess, and it does not increase the yield.
- Science brief: theoretical yield follows from the limiting reactant; actual yield is measured and is normally lower.

### Evidence check: limiting reactants

**Candidate claim:** A case report presents this as acceptable science: “identifying the limiting reactant as the one present in the smallest mass without converting to moles.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: the limiting reactant is the one that runs out first and therefore determines how much product can form. |
| E2 | established science reference | Science brief: the limiting reactant is identified by comparing available amounts in moles against the ratio the balanced equation requires, not by comparing masses. |
| E3 | established science reference | Science brief: reactant left over when the reaction stops is in excess, and it does not increase the yield. |
| E4 | established science reference | Science brief: theoretical yield follows from the limiting reactant; actual yield is measured and is normally lower. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs10-chemistry-u05::limiting reactants, derived from the reviewed correctness source policy/correctness/ma-hs10-chemistry.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about limiting reactants, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about limiting reactants, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about limiting reactants, then name one check that would catch an error or a weak claim.**
...
**Q1.** Without copying a sentence from the brief, evaluate the printed candidate case about limiting reactants. Cite E1-E4, write the reasoning in your own words, and correct the case claim so it fits exactly what the record establishes.

>

**Q2.** Explain limiting reactants in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q3.** Treat the printed candidate claim as the fresh case for limiting reactants. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q4.** Check the conclusion about limiting reactants, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q5.** Answer the printed unit question using limiting reactants as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q6.** Which one row in E1-E4 is strongest for evaluating the candidate claim about limiting reactants? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q7.** Using the supplied case and E1-E4, state one conclusion about limiting reactants that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q8.** Audit the inputs used in your response about limiting reactants. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about limiting reactants.
...
- Apply limiting reactants under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect limiting reactants to conservation of atoms and mass from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs10-chemistry-u05-l11` · lesson `ma-hs10-chemistry-u05-l11` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
