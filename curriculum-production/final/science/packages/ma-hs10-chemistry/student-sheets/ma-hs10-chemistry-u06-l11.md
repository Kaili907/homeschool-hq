# Unit assessment: computational energy models

**High School Chemistry (Grade 10)** · Unit 6: Energy in Chemical Systems · Day 11 of the unit (course day 71) · about 55–75 minutes
...
- Science brief: a computational energy model represents the system's energy inputs, outputs, and stores, and it must balance.
- Science brief: the model's output depends on the assumptions and parameters entered, and both are stated with the result.
- Science brief: a model result is a prediction, and it is compared against measurement rather than substituted for it.

### Evidence check: computational energy models

**Candidate claim:** A case report presents this as acceptable science: “presenting model output as though it were experimental data.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: a computational energy model represents the system's energy inputs, outputs, and stores, and it must balance. |
| E2 | established science reference | Science brief: the model's output depends on the assumptions and parameters entered, and both are stated with the result. |
| E3 | established science reference | Science brief: a model result is a prediction, and it is compared against measurement rather than substituted for it. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs10-chemistry-u06::computational energy models, derived from the reviewed correctness source policy/correctness/ma-hs10-chemistry.correctness.json.
...
**Q1.** Without copying a sentence from the brief, evaluate the printed candidate case about computational energy models. Cite E1-E3, write the reasoning in your own words, and correct the case claim so it fits exactly what the record establishes.
...
- Connect computational energy models to bond breaking and bond making from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs10-chemistry-u06-l11` · lesson `ma-hs10-chemistry-u06-l11` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
