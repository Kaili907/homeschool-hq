# Synthesis and review: iterative modeling

**Grade 8 Science** · Unit 9: Engineering and Scientific Argument Capstone · Day 10 of the unit (course day 106) · about 45–65 minutes

**Today's focus:** iterative modeling
...
- Connect iterative modeling to the unit’s other ideas and select evidence that shows readiness for assessment.
- Use subject-appropriate evidence or representation to communicate understanding of iterative modeling.
- Check, revise, or improve work about iterative modeling using stated success criteria.
...
- Science brief: each iteration tests the model or prototype, identifies a specific shortfall, and makes a change aimed at that shortfall.
- Science brief: changing one thing at a time keeps each change's effect identifiable.
- Science brief: the record of iterations, including the failures, is the evidence that the final design was arrived at rather than guessed.

### Evidence check: iterative modeling

**Candidate claim:** A case report presents this as acceptable science: “reporting only the successful final iteration.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: each iteration tests the model or prototype, identifies a specific shortfall, and makes a change aimed at that shortfall. |
| E2 | established science reference | Science brief: changing one thing at a time keeps each change's effect identifiable. |
| E3 | established science reference | Science brief: the record of iterations, including the failures, is the evidence that the final design was arrived at rather than guessed. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g8-science-u09::iterative modeling, derived from the reviewed correctness source policy/correctness/ma-g8-science.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about iterative modeling, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about iterative modeling, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about iterative modeling; then identify one check that would catch an error or weak claim.**
...
**Q1.** The supplied unit connections are: problem definition, criteria constraints and stakeholders, data-driven comparison, iterative modeling, optimization, scientific argument and communication. Choose one idea other than iterative modeling, connect it to one row in E1-E3, and explain one way treating the two ideas as identical would be misleading.

>

**Q2.** Communicate what the supplied record establishes about iterative modeling in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E3 and mark where the representation stops being supported.

>

**Q3.** Check your response about iterative modeling against each success criterion. Revise one sentence by adding or changing a citation to E1-E3; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using iterative modeling as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about iterative modeling? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about iterative modeling that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about iterative modeling. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about iterative modeling.
...
- Apply iterative modeling under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect iterative modeling to problem definition from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g8-science-u09-l10` · lesson `ma-g8-science-u09-l10` · built from `canonical-1.0.0` at `4056e31`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
