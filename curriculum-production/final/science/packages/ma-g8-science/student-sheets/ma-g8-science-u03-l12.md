# Correction and reflection: probability and model limits

**Grade 8 Science** · Unit 3: Growth, Reproduction, and Heredity · Day 12 of the unit (course day 36) · about 45–65 minutes

**Today's focus:** probability and model limits
...
- Use error evidence to choose a different representation or strategy for probability and model limits, then demonstrate corrected understanding.
- Use subject-appropriate evidence or representation to communicate understanding of probability and model limits.
- Check, revise, or improve work about probability and model limits using stated success criteria.
...
- Science brief: inheritance predictions are probabilities: they describe expected proportions across many offspring.
- Science brief: small samples depart from expected proportions by chance, and that departure is not evidence against the model.
- Science brief: a probability model is tested against large numbers of outcomes, not against individual cases.

### Evidence check: probability and model limits

**Candidate claim:** A case report presents this as acceptable science: “treating a three-to-one expectation as a promise about four offspring.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: inheritance predictions are probabilities: they describe expected proportions across many offspring. |
| E2 | established science reference | Science brief: small samples depart from expected proportions by chance, and that departure is not evidence against the model. |
| E3 | established science reference | Science brief: a probability model is tested against large numbers of outcomes, not against individual cases. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g8-science-u03::probability and model limits, derived from the reviewed correctness source policy/correctness/ma-g8-science.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about probability and model limits, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about probability and model limits, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about probability and model limits; then identify one check that would catch an error or weak claim.**
...
**Q1.** Locate the exact phrase in the candidate claim that conflicts with E1-E3. Name the decisive evidence ID, explain the error about probability and model limits, and write a corrected claim.

>

**Q2.** Communicate what the supplied record establishes about probability and model limits in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E3 and mark where the representation stops being supported.

>

**Q3.** Check your response about probability and model limits against each success criterion. Revise one sentence by adding or changing a citation to E1-E3; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using probability and model limits as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about probability and model limits? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about probability and model limits that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about probability and model limits. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about probability and model limits.
...
- Apply probability and model limits under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect probability and model limits to sexual and asexual reproduction from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g8-science-u03-l12` · lesson `ma-g8-science-u03-l12` · built from `canonical-1.0.0` at `4056e31`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
