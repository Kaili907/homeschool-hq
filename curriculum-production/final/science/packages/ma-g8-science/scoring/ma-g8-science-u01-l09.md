# Scoring authority — Performance task build: motion graphs

**Grade 8 Science** · Unit 1: Forces, Motion, and Interactions · Day 9 · lesson `ma-g8-science-u01-l09`
...
_Topic `ma-g8-science-u01::motion graphs` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g8-science.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- On a distance-time graph the slope represents speed; a horizontal line means the object is not moving.

**Accepted relationships and models.** What this lesson's learning target asserts:

- On a distance-time graph, a steeper slope means a greater speed and a flat line means no motion.
- A straight line on a distance-time graph means constant speed; a curve means the speed is changing.
- A graph shows only what was measured; the axes and their units determine what can be read from it.

**Also correct.** Accept any of these framings — do not require the wording above:

- Position-time and distance-time graphs are both acceptable here provided the axes are labelled.
...
- Reading a distance-time graph as though it showed the path the object travelled through space.
- Claiming a horizontal line on a distance-time graph means motion at a constant non-zero speed rather than rest.
- Reading a value off a graph with unlabelled axes.

**Grade boundary.** Velocity-time graphs, area under the curve, and acceleration from slope are High School work.
...
**Q1 — build product.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies motion graphs and the unit standards.
...
**Q2 — communicate representation.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of motion graphs.
...
**Q3 — check revise.** _Serves objective 3:_ Check, revise, or improve work about motion graphs using stated success criteria.
...
**Q4 — essential question.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies motion graphs and the unit standards.
...
**Q5 — evidence quality.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of motion graphs.
...
**Q6 — limitation.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies motion graphs and the unit standards.
...
**Q7 — provenance.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of motion graphs.
...
- **prerequisite gap** — Return to the smallest prerequisite needed for motion graphs, model it with a concrete or text-only representation, then retry one fresh item.
- **procedure without understanding** — Ask the learner to explain or represent why the process for motion graphs works before continuing.
- **correct but low confidence** — Confirm the reasoning specifically, offer one varied example of motion graphs, and avoid unnecessary remediation.
...
- **mastery evidence** — Require accurate independent application plus explanation on a later or different occasion before marking motion graphs mastered.
...
_Package `swp-ma-g8-science-u01-l09` · built from `canonical-1.0.0` at `4056e31`._
