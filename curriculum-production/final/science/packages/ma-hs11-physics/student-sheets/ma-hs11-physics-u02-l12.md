# Correction and reflection: analysing force data

**High School Physics (Grade 11)** · Unit 2: Forces and Newton's Second Law · Day 12 of the unit (course day 24) · about 55–75 minutes

**Today's focus:** analysing force data
...
- Explain analysing force data using evidence, a model, or a documented process rather than an unsupported answer.
- Apply analysing force data to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on analysing force data against the stated success criteria and name the next step.
...
- Science brief: force data is interpreted against a stated model, and the model predicts the shape of the relationship as well as its values.
- Science brief: a linear relationship between net force and acceleration supports the second law; departures indicate an unaccounted force such as friction.
- Science brief: the intercept and the scatter of a fitted line carry information about systematic and random error.

### Evidence check: analysing force data

**Candidate claim:** A case report presents this as acceptable science: “fitting a line through force-acceleration data and ignoring a substantial non-zero intercept.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: force data is interpreted against a stated model, and the model predicts the shape of the relationship as well as its values. |
| E2 | established science reference | Science brief: a linear relationship between net force and acceleration supports the second law; departures indicate an unaccounted force such as friction. |
| E3 | established science reference | Science brief: the intercept and the scatter of a fitted line carry information about systematic and random error. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs11-physics-u02::analysing force data, derived from the reviewed correctness source policy/correctness/ma-hs11-physics.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about analysing force data, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about analysing force data, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about analysing force data, then name one check that would catch an error or a weak claim.**
...
**Q1.** Explain analysing force data in your own words using at least two specifically named rows from E1-E3. Then explain why the candidate claim does or does not follow.

>

**Q2.** Treat the printed candidate claim as the fresh case for analysing force data. Decide whether E1-E3 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q3.** Check the conclusion about analysing force data, revise its weakest claim with a named row from E1-E3, and name the next observation or model input that would resolve the remaining limitation.

>

**Q4.** Answer the printed unit question using analysing force data as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about analysing force data? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about analysing force data that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about analysing force data. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about analysing force data.
...
- Apply analysing force data under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect analysing force data to free-body diagrams from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs11-physics-u02-l12` · lesson `ma-hs11-physics-u02-l12` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
