# Correction and reflection: vectors and components

**High School Physics (Grade 11)** · Unit 1: Motion, Measurement, and Quantitative Reasoning · Day 12 of the unit (course day 12) · about 55–75 minutes

**Today's focus:** vectors and components
...
- Explain vectors and components using evidence, a model, or a documented process rather than an unsupported answer.
- Apply vectors and components to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on vectors and components against the stated success criteria and name the next step.
...
- Science brief: vectors add by components or head-to-tail; they do not add by simple arithmetic unless they are along the same line.
- Science brief: a vector is fully specified by magnitude and direction and can be resolved into perpendicular components.
- Science brief: perpendicular components are independent: motion or force in one direction does not affect the other.
- Science brief: the magnitude of a vector sum is generally less than the sum of the magnitudes, and equals it only when the vectors are parallel.

### Evidence check: vectors and components

**Candidate claim:** A case report presents this as acceptable science: “adding vector magnitudes arithmetically when the vectors are not along the same line.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: vectors add by components or head-to-tail; they do not add by simple arithmetic unless they are along the same line. |
| E2 | established science reference | Science brief: a vector is fully specified by magnitude and direction and can be resolved into perpendicular components. |
| E3 | established science reference | Science brief: perpendicular components are independent: motion or force in one direction does not affect the other. |
| E4 | established science reference | Science brief: the magnitude of a vector sum is generally less than the sum of the magnitudes, and equals it only when the vectors are parallel. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs11-physics-u01::vectors and components, derived from the reviewed correctness source policy/correctness/ma-hs11-physics.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about vectors and components, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about vectors and components, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about vectors and components, then name one check that would catch an error or a weak claim.**
...
**Q1.** Explain vectors and components in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q2.** Treat the printed candidate claim as the fresh case for vectors and components. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q3.** Check the conclusion about vectors and components, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q4.** Answer the printed unit question using vectors and components as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E4 is strongest for evaluating the candidate claim about vectors and components? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E4, state one conclusion about vectors and components that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about vectors and components. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about vectors and components.
...
- Apply vectors and components under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect vectors and components to position, displacement, and reference frames from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs11-physics-u01-l12` · lesson `ma-hs11-physics-u01-l12` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
