# Reteach and varied practice: net force

**High School Physics (Grade 11)** · Unit 2: Forces and Newton's Second Law · Day 8 of the unit (course day 20) · about 55–75 minutes

**Today's focus:** net force
...
- Explain net force using evidence, a model, or a documented process rather than an unsupported answer.
- Apply net force to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on net force against the stated success criteria and name the next step.
...
- Science brief: net force is the vector sum of all forces acting on an object, and zero net force means zero acceleration.
- Science brief: an object with zero net force is either at rest or moving with constant velocity; both are equilibrium states.
- Science brief: net force determines acceleration, not velocity: a large net force on a slow object is entirely possible.
- Science brief: forces are summed as vectors, so perpendicular components are treated independently.

### Evidence check: net force

**Candidate claim:** A case report presents this as acceptable science: “an object moving at constant velocity must have a net force acting on it.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: net force is the vector sum of all forces acting on an object, and zero net force means zero acceleration. |
| E2 | established science reference | Science brief: an object with zero net force is either at rest or moving with constant velocity; both are equilibrium states. |
| E3 | established science reference | Science brief: net force determines acceleration, not velocity: a large net force on a slow object is entirely possible. |
| E4 | established science reference | Science brief: forces are summed as vectors, so perpendicular components are treated independently. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs11-physics-u02::net force, derived from the reviewed correctness source policy/correctness/ma-hs11-physics.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about net force, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about net force, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about net force, then name one check that would catch an error or a weak claim.**
...
**Q1.** Explain net force in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q2.** Treat the printed candidate claim as the fresh case for net force. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q3.** Check the conclusion about net force, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q4.** Answer the printed unit question using net force as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E4 is strongest for evaluating the candidate claim about net force? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E4, state one conclusion about net force that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about net force. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about net force.
...
- Apply net force under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect net force to free-body diagrams from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs11-physics-u02-l08` · lesson `ma-hs11-physics-u02-l08` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
