# Unit assessment: field lines as representations

**High School Physics (Grade 11)** · Unit 4: Gravitation and Electrostatic Forces · Day 11 of the unit (course day 47) · about 55–75 minutes

**Today's focus:** field lines as representations
...
- Explain field lines as representations using evidence, a model, or a documented process rather than an unsupported answer.
- Apply field lines as representations to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on field lines as representations against the stated success criteria and name the next step.
...
- Science brief: field lines are a representation of a field's direction and relative strength; they are not physical objects.
- Science brief: the direction of a field line at a point gives the direction of the force on a positive test charge, or on a test mass, placed there.
- Science brief: closer spacing of field lines represents a stronger field.
- Science brief: field lines never cross, because the field has one direction at each point.

### Evidence check: field lines as representations

**Candidate claim:** A case report presents this as acceptable science: “field lines are physically present in space.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: field lines are a representation of a field's direction and relative strength; they are not physical objects. |
| E2 | established science reference | Science brief: the direction of a field line at a point gives the direction of the force on a positive test charge, or on a test mass, placed there. |
| E3 | established science reference | Science brief: closer spacing of field lines represents a stronger field. |
| E4 | established science reference | Science brief: field lines never cross, because the field has one direction at each point. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs11-physics-u04::field lines as representations, derived from the reviewed correctness source policy/correctness/ma-hs11-physics.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about field lines as representations, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about field lines as representations, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about field lines as representations, then name one check that would catch an error or a weak claim.**
...
**Q1.** Without copying a sentence from the brief, evaluate the printed candidate case about field lines as representations. Cite E1-E4, write the reasoning in your own words, and correct the case claim so it fits exactly what the record establishes.

>

**Q2.** Explain field lines as representations in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q3.** Treat the printed candidate claim as the fresh case for field lines as representations. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q4.** Check the conclusion about field lines as representations, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q5.** Answer the printed unit question using field lines as representations as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q6.** Which one row in E1-E4 is strongest for evaluating the candidate claim about field lines as representations? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q7.** Using the supplied case and E1-E4, state one conclusion about field lines as representations that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q8.** Audit the inputs used in your response about field lines as representations. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about field lines as representations.
...
- Apply field lines as representations under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect field lines as representations to Newton's law of gravitation from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs11-physics-u04-l11` · lesson `ma-hs11-physics-u04-l11` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
