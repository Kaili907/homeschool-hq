# Correction and reflection: investigating induction

**High School Physics (Grade 11)** · Unit 7: Fields, Electromagnetism, and Induction · Day 12 of the unit (course day 84) · about 55–75 minutes

**Today's focus:** investigating induction
...
- Explain investigating induction using evidence, a model, or a documented process rather than an unsupported answer.
- Apply investigating induction to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on investigating induction against the stated success criteria and name the next step.
...
- Science brief: an induction investigation varies one factor at a time: relative speed, field strength, number of turns, or orientation.
- Science brief: detecting no induced effect when nothing is changing is a positive result confirming the model, not a failed trial.
- Science brief: measurements state the instrument, its sensitivity, and the conditions, since induced voltages are often small.

### Evidence check: investigating induction

**Candidate claim:** A case report presents this as acceptable science: “reporting an induced voltage under static conditions with no changing flux and attributing it to induction.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: an induction investigation varies one factor at a time: relative speed, field strength, number of turns, or orientation. |
| E2 | established science reference | Science brief: detecting no induced effect when nothing is changing is a positive result confirming the model, not a failed trial. |
| E3 | established science reference | Science brief: measurements state the instrument, its sensitivity, and the conditions, since induced voltages are often small. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs11-physics-u07::investigating induction, derived from the reviewed correctness source policy/correctness/ma-hs11-physics.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about investigating induction, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about investigating induction, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about investigating induction, then name one check that would catch an error or a weak claim.**
...
**Q1.** Explain investigating induction in your own words using at least two specifically named rows from E1-E3. Then explain why the candidate claim does or does not follow.

>

**Q2.** Treat the printed candidate claim as the fresh case for investigating induction. Decide whether E1-E3 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q3.** Check the conclusion about investigating induction, revise its weakest claim with a named row from E1-E3, and name the next observation or model input that would resolve the remaining limitation.

>

**Q4.** Answer the printed unit question using investigating induction as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about investigating induction? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about investigating induction that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about investigating induction. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about investigating induction.
...
- Apply investigating induction under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect investigating induction to electric and magnetic fields from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs11-physics-u07-l12` · lesson `ma-hs11-physics-u07-l12` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
