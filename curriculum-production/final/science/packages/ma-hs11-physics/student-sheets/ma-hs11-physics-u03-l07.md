# Investigation: momentum as mass times velocity

**High School Physics (Grade 11)** · Unit 3: Momentum, Collisions, and Protective Design · Day 7 of the unit (course day 31) · about 61–84 minutes

**Today's focus:** momentum as mass times velocity
...
- Explain momentum as mass times velocity using evidence, a model, or a documented process rather than an unsupported answer.
- Apply momentum as mass times velocity to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on momentum as mass times velocity against the stated success criteria and name the next step.

**Materials:**

- raw eggs or a small fragile object
- padding materials from the household
- measuring tape
- tray or tarp for the landing zone
- toy cars for the collision strand
...
- **Physical hazard.** Dropping objects from height risks falls and impacts.
  - What stops it: Drop from standing height at most, never from a ladder, chair, or window. The learner stands clear of the landing zone.
- **Physical hazard.** Raw egg is a salmonella exposure and a slip hazard.
  - What stops it: Contain every drop in a tray or tarp, wear gloves whenever anyone has a cut or graze, wash hands thoroughly after every trial without exception, disinfect the area, and never eat or drink in the work area.
...
- Science brief: momentum is mass times velocity, and it is a vector with the same direction as the velocity.
- Science brief: momentum depends on both mass and velocity, so a slow heavy object and a fast light one can carry the same momentum.
- Science brief: momentum is a vector, so its direction matters and momenta in opposite directions partly cancel.
- Science brief: momentum and kinetic energy are different quantities and are not interchangeable.

### Evidence check: momentum as mass times velocity

**Candidate claim:** A case report presents this as acceptable science: “treating momentum as a scalar and adding magnitudes regardless of direction.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: momentum is mass times velocity, and it is a vector with the same direction as the velocity. |
| E2 | established science reference | Science brief: momentum depends on both mass and velocity, so a slow heavy object and a fast light one can carry the same momentum. |
| E3 | established science reference | Science brief: momentum is a vector, so its direction matters and momenta in opposite directions partly cancel. |
| E4 | established science reference | Science brief: momentum and kinetic energy are different quantities and are not interchangeable. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs11-physics-u03::momentum as mass times velocity, derived from the reviewed correctness source policy/correctness/ma-hs11-physics.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about momentum as mass times velocity, and does it support the candidate claim?
...
| --- | --- | --- | --- | --- | --- |
| 1 | 1.0 | 3.0 | 1.0 | 0.0 | 1.5 |
| 2 | 2.0 | 3.0 | 1.0 | 0.0 | 2.0 |
| 3 | 1.0 | 2.0 | 1.0 | -1.0 | 0.5 |
| 4 | 0.5 | 4.0 | 1.5 | 0.0 | 1.0 |

**Model/source rule:** For a perfectly inelastic one-dimensional model, shared velocity is (m1v1+m2v2)/(m1+m2).
...
3. Verify momentum before and after, calculate kinetic energy before and after, and distinguish conservation of momentum from conservation of kinetic energy.
...
**Pinned H4 alternative statement (lineage evidence):** Use a sealed water balloon, a small beanbag, or a marshmallow instead of an egg - no biological hazard, no cleanup - or run the collision strand with toy cars only and analyse published crash-test data for the protective design. Both paths meet the same momentum and impulse target.
...
**My question.** Write the question you are testing about momentum as mass times velocity. It has to be a question your evidence could actually answer.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about momentum as mass times velocity, then name one check that would catch an error or a weak claim.**
...
**Q1.** Complete the route you chose. For the document/model route, use its printed procedure and input table; for the H4 physical route, use the printed safe order and your own blank record. Report the route, evidence IDs or model rows, conclusion about momentum as mass times velocity, uncertainty, and limitation.

>

**Q2.** Explain momentum as mass times velocity in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q3.** Treat the printed candidate claim as the fresh case for momentum as mass times velocity. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q4.** Check the conclusion about momentum as mass times velocity, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q5.** Answer the printed unit question using momentum as mass times velocity as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q6.** Which one row in E1-E4 is strongest for evaluating the candidate claim about momentum as mass times velocity? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q7.** Using the supplied case and E1-E4, state one conclusion about momentum as mass times velocity that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q8.** Audit the inputs used in your response about momentum as mass times velocity. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about momentum as mass times velocity.
...
- Apply momentum as mass times velocity under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect momentum as mass times velocity to conservation of momentum from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
- Take the unit's performance task — Design, test, and refine a device that minimises the force on an object during a collision, and use momentum and impulse reasoning plus your own trial data to justify each revision. — and write the one measurement or piece of evidence that would most change its outcome, then say why.
...
_Package `swp-ma-hs11-physics-u03-l07` · lesson `ma-hs11-physics-u03-l07` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
