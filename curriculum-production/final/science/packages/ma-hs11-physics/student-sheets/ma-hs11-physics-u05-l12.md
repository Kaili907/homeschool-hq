# Correction and reflection: building and checking an energy budget

**High School Physics (Grade 11)** · Unit 5: Energy: Modelling, Accounting, and Conservation · Day 12 of the unit (course day 60) · about 55–75 minutes

**Today's focus:** building and checking an energy budget
...
- Explain building and checking an energy budget using evidence, a model, or a documented process rather than an unsupported answer.
- Apply building and checking an energy budget to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on building and checking an energy budget against the stated success criteria and name the next step.
...
- Science brief: an energy budget lists the energy in the system at the start, the transfers in and out, and the energy at the end, and the two sides must agree within uncertainty.
- Science brief: a shortfall in the budget points to an unaccounted transfer, most often dissipation, and it is identified rather than absorbed into a residual.
- Science brief: every term in the budget carries the uncertainty of the measurements it came from.

### Evidence check: building and checking an energy budget

**Candidate claim:** A case report presents this as acceptable science: “balancing a budget by inserting an unexplained term.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: an energy budget lists the energy in the system at the start, the transfers in and out, and the energy at the end, and the two sides must agree within uncertainty. |
| E2 | established science reference | Science brief: a shortfall in the budget points to an unaccounted transfer, most often dissipation, and it is identified rather than absorbed into a residual. |
| E3 | established science reference | Science brief: every term in the budget carries the uncertainty of the measurements it came from. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs11-physics-u05::building and checking an energy budget, derived from the reviewed correctness source policy/correctness/ma-hs11-physics.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about building and checking an energy budget, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about building and checking an energy budget, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about building and checking an energy budget, then name one check that would catch an error or a weak claim.**
...
**Q1.** Explain building and checking an energy budget in your own words using at least two specifically named rows from E1-E3. Then explain why the candidate claim does or does not follow.

>

**Q2.** Treat the printed candidate claim as the fresh case for building and checking an energy budget. Decide whether E1-E3 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q3.** Check the conclusion about building and checking an energy budget, revise its weakest claim with a named row from E1-E3, and name the next observation or model input that would resolve the remaining limitation.

>

**Q4.** Answer the printed unit question using building and checking an energy budget as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about building and checking an energy budget? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about building and checking an energy budget that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about building and checking an energy budget. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about building and checking an energy budget.
...
- Apply building and checking an energy budget under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect building and checking an energy budget to kinetic and potential energy from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs11-physics-u05-l12` · lesson `ma-hs11-physics-u05-l12` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
