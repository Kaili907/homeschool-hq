# Correction and reflection: investigating energy distribution

**High School Physics (Grade 11)** · Unit 6: Thermal Energy and the Second Law · Day 12 of the unit (course day 72) · about 55–75 minutes

**Today's focus:** investigating energy distribution
...
- Explain investigating energy distribution using evidence, a model, or a documented process rather than an unsupported answer.
- Apply investigating energy distribution to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on investigating energy distribution against the stated success criteria and name the next step.
...
- Science brief: investigating thermal behaviour requires stating the system, its boundary, and the transfers that cross it.
- Science brief: measured temperature changes are evidence about energy transfer only when mass and material are also known.
- Science brief: thermal transfer to the environment is present in every real setup, so it belongs in the account rather than being assumed negligible without checking.

### Evidence check: investigating energy distribution

**Candidate claim:** A case report presents this as acceptable science: “concluding energy was not conserved from an incomplete account of transfers to the surroundings.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: investigating thermal behaviour requires stating the system, its boundary, and the transfers that cross it. |
| E2 | established science reference | Science brief: measured temperature changes are evidence about energy transfer only when mass and material are also known. |
| E3 | established science reference | Science brief: thermal transfer to the environment is present in every real setup, so it belongs in the account rather than being assumed negligible without checking. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs11-physics-u06::investigating energy distribution, derived from the reviewed correctness source policy/correctness/ma-hs11-physics.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about investigating energy distribution, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about investigating energy distribution, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about investigating energy distribution, then name one check that would catch an error or a weak claim.**
...
**Q1.** Explain investigating energy distribution in your own words using at least two specifically named rows from E1-E3. Then explain why the candidate claim does or does not follow.

>

**Q2.** Treat the printed candidate claim as the fresh case for investigating energy distribution. Decide whether E1-E3 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q3.** Check the conclusion about investigating energy distribution, revise its weakest claim with a named row from E1-E3, and name the next observation or model input that would resolve the remaining limitation.

>

**Q4.** Answer the printed unit question using investigating energy distribution as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about investigating energy distribution? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about investigating energy distribution that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about investigating energy distribution. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about investigating energy distribution.
...
- Apply investigating energy distribution under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect investigating energy distribution to temperature versus thermal energy from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs11-physics-u06-l12` · lesson `ma-hs11-physics-u06-l12` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
