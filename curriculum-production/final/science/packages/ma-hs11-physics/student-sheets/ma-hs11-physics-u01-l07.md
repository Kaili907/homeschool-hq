# Investigation: position, displacement, and reference frames

**High School Physics (Grade 11)** · Unit 1: Motion, Measurement, and Quantitative Reasoning · Day 7 of the unit (course day 7) · about 61–84 minutes

**Today's focus:** position, displacement, and reference frames
...
- Plan and carry out a safe investigation of position, displacement, and reference frames, or its stated alternative, and record the learner's own data with its uncertainty.
- Explain position, displacement, and reference frames using evidence, a model, or a documented process rather than an unsupported answer.
- Apply position, displacement, and reference frames to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on position, displacement, and reference frames against the stated success criteria and name the next step.

**Materials:**

- board or stiff cardboard for a ramp
- ball or toy car
- measuring tape
- stopwatch or phone timer
- books to set ramp height
- masking tape
- notebook
...
- **Physical hazard.** A ball leaving a ramp can trip someone or strike an object.
  - What stops it: Set the ramp so the run ends against a soft barrier, clear the floor path, keep the raised end no higher than 30 cm, use a rolling object under 200 g, and keep pets and young children out of the area.
...
**Clearing up.** Take the ramp down, peel off the masking tape, and put the ball and books away. Nothing hazardous is produced or left out.
...
- Science brief: displacement is a vector from start point to end point; distance is the scalar path length travelled.
- Science brief: position is stated relative to a chosen origin and reference frame, and the choice must be stated before positions mean anything.
- Science brief: displacement depends only on start and end points; distance depends on the whole path, so the two differ whenever the path is not straight.
- Science brief: motion described in one reference frame may be described quite differently in another, and neither description is wrong.

### Evidence check: position, displacement, and reference frames

**Candidate claim:** A case report presents this as acceptable science: “reporting displacement as equal to distance for a path that is not straight and one-directional.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: displacement is a vector from start point to end point; distance is the scalar path length travelled. |
| E2 | established science reference | Science brief: position is stated relative to a chosen origin and reference frame, and the choice must be stated before positions mean anything. |
| E3 | established science reference | Science brief: displacement depends only on start and end points; distance depends on the whole path, so the two differ whenever the path is not straight. |
| E4 | established science reference | Science brief: motion described in one reference frame may be described quite differently in another, and neither description is wrong. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs11-physics-u01::position, displacement, and reference frames, derived from the reviewed correctness source policy/correctness/ma-hs11-physics.correctness.json.

**Supplied unit connections:** position, displacement, and reference frames; velocity and acceleration; motion graphs and their slopes; uniform acceleration relationships; measurement uncertainty in motion; vectors and components

**Supplied unit performance task:** Measure a real accelerating motion, produce position, velocity, and acceleration representations from your own data, and report the acceleration with an honest uncertainty.

### Bound task

**Question:** What does evidence E1 through E4 justify about position, displacement, and reference frames, and does it support the candidate claim?
...
**Model/source rule:** Ground-frame position is 0.2t²+1.0t; the cart frame moves at 0.5 m/s, so x(cart)=x(ground)-0.5t.
...
3. Compute interval velocities and changes in velocity in both frames, state the sign convention, and explain which quantities change with reference frame.
...
**Pinned H4 alternative statement (lineage evidence):** Analyse a published motion dataset or a frame-numbered motion diagram supplied on paper, extracting velocity and acceleration by hand. No ramp, no timing, paper only.
...
**My question.** Write the question you are testing about position, displacement, and reference frames. It has to be a question your evidence could actually answer.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about position, displacement, and reference frames, then name one check that would catch an error or a weak claim.**
...
**Q1.** Complete the route you chose. For the document/model route, use its printed procedure and input table; for the H4 physical route, use the printed safe order and your own blank record. Report the route, evidence IDs or model rows, conclusion about position, displacement, and reference frames, uncertainty, and limitation.

>

**Q2.** Explain position, displacement, and reference frames in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q3.** Treat the printed candidate claim as the fresh case for position, displacement, and reference frames. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q4.** Check the conclusion about position, displacement, and reference frames, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q5.** Answer the printed unit question using position, displacement, and reference frames as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q6.** Which one row in E1-E4 is strongest for evaluating the candidate claim about position, displacement, and reference frames? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q7.** Using the supplied case and E1-E4, state one conclusion about position, displacement, and reference frames that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q8.** Audit the inputs used in your response about position, displacement, and reference frames. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about position, displacement, and reference frames.
...
- Apply position, displacement, and reference frames under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect position, displacement, and reference frames to velocity and acceleration from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
- Take the unit's performance task — Measure a real accelerating motion, produce position, velocity, and acceleration representations from your own data, and report the acceleration with an honest uncertainty. — and write the one measurement or piece of evidence that would most change its outcome, then say why.
...
_Package `swp-ma-hs11-physics-u01-l07` · lesson `ma-hs11-physics-u01-l07` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
