# Scoring authority — Reteach and varied practice: net force

**High School Physics (Grade 11)** · Unit 2: Forces and Newton's Second Law · Day 8 · lesson `ma-hs11-physics-u02-l08`
...
_Topic `ma-hs11-physics-u02::net force` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Net force is the vector sum of all forces acting on an object, and zero net force means zero acceleration.

**Accepted relationships and models.** What this lesson's learning target asserts:

- An object with zero net force is either at rest or moving with constant velocity; both are equilibrium states.
- Net force determines acceleration, not velocity: a large net force on a slow object is entirely possible.
- Forces are summed as vectors, so perpendicular components are treated independently.
...
- Claiming an object moving at constant velocity must have a net force acting on it.
- Claiming net force determines the direction of motion rather than the direction of acceleration.
- Adding force magnitudes arithmetically when the forces are not collinear.

**Grade boundary.** Torque, rotational equilibrium, and extended-body statics are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain net force using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply net force to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on net force against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply net force to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain net force using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply net force to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain net force using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for net force (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for net force (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for net force (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for net force (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for net force (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u02-l08` · built from `hs912-science-h4` at `a86780a`._
