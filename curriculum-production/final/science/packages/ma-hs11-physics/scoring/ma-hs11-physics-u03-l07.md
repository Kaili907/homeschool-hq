# Scoring authority — Investigation: momentum as mass times velocity

**High School Physics (Grade 11)** · Unit 3: Momentum, Collisions, and Protective Design · Day 7 · lesson `ma-hs11-physics-u03-l07`
...
_Topic `ma-hs11-physics-u03::momentum as mass times velocity` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Momentum is mass times velocity, and it is a vector with the same direction as the velocity.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Momentum depends on both mass and velocity, so a slow heavy object and a fast light one can carry the same momentum.
- Momentum is a vector, so its direction matters and momenta in opposite directions partly cancel.
- Momentum and kinetic energy are different quantities and are not interchangeable.
...
- Treating momentum as a scalar and adding magnitudes regardless of direction.
- Using momentum and kinetic energy as though they were the same quantity.
- Claiming momentum depends on speed alone.

**Grade boundary.** Relativistic momentum and angular momentum are beyond this course.
...
**Q1 — investigate.** _Serves objective 1:_ Plan and carry out a safe investigation of momentum as mass times velocity, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain momentum as mass times velocity using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply momentum as mass times velocity to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on momentum as mass times velocity against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply momentum as mass times velocity to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain momentum as mass times velocity using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 1:_ Plan and carry out a safe investigation of momentum as mass times velocity, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q8 — provenance.** _Serves objective 1:_ Plan and carry out a safe investigation of momentum as mass times velocity, or its stated alternative, and record the learner's own data with its uncertainty.
...
- **prerequisite-gap** — Run the prerequisite reteach route for momentum as mass times velocity (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for momentum as mass times velocity (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for momentum as mass times velocity (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for momentum as mass times velocity (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for momentum as mass times velocity (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Dropping objects from height risks falls and impacts. — mitigation: Drop from standing height at most, never from a ladder, chair, or window. The learner stands clear of the landing zone.
- physical: Raw egg is a salmonella exposure and a slip hazard. — mitigation: Contain every drop in a tray or tarp, wear gloves whenever anyone has a cut or graze, wash hands thoroughly after every trial without exception, disinfect the area, and never eat or drink in the work area.
...
_Package `swp-ma-hs11-physics-u03-l07` · built from `hs912-science-h4` at `a86780a`._
