# Scoring authority — Correction and reflection: investigating energy distribution

**High School Physics (Grade 11)** · Unit 6: Thermal Energy and the Second Law · Day 12 · lesson `ma-hs11-physics-u06-l12`
...
_Topic `ma-hs11-physics-u06::investigating energy distribution` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Investigating thermal behaviour requires stating the system, its boundary, and the transfers that cross it.
- Measured temperature changes are evidence about energy transfer only when mass and material are also known.
- Thermal transfer to the environment is present in every real setup, so it belongs in the account rather than being assumed negligible without checking.
...
- Concluding energy was not conserved from an incomplete account of transfers to the surroundings.
- Comparing temperature changes for different masses or materials as though they showed equal energy transfers.
- Assuming a container is perfectly insulating with no evidence.

**Grade boundary.** Calorimeter constant determination and formal uncertainty budgets are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain investigating energy distribution using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply investigating energy distribution to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on investigating energy distribution against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply investigating energy distribution to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain investigating energy distribution using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply investigating energy distribution to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain investigating energy distribution using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for investigating energy distribution (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for investigating energy distribution (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for investigating energy distribution (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for investigating energy distribution (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for investigating energy distribution (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u06-l12` · built from `hs912-science-h4` at `a86780a`._
