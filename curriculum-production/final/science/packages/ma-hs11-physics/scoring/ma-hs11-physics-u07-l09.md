# Scoring authority — Performance task build: electromagnetic induction

**High School Physics (Grade 11)** · Unit 7: Fields, Electromagnetism, and Induction · Day 9 · lesson `ma-hs11-physics-u07-l09`
...
_Topic `ma-hs11-physics-u07::electromagnetic induction` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- A voltage is induced in a circuit only when the magnetic flux through it is changing.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Relative motion between a conductor and a magnetic field, or a changing field strength, changes the flux and induces a voltage.
- A steady magnetic field with no relative motion induces nothing, however strong it is.
- The induced effect opposes the change that produced it, which is why generators require work to turn.

**Also correct.** Accept any of these framings — do not require the wording above:

- Faraday's and Lenz's descriptions, whether named or described in plain terms, are equally correct.
...
- Claiming a stationary magnet held near a stationary coil induces a steady voltage.
- Claiming induction produces energy rather than converting mechanical work into electrical energy.
- Claiming the induced effect reinforces the change that produced it.

**Grade boundary.** Quantitative flux calculation and transformer design equations are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for electromagnetic induction that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain electromagnetic induction using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply electromagnetic induction to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on electromagnetic induction against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply electromagnetic induction to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain electromagnetic induction using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply electromagnetic induction to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for electromagnetic induction that a reader could check independently.
...
- The learner completes the central task about electromagnetic induction.
...
- **prerequisite-gap** — Run the prerequisite reteach route for electromagnetic induction (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for electromagnetic induction (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for electromagnetic induction (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for electromagnetic induction (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for electromagnetic induction (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u07-l09` · built from `hs912-science-h4` at `a86780a`._
