# Scoring authority — Investigation: the electromagnetic spectrum

**High School Physics (Grade 11)** · Unit 9: Electromagnetic Radiation, Information, and a Physics Engineering Capstone · Day 7 · lesson `ma-hs11-physics-u09-l07`
...
_Topic `ma-hs11-physics-u09::the electromagnetic spectrum` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- In order of increasing frequency and decreasing wavelength the spectrum runs radio, microwave, infrared, visible, ultraviolet, X-ray, gamma.
- All electromagnetic waves travel at the same speed in vacuum, approximately three times ten to the eighth metres per second.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The regions of the spectrum differ in frequency and wavelength, not in their fundamental nature.
- Photon energy increases with frequency, so gamma rays carry far more energy per photon than radio waves.
- Only the higher-frequency regions, from roughly ultraviolet upward, carry enough energy per photon to ionise atoms.

**Also correct.** Accept any of these framings — do not require the wording above:

- Ordering the spectrum by wavelength or by frequency are equally correct, provided the direction is stated.
...
- Placing the spectrum regions out of order, such as putting ultraviolet below visible in frequency.
- Claiming different regions of the spectrum travel at different speeds in vacuum.
- Claiming radio waves or microwaves are ionising radiation.

**Grade boundary.** Quantitative photon energy calculation is acceptable but not required.
...
- Named data-source resource: `res-ma-hs11-physics-data-sources` (scope: lesson).

Provenance the curriculum source declares for this lesson, verbatim:

> Learner's own signal-strength observations across shielding materials and their own device performance trials. Published claims are evaluated, never restated as findings.
...
**Q1 — investigate.** _Serves objective 1:_ Plan and carry out a safe investigation of the electromagnetic spectrum, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain the electromagnetic spectrum using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply the electromagnetic spectrum to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on the electromagnetic spectrum against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply the electromagnetic spectrum to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain the electromagnetic spectrum using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 1:_ Plan and carry out a safe investigation of the electromagnetic spectrum, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q8 — provenance.** _Serves objective 1:_ Plan and carry out a safe investigation of the electromagnetic spectrum, or its stated alternative, and record the learner's own data with its uncertainty.
...
- **prerequisite-gap** — Run the prerequisite reteach route for the electromagnetic spectrum (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for the electromagnetic spectrum (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for the electromagnetic spectrum (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for the electromagnetic spectrum (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for the electromagnetic spectrum (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Building a device involves cutting and small parts. — mitigation: An adult performs all cutting with sharp tools, the learner uses blunt scissors or pre-cut parts, and small parts stay away from young children.
- physical: Aluminium foil laid across both terminals of a cell is a short circuit and a standard ignition demonstration. — mitigation: Foil is used for shielding only and never touches both terminals of any cell. The shielding strand runs first and uses no electricity at all.
- physical: Battery-powered builds can short and heat, and a stalled motor draws its full stall current. — mitigation: Single 1.5 V cells only - never mains electricity, never a lithium or button cell. Tape the terminals before and after use, disconnect between tests, disconnect immediately if a motor stalls or will not turn, and stop if anything warms.
- online: Evaluating published radiation claims means reading sources of uncertain quality. — mitigation: An adult approves each source in advance. No account creation, sign-in, purchase, or personal information is required, and the learner never contacts a site owner.
...
**Disposal and clearing up:** Disconnect and remove the cell and tape its terminals; used cells go to household battery recycling in a labelled container, never the general bin. Sweep up off-cuts and small parts and bin them away from young children. Recycle the foil and cardboard. Nothing is left connected or stored with a cell in it.
...
_Package `swp-ma-hs11-physics-u09-l07` · built from `hs912-science-h4` at `a86780a`._
