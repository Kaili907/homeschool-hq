# Scoring authority — Performance task build: impulse and contact time

**High School Physics (Grade 11)** · Unit 3: Momentum, Collisions, and Protective Design · Day 9 · lesson `ma-hs11-physics-u03-l09`
...
_Topic `ma-hs11-physics-u03::impulse and contact time` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Impulse is force multiplied by the time it acts, and it equals the change in momentum.

**Accepted relationships and models.** What this lesson's learning target asserts:

- For a given change in momentum, extending the contact time reduces the average force.
- This is why crumple zones, airbags, helmets, and catching techniques reduce peak force, without reducing the momentum change.
- Impulse is a vector in the direction of the average force.
...
- Claiming a protective device reduces the change in momentum rather than the peak force.
- Claiming impulse and force are the same quantity.
- Claiming a longer contact time increases the force for the same momentum change.

**Grade boundary.** Force-time curve integration for non-constant forces is acceptable but not required.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for impulse and contact time that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain impulse and contact time using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply impulse and contact time to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on impulse and contact time against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply impulse and contact time to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain impulse and contact time using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply impulse and contact time to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for impulse and contact time that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for impulse and contact time (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for impulse and contact time (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for impulse and contact time (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for impulse and contact time (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for impulse and contact time (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u03-l09` · built from `hs912-science-h4` at `a86780a`._
