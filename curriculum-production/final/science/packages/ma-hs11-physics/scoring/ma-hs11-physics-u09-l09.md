# Scoring authority — Performance task build: digital versus analog information

**High School Physics (Grade 11)** · Unit 9: Electromagnetic Radiation, Information, and a Physics Engineering Capstone · Day 9 · lesson `ma-hs11-physics-u09-l09`
...
_Topic `ma-hs11-physics-u09::digital versus analog information` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Digital signals encode information in discrete values; analog signals vary continuously.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Noise added to an analog signal becomes part of it, while a digital signal can be restored by rounding each value to the nearest allowed level.
- That restorability is why digital signals survive repeated transmission and copying far better than analog ones.
- Digitising a continuous signal involves sampling and rounding, so it loses some information; how much depends on the sampling and the levels used.
...
- Claiming digital signals are unaffected by noise or loss.
- Claiming digitisation preserves the original signal exactly.
- Claiming an analog signal can be cleaned of accumulated noise as a digital one can.

**Grade boundary.** Sampling theorem, quantisation error analysis, and coding theory are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for digital versus analog information that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain digital versus analog information using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply digital versus analog information to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on digital versus analog information against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply digital versus analog information to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain digital versus analog information using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply digital versus analog information to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for digital versus analog information that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for digital versus analog information (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for digital versus analog information (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for digital versus analog information (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for digital versus analog information (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for digital versus analog information (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u09-l09` · built from `hs912-science-h4` at `a86780a`._
