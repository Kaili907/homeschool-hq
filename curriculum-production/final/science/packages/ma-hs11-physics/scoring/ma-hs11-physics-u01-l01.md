# Scoring authority — Launch and diagnostic: position, displacement, and reference frames

**High School Physics (Grade 11)** · Unit 1: Motion, Measurement, and Quantitative Reasoning · Day 1 · lesson `ma-hs11-physics-u01-l01`
...
_Topic `ma-hs11-physics-u01::position, displacement, and reference frames` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._
...
**Q1 — phenomenon question.** _Serves objective 1:_ Observe the anchoring phenomenon, surface initial thinking about position, displacement, and reference frames without penalty, and pose a testable question.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain position, displacement, and reference frames using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply position, displacement, and reference frames to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on position, displacement, and reference frames against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply position, displacement, and reference frames to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain position, displacement, and reference frames using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply position, displacement, and reference frames to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Observe the anchoring phenomenon, surface initial thinking about position, displacement, and reference frames without penalty, and pose a testable question.
...
- **prerequisite-gap** — Run the prerequisite reteach route for position, displacement, and reference frames (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for position, displacement, and reference frames (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for position, displacement, and reference frames (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for position, displacement, and reference frames (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for position, displacement, and reference frames (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u01-l01` · built from `hs912-science-h4` at `a86780a`._
