# Scoring authority — Launch and diagnostic: kinetic and potential energy

**High School Physics (Grade 11)** · Unit 5: Energy: Modelling, Accounting, and Conservation · Day 1 · lesson `ma-hs11-physics-u05-l01`
...
_Topic `ma-hs11-physics-u05::kinetic and potential energy` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Kinetic energy depends on mass and on the square of the speed; gravitational potential energy near Earth's surface depends on mass, gravitational field strength, and height above a chosen reference.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Doubling an object's speed quadruples its kinetic energy, which is why stopping distance grows so steeply with speed.
- Potential energy is defined relative to a chosen reference level, and only differences in it are physically meaningful.
- Potential energy belongs to a system of interacting objects, such as an object and Earth, not to one object alone.
...
- Claiming kinetic energy is proportional to speed rather than to its square.
- Reporting a potential energy without stating the reference level.
- Claiming potential energy is stored inside a single object independently of any interaction.

**Grade boundary.** Elastic potential energy formulas and relativistic kinetic energy are acceptable but not required.


## What a complete response contains, question by question

**Q1 — phenomenon question.** _Serves objective 1:_ Observe the anchoring phenomenon, surface initial thinking about kinetic and potential energy without penalty, and pose a testable question.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain kinetic and potential energy using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply kinetic and potential energy to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on kinetic and potential energy against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply kinetic and potential energy to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain kinetic and potential energy using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply kinetic and potential energy to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Observe the anchoring phenomenon, surface initial thinking about kinetic and potential energy without penalty, and pose a testable question.
...
- **prerequisite-gap** — Run the prerequisite reteach route for kinetic and potential energy (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for kinetic and potential energy (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for kinetic and potential energy (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for kinetic and potential energy (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for kinetic and potential energy (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u05-l01` · built from `hs912-science-h4` at `a86780a`._
