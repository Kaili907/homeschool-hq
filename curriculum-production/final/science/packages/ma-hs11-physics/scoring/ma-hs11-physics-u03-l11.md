# Scoring authority — Unit assessment: collision analysis from data

**High School Physics (Grade 11)** · Unit 3: Momentum, Collisions, and Protective Design · Day 11 · lesson `ma-hs11-physics-u03-l11`
...
_Topic `ma-hs11-physics-u03::collision analysis from data` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Analysing a collision requires masses and velocities before and after, with directions, and each carries measurement uncertainty.
- A momentum total that does not balance within uncertainty indicates an unaccounted external force or a measurement problem, and both are investigated rather than assumed.
- Comparing kinetic energy before and after classifies the collision, and any energy shortfall is accounted for physically.
...
- Concluding momentum was not conserved without checking uncertainty or external forces.
- Omitting direction from velocities in a momentum calculation.
- Reporting a kinetic energy loss without identifying where the energy went.

**Grade boundary.** Video tracking analysis methods and statistical treatment of trial sets are beyond this course.


## What a complete response contains, question by question

**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for collision analysis from data on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain collision analysis from data using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply collision analysis from data to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on collision analysis from data against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply collision analysis from data to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain collision analysis from data using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply collision analysis from data to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for collision analysis from data on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for collision analysis from data (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for collision analysis from data (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for collision analysis from data (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for collision analysis from data (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for collision analysis from data (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u03-l11` · built from `hs912-science-h4` at `a86780a`._
