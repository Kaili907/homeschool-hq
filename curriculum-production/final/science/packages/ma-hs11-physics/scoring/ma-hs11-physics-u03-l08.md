# Scoring authority — Reteach and varied practice: conservation of momentum

**High School Physics (Grade 11)** · Unit 3: Momentum, Collisions, and Protective Design · Day 8 · lesson `ma-hs11-physics-u03-l08`
...
_Topic `ma-hs11-physics-u03::conservation of momentum` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- In the absence of a net external force, the total momentum of a system is unchanged by interactions within it.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Total momentum before an interaction equals total momentum after it, provided no net external force acts.
- Momentum conservation holds for every collision type, including those where kinetic energy is not conserved.
- Defining the system is part of applying the principle: a force is external or internal only with respect to a stated system.
...
- Claiming momentum is conserved only in elastic collisions.
- Applying momentum conservation to a system with a substantial unaccounted external force.
- Adding momenta as scalars and concluding momentum was not conserved.

**Grade boundary.** Centre-of-mass frame analysis and two-dimensional collision algebra are acceptable but not required.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain conservation of momentum using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply conservation of momentum to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on conservation of momentum against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply conservation of momentum to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain conservation of momentum using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply conservation of momentum to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain conservation of momentum using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for conservation of momentum (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for conservation of momentum (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for conservation of momentum (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for conservation of momentum (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for conservation of momentum (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u03-l08` · built from `hs912-science-h4` at `a86780a`._
