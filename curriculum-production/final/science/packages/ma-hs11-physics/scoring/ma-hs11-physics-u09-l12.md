# Scoring authority — Correction and reflection: energy conversion device design

**High School Physics (Grade 11)** · Unit 9: Electromagnetic Radiation, Information, and a Physics Engineering Capstone · Day 12 · lesson `ma-hs11-physics-u09-l12`
...
_Topic `ma-hs11-physics-u09::energy conversion device design` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- An energy conversion device is evaluated on measured input, measured useful output, and the difference between them.
- No device converts all input to useful output, so an efficiency claim of one hundred per cent indicates a measurement or accounting error.
- Design improvements are tested one change at a time against the stated criteria.
...
- Claiming a device produces more energy than it receives.
- Reporting an efficiency without stating what was measured as input and as useful output.
- Changing several design features at once and attributing the improvement to one.

**Grade boundary.** Thermodynamic efficiency limits for specific cycles and formal design optimisation are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain energy conversion device design using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply energy conversion device design to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on energy conversion device design against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply energy conversion device design to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain energy conversion device design using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply energy conversion device design to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain energy conversion device design using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for energy conversion device design (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for energy conversion device design (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for energy conversion device design (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for energy conversion device design (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for energy conversion device design (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u09-l12` · built from `hs912-science-h4` at `a86780a`._
