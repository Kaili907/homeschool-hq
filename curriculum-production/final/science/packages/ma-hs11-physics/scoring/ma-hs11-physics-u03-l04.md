# Scoring authority — Independent application A: elastic and inelastic collisions

**High School Physics (Grade 11)** · Unit 3: Momentum, Collisions, and Protective Design · Day 4 · lesson `ma-hs11-physics-u03-l04`
...
_Topic `ma-hs11-physics-u03::elastic and inelastic collisions` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._
...
- In an elastic collision total kinetic energy is unchanged; in an inelastic collision some is transferred to deformation, sound, and thermal energy.
- In a perfectly inelastic collision the objects move off together, and kinetic energy loss is at its maximum for that momentum.
- Real macroscopic collisions are never perfectly elastic, though some are close enough to model that way.
...
- Claiming kinetic energy is conserved in every collision.
- Claiming momentum is not conserved in an inelastic collision.
- Claiming energy is destroyed in an inelastic collision rather than transferred to other forms.

**Grade boundary.** Coefficient of restitution and elastic collision algebra in two dimensions are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain elastic and inelastic collisions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply elastic and inelastic collisions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on elastic and inelastic collisions against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply elastic and inelastic collisions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain elastic and inelastic collisions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply elastic and inelastic collisions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain elastic and inelastic collisions using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for elastic and inelastic collisions (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for elastic and inelastic collisions (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for elastic and inelastic collisions (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for elastic and inelastic collisions (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for elastic and inelastic collisions (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u03-l04` · built from `hs912-science-h4` at `a86780a`._
