# Scoring authority — Performance task build: computational energy models

**High School Physics (Grade 11)** · Unit 5: Energy: Modelling, Accounting, and Conservation · Day 9 · lesson `ma-hs11-physics-u05-l09`
...
_Topic `ma-hs11-physics-u05::computational energy models` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- A computational energy model steps a system forward using stated relationships, and its output depends on the assumptions and inputs supplied.
- The model must conserve energy unless a transfer out of the system is explicitly represented.
- Model output is a prediction to be compared with measurement, not a substitute for it.

**Also correct.** Accept any of these framings — do not require the wording above:

- Spreadsheet, code, and hand-iterated models are equally acceptable forms.
...
- Presenting model output as though it were measured data.
- Building a model in which energy is created or destroyed with no stated transfer.
- Reporting model results without stating the assumptions and inputs used.

**Grade boundary.** Numerical integration error analysis and simulation validation methods are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for computational energy models that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain computational energy models using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply computational energy models to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on computational energy models against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply computational energy models to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain computational energy models using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply computational energy models to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for computational energy models that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for computational energy models (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for computational energy models (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for computational energy models (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for computational energy models (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for computational energy models (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u05-l09` · built from `hs912-science-h4` at `a86780a`._
