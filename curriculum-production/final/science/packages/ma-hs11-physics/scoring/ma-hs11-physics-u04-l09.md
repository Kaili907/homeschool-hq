# Scoring authority — Performance task build: inverse-square relationships

**High School Physics (Grade 11)** · Unit 4: Gravitation and Electrostatic Forces · Day 9 · lesson `ma-hs11-physics-u04-l09`
...
_Topic `ma-hs11-physics-u04::inverse-square relationships` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- In an inverse-square relationship, doubling the distance reduces the quantity to one quarter.

**Accepted relationships and models.** What this lesson's learning target asserts:

- An inverse-square dependence arises whenever an influence spreads outward over a spherical surface whose area grows with the square of the radius.
- Doubling the distance quarters the quantity; tripling it reduces the quantity to one ninth.
- The relationship never reaches zero at finite distance, so the influence has no sharp boundary.
...
- Claiming doubling the distance halves an inverse-square quantity.
- Claiming an inverse-square force becomes exactly zero beyond some distance.
- Applying an inverse-square relationship to a quantity that does not spread over a spherical surface without justification.

**Grade boundary.** Solid angle formalism and flux integration are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for inverse-square relationships that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain inverse-square relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply inverse-square relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on inverse-square relationships against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply inverse-square relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain inverse-square relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply inverse-square relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for inverse-square relationships that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for inverse-square relationships (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for inverse-square relationships (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for inverse-square relationships (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for inverse-square relationships (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for inverse-square relationships (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u04-l09` · built from `hs912-science-h4` at `a86780a`._
