# Scoring authority — Reteach and varied practice: energy as a system-level account

**High School Physics (Grade 11)** · Unit 5: Energy: Modelling, Accounting, and Conservation · Day 8 · lesson `ma-hs11-physics-u05-l08`
...
_Topic `ma-hs11-physics-u05::energy as a system-level account` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Energy is conserved: the total energy of a closed system is constant, and energy transfers or transforms rather than being created or destroyed.

**Accepted relationships and models.** What this lesson's learning target asserts:

- An energy account requires a defined system and boundary, since what counts as a transfer depends on where the boundary lies.
- Energy changes form within the system and is transferred across the boundary by work, by heating, or by radiation.
- An account that does not balance indicates an unidentified transfer, not a failure of conservation.

**Also correct.** Accept any of these framings — do not require the wording above:

- Energy bar charts, tables, and equations are all acceptable forms for an energy account.
...
- Presenting an energy account in which energy is created or destroyed.
- Claiming energy is lost rather than transferred out of the system.
- Reporting an energy account with no system boundary stated.

**Grade boundary.** Mass-energy equivalence in nuclear processes and thermodynamic potentials are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain energy as a system-level account using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply energy as a system-level account to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on energy as a system-level account against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply energy as a system-level account to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain energy as a system-level account using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply energy as a system-level account to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain energy as a system-level account using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for energy as a system-level account (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for energy as a system-level account (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for energy as a system-level account (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for energy as a system-level account (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for energy as a system-level account (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u05-l08` · built from `hs912-science-h4` at `a86780a`._
