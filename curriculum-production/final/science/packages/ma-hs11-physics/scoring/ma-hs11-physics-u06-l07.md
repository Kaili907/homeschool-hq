# Scoring authority — Investigation: temperature versus thermal energy

**High School Physics (Grade 11)** · Unit 6: Thermal Energy and the Second Law · Day 7 · lesson `ma-hs11-physics-u06-l07`
...
_Topic `ma-hs11-physics-u06::temperature versus thermal energy` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Temperature measures the average kinetic energy of particles; thermal energy is the total energy of that random motion and also depends on how much material there is.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Two objects at the same temperature can hold very different amounts of thermal energy, because thermal energy depends on mass and material as well.
- Adding thermal energy to a substance raises its temperature, except during a change of state, when it changes particle arrangement instead.
- Temperature determines the direction of spontaneous thermal transfer; total thermal energy does not.
...
- Claiming the object with more thermal energy is necessarily at the higher temperature.
- Claiming temperature rises steadily while a substance is melting or boiling.

**Grade boundary.** Absolute temperature scales beyond conversion, and kinetic theory derivations, are beyond this course.
...
**Q1 — investigate.** _Serves objective 1:_ Plan and carry out a safe investigation of temperature versus thermal energy, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain temperature versus thermal energy using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply temperature versus thermal energy to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on temperature versus thermal energy against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply temperature versus thermal energy to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain temperature versus thermal energy using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 1:_ Plan and carry out a safe investigation of temperature versus thermal energy, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q8 — provenance.** _Serves objective 1:_ Plan and carry out a safe investigation of temperature versus thermal energy, or its stated alternative, and record the learner's own data with its uncertainty.
...
- **prerequisite-gap** — Run the prerequisite reteach route for temperature versus thermal energy (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for temperature versus thermal energy (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for temperature versus thermal energy (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for temperature versus thermal energy (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for temperature versus thermal energy (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Hot water scalds, and the risk rises with temperature. — mitigation: Keep all water below 50 degrees Celsius. An adult measures and pours every warm sample. The learner never carries an open container of warm water.
- physical: Thermometers can break; mercury thermometers must never be used. — mitigation: Use a digital or alcohol thermometer only. If any thermometer breaks, an adult clears it.
...
**Disposal and clearing up:** Pour the cooled water down the drain, dry the cups and thermos, and put the thermometer away in its case. If any thermometer breaks, an adult clears it: pieces are lifted with stiff card into a rigid container and binned separately, never with bare hands and never with a vacuum cleaner.
...
_Package `swp-ma-hs11-physics-u06-l07` · built from `hs912-science-h4` at `a86780a`._
