# Scoring authority — Independent application A: field models of interaction at a distance

**High School Physics (Grade 11)** · Unit 7: Fields, Electromagnetism, and Induction · Day 4 · lesson `ma-hs11-physics-u07-l04`
...
_Topic `ma-hs11-physics-u07::field models of interaction at a distance` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- The field model replaces action at a distance with a local interaction: the source establishes a field, and the field acts on whatever is present at a point.
- Changes in a field propagate at finite speed, not instantaneously.
- The field model applies to gravitational, electric, and magnetic interactions, with important differences in sign and in what responds.
...
- Claiming field changes propagate instantaneously across any distance.
- Claiming a field exists only where a second object is present to feel it.
- Treating gravitational and electric fields as identical in behaviour, ignoring that mass has one sign and charge has two.

**Grade boundary.** Field quantisation and retarded potentials are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain field models of interaction at a distance using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply field models of interaction at a distance to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on field models of interaction at a distance against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply field models of interaction at a distance to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain field models of interaction at a distance using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply field models of interaction at a distance to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain field models of interaction at a distance using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for field models of interaction at a distance (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for field models of interaction at a distance (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for field models of interaction at a distance (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for field models of interaction at a distance (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for field models of interaction at a distance (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u07-l04` · built from `hs912-science-h4` at `a86780a`._
