# Scoring authority — Performance task build: waves in different media

**High School Physics (Grade 11)** · Unit 8: Waves and Their Mathematics · Day 9 · lesson `ma-hs11-physics-u08-l09`
...
_Topic `ma-hs11-physics-u08::waves in different media` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._
...
- The speed of a mechanical wave is set by properties of the medium, principally its stiffness and its density.
- Light slows on entering a denser transparent medium and returns to its vacuum speed on leaving.
- The wave's frequency is a property of the source and does not change with the medium.
...
- Claiming sound travels faster in air than in water or steel.
- Claiming sound can travel through a vacuum.
- Claiming light travels faster in glass or water than in vacuum.

**Grade boundary.** Acoustic impedance and refractive index calculation are acceptable but not required.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for waves in different media that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain waves in different media using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply waves in different media to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on waves in different media against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply waves in different media to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain waves in different media using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply waves in different media to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for waves in different media that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for waves in different media (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for waves in different media (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for waves in different media (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for waves in different media (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for waves in different media (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u08-l09` · built from `hs912-science-h4` at `a86780a`._
