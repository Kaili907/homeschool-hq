# Scoring authority — Independent application A: reflection, refraction, and diffraction

**High School Physics (Grade 11)** · Unit 8: Waves and Their Mathematics · Day 4 · lesson `ma-hs11-physics-u08-l04`
...
_Topic `ma-hs11-physics-u08::reflection, refraction, and diffraction` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Reflection returns a wave from a boundary, refraction is the change of direction on entering a medium where the speed differs, and diffraction is the spreading of a wave past an edge or through a gap.

**Accepted relationships and models.** What this lesson's learning target asserts:

- In reflection from a smooth surface the angle of reflection equals the angle of incidence, both measured from the normal.
- Refraction occurs because the wave speed changes at the boundary; a wave entering along the normal changes speed without changing direction.
- Diffraction is most pronounced when the gap or obstacle is comparable in size to the wavelength.
...
- Claiming refraction occurs because the wave's frequency changes at the boundary.
- Claiming diffraction is significant regardless of the ratio of wavelength to gap size.
- Measuring angles of incidence and reflection from the surface rather than from the normal.

**Grade boundary.** Snell's law calculation, total internal reflection, and interference are acceptable but not required.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain reflection, refraction, and diffraction using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply reflection, refraction, and diffraction to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on reflection, refraction, and diffraction against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply reflection, refraction, and diffraction to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain reflection, refraction, and diffraction using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply reflection, refraction, and diffraction to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain reflection, refraction, and diffraction using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for reflection, refraction, and diffraction (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for reflection, refraction, and diffraction (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for reflection, refraction, and diffraction (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for reflection, refraction, and diffraction (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for reflection, refraction, and diffraction (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u08-l04` · built from `hs912-science-h4` at `a86780a`._
