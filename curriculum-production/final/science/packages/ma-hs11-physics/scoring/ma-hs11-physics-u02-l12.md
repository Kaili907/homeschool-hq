# Scoring authority — Correction and reflection: analysing force data

**High School Physics (Grade 11)** · Unit 2: Forces and Newton's Second Law · Day 12 · lesson `ma-hs11-physics-u02-l12`
...
_Topic `ma-hs11-physics-u02::analysing force data` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Force data is interpreted against a stated model, and the model predicts the shape of the relationship as well as its values.
- A linear relationship between net force and acceleration supports the second law; departures indicate an unaccounted force such as friction.
- The intercept and the scatter of a fitted line carry information about systematic and random error.
...
- Fitting a line through force-acceleration data and ignoring a substantial non-zero intercept.
- Attributing a departure from the expected relationship to error without identifying a candidate cause.
- Claiming agreement with the model from data whose scatter is too large to distinguish the alternatives.

**Grade boundary.** Regression statistics and residual analysis are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain analysing force data using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply analysing force data to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on analysing force data against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply analysing force data to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain analysing force data using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply analysing force data to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain analysing force data using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for analysing force data (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for analysing force data (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for analysing force data (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for analysing force data (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for analysing force data (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u02-l12` · built from `hs912-science-h4` at `a86780a`._
