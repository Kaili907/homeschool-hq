# Scoring authority — Performance task build: thermal equilibrium

**High School Physics (Grade 11)** · Unit 6: Thermal Energy and the Second Law · Day 9 · lesson `ma-hs11-physics-u06-l09`
...
_Topic `ma-hs11-physics-u06::thermal equilibrium` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Objects in thermal contact exchange energy until they reach the same temperature; net transfer is then zero.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Net thermal energy flows from higher to lower temperature, and the flow stops when the temperatures are equal.
- At equilibrium, exchange continues in both directions at equal rates, so equilibrium is dynamic rather than static.
- Energy is conserved during equilibration: what one body loses, the other and the surroundings gain.
...
- Claiming cold flows from the cooler body to the warmer one.
- Claiming all exchange ceases at thermal equilibrium.
- Claiming equilibrium means equal thermal energy rather than equal temperature.

**Grade boundary.** The zeroth law stated formally and quantitative equilibration timescales are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for thermal equilibrium that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain thermal equilibrium using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply thermal equilibrium to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on thermal equilibrium against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply thermal equilibrium to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain thermal equilibrium using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply thermal equilibrium to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for thermal equilibrium that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for thermal equilibrium (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for thermal equilibrium (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for thermal equilibrium (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for thermal equilibrium (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for thermal equilibrium (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u06-l09` · built from `hs912-science-h4` at `a86780a`._
