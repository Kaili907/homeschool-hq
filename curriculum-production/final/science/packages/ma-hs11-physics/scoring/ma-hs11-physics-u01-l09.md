# Scoring authority — Performance task build: motion graphs and their slopes

**High School Physics (Grade 11)** · Unit 1: Motion, Measurement, and Quantitative Reasoning · Day 9 · lesson `ma-hs11-physics-u01-l09`
...
_Topic `ma-hs11-physics-u01::motion graphs and their slopes` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- On a position-time graph the slope is velocity; on a velocity-time graph the slope is acceleration and the area under the curve is displacement.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The slope of a position-time graph gives velocity, including its sign, which carries direction.
- The slope of a velocity-time graph gives acceleration, and the area between the curve and the time axis gives displacement.
- A graph represents the motion; it is not a picture of the path the object took through space.
...
- Reading a position-time graph as a map of the object's path.
- Claiming the slope of a velocity-time graph gives velocity, or that its area gives acceleration.
- Claiming a horizontal line on a position-time graph shows motion at a constant non-zero speed rather than rest.

**Grade boundary.** Curve fitting and numerical differentiation of experimental data are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for motion graphs and their slopes that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain motion graphs and their slopes using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply motion graphs and their slopes to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on motion graphs and their slopes against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply motion graphs and their slopes to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain motion graphs and their slopes using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply motion graphs and their slopes to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for motion graphs and their slopes that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for motion graphs and their slopes (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for motion graphs and their slopes (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for motion graphs and their slopes (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for motion graphs and their slopes (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for motion graphs and their slopes (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: A ball leaving a ramp can trip someone or strike an object. — mitigation: Set the ramp so the run ends against a soft barrier, clear the floor path, keep the raised end no higher than 30 cm, use a rolling object under 200 g, and keep pets and young children out of the area.
...
**Disposal and clearing up:** Take the ramp down, peel off the masking tape, and put the ball and books away. Nothing hazardous is produced or left out.
...
_Package `swp-ma-hs11-physics-u01-l09` · built from `hs912-science-h4` at `a86780a`._
