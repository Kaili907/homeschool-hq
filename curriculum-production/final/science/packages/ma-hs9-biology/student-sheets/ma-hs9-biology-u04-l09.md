# Performance task build: positive feedback loops

**High School Biology (Grade 9)** · Unit 4: Homeostasis and Feedback Mechanisms · Day 9 of the unit (course day 45) · about 61–84 minutes

**Today's focus:** positive feedback loops
...
- Produce performance-task evidence for positive feedback loops that a reader could check independently.
- Explain positive feedback loops using evidence, a model, or a documented process rather than an unsupported answer.
- Apply positive feedback loops to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on positive feedback loops against the stated success criteria and name the next step.

**Materials:**

- two similar potted plants
- water
- clear bags
- thermometer
- insulated container
...
- **Physical hazard.** Warm water can scald.
  - What stops it: Keep all water below 50 degrees Celsius, an adult fills the container, and the learner never handles boiling water.
- **Emotional hazard.** Health and body topics can be distressing when they touch personal or family medical experience.
  - What stops it: Examples use non-human systems and published cases only; the learner is never asked about their own or a family member's health, and may skip any example without penalty.
...
**Clearing up.** Pour the cooled water down the drain, dry the containers, and return the plants to their usual place. No sample and no body or health measurement is taken or kept.
...
- Science brief: positive feedback amplifies the change that triggered it and drives the system away from the starting condition.
- Science brief: in positive feedback the response reinforces the original change rather than opposing it.
- Science brief: positive feedback drives processes to completion, such as childbirth, blood clotting, and the rising phase of a nerve impulse.
- Science brief: positive feedback requires an external stopping condition, because the loop itself does not stabilise.

### Evidence check: positive feedback loops

**Candidate claim:** A case report presents this as acceptable science: “positive feedback means a beneficial outcome and negative feedback a harmful one.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: positive feedback amplifies the change that triggered it and drives the system away from the starting condition. |
| E2 | established science reference | Science brief: in positive feedback the response reinforces the original change rather than opposing it. |
| E3 | established science reference | Science brief: positive feedback drives processes to completion, such as childbirth, blood clotting, and the rising phase of a nerve impulse. |
| E4 | established science reference | Science brief: positive feedback requires an external stopping condition, because the loop itself does not stabilise. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs9-biology-u04::positive feedback loops, derived from the reviewed correctness source policy/correctness/ma-hs9-biology.correctness.json.

**Supplied unit connections:** setpoints and tolerance ranges; negative feedback loops; positive feedback loops; feedback failure and disease; regulation across organ systems; modelling feedback quantitatively

**Supplied unit performance task:** Plan and conduct an investigation using a non-human system that provides evidence a feedback mechanism is maintaining a stable internal condition, then model the loop and identify what would break it.

### Bound task

**Question:** What does evidence E1 through E4 justify about positive feedback loops, and does it support the candidate claim?
...
**Model/source rule:** A fictional ectotherm-control simulation increases a cooling-response index when model temperature exceeds 38.2 °C; the response removes 0.15 °C per index per step.
...
3. Plot model temperature and response through time, identify the stimulus-response loop, and state why the table cannot be used as a learner health measurement.
...
**Pinned H4 alternative statement (lineage evidence):** Use a published, openly licensed physiological dataset - for example thermoregulation in a non-human animal - and model the feedback loop from that data. Needs only the dataset and paper.
...
**My question.** Write the question you are testing about positive feedback loops. It has to be a question your evidence could actually answer.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about positive feedback loops, then name one check that would catch an error or a weak claim.**
...
**Q1.** Build a checkable contribution to the supplied unit performance task — Plan and conduct an investigation using a non-human system that provides evidence a feedback mechanism is maintaining a stable internal condition, then model the loop and identify what would break it. — using the case and E1-E4. Include claim, evidence, reasoning, limitation, and a recorded revision.

>

**Q2.** Explain positive feedback loops in your own words using at least two specifically named rows from E1-E4. Then explain why the candidate claim does or does not follow.

>

**Q3.** Treat the printed candidate claim as the fresh case for positive feedback loops. Decide whether E1-E4 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q4.** Check the conclusion about positive feedback loops, revise its weakest claim with a named row from E1-E4, and name the next observation or model input that would resolve the remaining limitation.

>

**Q5.** Answer the printed unit question using positive feedback loops as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q6.** Which one row in E1-E4 is strongest for evaluating the candidate claim about positive feedback loops? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q7.** Using the supplied case and E1-E4, state one conclusion about positive feedback loops that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q8.** Audit the inputs used in your response about positive feedback loops. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about positive feedback loops.
...
- Apply positive feedback loops under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect positive feedback loops to setpoints and tolerance ranges from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
- Take the unit's performance task — Plan and conduct an investigation using a non-human system that provides evidence a feedback mechanism is maintaining a stable internal condition, then model the loop and identify what would break it. — and write the one measurement or piece of evidence that would most change its outcome, then say why.
...
_Package `swp-ma-hs9-biology-u04-l09` · lesson `ma-hs9-biology-u04-l09` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
