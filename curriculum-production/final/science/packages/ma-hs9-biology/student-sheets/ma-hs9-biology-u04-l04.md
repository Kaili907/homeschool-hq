# Independent application A: feedback failure and disease

**High School Biology (Grade 9)** · Unit 4: Homeostasis and Feedback Mechanisms · Day 4 of the unit (course day 40) · about 55–75 minutes

**Today's focus:** feedback failure and disease
...
- Explain feedback failure and disease using evidence, a model, or a documented process rather than an unsupported answer.
- Apply feedback failure and disease to a task the learner has not seen before, showing the reasoning that produced the result.
- Check and revise the work on feedback failure and disease against the stated success criteria and name the next step.
...
- Science brief: disease can result when a sensor, a control centre, an effector, or the signal between them fails.
- Science brief: the same symptom can arise from failures at different points in the loop, so the symptom alone does not identify the cause.
- Science brief: treatment that replaces an effector's output does not restore regulation unless the loop can still detect and respond.

### Evidence check: feedback failure and disease

**Candidate claim:** A case report presents this as acceptable science: “inferring the point of failure from the symptom alone with no further evidence.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: disease can result when a sensor, a control centre, an effector, or the signal between them fails. |
| E2 | established science reference | Science brief: the same symptom can arise from failures at different points in the loop, so the symptom alone does not identify the cause. |
| E3 | established science reference | Science brief: treatment that replaces an effector's output does not restore regulation unless the loop can still detect and respond. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-hs9-biology-u04::feedback failure and disease, derived from the reviewed correctness source policy/correctness/ma-hs9-biology.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about feedback failure and disease, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about feedback failure and disease, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about feedback failure and disease, then name one check that would catch an error or a weak claim.**
...
**Q1.** Explain feedback failure and disease in your own words using at least two specifically named rows from E1-E3. Then explain why the candidate claim does or does not follow.

>

**Q2.** Treat the printed candidate claim as the fresh case for feedback failure and disease. Decide whether E1-E3 supports it, show the evidence-to-conclusion reasoning, and write a more defensible replacement claim if it does not.

>

**Q3.** Check the conclusion about feedback failure and disease, revise its weakest claim with a named row from E1-E3, and name the next observation or model input that would resolve the remaining limitation.

>

**Q4.** Answer the printed unit question using feedback failure and disease as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about feedback failure and disease? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about feedback failure and disease that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about feedback failure and disease. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about feedback failure and disease.
...
- Apply feedback failure and disease under a new constraint, compare two approaches or two sources, or teach the idea with an original example. Extension never means completing another learner's graded work.
- Connect feedback failure and disease to setpoints and tolerance ranges from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-hs9-biology-u04-l04` · lesson `ma-hs9-biology-u04-l04` · built from `hs912-science-h4` at `a86780a`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
