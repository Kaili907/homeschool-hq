# Scoring authority — Performance task build: the carbon cycle in ecosystems

**High School Biology (Grade 9)** · Unit 6: Ecosystems: Matter Cycling and Energy Flow · Day 9 · lesson `ma-hs9-biology-u06-l09`
...
_Topic `ma-hs9-biology-u06::the carbon cycle in ecosystems` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Photosynthesis moves carbon from the atmosphere into organisms; respiration, decomposition, and combustion return it.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Carbon moves between the atmosphere, organisms, soil, water, and rock, and the timescales of those movements differ by orders of magnitude.
- Fossil fuel combustion transfers carbon from a long-term geological reservoir into the atmosphere far faster than natural processes return it.
- A carbon cycle account balances the flows in and out of each reservoir.
...
- Claiming carbon is created or destroyed as it moves through the cycle.
- Claiming only plants and animals participate in the carbon cycle.
- Claiming the fast biological cycle and the slow geological cycle operate at comparable rates.

**Grade boundary.** Quantitative reservoir sizes, flux measurements, and ocean carbonate chemistry are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for the carbon cycle in ecosystems that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain the carbon cycle in ecosystems using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply the carbon cycle in ecosystems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on the carbon cycle in ecosystems against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply the carbon cycle in ecosystems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain the carbon cycle in ecosystems using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply the carbon cycle in ecosystems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for the carbon cycle in ecosystems that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for the carbon cycle in ecosystems (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for the carbon cycle in ecosystems (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for the carbon cycle in ecosystems (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for the carbon cycle in ecosystems (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for the carbon cycle in ecosystems (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u06-l09` · built from `hs912-science-h4` at `a86780a`._
