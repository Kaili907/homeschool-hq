# Scoring authority — Correction and reflection: distribution of expressed traits

**High School Biology (Grade 9)** · Unit 8: Inheritance and Variation of Traits · Day 12 · lesson `ma-hs9-biology-u08-l12`
...
_Topic `ma-hs9-biology-u08::distribution of expressed traits` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Traits influenced by many genes and by the environment are distributed continuously in a population, often clustering around a central value.
- The distribution is a property of the population, and no individual measurement characterises it.
- Comparing distributions requires comparable measurement methods and sample sizes.

**Also correct.** Accept any of these framings — do not require the wording above:

- Describing the shape as a normal or bell-shaped distribution is correct; describing it in plain terms is equally acceptable.
...
- Drawing a conclusion about a population's trait distribution from one individual.
- Claiming a continuous trait falls into discrete categories because the data was binned that way.
- Comparing trait distributions measured by different methods as though they were equivalent.

**Grade boundary.** Descriptive and inferential statistics beyond reading a distribution are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain distribution of expressed traits using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply distribution of expressed traits to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on distribution of expressed traits against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply distribution of expressed traits to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain distribution of expressed traits using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply distribution of expressed traits to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain distribution of expressed traits using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for distribution of expressed traits (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for distribution of expressed traits (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for distribution of expressed traits (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for distribution of expressed traits (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for distribution of expressed traits (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u08-l12` · built from `hs912-science-h4` at `a86780a`._
