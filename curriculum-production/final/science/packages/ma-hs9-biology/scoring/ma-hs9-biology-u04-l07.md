# Scoring authority — Investigation: setpoints and tolerance ranges

**High School Biology (Grade 9)** · Unit 4: Homeostasis and Feedback Mechanisms · Day 7 · lesson `ma-hs9-biology-u04-l07`
...
_Topic `ma-hs9-biology-u04::setpoints and tolerance ranges` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Homeostasis maintains internal conditions within a tolerance range, not at a single unchanging value.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A regulated variable has a setpoint and a range within which the organism functions.
- Values fluctuate within the range continuously; regulation acts when a departure is detected, so variation is expected.
- Different variables have different ranges, and exceeding a range has consequences specific to that variable.
...
- Claiming a homeostatically regulated variable is held perfectly constant.
- Treating any fluctuation within the tolerance range as a regulatory failure.
- Claiming all regulated variables share one tolerance range.

**Grade boundary.** Control theory and quantitative physiological modelling are beyond this course.
...
> Learner-collected plant or thermal-model measurements, or a published non-human dataset. NO LEARNER BODY OR HEALTH MEASUREMENT IS REQUESTED ANYWHERE IN THIS UNIT.
...
**Q1 — investigate.** _Serves objective 1:_ Plan and carry out a safe investigation of setpoints and tolerance ranges, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain setpoints and tolerance ranges using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply setpoints and tolerance ranges to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on setpoints and tolerance ranges against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply setpoints and tolerance ranges to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain setpoints and tolerance ranges using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 1:_ Plan and carry out a safe investigation of setpoints and tolerance ranges, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q8 — provenance.** _Serves objective 1:_ Plan and carry out a safe investigation of setpoints and tolerance ranges, or its stated alternative, and record the learner's own data with its uncertainty.
...
- **prerequisite-gap** — Run the prerequisite reteach route for setpoints and tolerance ranges (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for setpoints and tolerance ranges (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for setpoints and tolerance ranges (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for setpoints and tolerance ranges (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for setpoints and tolerance ranges (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Warm water can scald. — mitigation: Keep all water below 50 degrees Celsius, an adult fills the container, and the learner never handles boiling water.
- emotional: Health and body topics can be distressing when they touch personal or family medical experience. — mitigation: Examples use non-human systems and published cases only; the learner is never asked about their own or a family member's health, and may skip any example without penalty.
...
**Disposal and clearing up:** Pour the cooled water down the drain, dry the containers, and return the plants to their usual place. No sample and no body or health measurement is taken or kept.
...
_Package `swp-ma-hs9-biology-u04-l07` · built from `hs912-science-h4` at `a86780a`._
