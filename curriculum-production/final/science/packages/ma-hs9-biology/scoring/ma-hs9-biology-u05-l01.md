# Scoring authority — Launch and diagnostic: light energy capture

**High School Biology (Grade 9)** · Unit 5: Photosynthesis, Cellular Respiration, and Energy in Living Systems · Day 1 · lesson `ma-hs9-biology-u05-l01`
...
_Topic `ma-hs9-biology-u05::light energy capture` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Chlorophyll absorbs light most strongly in the blue and red regions and reflects green, which is why plants appear green.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Pigments absorb particular wavelengths of light and transfer that energy into the photosynthetic process.
- Light absorbed is used; light reflected or transmitted is not, which is why leaf colour indicates which wavelengths are least used.
...
- Naming accessory pigments such as carotenoids alongside chlorophyll is correct and not required.
...
- Claiming chlorophyll absorbs green light most strongly.
- Claiming plants take in light and store it as light energy.
- Claiming photosynthesis occurs in the mitochondria.

**Grade boundary.** Photosystem biochemistry, electron transport detail, and absorption spectra analysis are beyond this course.
...
**Q1 — phenomenon question.** _Serves objective 1:_ Observe the anchoring phenomenon, surface initial thinking about light energy capture without penalty, and pose a testable question.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain light energy capture using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply light energy capture to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on light energy capture against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply light energy capture to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain light energy capture using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply light energy capture to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Observe the anchoring phenomenon, surface initial thinking about light energy capture without penalty, and pose a testable question.
...
- **prerequisite-gap** — Run the prerequisite reteach route for light energy capture (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for light energy capture (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for light energy capture (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for light energy capture (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for light energy capture (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u05-l01` · built from `hs912-science-h4` at `a86780a`._
