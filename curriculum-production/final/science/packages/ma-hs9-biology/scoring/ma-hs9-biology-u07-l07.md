# Scoring authority — Investigation: exponential and logistic growth

**High School Biology (Grade 9)** · Unit 7: Population Dynamics, Carrying Capacity, and Biodiversity · Day 7 · lesson `ma-hs9-biology-u07-l07`
...
_Topic `ma-hs9-biology-u07::exponential and logistic growth` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Exponential growth has no limiting factor and cannot be sustained; logistic growth slows as the population approaches carrying capacity.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A population grows exponentially when resources are effectively unlimited, with growth rate proportional to population size.
- As resources become limiting, per-individual growth rate declines and the curve levels off near the carrying capacity.
- No real population grows exponentially indefinitely, because no environment supplies unlimited resources.

**Also correct.** Accept any of these framings — do not require the wording above:

- Describing the curves as J-shaped and S-shaped is correct and equivalent.
...
- Claiming a population can sustain exponential growth indefinitely.
- Claiming logistic growth means the population stops changing once it reaches carrying capacity.
- Confusing the growth rate with the population size.

**Grade boundary.** Differential equation forms, intrinsic rate of increase, and age-structured models are beyond this course.
...
> A published long-term population dataset the family retrieves, plus the learner's own sampling counts. Growth constants are fitted by the learner, not supplied.
...
**Q1 — investigate.** _Serves objective 1:_ Plan and carry out a safe investigation of exponential and logistic growth, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain exponential and logistic growth using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply exponential and logistic growth to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on exponential and logistic growth against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply exponential and logistic growth to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain exponential and logistic growth using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 1:_ Plan and carry out a safe investigation of exponential and logistic growth, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q8 — provenance.** _Serves objective 1:_ Plan and carry out a safe investigation of exponential and logistic growth, or its stated alternative, and record the learner's own data with its uncertainty.
...
- **prerequisite-gap** — Run the prerequisite reteach route for exponential and logistic growth (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for exponential and logistic growth (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for exponential and logistic growth (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for exponential and logistic growth (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for exponential and logistic growth (evidence type: application, require explanation: True, review timing: same-day).
...
- privacy: Online dataset searching can expose the learner to tracking or to accounts. — mitigation: An adult supplies the dataset file or the exact source; no account creation, sign-in, or purchase is required.
- physical: Small counters are a choking hazard around young children, and dried beans used as counters are no longer food - raw kidney beans in particular are toxic if eaten. — mitigation: Keep counters away from younger siblings, count them back into a labelled container at the end, and never return beans used as counters to food storage or eat them.
...
**Disposal and clearing up:** Beans and counters go back into a labelled container stored out of reach of young children. Beans used as counters are never eaten and never returned to food storage. Dataset files stay on the adult's device; nothing is uploaded.
...
_Package `swp-ma-hs9-biology-u07-l07` · built from `hs912-science-h4` at `a86780a`._
