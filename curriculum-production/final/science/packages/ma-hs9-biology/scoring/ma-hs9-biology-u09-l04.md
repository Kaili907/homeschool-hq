# Scoring authority — Independent application A: adaptation of populations

**High School Biology (Grade 9)** · Unit 9: Natural Selection, Evolution, and Biodiversity Engineering · Day 4 · lesson `ma-hs9-biology-u09-l04`
...
_Topic `ma-hs9-biology-u09::adaptation of populations` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Adaptation is a population-level outcome across generations, not a change an individual makes during its life.

**Accepted relationships and models.** What this lesson's learning target asserts:

- An adaptation is a heritable trait that has become common in a population because it improved survival and reproduction in that environment.
- A trait that is adaptive in one environment can be neutral or harmful in another, so adaptation is always relative to conditions.
- Not every trait is an adaptation; some persist through chance, through linkage to other traits, or as by-products.
...
- Describing an individual organism as adapting to its environment during its lifetime as an evolutionary process.
- Claiming every trait an organism has is an adaptation.
- Claiming an adaptation is a trait the organism developed because it was needed.

**Grade boundary.** Adaptationist critique, exaptation theory, and comparative methods are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain adaptation of populations using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply adaptation of populations to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on adaptation of populations against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply adaptation of populations to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain adaptation of populations using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply adaptation of populations to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain adaptation of populations using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for adaptation of populations (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for adaptation of populations (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for adaptation of populations (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for adaptation of populations (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for adaptation of populations (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u09-l04` · built from `hs912-science-h4` at `a86780a`._
