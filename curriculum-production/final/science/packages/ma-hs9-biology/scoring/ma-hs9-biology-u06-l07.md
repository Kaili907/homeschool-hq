# Scoring authority — Investigation: trophic levels and energy transfer

**High School Biology (Grade 9)** · Unit 6: Ecosystems: Matter Cycling and Energy Flow · Day 7 · lesson `ma-hs9-biology-u06-l07`
...
_Topic `ma-hs9-biology-u06::trophic levels and energy transfer` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Only a small fraction of the energy at one trophic level, on the order of ten per cent, is incorporated into the next.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Energy enters most ecosystems through producers and passes upward through consumers, with most released as thermal energy at each transfer.
- Because so little energy passes upward, food chains have few levels and higher levels support far less biomass.
- Energy flows through an ecosystem in one direction and is not recycled.

**Also correct.** Accept any of these framings — do not require the wording above:

- Approximate statements such as 'about ten per cent' or 'roughly a tenth' are correct; an exact figure is not.
...
- Claiming energy cycles through an ecosystem the way matter does.
- Claiming most of the energy at one trophic level passes to the next.
- Claiming a long food chain supports as much biomass at the top as at the bottom.

**Grade boundary.** Quantitative production ecology and ecological efficiency measurement are beyond this course.
...
> Learner-collected mass-loss measurements where run, plus a published ecosystem energy dataset. Trophic efficiency is derived from data, never asserted.
...
**Q1 — investigate.** _Serves objective 1:_ Plan and carry out a safe investigation of trophic levels and energy transfer, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain trophic levels and energy transfer using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply trophic levels and energy transfer to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on trophic levels and energy transfer against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply trophic levels and energy transfer to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain trophic levels and energy transfer using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 1:_ Plan and carry out a safe investigation of trophic levels and energy transfer, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q8 — provenance.** _Serves objective 1:_ Plan and carry out a safe investigation of trophic levels and energy transfer, or its stated alternative, and record the learner's own data with its uncertainty.
...
- **prerequisite-gap** — Run the prerequisite reteach route for trophic levels and energy transfer (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for trophic levels and energy transfer (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for trophic levels and energy transfer (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for trophic levels and energy transfer (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for trophic levels and energy transfer (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Decomposing material grows mould, and mould aerosol is far more dangerous to an asthmatic than to someone with hay fever. — mitigation: Anyone with asthma, an immune condition, or a mould or spore allergy uses the alternative and opens nothing. Containers stay outdoors, labelled, covered LOOSELY so gas escapes - never sealed. Gloves on; open outdoors only.
- physical: Mould-colonised matter on a food scale contaminates everything weighed afterwards. — mitigation: Weigh in a disposable tray placed on the scale; the sample never touches the scale surface. The containers, trays, and gloves belong to this investigation only and are binned at the end. None of it returns to kitchen or food use.
- privacy: Recording an outdoor location can disclose where the family lives. — mitigation: Record habitat type and general conditions only. Never record an address, GPS coordinate, or identifiable landmark.
...
_Package `swp-ma-hs9-biology-u06-l07` · built from `hs912-science-h4` at `a86780a`._
