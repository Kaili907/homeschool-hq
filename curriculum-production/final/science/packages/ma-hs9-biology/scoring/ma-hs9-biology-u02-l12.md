# Scoring authority — Correction and reflection: mutation and its consequences

**High School Biology (Grade 9)** · Unit 2: DNA, Proteins, and the Information Flow of the Cell · Day 12 · lesson `ma-hs9-biology-u02-l12`
...
_Topic `ma-hs9-biology-u02::mutation and its consequences` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- A mutation is a change in DNA sequence; its effect may be harmful, beneficial, or neutral, and depends on where it occurs and what it changes.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Mutations arise from replication errors and from chemical and radiation damage, and most are repaired.
- A substitution may change no amino acid, change one, or introduce a stop; an insertion or deletion that is not a multiple of three shifts the reading frame and usually has a larger effect.
- Only mutations in the germ line are inherited; somatic mutations affect the individual alone.

**Also correct.** Accept any of these framings — do not require the wording above:

- Silent, missense, nonsense, and frameshift are correct names; describing the effect without the labels is equally acceptable.
...
- Claiming mutations occur because the organism or population needs a particular change.
- Claiming a somatic mutation is passed to offspring.

**Grade boundary.** Repair pathway biochemistry and mutational signature analysis are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain mutation and its consequences using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply mutation and its consequences to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on mutation and its consequences against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply mutation and its consequences to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain mutation and its consequences using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply mutation and its consequences to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain mutation and its consequences using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for mutation and its consequences (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for mutation and its consequences (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for mutation and its consequences (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for mutation and its consequences (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for mutation and its consequences (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u02-l12` · built from `hs912-science-h4` at `a86780a`._
