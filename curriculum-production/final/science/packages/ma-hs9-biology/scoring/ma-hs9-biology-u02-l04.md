# Scoring authority — Independent application A: translation and the genetic code

**High School Biology (Grade 9)** · Unit 2: DNA, Proteins, and the Information Flow of the Cell · Day 4 · lesson `ma-hs9-biology-u02-l04`
...
_Topic `ma-hs9-biology-u02::translation and the genetic code` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The genetic code is read in three-nucleotide codons, and it is redundant: most amino acids are specified by more than one codon.
- Proteins are built from twenty standard amino acids.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Ribosomes read messenger RNA in codons of three nucleotides and assemble the corresponding amino acids into a chain.
- Transfer RNA molecules match codons to amino acids, which is how the nucleotide sequence is converted into an amino acid sequence.
- The code is redundant but not ambiguous: one codon specifies one amino acid, while one amino acid may have several codons.

**Also correct.** Accept any of these framings — do not require the wording above:

- Describing the code as degenerate or as redundant are the same claim.
...
- Claiming one nucleotide, or two, specifies one amino acid.
- Claiming each amino acid has exactly one codon.
- Claiming translation occurs in the nucleus in eukaryotic cells.

**Grade boundary.** Ribosome structure, elongation factor biochemistry, and codon usage bias are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain translation and the genetic code using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply translation and the genetic code to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on translation and the genetic code against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply translation and the genetic code to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain translation and the genetic code using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply translation and the genetic code to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain translation and the genetic code using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for translation and the genetic code (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for translation and the genetic code (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for translation and the genetic code (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for translation and the genetic code (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for translation and the genetic code (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u02-l04` · built from `hs912-science-h4` at `a86780a`._
