# Scoring authority — Performance task build: positive feedback loops

**High School Biology (Grade 9)** · Unit 4: Homeostasis and Feedback Mechanisms · Day 9 · lesson `ma-hs9-biology-u04-l09`
...
_Topic `ma-hs9-biology-u04::positive feedback loops` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Positive feedback amplifies the change that triggered it and drives the system away from the starting condition.

**Accepted relationships and models.** What this lesson's learning target asserts:

- In positive feedback the response reinforces the original change rather than opposing it.
- Positive feedback drives processes to completion, such as childbirth, blood clotting, and the rising phase of a nerve impulse.
- Positive feedback requires an external stopping condition, because the loop itself does not stabilise.
...
- Claiming positive feedback means a beneficial outcome and negative feedback a harmful one.
- Describing a stabilising loop as positive feedback.
- Claiming positive feedback maintains a setpoint.

**Grade boundary.** Bistability and formal dynamical systems analysis are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for positive feedback loops that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain positive feedback loops using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply positive feedback loops to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on positive feedback loops against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply positive feedback loops to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain positive feedback loops using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply positive feedback loops to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for positive feedback loops that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for positive feedback loops (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for positive feedback loops (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for positive feedback loops (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for positive feedback loops (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for positive feedback loops (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u04-l09` · built from `hs912-science-h4` at `a86780a`._
