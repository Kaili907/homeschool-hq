# Scoring authority — Reteach and varied practice: carrying capacity and limiting factors

**High School Biology (Grade 9)** · Unit 7: Population Dynamics, Carrying Capacity, and Biodiversity · Day 8 · lesson `ma-hs9-biology-u07-l08`
...
_Topic `ma-hs9-biology-u07::carrying capacity and limiting factors` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Carrying capacity is the population size an environment can sustain given its limiting resources, and it changes when those resources change.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The limiting factor is the resource in shortest supply relative to demand; increasing any other resource does not raise the population.
- Density-dependent factors such as competition, disease, and predation intensify as population rises; density-independent factors such as weather do not.
- Carrying capacity is a property of the environment and the population together, and it is not fixed.
...
- Treating carrying capacity as a permanent fixed number for a place.
- Claiming increasing a non-limiting resource raises the sustainable population.
- Classifying weather events as density-dependent factors.

**Grade boundary.** Quantitative estimation of carrying capacity and metapopulation theory are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain carrying capacity and limiting factors using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply carrying capacity and limiting factors to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on carrying capacity and limiting factors against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply carrying capacity and limiting factors to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain carrying capacity and limiting factors using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply carrying capacity and limiting factors to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain carrying capacity and limiting factors using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for carrying capacity and limiting factors (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for carrying capacity and limiting factors (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for carrying capacity and limiting factors (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for carrying capacity and limiting factors (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for carrying capacity and limiting factors (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u07-l08` · built from `hs912-science-h4` at `a86780a`._
