# Scoring authority — Reteach and varied practice: the four factors driving natural selection

**High School Biology (Grade 9)** · Unit 9: Natural Selection, Evolution, and Biodiversity Engineering · Day 8 · lesson `ma-hs9-biology-u09-l08`
...
_Topic `ma-hs9-biology-u09::the four factors driving natural selection` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Natural selection requires: more offspring produced than can survive; heritable variation among individuals; competition for limited resources; and differential survival and reproduction of individuals with particular heritable traits.

**Accepted relationships and models.** What this lesson's learning target asserts:

- All four conditions are necessary; if any is absent, natural selection does not operate.
- Selection acts on phenotypes, and the effect is heritable only to the extent that the variation is genetic.
- The outcome is a change in the frequency of heritable traits in the population across generations.

**Also correct.** Accept any of these framings — do not require the wording above:

- Describing the fourth factor as differential reproductive success, or as differences in survival and reproduction, are equivalent.
...
- Claiming individual organisms evolve or adapt themselves during their lifetimes.
- Claiming variation arises because the population needs it.
- Claiming natural selection has a goal, a direction toward complexity, or an endpoint.

**Grade boundary.** Hardy-Weinberg equilibrium, genetic drift quantification, and speciation modelling are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain the four factors driving natural selection using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply the four factors driving natural selection to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on the four factors driving natural selection against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply the four factors driving natural selection to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain the four factors driving natural selection using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply the four factors driving natural selection to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain the four factors driving natural selection using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for the four factors driving natural selection (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for the four factors driving natural selection (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for the four factors driving natural selection (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for the four factors driving natural selection (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for the four factors driving natural selection (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u09-l08` · built from `hs912-science-h4` at `a86780a`._
