# Scoring authority — Reteach and varied practice: negative feedback loops

**High School Biology (Grade 9)** · Unit 4: Homeostasis and Feedback Mechanisms · Day 8 · lesson `ma-hs9-biology-u04-l08`
...
_Topic `ma-hs9-biology-u04::negative feedback loops` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Negative feedback opposes the change that triggered it and returns the variable toward its setpoint.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A negative feedback loop requires a sensor that detects the change, a control centre that compares it with the setpoint, and an effector that acts to reverse it.
- Because the response opposes the deviation, the system is stabilised around the setpoint.
- Most physiological regulation, including of body temperature, blood glucose, and water balance, is negative feedback.

**Also correct.** Accept any of these framings — do not require the wording above:

- Naming the three components sensor, control centre, and effector, or receptor, integrator, and effector, are equally correct.
...
- Describing a loop as negative feedback when the response amplifies the original change.
- Omitting the detection step and claiming the effector acts on its own.
- Claiming negative feedback means the outcome is harmful.

**Grade boundary.** Gain, time constants, and formal control-system analysis are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain negative feedback loops using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply negative feedback loops to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on negative feedback loops against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply negative feedback loops to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain negative feedback loops using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply negative feedback loops to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain negative feedback loops using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for negative feedback loops (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for negative feedback loops (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for negative feedback loops (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for negative feedback loops (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for negative feedback loops (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u04-l08` · built from `hs912-science-h4` at `a86780a`._
