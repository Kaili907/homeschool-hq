# Scoring authority — Performance task build: transcription

**High School Biology (Grade 9)** · Unit 2: DNA, Proteins, and the Information Flow of the Cell · Day 9 · lesson `ma-hs9-biology-u02-l09`
...
_Topic `ma-hs9-biology-u02::transcription` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Transcription copies a DNA sequence into RNA; RNA contains uracil in place of thymine and ribose in place of deoxyribose.
...
- Transcription produces a single-stranded RNA copy complementary to one DNA strand.
- Only part of the genome is transcribed in any given cell at any given time, which is why cells with identical DNA differ.
- The messenger RNA carries the sequence information out of the nucleus to the ribosome.

**Also correct.** Accept any of these framings — do not require the wording above:

- Describing the copied strand as the template or non-coding strand is correct; naming the other strand the coding or sense strand is also correct.
...
- Claiming RNA contains thymine.
- Claiming transcription produces protein.
- Claiming every gene is transcribed in every cell at all times.

**Grade boundary.** Promoter structure, splicing detail, and transcription factor biochemistry are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for transcription that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain transcription using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply transcription to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on transcription against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply transcription to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain transcription using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply transcription to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for transcription that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for transcription (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for transcription (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for transcription (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for transcription (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for transcription (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u02-l09` · built from `hs912-science-h4` at `a86780a`._
