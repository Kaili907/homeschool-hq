# Scoring authority — Performance task build: interspecific interactions

**High School Biology (Grade 9)** · Unit 7: Population Dynamics, Carrying Capacity, and Biodiversity · Day 9 · lesson `ma-hs9-biology-u07-l09`
...
_Topic `ma-hs9-biology-u07::interspecific interactions` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- In mutualism both species benefit; in commensalism one benefits and the other is essentially unaffected; in parasitism and predation one benefits at the other's expense; in competition both are harmed.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Interactions are classified by the effect on each participant, not by intent or by how the relationship appears.
- Interspecific competition occurs where species use the same limiting resource, and it can shift the resources each species uses.
- Predator and prey populations influence one another, typically with a delay between the change in one and the response of the other.
...
- Classifying a relationship as mutualism when only one participant benefits.
- Claiming predators normally drive their prey to extinction.
- Claiming competition occurs only between members of different species.

**Grade boundary.** Competitive exclusion modelling and niche partitioning theory in depth are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for interspecific interactions that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain interspecific interactions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply interspecific interactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on interspecific interactions against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply interspecific interactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain interspecific interactions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply interspecific interactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for interspecific interactions that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for interspecific interactions (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for interspecific interactions (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for interspecific interactions (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for interspecific interactions (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for interspecific interactions (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u07-l09` · built from `hs912-science-h4` at `a86780a`._
