# Scoring authority — Investigation: multiple lines of evidence for common ancestry

**High School Biology (Grade 9)** · Unit 9: Natural Selection, Evolution, and Biodiversity Engineering · Day 7 · lesson `ma-hs9-biology-u09-l07`
...
_Topic `ma-hs9-biology-u09::multiple lines of evidence for common ancestry` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Evidence for common ancestry comes from the fossil record, comparative anatomy, embryology, biogeography, and molecular sequence comparison.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Independent lines of evidence converging on the same pattern of relationship is what makes common ancestry a robust conclusion.
- Shared DNA and protein sequences among species match the pattern inferred from anatomy and fossils.
- Shared underlying structure used for different functions indicates common ancestry; similar function achieved by different structures does not.

**Also correct.** Accept any of these framings — do not require the wording above:

- Naming homologous and analogous structures is correct but not required, provided the distinction is used correctly.
...
- Treating similarity of function as evidence of close relationship.
- Claiming a single line of evidence establishes the relationships on its own.
- Claiming one living species is the ancestor of another living species.

**Grade boundary.** Phylogenetic tree-building algorithms and molecular clock calibration are beyond this course.
...
> Learner's own predation-simulation counts across generations and a published conservation case study. Allele-frequency change is computed by the learner from their own trials.
...
**Q1 — investigate.** _Serves objective 1:_ Plan and carry out a safe investigation of multiple lines of evidence for common ancestry, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain multiple lines of evidence for common ancestry using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply multiple lines of evidence for common ancestry to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on multiple lines of evidence for common ancestry against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply multiple lines of evidence for common ancestry to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain multiple lines of evidence for common ancestry using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 1:_ Plan and carry out a safe investigation of multiple lines of evidence for common ancestry, or its stated alternative, and record the learner's own data with its uncertainty.
...
**Q8 — provenance.** _Serves objective 1:_ Plan and carry out a safe investigation of multiple lines of evidence for common ancestry, or its stated alternative, and record the learner's own data with its uncertainty.
...
- **prerequisite-gap** — Run the prerequisite reteach route for multiple lines of evidence for common ancestry (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for multiple lines of evidence for common ancestry (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for multiple lines of evidence for common ancestry (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for multiple lines of evidence for common ancestry (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for multiple lines of evidence for common ancestry (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Timed grabbing games can knock over nearby objects. — mitigation: Clear the work surface first and keep the play area away from edges and breakables.
- emotional: Extinction and habitat loss can be genuinely upsetting. — mitigation: Frame the unit around designed responses and documented recoveries; the learner may pause or choose a different case at any point.
...
**Disposal and clearing up:** Beans, paper dots, and cloth go back into a labelled container out of reach of young children; beans used as counters are never eaten. Case-study printouts are filed or recycled. No location is recorded and nothing is uploaded.
...
_Package `swp-ma-hs9-biology-u09-l07` · built from `hs912-science-h4` at `a86780a`._
