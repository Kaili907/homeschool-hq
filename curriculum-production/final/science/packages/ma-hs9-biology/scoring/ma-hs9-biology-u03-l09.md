# Scoring authority — Performance task build: tissues, organs, and organ systems

**High School Biology (Grade 9)** · Unit 3: Cells, Hierarchical Organization, and Cellular Division · Day 9 · lesson `ma-hs9-biology-u03-l09`
...
_Topic `ma-hs9-biology-u03::tissues, organs, and organ systems` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The hierarchy runs cells, tissues, organs, organ systems, organism.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Specialised cells of similar type form tissues; tissues combine into organs; organs work together as organ systems.
- Each level shows capabilities the level below does not have on its own, which is what makes the hierarchy meaningful.
- Organ systems are interdependent, so a failure in one propagates to others.
...
- Placing the levels of organisation out of order.
- Claiming an organ consists of a single tissue type.
- Claiming organ systems operate independently.

**Grade boundary.** Histology and comparative anatomy are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for tissues, organs, and organ systems that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain tissues, organs, and organ systems using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply tissues, organs, and organ systems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on tissues, organs, and organ systems against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply tissues, organs, and organ systems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain tissues, organs, and organ systems using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply tissues, organs, and organ systems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for tissues, organs, and organ systems that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for tissues, organs, and organ systems (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for tissues, organs, and organ systems (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for tissues, organs, and organ systems (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for tissues, organs, and organ systems (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for tissues, organs, and organ systems (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Cutting cubes to size uses a knife. — mitigation: An adult does all cutting, or the learner uses clay shaped by hand with no cutting tool.
- physical: Microscope lamps get warm and slides have sharp edges. — mitigation: Handle slides by their edges, let the lamp cool before storage, and treat the microscope step as optional.
- physical: Broken slide glass cuts whoever empties the bin. — mitigation: An adult sweeps fragments with stiff card, wraps them in thick paper and tape, and bins them separately. Never by hand, never loose, never with a vacuum cleaner.
...
_Package `swp-ma-hs9-biology-u03-l09` · built from `hs912-science-h4` at `a86780a`._
