# Scoring authority — Unit assessment: polygenic and environmental effects

**High School Biology (Grade 9)** · Unit 8: Inheritance and Variation of Traits · Day 11 · lesson `ma-hs9-biology-u08-l11`
...
_Topic `ma-hs9-biology-u08::polygenic and environmental effects` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Many traits are influenced by multiple genes and by the environment, and these show continuous rather than discrete variation.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A polygenic trait is influenced by several genes, each contributing part of the effect, which produces a continuous range of phenotypes.
- The environment modifies the expression of many traits, so genotype does not determine phenotype on its own.
- Simple dominant-recessive models apply to a minority of traits and mislead when applied to polygenic ones.
...
- Modelling a continuously varying trait such as height as a single gene with two alleles and presenting the result as established.
- Claiming genotype fully determines phenotype regardless of environment.
- Claiming an environmentally induced change in an individual is inherited through the genome.

**Grade boundary.** Heritability estimation, quantitative trait loci, and genome-wide association methods are beyond this course.


## What a complete response contains, question by question

**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for polygenic and environmental effects on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain polygenic and environmental effects using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply polygenic and environmental effects to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on polygenic and environmental effects against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply polygenic and environmental effects to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain polygenic and environmental effects using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply polygenic and environmental effects to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for polygenic and environmental effects on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for polygenic and environmental effects (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for polygenic and environmental effects (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for polygenic and environmental effects (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for polygenic and environmental effects (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for polygenic and environmental effects (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u08-l11` · built from `hs912-science-h4` at `a86780a`._
