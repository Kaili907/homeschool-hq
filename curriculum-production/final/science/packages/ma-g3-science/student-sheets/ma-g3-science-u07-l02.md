# Concept model A: reading rock layers

**Grade 3 Science** · Unit 7: Fossils and Environments Long Ago · Day 2 of the unit (course day 74) · about 40–55 minutes

**Today's focus:** reading rock layers
...
- Build a clear mental model of reading rock layers through an explicit worked example, demonstration, text, or phenomenon.
- Use subject-appropriate evidence or representation to communicate understanding of reading rock layers.
- Check, revise, or improve work about reading rock layers using stated success criteria.
...
- Science brief: in undisturbed sedimentary layers, a lower layer is older than a layer above it.
- Science brief: sedimentary rock forms in layers, with newer layers deposited on top of older ones.
- Science brief: in undisturbed layers, a lower layer is older than the layer above it.
- Science brief: layers can be tilted, folded, or broken after they form, so whether a sequence is undisturbed has to be checked rather than assumed.

### Evidence check: reading rock layers

**Candidate claim:** A case report presents this as acceptable science: “the top layer is the oldest.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: in undisturbed sedimentary layers, a lower layer is older than a layer above it. |
| E2 | established science reference | Science brief: sedimentary rock forms in layers, with newer layers deposited on top of older ones. |
| E3 | established science reference | Science brief: in undisturbed layers, a lower layer is older than the layer above it. |
| E4 | established science reference | Science brief: layers can be tilted, folded, or broken after they form, so whether a sequence is undisturbed has to be checked rather than assumed. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g3-science-u07::reading rock layers, derived from the reviewed correctness source policy/correctness/ma-g3-science.correctness.json.
...
**Question:** What does evidence E1 through E4 justify about reading rock layers, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E4 justify about reading rock layers, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about reading rock layers; then identify one check that would catch an error or weak claim.**
...
**Q1.** Build a labelled model of reading rock layers that represents every supplied row in E1-E4. Annotate each model feature with its evidence ID, then show where the candidate claim fails or remains uncertain.

>

**Q2.** Communicate what the supplied record establishes about reading rock layers in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E4 and mark where the representation stops being supported.

>

**Q3.** Check your response about reading rock layers against each success criterion. Revise one sentence by adding or changing a citation to E1-E4; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using reading rock layers as the example. Cite one row from E1-E4, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E4 is strongest for evaluating the candidate claim about reading rock layers? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E4, state one conclusion about reading rock layers that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about reading rock layers. Label E1-E4 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about reading rock layers.
...
- Apply reading rock layers under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect reading rock layers to what fossils are from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g3-science-u07-l02` · lesson `ma-g3-science-u07-l02` · built from `g34-committed` at `4c6ca4e`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
