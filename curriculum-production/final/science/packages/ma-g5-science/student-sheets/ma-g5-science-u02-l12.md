# Correction and reflection: measurement precision

**Grade 5 Science** · Unit 2: Properties of Matter · Day 12 of the unit (course day 24) · about 45–65 minutes

**Today's focus:** measurement precision
...
- Use error evidence to choose a different representation or strategy for measurement precision, then demonstrate corrected understanding.
- Use subject-appropriate evidence or representation to communicate understanding of measurement precision.
- Check, revise, or improve work about measurement precision using stated success criteria.
...
- Science brief: a measuring tool can only resolve differences down to its smallest division; smaller differences than that are not measured, they are guessed.
- Science brief: repeating a measurement and getting similar values shows the method is reproducible.
- Science brief: precision is about how closely repeated measurements agree; being close together is not the same as being right.

### Evidence check: measurement precision

**Candidate claim:** A case report presents this as acceptable science: “reporting a measurement to more places than the tool can resolve.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: a measuring tool can only resolve differences down to its smallest division; smaller differences than that are not measured, they are guessed. |
| E2 | established science reference | Science brief: repeating a measurement and getting similar values shows the method is reproducible. |
| E3 | established science reference | Science brief: precision is about how closely repeated measurements agree; being close together is not the same as being right. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g5-science-u02::measurement precision, derived from the reviewed correctness source policy/correctness/ma-g5-science.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about measurement precision, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about measurement precision, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about measurement precision; then identify one check that would catch an error or weak claim.**
...
**Q1.** Locate the exact phrase in the candidate claim that conflicts with E1-E3. Name the decisive evidence ID, explain the error about measurement precision, and write a corrected claim.

>

**Q2.** Communicate what the supplied record establishes about measurement precision in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E3 and mark where the representation stops being supported.

>

**Q3.** Check your response about measurement precision against each success criterion. Revise one sentence by adding or changing a citation to E1-E3; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using measurement precision as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about measurement precision? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about measurement precision that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about measurement precision. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about measurement precision.
...
- Apply measurement precision under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect measurement precision to particles and matter models from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g5-science-u02-l12` · lesson `ma-g5-science-u02-l12` · built from `canonical-1.0.0` at `4056e31`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
