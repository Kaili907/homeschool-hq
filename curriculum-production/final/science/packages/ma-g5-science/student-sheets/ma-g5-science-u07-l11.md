# Unit assessment: data over time

**Grade 5 Science** · Unit 7: Patterns in the Sky · Day 11 of the unit (course day 83) · about 45–65 minutes

**Today's focus:** data over time
...
- Demonstrate current mastery of data over time through selected-response, constructed-response, and applied evidence.
- Use subject-appropriate evidence or representation to communicate understanding of data over time.
- Check, revise, or improve work about data over time using stated success criteria.
...
- Science brief: patterns in the sky are found by recording observations over days, months, or a year, not from single observations.
- Science brief: the record states the date, time, and place of each observation, because all three affect what is seen.
- Science brief: a pattern found over one period supports predictions only for conditions the record actually covered.

### Evidence check: data over time

**Candidate claim:** A case report presents this as acceptable science: “a yearly pattern from a few days of observation.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: patterns in the sky are found by recording observations over days, months, or a year, not from single observations. |
| E2 | established science reference | Science brief: the record states the date, time, and place of each observation, because all three affect what is seen. |
| E3 | established science reference | Science brief: a pattern found over one period supports predictions only for conditions the record actually covered. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g5-science-u07::data over time, derived from the reviewed correctness source policy/correctness/ma-g5-science.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about data over time, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about data over time, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? Rate confidence in data over time, cite one piece of evidence from today, and choose: ready to extend, ready to review later, or need a different explanation.**
...
**Q1.** Complete all three parts for data over time. (1) Select A: supported, B: partly supported, or C: not supported for the candidate claim. (2) Justify the selection with E1-E3. (3) Rewrite the claim so every word is supported by the supplied record.

>

**Q2.** Communicate what the supplied record establishes about data over time in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E3 and mark where the representation stops being supported.

>

**Q3.** Check your response about data over time against each success criterion. Revise one sentence by adding or changing a citation to E1-E3; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using data over time as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about data over time? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about data over time that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about data over time. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about data over time.
...
- Apply data over time under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect data over time to star brightness and distance from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g5-science-u07-l11` · lesson `ma-g5-science-u07-l11` · built from `canonical-1.0.0` at `4056e31`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
