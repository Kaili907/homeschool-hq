# Correction and reflection: scale models and limitations

**Grade 5 Science** · Unit 7: Patterns in the Sky · Day 12 of the unit (course day 84) · about 45–65 minutes

**Today's focus:** scale models and limitations
...
- Use error evidence to choose a different representation or strategy for scale models and limitations, then demonstrate corrected understanding.
- Use subject-appropriate evidence or representation to communicate understanding of scale models and limitations.
- Check, revise, or improve work about scale models and limitations using stated success criteria.
...
- Science brief: a scale model of the sky or the solar system cannot usually keep both sizes and distances to the same scale, because the distances are so much larger.
- Science brief: the model's limitation is stated alongside it, so nobody reads the wrong lesson from it.
- Science brief: a model is useful for the relationships it does represent, even when it distorts others.

### Evidence check: scale models and limitations

**Candidate claim:** A case report presents this as acceptable science: “reading planet sizes or distances off a classroom model as though they were to scale.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: a scale model of the sky or the solar system cannot usually keep both sizes and distances to the same scale, because the distances are so much larger. |
| E2 | established science reference | Science brief: the model's limitation is stated alongside it, so nobody reads the wrong lesson from it. |
| E3 | established science reference | Science brief: a model is useful for the relationships it does represent, even when it distorts others. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g5-science-u07::scale models and limitations, derived from the reviewed correctness source policy/correctness/ma-g5-science.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about scale models and limitations, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about scale models and limitations, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about scale models and limitations; then identify one check that would catch an error or weak claim.**
...
**Q1.** Locate the exact phrase in the candidate claim that conflicts with E1-E3. Name the decisive evidence ID, explain the error about scale models and limitations, and write a corrected claim.

>

**Q2.** Communicate what the supplied record establishes about scale models and limitations in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E3 and mark where the representation stops being supported.

>

**Q3.** Check your response about scale models and limitations against each success criterion. Revise one sentence by adding or changing a citation to E1-E3; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using scale models and limitations as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about scale models and limitations? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about scale models and limitations that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about scale models and limitations. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about scale models and limitations.
...
- Apply scale models and limitations under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect scale models and limitations to star brightness and distance from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g5-science-u07-l12` · lesson `ma-g5-science-u07-l12` · built from `canonical-1.0.0` at `4056e31`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
