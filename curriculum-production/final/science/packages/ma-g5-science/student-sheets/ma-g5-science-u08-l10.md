# Synthesis and review: community tradeoffs

**Grade 5 Science** · Unit 8: Human Impact and Environmental Decisions · Day 10 of the unit (course day 94) · about 45–65 minutes

**Today's focus:** community tradeoffs
...
- Connect community tradeoffs to the unit’s other ideas and select evidence that shows readiness for assessment.
- Use subject-appropriate evidence or representation to communicate understanding of community tradeoffs.
- Check, revise, or improve work about community tradeoffs using stated success criteria.
...
- Science brief: environmental decisions involve trade-offs: a benefit for one group or one goal often comes with a cost to another.
- Science brief: who bears the cost and who receives the benefit are part of the evidence, not a separate question.
- Science brief: a decision that ignores a trade-off has not removed it.

### Evidence check: community tradeoffs

**Candidate claim:** A case report presents this as acceptable science: “presenting a solution as having benefits and no costs.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: environmental decisions involve trade-offs: a benefit for one group or one goal often comes with a cost to another. |
| E2 | established science reference | Science brief: who bears the cost and who receives the benefit are part of the evidence, not a separate question. |
| E3 | established science reference | Science brief: a decision that ignores a trade-off has not removed it. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g5-science-u08::community tradeoffs, derived from the reviewed correctness source policy/correctness/ma-g5-science.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about community tradeoffs, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about community tradeoffs, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about community tradeoffs; then identify one check that would catch an error or weak claim.**
...
**Q1.** The supplied unit connections are: resource use, waste and pollution, conservation strategies, community tradeoffs, evidence-based decisions, solution criteria. Choose one idea other than community tradeoffs, connect it to one row in E1-E3, and explain one way treating the two ideas as identical would be misleading.

>

**Q2.** Communicate what the supplied record establishes about community tradeoffs in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E3 and mark where the representation stops being supported.

>

**Q3.** Check your response about community tradeoffs against each success criterion. Revise one sentence by adding or changing a citation to E1-E3; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using community tradeoffs as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about community tradeoffs? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about community tradeoffs that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about community tradeoffs. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about community tradeoffs.
...
- Apply community tradeoffs under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect community tradeoffs to resource use from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g5-science-u08-l10` · lesson `ma-g5-science-u08-l10` · built from `canonical-1.0.0` at `4056e31`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
