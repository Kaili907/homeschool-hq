...
**Grade 5 Science** · Unit 1: Science and Engineering Practices · Day 12 of the unit (course day 12) · about 45–65 minutes
...
- Science brief: criteria state what a successful solution must achieve; constraints state the limits it must respect, including materials, cost, time, and safety.
- Science brief: criteria and constraints are set before solutions are generated, so all options are judged the same way.
- Science brief: a solution that violates a constraint has not solved the problem, however well it scores on the criteria.
...
| --- | --- | --- |
| E1 | established science reference | Science brief: criteria state what a successful solution must achieve; constraints state the limits it must respect, including materials, cost, time, and safety. |
| E2 | established science reference | Science brief: criteria and constraints are set before solutions are generated, so all options are judged the same way. |
| E3 | established science reference | Science brief: a solution that violates a constraint has not solved the problem, however well it scores on the criteria. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g5-science-u01::criteria and constraints, derived from the reviewed correctness source policy/correctness/ma-g5-science.correctness.json.
...
- Connect criteria and constraints to observable phenomena from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g5-science-u01-l12` · lesson `ma-g5-science-u01-l12` · built from `canonical-1.0.0` at `4056e31`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
