...
**Grade 5 Science** · Unit 1: Science and Engineering Practices · Day 10 · lesson `ma-g5-science-u01-l10`
...
_Topic `ma-g5-science-u01::fair tests` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g5-science.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- A fair test changes one variable and holds the others constant.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A fair test changes one thing on purpose and holds constant everything else that could affect the result.
- If two conditions change together, the result cannot be attributed to either.
- Repeating trials shows whether a result is reproducible; a single trial cannot.
...
- Calling a test fair when more than one condition changed.
- Treating a single trial as proof of a result.
- Discarding a trial because it disagreed with the others, without saying so and why.
...
_Package `swp-ma-g5-science-u01-l10` · built from `canonical-1.0.0` at `4056e31`._
