# Scoring authority — Performance task build: daily shadow patterns

**Grade 5 Science** · Unit 7: Patterns in the Sky · Day 9 · lesson `ma-g5-science-u07-l09`
...
_Topic `ma-g5-science-u07::daily shadow patterns` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g5-science.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Day and night are caused by Earth rotating on its axis, not by the Sun moving around Earth.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Earth's rotation on its axis causes the daily cycle of day and night and the apparent movement of the Sun across the sky.
- A shadow is longest when the Sun is lowest in the sky and shortest when it is highest, so shadows change in a repeating daily pattern.
- A shadow points away from the light source, so shadow direction changes through the day as the Sun's apparent position changes.
...
- Claiming the Sun orbits Earth each day.
- Claiming shadows are longest at midday.
- Claiming day and night are caused by Earth's orbit around the Sun rather than by its rotation.

**Grade boundary.** Solar altitude calculations, solar time, and the analemma are later work.
...
**Q1 — build product.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies daily shadow patterns and the unit standards.
...
**Q2 — communicate representation.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of daily shadow patterns.
...
**Q3 — check revise.** _Serves objective 3:_ Check, revise, or improve work about daily shadow patterns using stated success criteria.
...
**Q4 — essential question.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies daily shadow patterns and the unit standards.
...
**Q5 — evidence quality.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of daily shadow patterns.
...
**Q6 — limitation.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies daily shadow patterns and the unit standards.
...
**Q7 — provenance.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of daily shadow patterns.
...
- **prerequisite gap** — Return to the smallest prerequisite needed for daily shadow patterns, model it with a concrete or text-only representation, then retry one fresh item.
- **procedure without understanding** — Ask the learner to explain or represent why the process for daily shadow patterns works before continuing.
- **correct but low confidence** — Confirm the reasoning specifically, offer one varied example of daily shadow patterns, and avoid unnecessary remediation.
...
- **mastery evidence** — Require accurate independent application plus explanation on a later or different occasion before marking daily shadow patterns mastered.
...
_Package `swp-ma-g5-science-u07-l09` · built from `canonical-1.0.0` at `4056e31`._
