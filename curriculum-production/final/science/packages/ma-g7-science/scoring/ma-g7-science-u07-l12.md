# Scoring authority — Correction and reflection: model uncertainty

**Grade 7 Science** · Unit 7: Weather, Climate, and Earth Systems · Day 12 · lesson `ma-g7-science-u07-l12`
...
_Topic `ma-g7-science-u07::model uncertainty` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g7-science.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Weather and climate models are simplifications, so their predictions carry uncertainty that grows with the length of the forecast.
- Uncertainty in a forecast is a statement about the model and the data, not an admission that the science is unreliable.
- A forecast is evaluated over many predictions, not by whether one turned out right.
...
- Judging a probabilistic forecast wrong because a single day differed from it.
- Claiming uncertainty means the model carries no information.
- Presenting a model prediction as a measurement.

**Grade boundary.** Ensemble forecasting and formal skill scores are High School work.


## What a complete response contains, question by question

**Q1 — error analysis.** _Serves objective 1:_ Use error evidence to choose a different representation or strategy for model uncertainty, then demonstrate corrected understanding.
...
**Q2 — communicate representation.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of model uncertainty.
...
**Q3 — check revise.** _Serves objective 3:_ Check, revise, or improve work about model uncertainty using stated success criteria.
...
**Q4 — essential question.** _Serves objective 1:_ Use error evidence to choose a different representation or strategy for model uncertainty, then demonstrate corrected understanding.
...
**Q5 — evidence quality.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of model uncertainty.
...
**Q6 — limitation.** _Serves objective 1:_ Use error evidence to choose a different representation or strategy for model uncertainty, then demonstrate corrected understanding.
...
**Q7 — provenance.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of model uncertainty.
...
- **prerequisite gap** — Return to the smallest prerequisite needed for model uncertainty, model it with a concrete or text-only representation, then retry one fresh item.
- **procedure without understanding** — Ask the learner to explain or represent why the process for model uncertainty works before continuing.
- **correct but low confidence** — Confirm the reasoning specifically, offer one varied example of model uncertainty, and avoid unnecessary remediation.
...
- **mastery evidence** — Require accurate independent application plus explanation on a later or different occasion before marking model uncertainty mastered.
...
_Package `swp-ma-g7-science-u07-l12` · built from `canonical-1.0.0` at `4056e31`._
