# Unit assessment: model limitations

**Grade 7 Science** · Unit 1: Matter, Particles, and Properties · Day 11 of the unit (course day 11) · about 45–65 minutes
...
- Demonstrate current mastery of model limitations through selected-response, constructed-response, and applied evidence.
...
- Science brief: every model of matter simplifies, and the simplification determines what the model can and cannot explain.
- Science brief: a model is chosen for the question being asked; a model that is wrong for one purpose can be right for another.
- Science brief: stating a model's limitation is part of using it correctly, not an admission of failure.

### Evidence check: model limitations

**Candidate claim:** A case report presents this as acceptable science: “treating a model's simplification as a property of the real system, such as claiming atoms are coloured spheres.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: every model of matter simplifies, and the simplification determines what the model can and cannot explain. |
| E2 | established science reference | Science brief: a model is chosen for the question being asked; a model that is wrong for one purpose can be right for another. |
| E3 | established science reference | Science brief: stating a model's limitation is part of using it correctly, not an admission of failure. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g7-science-u01::model limitations, derived from the reviewed correctness source policy/correctness/ma-g7-science.correctness.json.
...
**What did you check, what did you change, and what is your next step? Rate confidence in model limitations, cite one piece of evidence from today, and choose: ready to extend, ready to review later, or need a different explanation.**
...
**Q1.** Complete all three parts for model limitations. (1) Select A: supported, B: partly supported, or C: not supported for the candidate claim. (2) Justify the selection with E1-E3. (3) Rewrite the claim so every word is supported by the supplied record.
...
- Connect model limitations to atomic and molecular models from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g7-science-u01-l11` · lesson `ma-g7-science-u01-l11` · built from `canonical-1.0.0` at `4056e31`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
