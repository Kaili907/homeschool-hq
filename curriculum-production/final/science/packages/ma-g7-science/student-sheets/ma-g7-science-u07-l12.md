# Correction and reflection: model uncertainty

**Grade 7 Science** · Unit 7: Weather, Climate, and Earth Systems · Day 12 of the unit (course day 84) · about 45–65 minutes

**Today's focus:** model uncertainty
...
- Use error evidence to choose a different representation or strategy for model uncertainty, then demonstrate corrected understanding.
- Use subject-appropriate evidence or representation to communicate understanding of model uncertainty.
- Check, revise, or improve work about model uncertainty using stated success criteria.
...
- Science brief: weather and climate models are simplifications, so their predictions carry uncertainty that grows with the length of the forecast.
- Science brief: uncertainty in a forecast is a statement about the model and the data, not an admission that the science is unreliable.
- Science brief: a forecast is evaluated over many predictions, not by whether one turned out right.

### Evidence check: model uncertainty

**Candidate claim:** A case report presents this as acceptable science: “judging a probabilistic forecast wrong because a single day differed from it.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: weather and climate models are simplifications, so their predictions carry uncertainty that grows with the length of the forecast. |
| E2 | established science reference | Science brief: uncertainty in a forecast is a statement about the model and the data, not an admission that the science is unreliable. |
| E3 | established science reference | Science brief: a forecast is evaluated over many predictions, not by whether one turned out right. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g7-science-u07::model uncertainty, derived from the reviewed correctness source policy/correctness/ma-g7-science.correctness.json.
...
**Question:** What does evidence E1 through E3 justify about model uncertainty, and does it support the candidate claim?
...
**The task.** What does evidence E1 through E3 justify about model uncertainty, and does it support the candidate claim? Follow the five bound steps printed above and cite the supplied evidence IDs.
...
**What did you check, what did you change, and what is your next step? In one concise response, show or explain the most important idea about model uncertainty; then identify one check that would catch an error or weak claim.**
...
**Q1.** Locate the exact phrase in the candidate claim that conflicts with E1-E3. Name the decisive evidence ID, explain the error about model uncertainty, and write a corrected claim.

>

**Q2.** Communicate what the supplied record establishes about model uncertainty in a labelled diagram, table, or paragraph. Include citations to at least two rows from E1-E3 and mark where the representation stops being supported.

>

**Q3.** Check your response about model uncertainty against each success criterion. Revise one sentence by adding or changing a citation to E1-E3; state exactly what the revision fixes.

>

**Q4.** Answer the printed unit question using model uncertainty as the example. Cite one row from E1-E3, connect it to the answer, and state what the supplied record cannot decide.

>

**Q5.** Which one row in E1-E3 is strongest for evaluating the candidate claim about model uncertainty? Name the row, explain why it is more direct than another supplied row, and name one input that would strengthen it.

>

**Q6.** Using the supplied case and E1-E3, state one conclusion about model uncertainty that the record supports and one question it cannot answer. Name the different observation, measurement, or model input the unanswered question would require.

>

**Q7.** Audit the inputs used in your response about model uncertainty. Label E1-E3 as SUPPLIED reference information with the provenance printed below the table; label any model value as MODEL OUTPUT and any value you calculate as CALCULATED.
...
- The learner completes the central task about model uncertainty.
...
- Apply model uncertainty under a new constraint, compare two approaches or perspectives, or teach the idea with an original example without completing another learner’s graded work.
- Connect model uncertainty to water cycle driven by energy and gravity from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g7-science-u07-l12` · lesson `ma-g7-science-u07-l12` · built from `canonical-1.0.0` at `4056e31`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
