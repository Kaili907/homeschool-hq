...
**Grade 7 Science** · Unit 9: Engineering Design Capstone · Day 10 of the unit (course day 106) · about 45–65 minutes
...
- Science brief: a model is revised when its prediction and the test result disagree, and the revision names what the disagreement showed.
- Science brief: changing one aspect at a time keeps the effect of each change identifiable.
- Science brief: a model that has never been tested against data has not yet been evaluated, however reasonable it looks.

### Evidence check: iterative modeling

**Candidate claim:** A case report presents this as acceptable science: “revising a model to fit data without saying what the disagreement showed.”
...
| --- | --- | --- |
| E1 | established science reference | Science brief: a model is revised when its prediction and the test result disagree, and the revision names what the disagreement showed. |
| E2 | established science reference | Science brief: changing one aspect at a time keeps the effect of each change identifiable. |
| E3 | established science reference | Science brief: a model that has never been tested against data has not yet been evaluated, however reasonable it looks. |

**Input provenance:** Manuel Academy Science learner brief for topic ma-g7-science-u09::iterative modeling, derived from the reviewed correctness source policy/correctness/ma-g7-science.correctness.json.
...
**Q1.** The supplied unit connections are: defining criteria and constraints, evaluating competing solutions, test data, iterative modeling, optimization, communicating tradeoffs. Choose one idea other than iterative modeling, connect it to one row in E1-E3, and explain one way treating the two ideas as identical would be misleading.
...
- Connect iterative modeling to defining criteria and constraints from this unit: state one thing that must be true of both, and one case where treating them the same would mislead you.
...
_Package `swp-ma-g7-science-u09-l10` · lesson `ma-g7-science-u09-l10` · built from `canonical-1.0.0` at `4056e31`. Supplied reference and model inputs are labelled; physical-investigation observations and expected results are never supplied before collection._
