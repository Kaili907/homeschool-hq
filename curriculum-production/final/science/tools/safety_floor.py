...
fails if a clause has no variant.
  ...
  hs-h4   mac/hs912-science-h4 @ a86780a — the eleven non-disableable
          prohibitions, the required privacy declarations, and the global stop
          conditions carried by every lesson in the H4 package. H4 supersedes
          H3 and H2; the count rose from ten when H3 added the unconditional open-flame
...
def _common_stop_conditions(hs_lessons_by_course: dict[str, list[dict]]) -> list[str]:
    ...
    Taken as the intersection over all H4 lessons so a condition that only some
...
def build_safety_floor(reader: SourceReader, lessons_by_course: dict[str, list[dict]]) -> dict:
    policy_set = reader.read_json("hs912-science-h4", HS_H4_COMMIT, HS_POLICY_SET_PATH)
    ...
    return {
        ...
        "attribution": {
            "hs-h4": {
                ...
                "commit": HS_H4_COMMIT,
                "note": (
                    "H4 source-consistency fix; supersedes H3 and H2. No superseded "
                    "High School source blob is read by this build."
                ),
            },
            "g34": {
