 ...
 *
 * A banned term is a violation only when a preceding, same-clause negation
 * does not govern it. This lexical, forward-only rule is the accepted Grade 3
 * Health H2 behavior: `no` cannot be inferred from `now`/`note`, and a
 * prohibition in one clause cannot launder a later requirement. This is a heuristic scan for
...
const BANNED_PATTERNS = [
  ...
  { id: 'diagnosis', re: /\b(?:require|provide|share|report|submit|reveal|state)\w*\b[^.]{0,60}\b(?:medical|mental[- ]health)\s+diagnos(?:e|es|ed|ing|is)\b|\bdiagnos(?:e|es|ed|ing)\b[^.]{0,40}\b(?:medical|mental[- ]health)\b/i },
...
const COLLECT_VERB =
  'record|records|log|logs|take|takes|measure|measures|count|counts|weigh|weighs|track|tracks|submit|submits|require|requires|enter|enters|report|reports|write|writes|photograph|photographs|upload|uploads|diagnose|diagnoses'
  ...
  for (const { path, text } of strings) {
    for (const sentence of sentences(text)) {
      for (const clause of sentence.split(CLAUSE_BREAK)) {
        if (!clause) continue
        const guard = NEGATION_CUE.exec(clause)?.index ?? -1
        for (const { id, re } of BANNED_PATTERNS) {
          const match = re.exec(clause)
          if (!match) continue
          if (guard !== -1 && guard < match.index) continue
          violations.push({ fileLabel, path, patternId: id, sentence: clause.trim() })
        }
      }
    }
  }
