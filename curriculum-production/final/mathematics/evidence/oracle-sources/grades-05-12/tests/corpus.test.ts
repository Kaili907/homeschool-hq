...
describe('lesson sources', () => {
  it('covers every eligible grade with a full 180-lesson course', () => {
    ...
    expect(allLessons).toHaveLength(1260)
  })

  it('excludes grades 3 and 4, whose authoring branch is still moving', () => {
    expect(ELIGIBLE_GRADES).not.toContain(3)
    expect(ELIGIBLE_GRADES).not.toContain(4)
    expect(allLessons.every((lesson) => lesson.ref.grade >= 5)).toBe(true)
  })
})
...
describe('corpus emission', () => {
  ...
  it('is deterministic: the same lesson emits identical output on every run', () => {
    for (const lesson of [allLessons[0], allLessons[600], allLessons[1259]]) {
  ...
  it('varies package composition by lesson phase rather than reusing one template', () => {
    const unitLessons = allLessons.filter(
      (lesson) => lesson.ref.courseId === 'ma-g9-mathematics' && lesson.ref.unitNumber === 3,
    )
  ...
  it('gives lessons in the same unit different questions', () => {
    const unitLessons = allLessons.filter(
      (lesson) => lesson.ref.courseId === 'ma-g12-mathematics' && lesson.ref.unitNumber === 6,
    )
    const first = new Set(
...
describe('answer separation', () => {
  ...
  it('keeps the answer key out of the student projection entirely', () => {
    // workedSolution is deliberately absent from this list: an instructional
    // worked example is meant to show its own solution to the learner. What must
    // never appear is anything revealing the answer to a graded item.
    const neverInPackage = ['answerIndex', 'solutionReasoning', 'commonErrors', 'given']
    for (const lesson of allLessons.slice(0, 120)) {
...
describe('answer authority', () => {
  it('recomputes every high-school answer from parameters on an independent oracle', () => {
    // makeHsUnitBank throws when its oracle disagrees with the built answer, so
    // exercising every high-school item type across difficulties is the check.
    let generated = 0
  ...
  it('records how each answer was established', () => {
    for (const lesson of [allLessons[0], allLessons[1000]]) {
      const emitted = emitLesson(lesson)
      for (const entry of emitted.answerKey.answers) {
        expect(['recomputed', 'generator-authority']).toContain(entry.verification.method)
  ...
  it('reasons about the item itself, never about the reference example', () => {
    // The canonical generators store one worked example per item type, so the
    // reference example is a different problem. The solution reasoning must not
    // be a copy of it.
    for (const lesson of allLessons.slice(0, 300)) {
      for (const entry of emitLesson(lesson).answerKey.answers) {
        expect(entry.solutionReasoning.steps).not.toEqual(entry.referenceExample.steps)
...
describe('standard alignment', () => {
  it('treats a sub-standard as covering its parent and nothing else', () => {
    expect(standardsAlign('8.EE.8a', '8.EE.8')).toBe(true)
    expect(standardsAlign('8.EE.8', '8.EE.8a')).toBe(true)
    expect(standardsAlign('8.EE.8', '8.EE.80')).toBe(false)
    expect(standardsAlign('8.EE.8', '8.EE.7')).toBe(false)
    expect(standardCoveredBy('N-RN.2', ['N-Q.1', 'N-RN.2'])).toBe(true)
    expect(standardCoveredBy('N-RN.2', ['N-Q.1'])).toBe(false)
  })
...
describe('course identity', () => {
  ...
  it('does not reuse questions across different courses', () => {
    const grade8 = new Set(
      emitLesson(readLessons(8)[90]).package.sections.flatMap((section) =>
        section.items.map((item) => item.prompt),
      ),
    )
    const grade12 = emitLesson(readLessons(12)[90]).package.sections.flatMap((section) =>
      section.items.map((item) => item.prompt),
    )
    expect(grade12.filter((prompt) => grade8.has(prompt))).toEqual([])
  })
})
...
describe('equivalent-distractor guard', () => {
  it('parses the numeric forms the corpus renders', () => {
    ...
    expect(numericValue('√576')).toBe(24)
    expect(numericValue('6√2')).toBeCloseTo(8.485281, 5)
  ...
  it('flags a distractor equal in value to the answer but written differently', () => {
    expect(isEquivalentButDifferent('√576', '24')).toBe(true)
    ...
    expect(isEquivalentButDifferent('irrational', 'rational')).toBe(false)
  })
