/**
 * Machine-readable student-work material format for Manuel Academy mathematics.
 *
 ...
 *
 * The same package feeds digital Study Engine rendering and (later) printable
 * worksheet rendering; nothing in this format is renderer-specific.
 */
 ...
 * that produced the item. That is the standard for grades 9-12, whose
 * generators are authored here.
 *
 * `generator-authority` means the answer comes from a canonical curriculum
 * generator in src/curriculum whose arithmetic is independently verified by its
 * own oracle test suite (see `oracle`), rather than being re-derived here. That
 * is the standard for grades 5, 7, and 8, where the generators and their
 * independent oracles already existed.
 */
 ...
 *
 * This is a *different problem* from the item it accompanies — the canonical
 * generators store one worked example per item type rather than per item. It is
 * kept because it shows the method, and it is named `referenceExample` so no
 * renderer can present it as this item's solution.
 */
