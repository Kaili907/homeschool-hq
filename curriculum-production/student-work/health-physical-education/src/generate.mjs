...
function trustedAdultSentence(lesson) {
  ...
  return sentence ?? 'Direct any urgent safety, health, or mental-health concern to a trusted adult or qualified professional; this course is not diagnosis, therapy, or treatment.'
}
...
function buildLessonArtifacts(lesson, unit, subject, grade) {
  ...
  const pkg = {
    ...
    sourceProvenance: { sourceBranch: sourceBranchLabel(grade), sourceLessonId: lesson.lesson_id },
  }
...
function buildAssessmentArtifacts(assessment, unit, subject, grade, courseId) {
  ...
  const pkg = {
    ...
    neverRequires: NEVER_REQUIRES,
    sourceProvenance: { sourceBranch: sourceBranchLabel(grade), sourceAssessmentId: assessmentId },
  }
...
function main() {
  ...
  const manifest = {
    ...
    generatedFrom: {
      grade34: 'mac/g34-health-pe-r1@d0ebaa0 (curriculum-authoring/full-family-grade34)',
      canonical578: 'shared base@656efba (curriculum-content/manuel-academy/1.0.0, grades 5, 7, 8)',
      hs912: 'mac/hs912-health-pe-r1@e39e2b3 (curriculum-authoring/full-family-highschool-9-12)',
