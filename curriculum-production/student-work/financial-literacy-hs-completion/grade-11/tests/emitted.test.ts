...
const ADULT_ONLY_KEYS = [
  ...
  'scoringAuthority', 'judgment', 'substituted', 'why', 'expression', 'exactKey', 'decision', 'expr',
]
...
describe('the emitted corpus', () => {
  ...
  it('composes deterministically: the same spec produces byte-identical output twice', () => {
    const unstable: string[] = []
    for (const spec of ALL_SPECS) {
      const src = source.get(spec.lessonId)!
      const a = `${JSON.stringify(composeTaskSheet(spec, src), null, 2)}\n${JSON.stringify(composeScoringRecord(spec, src), null, 2)}`
      const b = `${JSON.stringify(composeTaskSheet(spec, src), null, 2)}\n${JSON.stringify(composeScoringRecord(spec, src), null, 2)}`
      if (a !== b) unstable.push(spec.lessonId)
    }
  ...
  it('ships one task sheet and one scoring record per authored lesson and nothing else', () => {
    ...
    expect(walk(join(ROOT, 'packages'), '.json')).toHaveLength(ALL_SPECS.length)
    expect(walk(join(ROOT, 'scoring'), '.json')).toHaveLength(ALL_SPECS.length)
  })

  it('keeps every answer, rubric descriptor, worked solution, and look-for out of the learner-facing sheet', () => {
    const leaks: string[] = []
    for (const spec of ALL_SPECS) {
  ...
  it('never prints an authored answer string in the learner-facing sheet', () => {
    const leaks: string[] = []
    for (const spec of ALL_SPECS) {
      const sheetText = JSON.stringify(composeTaskSheet(spec, source.get(spec.lessonId)!))
      for (const task of spec.tasks) {
        for (const item of task.items) {
          if (item.kind === 'judgment') continue
          // A keyed choice necessarily prints its options; a computed answer must not appear.
          if (item.kind === 'numeric' && sheetText.includes(`"${item.answer}"`)) {
            leaks.push(`${spec.lessonId} ${item.ref}: the answer ${item.answer} appears as a value in the sheet`)
          }
        }
      }
    }
  ...
  it('gives every lesson both a fixed scoring authority and a rubric authority', () => {
    const wrong: string[] = []
    for (const spec of ALL_SPECS) {
      const rec = composeScoringRecord(spec, source.get(spec.lessonId)!) as {
        scoringAuthority: { kind: string; items?: unknown[]; judgment?: unknown[]; criteria?: unknown[]; workedSolution?: unknown[] }
      }
      const a = rec.scoringAuthority
      if (a.kind !== 'HYBRID') wrong.push(`${spec.lessonId}: scoring authority is ${a.kind}, not HYBRID`)
      if (!a.items?.length) wrong.push(`${spec.lessonId}: no fixed scoring authority`)
      if (!a.judgment?.length) wrong.push(`${spec.lessonId}: no rubric authority`)
      if (!a.criteria?.length) wrong.push(`${spec.lessonId}: no rubric criteria`)
      if (!a.workedSolution?.length) wrong.push(`${spec.lessonId}: no worked solution`)
    }
