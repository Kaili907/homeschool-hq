...
/** The lane's own scoring vocabulary, derived from what the lesson actually carries. */
export function scoringKind(spec: LessonSpec): 'ANSWER_KEY' | 'RUBRIC' | 'HYBRID' {
  ...
  const judged = items.some((i) => i.kind === 'judgment')
  return fixed && judged ? 'HYBRID' : fixed ? 'ANSWER_KEY' : 'RUBRIC'
}
...
export function composeScoringRecord(spec: LessonSpec, src: SourceLesson): Record<string, unknown> {
  ...
  const kind = items.length > 0 && judgment.length > 0
    ? 'HYBRID'
    : items.length > 0
      ? 'ANSWER_KEY'
      : 'RUBRIC'
