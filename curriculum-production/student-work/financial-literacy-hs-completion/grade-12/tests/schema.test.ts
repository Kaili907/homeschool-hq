...
/**
 * Conformance of the emitted corpus to the three JSON Schemas in schema/.
 *
 * No JSON Schema validator is available to this lane without adding a
 * dependency to a repository it does not own, so this checks the schemas'
 ...
 * schema pins. It reads the constraints out of the schema files rather than
 * restating them, so a schema edit is felt here rather than silently diverging
 * from what ships.
 */
...
describe('the emitted corpus conforms to the lane schemas', () => {
  ...
  it('the gate metadata matches schema/gate-metadata.schema.json', () => {
    const schema = loadSchema('gate-metadata.schema.json')
    expect(conform(buildGateMetadata(ALL_SPECS) as unknown as Record<string, unknown>, schema, 'gate/gate-metadata.json')).toEqual([])
  })
  ...
  it('requires at least two integrated domains on every scoring record', () => {
    const schema = loadSchema('scoring-record.schema.json') as unknown as {
      properties: { integratedDomains: { minItems: number } }
    }
    expect(schema.properties.integratedDomains.minItems).toBe(2)
    const records = ALL_SPECS.map((s) => composeScoringRecord(s, source.get(s.lessonId)!) as { integratedDomains: string[] })
    expect(records.every((r) => r.integratedDomains.length >= 2)).toBe(true)
  })
})
