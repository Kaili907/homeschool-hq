...
describe('loadCorpus', () => {
  ...
  it('every entry is Grade 8 ready-for-life with subjectFamily ARTS_RFL_PE_PROJECT', () => {
    for (const { pkg } of entries) {
      expect(pkg.lessonRef.subject).toBe('ready-for-life')
      expect(pkg.lessonRef.grade).toBe(8)
      expect(pkg.lessonRef.courseId).toBe('ma-g8-ready-for-life')
