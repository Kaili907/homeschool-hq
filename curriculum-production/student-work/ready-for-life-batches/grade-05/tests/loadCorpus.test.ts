...
describe('loadCorpus', () => {
  ...
  it('every entry is subject ready-for-life, grade 5, subjectFamily ARTS_RFL_PE_PROJECT', () => {
    for (const { pkg } of entries) {
      expect(pkg.lessonRef.subject).toBe('ready-for-life')
      expect(pkg.lessonRef.grade).toBe(5)
      expect(pkg.subjectFamily).toBe('ARTS_RFL_PE_PROJECT')
    }
  })

  it('loads exactly 36 lessons — the full grade-5 Ready for Life inventory', () => {
