...
describe('local production-readiness checks over the authored corpus', () => {
  ...
  it('at least one lesson is guardian-attested (a genuine adult-observed component exists)', () => {
    const guardianCount = entries.filter((e) => e.pkg.completionAuthority === 'guardian').length
    expect(guardianCount).toBeGreaterThan(0)
  })

  it('not every lesson is guardian-attested (attestation is reserved, not uniform)', () => {
