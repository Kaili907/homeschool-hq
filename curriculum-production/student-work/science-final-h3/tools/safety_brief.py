...
High School briefs are built from the H3 package (mac/hs912-science-h3 @
e7551b9), which supersedes H2. Where H3 already authored a student-visible `safety-review` segment,
...
def build_hs_brief(lesson: dict, floor: dict) -> dict:
    ...
    return {
        ...
        "h3_safety_segment_verbatim": segment,
        "h3_lab_safety_summary_verbatim": summary,
    }
...
def brief_lines(resolved: dict) -> list[str]:
    ...
    if resolved.get("h3_safety_segment_verbatim"):
        lines.append(resolved["h3_safety_segment_verbatim"])
