...
fails if a clause has no variant.
  ...
  hs-h3   mac/hs912-science-h3 @ e7551b9 — the eleven non-disableable
          prohibitions, the required privacy declarations, and the global stop
          conditions carried by every lesson in the H3 package. H3 supersedes
          H2: the count rose from ten when H3 added the unconditional open-flame
          prohibition, and the floor takes whatever the pinned policy set states
...
def _common_stop_conditions(hs_lessons_by_course: dict[str, list[dict]]) -> list[str]:
    ...
    Taken as the intersection over all H3 lessons so a condition that only some
...
def build_safety_floor(reader: SourceReader, lessons_by_course: dict[str, list[dict]]) -> dict:
    policy_set = reader.read_json("hs912-science-h3", HS_H3_COMMIT, HS_POLICY_SET_PATH)
    ...
    return {
        ...
        "attribution": {
            "hs-h3": {
                "branch": "mac/hs912-science-h3",
                "commit": HS_H3_COMMIT,
                "note": (
                    "H3 learner-use safety fix; supersedes H2. Neither the superseded H2 "
                    "commit 265ea3a nor the failed base candidate f58f7f1 is read by this build."
                ),
            },
            "g34": {
