# Scoring authority — Independent application A: evaluating published claims about radiation

**High School Physics (Grade 11)** · Unit 9: Electromagnetic Radiation, Information, and a Physics Engineering Capstone · Day 4 · lesson `ma-hs11-physics-u09-l04`
...
_Topic `ma-hs11-physics-u09::evaluating published claims about radiation` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The distinction between ionising and non-ionising radiation is set by photon energy, which depends on frequency, not by the power of the source.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Evaluating a claim about radiation means identifying which part of the spectrum is involved, at what intensity, and over what exposure.
- A claim's strength depends on the study design and on whether independent work agrees, not on the confidence of the presentation.
- The word radiation covers everything from radio waves to gamma rays, so a claim about radiation that does not specify which is not evaluable.
...
- Treating all radiation as equivalent in its effects on matter or on people.
- Claiming a non-ionising source becomes ionising at high power.
- Accepting or rejecting a published claim without reference to the evidence or the study design behind it.

**Grade boundary.** Epidemiological methods, exposure standards, and health physics are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain evaluating published claims about radiation using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply evaluating published claims about radiation to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on evaluating published claims about radiation against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply evaluating published claims about radiation to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain evaluating published claims about radiation using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply evaluating published claims about radiation to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain evaluating published claims about radiation using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for evaluating published claims about radiation (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for evaluating published claims about radiation (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for evaluating published claims about radiation (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for evaluating published claims about radiation (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for evaluating published claims about radiation (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u09-l04` · built from `hs912-science-h3` at `e7551b9`._
