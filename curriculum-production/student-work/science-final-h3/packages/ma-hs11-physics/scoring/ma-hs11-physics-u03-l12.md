# Scoring authority — Correction and reflection: engineering a protective device

**High School Physics (Grade 11)** · Unit 3: Momentum, Collisions, and Protective Design · Day 12 · lesson `ma-hs11-physics-u03-l12`
...
_Topic `ma-hs11-physics-u03::engineering a protective device` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- A protective device works by extending the time or the distance over which momentum changes, thereby reducing peak force.
- It can also spread the force over a larger area, reducing pressure, which is a distinct mechanism from reducing the force.
- The device is evaluated against measured peak force or deceleration under stated test conditions, not against how sturdy it appears.
...
- Claiming a protective device removes the momentum change rather than extending the time over which it occurs.
- Confusing spreading force over area with reducing the total force.
- Declaring a design effective without a measurement against the stated criterion.

**Grade boundary.** Materials testing standards and biomechanical injury criteria are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain engineering a protective device using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply engineering a protective device to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on engineering a protective device against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply engineering a protective device to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain engineering a protective device using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply engineering a protective device to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain engineering a protective device using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for engineering a protective device (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for engineering a protective device (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for engineering a protective device (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for engineering a protective device (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for engineering a protective device (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u03-l12` · built from `hs912-science-h3` at `e7551b9`._
