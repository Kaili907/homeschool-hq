# Scoring authority — Correction and reflection: vectors and components

**High School Physics (Grade 11)** · Unit 1: Motion, Measurement, and Quantitative Reasoning · Day 12 · lesson `ma-hs11-physics-u01-l12`
...
_Topic `ma-hs11-physics-u01::vectors and components` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Vectors add by components or head-to-tail; they do not add by simple arithmetic unless they are along the same line.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A vector is fully specified by magnitude and direction and can be resolved into perpendicular components.
- Perpendicular components are independent: motion or force in one direction does not affect the other.
- The magnitude of a vector sum is generally less than the sum of the magnitudes, and equals it only when the vectors are parallel.

**Also correct.** Accept any of these framings — do not require the wording above:

- Component method and scale diagram method give the same result and are both acceptable.
...
- Adding vector magnitudes arithmetically when the vectors are not along the same line.
- Claiming horizontal and vertical components of projectile motion affect one another.
- Reporting a vector result with magnitude only where direction is required.

**Grade boundary.** Three-dimensional vector algebra, dot and cross products are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain vectors and components using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply vectors and components to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on vectors and components against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply vectors and components to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain vectors and components using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply vectors and components to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain vectors and components using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for vectors and components (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for vectors and components (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for vectors and components (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for vectors and components (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for vectors and components (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u01-l12` · built from `hs912-science-h3` at `e7551b9`._
