# Scoring authority — Reteach and varied practice: velocity and acceleration

**High School Physics (Grade 11)** · Unit 1: Motion, Measurement, and Quantitative Reasoning · Day 8 · lesson `ma-hs11-physics-u01-l08`
...
_Topic `ma-hs11-physics-u01::velocity and acceleration` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._
...
- Acceleration is non-zero whenever speed changes, direction changes, or both.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Velocity is a vector: it has magnitude and direction, and speed is its magnitude.
- An object moving at constant speed around a curve is accelerating, because its direction, and therefore its velocity, is changing.
- Acceleration and velocity are independent in direction: an object can be moving one way and accelerating the other, which is what slowing down is.
...
- Claiming acceleration is always in the direction of motion.
- Claiming an object moving at constant speed is never accelerating.
- Using speed and velocity interchangeably where direction matters.

**Grade boundary.** Calculus definitions of instantaneous rate are acceptable but not required.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain velocity and acceleration using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply velocity and acceleration to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on velocity and acceleration against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply velocity and acceleration to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain velocity and acceleration using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply velocity and acceleration to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain velocity and acceleration using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for velocity and acceleration (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for velocity and acceleration (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for velocity and acceleration (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for velocity and acceleration (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for velocity and acceleration (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u01-l08` · built from `hs912-science-h3` at `e7551b9`._
