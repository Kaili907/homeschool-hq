# Scoring authority — Correction and reflection: investigating induction

**High School Physics (Grade 11)** · Unit 7: Fields, Electromagnetism, and Induction · Day 12 · lesson `ma-hs11-physics-u07-l12`
...
_Topic `ma-hs11-physics-u07::investigating induction` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- An induction investigation varies one factor at a time: relative speed, field strength, number of turns, or orientation.
- Detecting no induced effect when nothing is changing is a positive result confirming the model, not a failed trial.
- Measurements state the instrument, its sensitivity, and the conditions, since induced voltages are often small.
...
- Reporting an induced voltage under static conditions with no changing flux and attributing it to induction.
- Varying several factors at once and attributing the effect to one.
- Treating an instrument's limit of detection as evidence that no effect exists.

**Grade boundary.** Instrumentation design and signal conditioning are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain investigating induction using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply investigating induction to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on investigating induction against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply investigating induction to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain investigating induction using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply investigating induction to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain investigating induction using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for investigating induction (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for investigating induction (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for investigating induction (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for investigating induction (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for investigating induction (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u07-l12` · built from `hs912-science-h3` at `e7551b9`._
