# Scoring authority — Independent application A: specific heat capacity

**High School Physics (Grade 11)** · Unit 6: Thermal Energy and the Second Law · Day 4 · lesson `ma-hs11-physics-u06-l04`
...
_Topic `ma-hs11-physics-u06::specific heat capacity` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Specific heat capacity is the energy required to raise the temperature of one kilogram of a substance by one kelvin, and it differs between substances.

**Accepted relationships and models.** What this lesson's learning target asserts:

- For the same energy input and the same mass, a substance with higher specific heat capacity shows a smaller temperature rise.
- Water has an unusually high specific heat capacity, which is why large bodies of water moderate nearby climates.
- The energy transferred is the product of mass, specific heat capacity, and temperature change.
...
- Claiming equal energy input produces equal temperature change regardless of substance or mass.
- Confusing specific heat capacity with thermal conductivity.
- Applying the specific heat relationship across a change of state.

**Grade boundary.** Latent heat calculations and calorimetry error analysis are acceptable but not required.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain specific heat capacity using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply specific heat capacity to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on specific heat capacity against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply specific heat capacity to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain specific heat capacity using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply specific heat capacity to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain specific heat capacity using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for specific heat capacity (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for specific heat capacity (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for specific heat capacity (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for specific heat capacity (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for specific heat capacity (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u06-l04` · built from `hs912-science-h3` at `e7551b9`._
