# Scoring authority — Correction and reflection: building and checking an energy budget

**High School Physics (Grade 11)** · Unit 5: Energy: Modelling, Accounting, and Conservation · Day 12 · lesson `ma-hs11-physics-u05-l12`
...
_Topic `ma-hs11-physics-u05::building and checking an energy budget` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- An energy budget lists the energy in the system at the start, the transfers in and out, and the energy at the end, and the two sides must agree within uncertainty.
- A shortfall in the budget points to an unaccounted transfer, most often dissipation, and it is identified rather than absorbed into a residual.
- Every term in the budget carries the uncertainty of the measurements it came from.
...
- Balancing a budget by inserting an unexplained term.
- Claiming a budget balances when the discrepancy exceeds the measurement uncertainty.
- Omitting dissipation from a budget for a system with friction or inelastic interaction.

**Grade boundary.** Formal uncertainty propagation through multi-term budgets is beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain building and checking an energy budget using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply building and checking an energy budget to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on building and checking an energy budget against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply building and checking an energy budget to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain building and checking an energy budget using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply building and checking an energy budget to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain building and checking an energy budget using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for building and checking an energy budget (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for building and checking an energy budget (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for building and checking an energy budget (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for building and checking an energy budget (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for building and checking an energy budget (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u05-l12` · built from `hs912-science-h3` at `e7551b9`._
