# Scoring authority — Correction and reflection: evaluating competing models

**High School Physics (Grade 11)** · Unit 8: Waves and Their Mathematics · Day 12 · lesson `ma-hs11-physics-u08-l12`
...
_Topic `ma-hs11-physics-u08::evaluating competing models` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Competing models are evaluated on the range of observations each accounts for and on the predictions that distinguish them.
- A decisive test is one where the models predict different outcomes, so an observation consistent with both settles nothing.
- A model retained for a limited domain is not thereby wrong; its domain is part of the model.
...
- Selecting between models on evidence consistent with both.
- Claiming a model with a limited domain has been refuted.
- Choosing a model on simplicity or familiarity alone with no evidential comparison.

**Grade boundary.** Formal model selection criteria and philosophy of science are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain evaluating competing models using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply evaluating competing models to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on evaluating competing models against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply evaluating competing models to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain evaluating competing models using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply evaluating competing models to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain evaluating competing models using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for evaluating competing models (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for evaluating competing models (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for evaluating competing models (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for evaluating competing models (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for evaluating competing models (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u08-l12` · built from `hs912-science-h3` at `e7551b9`._
