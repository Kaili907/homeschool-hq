# Scoring authority — Reteach and varied practice: absorption of radiation by matter

**High School Physics (Grade 11)** · Unit 9: Electromagnetic Radiation, Information, and a Physics Engineering Capstone · Day 8 · lesson `ma-hs11-physics-u09-l08`
...
_Topic `ma-hs11-physics-u09::absorption of radiation by matter` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Whether radiation is absorbed, transmitted, or reflected depends on both the material and the frequency of the radiation.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Absorption transfers energy from the radiation to the material, commonly raising its temperature or exciting its electrons.
- A material transparent to one part of the spectrum can be opaque to another, which is why glass passes visible light but blocks much ultraviolet.
- Ionising radiation damages matter by removing electrons from atoms, which non-ionising radiation cannot do however intense it is.
...
- Claiming a material behaves the same way toward all frequencies of radiation.
- Claiming non-ionising radiation causes ionisation if it is intense enough.
- Claiming absorbed radiation energy disappears rather than being transferred to the material.

**Grade boundary.** Attenuation coefficients, dosimetry, and spectroscopy are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain absorption of radiation by matter using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply absorption of radiation by matter to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on absorption of radiation by matter against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply absorption of radiation by matter to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain absorption of radiation by matter using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply absorption of radiation by matter to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain absorption of radiation by matter using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for absorption of radiation by matter (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for absorption of radiation by matter (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for absorption of radiation by matter (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for absorption of radiation by matter (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for absorption of radiation by matter (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u09-l08` · built from `hs912-science-h3` at `e7551b9`._
