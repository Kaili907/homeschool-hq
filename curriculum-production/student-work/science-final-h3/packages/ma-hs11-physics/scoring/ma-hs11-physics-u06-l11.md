# Scoring authority — Unit assessment: the second law of thermodynamics

**High School Physics (Grade 11)** · Unit 6: Thermal Energy and the Second Law · Day 11 · lesson `ma-hs11-physics-u06-l11`
...
_Topic `ma-hs11-physics-u06::the second law of thermodynamics` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Thermal energy does not flow spontaneously from a cooler body to a warmer one, and no cyclic device converts thermal energy entirely into work.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Energy spontaneously disperses, so the total disorder of a system and its surroundings does not decrease.
- A refrigerator or heat pump moves energy from cooler to warmer only because work is supplied, so the overall dispersal still increases.
- This sets an upper limit on the efficiency of any heat engine that is strictly below one hundred per cent.

**Also correct.** Accept any of these framings — do not require the wording above:

- Formulations in terms of entropy, of energy dispersal, or of the impossibility of a perfect engine are equivalent.
...
- Claiming a heat engine can be one hundred per cent efficient.
- Claiming a local decrease in disorder violates the second law without accounting for the surroundings.
- Confusing the first law's conservation of energy with a claim that energy stays equally useful.

**Grade boundary.** Carnot efficiency calculation and statistical entropy are acceptable but not required.


## What a complete response contains, question by question

**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for the second law of thermodynamics on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain the second law of thermodynamics using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply the second law of thermodynamics to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on the second law of thermodynamics against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply the second law of thermodynamics to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain the second law of thermodynamics using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply the second law of thermodynamics to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for the second law of thermodynamics on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for the second law of thermodynamics (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for the second law of thermodynamics (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for the second law of thermodynamics (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for the second law of thermodynamics (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for the second law of thermodynamics (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u06-l11` · built from `hs912-science-h3` at `e7551b9`._
