# Scoring authority — Independent application A: uniform acceleration relationships

**High School Physics (Grade 11)** · Unit 1: Motion, Measurement, and Quantitative Reasoning · Day 4 · lesson `ma-hs11-physics-u01-l04`
...
_Topic `ma-hs11-physics-u01::uniform acceleration relationships` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The kinematic relationships for uniform acceleration apply only while the acceleration is constant.

**Accepted relationships and models.** What this lesson's learning target asserts:

- For constant acceleration, final velocity, displacement, and time are related by the standard kinematic relationships.
- Near Earth's surface, an object in free fall with negligible air resistance accelerates downward at approximately nine point eight metres per second squared, independently of its mass.
- Applying the relationships requires the acceleration to be constant over the whole interval considered.

**Also correct.** Accept any of these framings — do not require the wording above:

- Any of the standard kinematic equations, or graphical reasoning giving the same result, are equally correct.
...
- Applying the constant-acceleration relationships across an interval where the acceleration changes.
- Claiming heavier objects fall with greater acceleration when air resistance is negligible.
- Claiming an object at the top of its flight has zero acceleration because its velocity is momentarily zero.

**Grade boundary.** Air resistance modelling and terminal velocity calculation are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain uniform acceleration relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply uniform acceleration relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on uniform acceleration relationships against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply uniform acceleration relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain uniform acceleration relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply uniform acceleration relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain uniform acceleration relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for uniform acceleration relationships (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for uniform acceleration relationships (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for uniform acceleration relationships (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for uniform acceleration relationships (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for uniform acceleration relationships (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u01-l04` · built from `hs912-science-h3` at `e7551b9`._
