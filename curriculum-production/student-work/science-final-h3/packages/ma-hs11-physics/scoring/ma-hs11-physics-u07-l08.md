# Scoring authority — Reteach and varied practice: current and magnetic field production

**High School Physics (Grade 11)** · Unit 7: Fields, Electromagnetism, and Induction · Day 8 · lesson `ma-hs11-physics-u07-l08`
...
_Topic `ma-hs11-physics-u07::current and magnetic field production` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._
...
- A current-carrying wire produces a magnetic field encircling it, and the field strength increases with the current.
- Coiling the wire concentrates the field, and adding an iron core strengthens it further, which is how electromagnets work.
- A current-carrying conductor in an external magnetic field experiences a force, which is the basis of the electric motor.

**Also correct.** Accept any of these framings — do not require the wording above:

- Right-hand rule conventions in any consistent form are acceptable.
...
- Claiming a stationary charge produces a magnetic field.
- Claiming an electromagnet's field persists unchanged when the current is switched off, with no ferromagnetic retention involved.
- Claiming the magnetic field of a straight wire points along the wire.

**Grade boundary.** Ampere's law and quantitative field calculation are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain current and magnetic field production using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply current and magnetic field production to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on current and magnetic field production against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply current and magnetic field production to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain current and magnetic field production using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply current and magnetic field production to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain current and magnetic field production using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for current and magnetic field production (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for current and magnetic field production (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for current and magnetic field production (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for current and magnetic field production (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for current and magnetic field production (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u07-l08` · built from `hs912-science-h3` at `e7551b9`._
