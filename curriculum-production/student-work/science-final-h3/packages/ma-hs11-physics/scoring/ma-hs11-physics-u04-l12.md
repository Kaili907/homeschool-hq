# Scoring authority — Correction and reflection: predicting forces mathematically

**High School Physics (Grade 11)** · Unit 4: Gravitation and Electrostatic Forces · Day 12 · lesson `ma-hs11-physics-u04-l12`
...
_Topic `ma-hs11-physics-u04::predicting forces mathematically` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- A predicted force follows from the stated relationship, the measured quantities, and their units, and it carries the uncertainty of those measurements.
- A prediction states the assumptions it rests on, such as treating a body as a point mass or ignoring intervening material.
- The prediction is then compared against measurement, and disagreement is investigated rather than explained away.
...
- Reporting a calculated force with more precision than the inputs support.
- Applying a point-mass or point-charge relationship to extended bodies at close range without saying so.
- Attributing a disagreement between prediction and measurement to error with no candidate cause identified.

**Grade boundary.** Numerical modelling of multi-body force systems is beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain predicting forces mathematically using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply predicting forces mathematically to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on predicting forces mathematically against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply predicting forces mathematically to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain predicting forces mathematically using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply predicting forces mathematically to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain predicting forces mathematically using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for predicting forces mathematically (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for predicting forces mathematically (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for predicting forces mathematically (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for predicting forces mathematically (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for predicting forces mathematically (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u04-l12` · built from `hs912-science-h3` at `e7551b9`._
