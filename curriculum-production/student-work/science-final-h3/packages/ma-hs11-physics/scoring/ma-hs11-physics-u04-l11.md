# Scoring authority — Unit assessment: field lines as representations

**High School Physics (Grade 11)** · Unit 4: Gravitation and Electrostatic Forces · Day 11 · lesson `ma-hs11-physics-u04-l11`
...
_Topic `ma-hs11-physics-u04::field lines as representations` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Field lines are a representation of a field's direction and relative strength; they are not physical objects.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The direction of a field line at a point gives the direction of the force on a positive test charge, or on a test mass, placed there.
- Closer spacing of field lines represents a stronger field.
- Field lines never cross, because the field has one direction at each point.

**Also correct.** Accept any of these framings — do not require the wording above:

- Field vector diagrams and field line diagrams convey the same information and are both acceptable.
...
- Claiming field lines are physically present in space.
- Drawing field lines that cross one another.
- Claiming the field exists only along the drawn lines and not between them.

**Grade boundary.** Field potential surfaces and quantitative field mapping are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for field lines as representations on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain field lines as representations using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply field lines as representations to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on field lines as representations against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply field lines as representations to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain field lines as representations using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply field lines as representations to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for field lines as representations on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for field lines as representations (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for field lines as representations (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for field lines as representations (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for field lines as representations (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for field lines as representations (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u04-l11` · built from `hs912-science-h3` at `e7551b9`._
