# Scoring authority — Reteach and varied practice: Coulomb's law

**High School Physics (Grade 11)** · Unit 4: Gravitation and Electrostatic Forces · Day 8 · lesson `ma-hs11-physics-u04-l08`
...
_Topic `ma-hs11-physics-u04::Coulomb's law` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._
...
- Coulomb's law has the same inverse-square form as the gravitational law, but electric charge comes in two signs, so the force can attract or repel.
- The two charges exert equal and opposite forces on each other regardless of their relative magnitudes.
- The presence of intervening material changes the force, unlike the gravitational case.
...
- Claiming the larger charge exerts the greater force in a pair.
- Claiming electrostatic force is always attractive.
- Claiming the force falls off in direct proportion to distance rather than to its square.

**Grade boundary.** Dielectric theory, continuous charge distributions, and Gauss's law are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain Coulomb's law using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply Coulomb's law to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on Coulomb's law against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply Coulomb's law to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain Coulomb's law using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply Coulomb's law to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain Coulomb's law using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for Coulomb's law (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for Coulomb's law (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for Coulomb's law (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for Coulomb's law (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for Coulomb's law (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u04-l08` · built from `hs912-science-h3` at `e7551b9`._
