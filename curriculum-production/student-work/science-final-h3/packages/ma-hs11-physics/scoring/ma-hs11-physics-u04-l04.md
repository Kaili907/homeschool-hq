# Scoring authority — Independent application A: comparing gravitational and electrostatic strength

**High School Physics (Grade 11)** · Unit 4: Gravitation and Electrostatic Forces · Day 4 · lesson `ma-hs11-physics-u04-l04`
...
_Topic `ma-hs11-physics-u04::comparing gravitational and electrostatic strength` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The electrostatic force between two charged subatomic particles is enormously stronger than the gravitational force between them.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Both forces obey an inverse-square law, but the electrostatic force is vastly stronger for elementary particles.
- Gravity nonetheless dominates at astronomical scale because bulk matter is very nearly electrically neutral, so positive and negative contributions cancel.
- Gravity has no cancellation mechanism, because mass has only one sign.
...
- Claiming gravity is intrinsically the stronger of the two forces.
- Claiming electrostatic forces are irrelevant at atomic scale.
- Explaining gravity's astronomical dominance by strength rather than by the absence of cancellation.

**Grade boundary.** The full hierarchy of fundamental forces and coupling constants is beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain comparing gravitational and electrostatic strength using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply comparing gravitational and electrostatic strength to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on comparing gravitational and electrostatic strength against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply comparing gravitational and electrostatic strength to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain comparing gravitational and electrostatic strength using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply comparing gravitational and electrostatic strength to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain comparing gravitational and electrostatic strength using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for comparing gravitational and electrostatic strength (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for comparing gravitational and electrostatic strength (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for comparing gravitational and electrostatic strength (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for comparing gravitational and electrostatic strength (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for comparing gravitational and electrostatic strength (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u04-l04` · built from `hs912-science-h3` at `e7551b9`._
