# Scoring authority — Unit assessment: wave-based technologies

**High School Physics (Grade 11)** · Unit 9: Electromagnetic Radiation, Information, and a Physics Engineering Capstone · Day 11 · lesson `ma-hs11-physics-u09-l11`
...
_Topic `ma-hs11-physics-u09::wave-based technologies` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Wave-based technologies work by encoding information onto a wave, transmitting it, and decoding it, and each stage constrains performance.
- The choice of frequency band is set by how the wave interacts with the environment: penetration, range, and available bandwidth all depend on it.
- Every link has limits on range, rate, and reliability set by physics as well as by equipment.
...
- Claiming a communication technology has no physical limit on range or data rate.
- Claiming frequency band choice is arbitrary rather than driven by propagation behaviour.
- Claiming information travels without any physical carrier.

**Grade boundary.** Modulation schemes, antenna theory, and link budget calculation are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for wave-based technologies on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain wave-based technologies using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply wave-based technologies to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on wave-based technologies against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply wave-based technologies to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain wave-based technologies using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply wave-based technologies to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for wave-based technologies on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for wave-based technologies (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for wave-based technologies (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for wave-based technologies (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for wave-based technologies (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for wave-based technologies (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u09-l11` · built from `hs912-science-h3` at `e7551b9`._
