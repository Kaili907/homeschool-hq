# Scoring authority — Performance task build: Newton's second law as a mathematical relationship

**High School Physics (Grade 11)** · Unit 2: Forces and Newton's Second Law · Day 9 · lesson `ma-hs11-physics-u02-l09`
...
_Topic `ma-hs11-physics-u02::Newton's second law as a mathematical relationship` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Acceleration is proportional to net force and inversely proportional to mass, and it is in the direction of the net force.

**Accepted relationships and models.** What this lesson's learning target asserts:

- For a given mass, doubling the net force doubles the acceleration; for a given net force, doubling the mass halves it.
- The relationship applies to the net force, not to any single force acting on the object.
- Acceleration is always in the direction of the net force, whatever the direction of motion.

**Also correct.** Accept any of these framings — do not require the wording above:

- Expressing the relationship as an equation, as a proportionality, or in words are equally correct.
...
- Applying the relationship to one individual force rather than the net force.
- Claiming acceleration is in the direction of motion rather than of net force.
- Claiming force is proportional to velocity rather than to acceleration.

**Grade boundary.** Variable-mass systems and the momentum form of the second law are acceptable but not required.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for Newton's second law as a mathematical relationship that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain Newton's second law as a mathematical relationship using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply Newton's second law as a mathematical relationship to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on Newton's second law as a mathematical relationship against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply Newton's second law as a mathematical relationship to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain Newton's second law as a mathematical relationship using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply Newton's second law as a mathematical relationship to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for Newton's second law as a mathematical relationship that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for Newton's second law as a mathematical relationship (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for Newton's second law as a mathematical relationship (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for Newton's second law as a mathematical relationship (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for Newton's second law as a mathematical relationship (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for Newton's second law as a mathematical relationship (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Falling weights can land on feet or break flooring. — mitigation: Keep the hanging mass under 200 g, place a cushion or towel below the drop zone, keep feet clear, and never stand under the hanging mass.
- physical: String under tension can snap back toward the face. — mitigation: Wear eye protection whenever the string is under tension, use light loads only, inspect the string along its whole length before each run, and stand to the side of the string line rather than in it.
...
**Disposal and clearing up:** Coil the string and put the weights back into a container out of reach of young children. Nothing hazardous is produced.
...
_Package `swp-ma-hs11-physics-u02-l09` · built from `hs912-science-h3` at `e7551b9`._
