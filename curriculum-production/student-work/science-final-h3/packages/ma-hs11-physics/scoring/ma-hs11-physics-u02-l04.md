# Scoring authority — Independent application A: mass versus weight

**High School Physics (Grade 11)** · Unit 2: Forces and Newton's Second Law · Day 4 · lesson `ma-hs11-physics-u02-l04`
...
_Topic `ma-hs11-physics-u02::mass versus weight` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs11-physics.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Mass is a property of the object and does not change with location; weight is the gravitational force on it and does.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Mass measures how strongly an object resists a change in motion and is measured in kilograms.
- Weight is a force, measured in newtons, equal to mass times the local gravitational field strength.
- An object in free fall or in orbit still has weight; what it lacks is a supporting normal force, which is why it feels weightless.
...
- Treating mass and weight as the same quantity or reporting one in the other's units.
- Claiming an object's mass changes on the Moon.
- Claiming orbiting astronauts are weightless because there is no gravity at that altitude.

**Grade boundary.** Gravitational versus inertial mass equivalence is beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain mass versus weight using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply mass versus weight to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on mass versus weight against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply mass versus weight to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain mass versus weight using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply mass versus weight to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain mass versus weight using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for mass versus weight (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for mass versus weight (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for mass versus weight (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for mass versus weight (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for mass versus weight (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs11-physics-u02-l04` · built from `hs912-science-h3` at `e7551b9`._
