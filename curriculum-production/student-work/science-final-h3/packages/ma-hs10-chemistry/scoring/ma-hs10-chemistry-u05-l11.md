# Scoring authority — Unit assessment: limiting reactants

**High School Chemistry (Grade 10)** · Unit 5: Conservation of Mass and Quantitative Reasoning in Reactions · Day 11 · lesson `ma-hs10-chemistry-u05-l11`
...
_Topic `ma-hs10-chemistry-u05::limiting reactants` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The limiting reactant is the one that runs out first and therefore determines how much product can form.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The limiting reactant is identified by comparing available amounts in moles against the ratio the balanced equation requires, not by comparing masses.
- Reactant left over when the reaction stops is in excess, and it does not increase the yield.
- Theoretical yield follows from the limiting reactant; actual yield is measured and is normally lower.
...
- Identifying the limiting reactant as the one present in the smallest mass without converting to moles.
- Claiming adding more of the excess reactant increases the product yield.
- Calculating theoretical yield from the excess reactant.

**Grade boundary.** Percent yield error analysis and multi-step yield propagation are acceptable but not required.


## What a complete response contains, question by question

**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for limiting reactants on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain limiting reactants using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply limiting reactants to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on limiting reactants against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply limiting reactants to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain limiting reactants using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply limiting reactants to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for limiting reactants on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for limiting reactants (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for limiting reactants (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for limiting reactants (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for limiting reactants (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for limiting reactants (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u05-l11` · built from `hs912-science-h3` at `e7551b9`._
