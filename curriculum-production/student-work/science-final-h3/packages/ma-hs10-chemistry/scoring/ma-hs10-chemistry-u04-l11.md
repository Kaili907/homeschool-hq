# Scoring authority — Unit assessment: predicting products from periodic trends

**High School Chemistry (Grade 10)** · Unit 4: Chemical Reactions and Electron Rearrangement · Day 11 · lesson `ma-hs10-chemistry-u04-l11`
...
_Topic `ma-hs10-chemistry-u04::predicting products from periodic trends` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Position in the periodic table indicates likely charges for main-group ions, and therefore likely formulas for the compounds formed.
- A predicted product is a hypothesis to be checked against conservation of atoms, charge balance, and experimental evidence.
- Trends give likely outcomes, not guarantees; exceptions exist and are part of the chemistry.
...
- Predicting a compound whose formula does not balance charge.
- Presenting a trend-based prediction as an established result with no verification.
- Claiming a reaction must occur because the products would be reasonable.

**Grade boundary.** Thermodynamic prediction of spontaneity and quantitative equilibrium prediction are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for predicting products from periodic trends on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain predicting products from periodic trends using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply predicting products from periodic trends to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on predicting products from periodic trends against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply predicting products from periodic trends to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain predicting products from periodic trends using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply predicting products from periodic trends to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for predicting products from periodic trends on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for predicting products from periodic trends (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for predicting products from periodic trends (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for predicting products from periodic trends (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for predicting products from periodic trends (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for predicting products from periodic trends (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u04-l11` · built from `hs912-science-h3` at `e7551b9`._
