# Scoring authority — Independent application A: periodic trends in atomic radius and ionization

**High School Chemistry (Grade 10)** · Unit 2: Atomic Structure and the Periodic Table as a Model · Day 4 · lesson `ma-hs10-chemistry-u02-l04`
...
_Topic `ma-hs10-chemistry-u02::periodic trends in atomic radius and ionization` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._
...
- First ionisation energy generally increases across a period from left to right and decreases down a group.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Across a period, nuclear charge rises while electrons are added to the same energy level, so the outer electrons are pulled in more strongly.
- Down a group, outer electrons occupy higher energy levels and are shielded by more inner electrons, so they are held less tightly.
- The trends in radius and ionisation energy run in opposite directions because both follow from the same effective nuclear attraction.

**Also correct.** Accept any of these framings — do not require the wording above:

- Explaining the trends through effective nuclear charge, or through nuclear charge together with shielding and energy level, are equivalent.
...
- Claiming atomic radius increases across a period from left to right.
- Claiming ionisation energy increases down a group.
- Claiming atoms get larger across a period because more electrons are added.

**Grade boundary.** Slater's rules, quantitative effective nuclear charge, and irregularities in specific trends are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain periodic trends in atomic radius and ionization using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply periodic trends in atomic radius and ionization to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on periodic trends in atomic radius and ionization against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply periodic trends in atomic radius and ionization to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain periodic trends in atomic radius and ionization using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply periodic trends in atomic radius and ionization to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain periodic trends in atomic radius and ionization using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for periodic trends in atomic radius and ionization (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for periodic trends in atomic radius and ionization (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for periodic trends in atomic radius and ionization (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for periodic trends in atomic radius and ionization (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for periodic trends in atomic radius and ionization (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u02-l04` · built from `hs912-science-h3` at `e7551b9`._
