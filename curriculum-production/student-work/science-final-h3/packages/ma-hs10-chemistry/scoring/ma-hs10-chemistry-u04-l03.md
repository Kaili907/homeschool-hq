# Scoring authority — Guided practice A: outermost electron states in reactions

**High School Chemistry (Grade 10)** · Unit 4: Chemical Reactions and Electron Rearrangement · Day 3 · lesson `ma-hs10-chemistry-u04-l03`
...
_Topic `ma-hs10-chemistry-u04::outermost electron states in reactions` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Chemical reactions involve rearrangement of outer electrons; nuclei are unchanged.

**Accepted relationships and models.** What this lesson's learning target asserts:

- In a chemical reaction, valence electrons are transferred, shared, or redistributed, and the nuclei retain their identity.
- This is why elements are conserved in chemical reactions: no atom becomes an atom of a different element.
- The stability of the resulting outer-electron arrangements is part of why some rearrangements are favourable.
...
- Claiming atoms of one element are converted into another element by a chemical reaction.
- Claiming nuclear particles are rearranged during ordinary chemical reactions.
- Claiming inner-shell electrons participate in ordinary chemical bonding.

**Grade boundary.** Nuclear reactions belong to unit 9; orbital hybridisation is beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain outermost electron states in reactions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply outermost electron states in reactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on outermost electron states in reactions against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply outermost electron states in reactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain outermost electron states in reactions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply outermost electron states in reactions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain outermost electron states in reactions using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for outermost electron states in reactions (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for outermost electron states in reactions (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for outermost electron states in reactions (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for outermost electron states in reactions (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for outermost electron states in reactions (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u04-l03` · built from `hs912-science-h3` at `e7551b9`._
