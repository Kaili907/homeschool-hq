# Scoring authority — Reteach and varied practice: measurement, precision, and significant figures

**High School Chemistry (Grade 10)** · Unit 1: Matter, Measurement, and Laboratory Practice · Day 8 · lesson `ma-hs10-chemistry-u01-l08`
...
_Topic `ma-hs10-chemistry-u01::measurement, precision, and significant figures` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The number of significant figures reported must reflect the precision of the instrument used.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A measurement is recorded to the smallest division of the instrument plus one estimated digit.
- Precision describes how closely repeated measurements agree; accuracy describes how close they are to the accepted value, and the two are independent.
- Calculations cannot create precision: a result is reported to no more significant figures than the least precise input allows.

**Also correct.** Accept any of these framings — do not require the wording above:

- Reporting a value with an explicit uncertainty, or with significant figures alone, are both acceptable provided the precision claimed is real.
...
- Reporting a calculated result to more significant figures than the measurements support.
- Treating precision and accuracy as the same property.
- Claiming a measurement is accurate because repeated trials agreed closely.

**Grade boundary.** Formal error propagation and statistical uncertainty analysis are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain measurement, precision, and significant figures using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply measurement, precision, and significant figures to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on measurement, precision, and significant figures against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply measurement, precision, and significant figures to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain measurement, precision, and significant figures using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply measurement, precision, and significant figures to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain measurement, precision, and significant figures using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for measurement, precision, and significant figures (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for measurement, precision, and significant figures (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for measurement, precision, and significant figures (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for measurement, precision, and significant figures (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for measurement, precision, and significant figures (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u01-l08` · built from `hs912-science-h3` at `e7551b9`._
