# Scoring authority — Correction and reflection: the second law in everyday systems

**High School Chemistry (Grade 10)** · Unit 6: Energy in Chemical Systems · Day 12 · lesson `ma-hs10-chemistry-u06-l12`
...
_Topic `ma-hs10-chemistry-u06::the second law in everyday systems` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Energy spontaneously disperses; concentrating it in a system requires energy input from elsewhere.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Energy is conserved in every process, but its usefulness decreases as it disperses into the surroundings.
- A refrigerator or heat pump moves thermal energy from cooler to warmer only because work is done on it, so the overall dispersal still increases.
- This is why no device converts all energy input into useful output, and why perpetual motion machines are impossible.

**Also correct.** Accept any of these framings — do not require the wording above:

- Describing the principle through entropy, through energy dispersal, or through the impossibility of a perfect engine are all correct.
...
- Claiming a device can be one hundred per cent efficient in converting energy to useful work.
- Claiming the second law is violated by any local decrease in disorder, without accounting for the surroundings.
- Confusing conservation of energy with conservation of useful energy.

**Grade boundary.** Quantitative entropy calculation and statistical thermodynamics are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain the second law in everyday systems using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply the second law in everyday systems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on the second law in everyday systems against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply the second law in everyday systems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain the second law in everyday systems using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply the second law in everyday systems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain the second law in everyday systems using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for the second law in everyday systems (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for the second law in everyday systems (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for the second law in everyday systems (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for the second law in everyday systems (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for the second law in everyday systems (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u06-l12` · built from `hs912-science-h3` at `e7551b9`._
