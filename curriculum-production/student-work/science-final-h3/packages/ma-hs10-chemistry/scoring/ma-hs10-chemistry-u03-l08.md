# Scoring authority — Reteach and varied practice: intermolecular forces

**High School Chemistry (Grade 10)** · Unit 3: Bonding, Structure, and the Properties of Designed Materials · Day 8 · lesson `ma-hs10-chemistry-u03-l08`
...
_Topic `ma-hs10-chemistry-u03::intermolecular forces` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Intermolecular forces are attractions between molecules and are much weaker than the covalent bonds within a molecule.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Intermolecular attractions include dispersion forces, which act between all molecules, dipole-dipole attractions between polar molecules, and hydrogen bonding where hydrogen is bonded to nitrogen, oxygen, or fluorine.
- Boiling a molecular substance overcomes the forces between molecules, not the bonds within them.
- Stronger intermolecular forces raise boiling point, raise viscosity, and lower vapour pressure.

**Also correct.** Accept any of these framings — do not require the wording above:

- London dispersion, van der Waals, and induced-dipole forces are acceptable names for the same interaction at this level.
...
- Claiming boiling water breaks the covalent bonds within water molecules.
- Claiming hydrogen bonds are covalent bonds involving hydrogen.
- Claiming non-polar molecules experience no intermolecular attraction at all.

**Grade boundary.** Quantitative interaction energies and polarisability calculations are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain intermolecular forces using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply intermolecular forces to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on intermolecular forces against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply intermolecular forces to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain intermolecular forces using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply intermolecular forces to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain intermolecular forces using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for intermolecular forces (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for intermolecular forces (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for intermolecular forces (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for intermolecular forces (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for intermolecular forces (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u03-l08` · built from `hs912-science-h3` at `e7551b9`._
