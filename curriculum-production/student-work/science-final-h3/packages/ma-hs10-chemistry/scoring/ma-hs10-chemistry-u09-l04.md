# Scoring authority — Independent application A: fusion

**High School Chemistry (Grade 10)** · Unit 9: The Nucleus, Nuclear Energy, and a Chemistry Engineering Capstone · Day 4 · lesson `ma-hs10-chemistry-u09-l04`
...
_Topic `ma-hs10-chemistry-u09::fusion` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Fusion is the combining of light nuclei into a heavier nucleus, releasing energy.
- Fusion powers the Sun and other stars.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Fusion releases energy because the product nucleus is more tightly bound per nucleon than the light nuclei that formed it.
- Fusion requires extremely high temperature and pressure, because positively charged nuclei repel each other strongly at short range.
- That requirement is why sustained controlled fusion for power generation remains an engineering problem.
...
- Claiming fusion is the splitting of nuclei.
- Claiming fusion occurs readily at ordinary temperatures.
- Claiming the Sun is powered by chemical combustion.

**Grade boundary.** Plasma physics and confinement approaches are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain fusion using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply fusion to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on fusion against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply fusion to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain fusion using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply fusion to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain fusion using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for fusion (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for fusion (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for fusion (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for fusion (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for fusion (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u09-l04` · built from `hs912-science-h3` at `e7551b9`._
