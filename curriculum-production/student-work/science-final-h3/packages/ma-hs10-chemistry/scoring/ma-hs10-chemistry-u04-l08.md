# Scoring authority — Reteach and varied practice: reaction types and patterns

**High School Chemistry (Grade 10)** · Unit 4: Chemical Reactions and Electron Rearrangement · Day 8 · lesson `ma-hs10-chemistry-u04-l08`
...
_Topic `ma-hs10-chemistry-u04::reaction types and patterns` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Common reaction patterns include synthesis, decomposition, single replacement, double replacement, and combustion.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Classifying a reaction by pattern supports predicting its products, because each pattern has a characteristic rearrangement.
- The classification describes what happens to the atoms, not why the reaction is favourable.
- Some reactions fit more than one pattern, and some fit none; the categories are a useful model rather than an exhaustive partition.

**Also correct.** Accept any of these framings — do not require the wording above:

- Combination and synthesis are the same category; single and double displacement and replacement are the same categories.
...
- Predicting products by pattern while producing an equation in which atoms are not conserved.
- Claiming classification by pattern establishes that a reaction will occur.
- Claiming every reaction fits exactly one of the standard categories.

**Grade boundary.** Reaction mechanisms, activity series in quantitative form, and organic reaction classification are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain reaction types and patterns using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply reaction types and patterns to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on reaction types and patterns against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply reaction types and patterns to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain reaction types and patterns using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply reaction types and patterns to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain reaction types and patterns using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for reaction types and patterns (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for reaction types and patterns (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for reaction types and patterns (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for reaction types and patterns (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for reaction types and patterns (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u04-l08` · built from `hs912-science-h3` at `e7551b9`._
