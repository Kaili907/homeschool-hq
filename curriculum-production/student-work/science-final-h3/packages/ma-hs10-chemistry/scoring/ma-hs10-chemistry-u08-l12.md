# Scoring authority — Correction and reflection: refining a chemical system design

**High School Chemistry (Grade 10)** · Unit 8: Equilibrium and the Design of Chemical Systems · Day 12 · lesson `ma-hs10-chemistry-u08-l12`
...
_Topic `ma-hs10-chemistry-u08::refining a chemical system design` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Refining a chemical system means changing one condition at a time and measuring the effect against stated criteria.
- A change that improves one criterion commonly worsens another, and the trade-off is reported.
- Safety and waste constraints bound the design space and are not traded for yield or rate.
...
- Changing several conditions at once and attributing the improvement to one.
- Reporting a design as improved without measuring against the stated criteria.
- Trading a safety or waste constraint for better yield.

**Grade boundary.** Formal process optimisation and life-cycle assessment are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain refining a chemical system design using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply refining a chemical system design to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on refining a chemical system design against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply refining a chemical system design to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain refining a chemical system design using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply refining a chemical system design to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain refining a chemical system design using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for refining a chemical system design (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for refining a chemical system design (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for refining a chemical system design (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for refining a chemical system design (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for refining a chemical system design (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u08-l12` · built from `hs912-science-h3` at `e7551b9`._
