# Scoring authority — Independent application A: shifting equilibrium by changing conditions

**High School Chemistry (Grade 10)** · Unit 8: Equilibrium and the Design of Chemical Systems · Day 4 · lesson `ma-hs10-chemistry-u08-l04`
...
_Topic `ma-hs10-chemistry-u08::shifting equilibrium by changing conditions` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Increasing the concentration of a reactant shifts the equilibrium toward products; increasing pressure on a gas-phase equilibrium shifts it toward the side with fewer gas particles.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Adding a substance shifts the system away from that substance; removing one shifts it toward that substance.
- For gas-phase equilibria, raising pressure by reducing volume favours the side with fewer moles of gas.
...
- Claiming raising temperature always increases product yield regardless of the reaction's energetics.
- Claiming pressure changes shift an equilibrium with equal moles of gas on both sides.
- Claiming a catalyst changes the yield at equilibrium.

**Grade boundary.** Van't Hoff analysis and quantitative yield prediction are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain shifting equilibrium by changing conditions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply shifting equilibrium by changing conditions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on shifting equilibrium by changing conditions against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply shifting equilibrium by changing conditions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain shifting equilibrium by changing conditions using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply shifting equilibrium by changing conditions to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain shifting equilibrium by changing conditions using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for shifting equilibrium by changing conditions (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for shifting equilibrium by changing conditions (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for shifting equilibrium by changing conditions (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for shifting equilibrium by changing conditions (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for shifting equilibrium by changing conditions (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u08-l04` · built from `hs912-science-h3` at `e7551b9`._
