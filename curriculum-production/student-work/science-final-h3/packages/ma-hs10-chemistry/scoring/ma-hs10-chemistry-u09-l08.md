# Scoring authority — Reteach and varied practice: radioactive decay and half-life

**High School Chemistry (Grade 10)** · Unit 9: The Nucleus, Nuclear Energy, and a Chemistry Engineering Capstone · Day 8 · lesson `ma-hs10-chemistry-u09-l08`
...
_Topic `ma-hs10-chemistry-u09::radioactive decay and half-life` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Half-life is the time for half of the nuclei in a sample to decay, and it is unaffected by temperature, pressure, or chemical combination.
- Alpha decay reduces the atomic number by two and the mass number by four; beta-minus decay increases the atomic number by one and leaves the mass number unchanged.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Radioactive decay is a nuclear process, so it changes the element, unlike chemical reactions.
- Decay of an individual nucleus is random, while the behaviour of a large sample follows a predictable exponential pattern.
- Half-life is a constant for a given nuclide and does not depend on how much of the sample remains.

**Also correct.** Accept any of these framings — do not require the wording above:

- Decay described by half-life, or by fraction remaining after a number of half-lives, are equivalent.
...
- Claiming half-life can be changed by heating, cooling, or chemically combining the substance.
- Claiming a sample is completely gone after two half-lives.
- Claiming radioactive decay is a chemical reaction involving electrons.

**Grade boundary.** Decay constants, activity units, and dosimetry are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain radioactive decay and half-life using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply radioactive decay and half-life to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on radioactive decay and half-life against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply radioactive decay and half-life to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain radioactive decay and half-life using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply radioactive decay and half-life to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain radioactive decay and half-life using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for radioactive decay and half-life (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for radioactive decay and half-life (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for radioactive decay and half-life (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for radioactive decay and half-life (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for radioactive decay and half-life (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u09-l08` · built from `hs912-science-h3` at `e7551b9`._
