# Scoring authority — Correction and reflection: the periodic table as a predictive model

**High School Chemistry (Grade 10)** · Unit 2: Atomic Structure and the Periodic Table as a Model · Day 12 · lesson `ma-hs10-chemistry-u02-l12`
...
_Topic `ma-hs10-chemistry-u02::the periodic table as a predictive model` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs10-chemistry.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The periodic table is arranged by increasing atomic number, and elements with similar properties fall in the same group.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Because properties repeat with atomic number, the table predicts the behaviour of an element from its position.
- The table's predictive power was demonstrated historically by successful predictions about elements not yet discovered.
- The table is a model built on periodicity, and its predictions are approximate, with known exceptions.
...
- Claiming the table is ordered by atomic mass rather than atomic number.
- Claiming the table's predictions hold exactly with no exceptions.
- Claiming position in the table causes an element's properties rather than reflecting its electron arrangement.

**Grade boundary.** Relativistic effects on heavy element properties and superheavy element chemistry are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain the periodic table as a predictive model using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply the periodic table as a predictive model to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on the periodic table as a predictive model against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply the periodic table as a predictive model to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain the periodic table as a predictive model using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply the periodic table as a predictive model to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain the periodic table as a predictive model using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for the periodic table as a predictive model (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for the periodic table as a predictive model (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for the periodic table as a predictive model (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for the periodic table as a predictive model (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for the periodic table as a predictive model (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs10-chemistry-u02-l12` · built from `hs912-science-h3` at `e7551b9`._
