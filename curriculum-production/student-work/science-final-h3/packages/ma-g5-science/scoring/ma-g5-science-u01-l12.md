...
**Grade 5 Science** · Unit 1: Science and Engineering Practices · Day 12 · lesson `ma-g5-science-u01-l12`
...
_Topic `ma-g5-science-u01::criteria and constraints` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g5-science.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Criteria state what a successful solution must achieve; constraints state the limits it must respect, including materials, cost, time, and safety.
- Criteria and constraints are set before solutions are generated, so all options are judged the same way.
- A solution that violates a constraint has not solved the problem, however well it scores on the criteria.
...
- Treating criteria and constraints as interchangeable.
- Declaring a solution best without stating the criteria.
- Revising the criteria after testing so a preferred solution wins.

**Grade boundary.** Weighted decision matrices and formal optimisation are later work.
...
_Package `swp-ma-g5-science-u01-l12` · built from `canonical-1.0.0` at `4056e31`._
