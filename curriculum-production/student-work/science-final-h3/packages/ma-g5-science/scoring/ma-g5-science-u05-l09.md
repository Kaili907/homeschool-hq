# Scoring authority — Performance task build: producers consumers decomposers

**Grade 5 Science** · Unit 5: Plants, Ecosystems, and Matter Cycles · Day 9 · lesson `ma-g5-science-u05-l09`
...
_Topic `ma-g5-science-u05::producers consumers decomposers` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g5-science.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Producers make their own food; consumers eat other organisms; decomposers break down dead material and wastes.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Producers make their own food, usually using light; consumers get food by eating other organisms; decomposers break down dead organisms and wastes.
- Decomposers return matter from dead organisms to the soil, water, and air, where producers can use it again.
- Without decomposers, matter would stay locked in dead material and the cycle would stop.

**Also correct.** Accept any of these framings — do not require the wording above:

- Naming fungi and bacteria as decomposers, or describing detritivores among them, are both acceptable.
...
- Classifying a plant as a consumer because it takes in water and minerals.
- Claiming decomposers are harmful organisms with no role in the ecosystem.
- Claiming dead material simply disappears without decomposers acting on it.

**Grade boundary.** Chemoautotrophs, trophic-level arithmetic, and nutrient-cycle chemistry are later work.
...
**Q1 — build product.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies producers consumers decomposers and the unit standards.

A complete response records the criteria before the first test, logs every test including the failures, and attributes the largest improvement to a specific revision with a reason for believing it was that revision. A product with only a successful final test has no evidence of iteration.

**Q2 — communicate representation.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of producers consumers decomposers.
...
**Q3 — check revise.** _Serves objective 3:_ Check, revise, or improve work about producers consumers decomposers using stated success criteria.
...
**Q4 — essential question.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies producers consumers decomposers and the unit standards.
...
**Q5 — evidence quality.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of producers consumers decomposers.
...
**Q6 — limitation.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies producers consumers decomposers and the unit standards.
...
**Q7 — provenance.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of producers consumers decomposers.
...
- **prerequisite gap** — Return to the smallest prerequisite needed for producers consumers decomposers, model it with a concrete or text-only representation, then retry one fresh item.
- **procedure without understanding** — Ask the learner to explain or represent why the process for producers consumers decomposers works before continuing.
- **correct but low confidence** — Confirm the reasoning specifically, offer one varied example of producers consumers decomposers, and avoid unnecessary remediation.
...
- **mastery evidence** — Require accurate independent application plus explanation on a later or different occasion before marking producers consumers decomposers mastered.
...
_Package `swp-ma-g5-science-u05-l09` · built from `canonical-1.0.0` at `4056e31`._
