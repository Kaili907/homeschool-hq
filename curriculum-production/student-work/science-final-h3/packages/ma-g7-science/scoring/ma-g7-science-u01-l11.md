# Scoring authority — Unit assessment: model limitations

**Grade 7 Science** · Unit 1: Matter, Particles, and Properties · Day 11 · lesson `ma-g7-science-u01-l11`
...
_Topic `ma-g7-science-u01::model limitations` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g7-science.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Every model of matter simplifies, and the simplification determines what the model can and cannot explain.
- A model is chosen for the question being asked; a model that is wrong for one purpose can be right for another.
- Stating a model's limitation is part of using it correctly, not an admission of failure.
...
- Treating a model's simplification as a property of the real system, such as claiming atoms are coloured spheres.
- Claiming a more detailed model is always the better one.
- Using a model outside the range it was built for without saying so.
...
**Q1 — demonstrate mastery.** _Serves objective 1:_ Demonstrate current mastery of model limitations through selected-response, constructed-response, and applied evidence.
...
**Q4 — essential question.** _Serves objective 1:_ Demonstrate current mastery of model limitations through selected-response, constructed-response, and applied evidence.
...
**Q6 — limitation.** _Serves objective 1:_ Demonstrate current mastery of model limitations through selected-response, constructed-response, and applied evidence.
...
_Package `swp-ma-g7-science-u01-l11` · built from `canonical-1.0.0` at `4056e31`._
