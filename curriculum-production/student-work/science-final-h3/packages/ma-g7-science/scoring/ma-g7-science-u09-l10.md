...
**Grade 7 Science** · Unit 9: Engineering Design Capstone · Day 10 · lesson `ma-g7-science-u09-l10`
...
_Topic `ma-g7-science-u09::iterative modeling` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g7-science.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- A model is revised when its prediction and the test result disagree, and the revision names what the disagreement showed.
- Changing one aspect at a time keeps the effect of each change identifiable.
- A model that has never been tested against data has not yet been evaluated, however reasonable it looks.
...
- Revising a model to fit data without saying what the disagreement showed.
- Changing several aspects at once and attributing the improvement to one.
- Treating an untested model as validated.

**Grade boundary.** Formal model calibration and parameter fitting are later work.
...
_Package `swp-ma-g7-science-u09-l10` · built from `canonical-1.0.0` at `4056e31`._
