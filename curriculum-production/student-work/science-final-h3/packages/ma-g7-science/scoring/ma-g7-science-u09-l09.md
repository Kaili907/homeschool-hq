# Scoring authority — Performance task build: test data

**Grade 7 Science** · Unit 9: Engineering Design Capstone · Day 9 · lesson `ma-g7-science-u09-l09`
...
_Topic `ma-g7-science-u09::test data` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g7-science.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Test data is only interpretable when the test conditions, the measured quantity, and the number of trials are recorded.
- Repeated trials show whether a difference between designs is larger than the variation within a design.
- A test that produced an unexpected result is reported as it happened.
...
- Claiming one design outperforms another from a difference smaller than the variation between trials.
- Reporting test results without the conditions under which they were obtained.
- Omitting a trial because its result was inconvenient.
...
**Q1 — build product.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies test data and the unit standards.
...
**Q2 — communicate representation.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of test data.
...
**Q3 — check revise.** _Serves objective 3:_ Check, revise, or improve work about test data using stated success criteria.
...
**Q4 — essential question.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies test data and the unit standards.
...
**Q5 — evidence quality.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of test data.
...
**Q6 — limitation.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies test data and the unit standards.
...
**Q7 — provenance.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of test data.
...
- **prerequisite gap** — Return to the smallest prerequisite needed for test data, model it with a concrete or text-only representation, then retry one fresh item.
- **procedure without understanding** — Ask the learner to explain or represent why the process for test data works before continuing.
- **correct but low confidence** — Confirm the reasoning specifically, offer one varied example of test data, and avoid unnecessary remediation.
...
- **mastery evidence** — Require accurate independent application plus explanation on a later or different occasion before marking test data mastered.
...
_Package `swp-ma-g7-science-u09-l09` · built from `canonical-1.0.0` at `4056e31`._
