# Scoring authority — Performance task build: proxy data and indirect evidence

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 1: Earth and Space Systems, Scale, and Data Practice · Day 9 · lesson `ma-hs12-earth-space-environmental-u01-l09`
...
_Topic `ma-hs12-earth-space-environmental-u01::proxy data and indirect evidence` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- A proxy is a measurable quantity that stands in for a quantity that cannot be measured directly, and it requires a calibrated relationship between the two.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Ice cores, tree rings, sediment layers, corals, and isotope ratios are proxies for past conditions.
- A proxy carries the uncertainty of the calibration as well as of the measurement itself.
- Independent proxies agreeing is what makes a reconstruction robust; a single proxy carries much weaker support.
...
- Treating a proxy measurement as a direct measurement of the quantity it stands for.
- Reporting a proxy reconstruction with no reference to its calibration or uncertainty.
- Basing a reconstruction on one proxy and presenting it as settled.

**Grade boundary.** Specific proxy calibration methodology and age-model construction are beyond this course.
...
> Published datasets the family retrieves plus the learner's own graphs and scale calculations.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for proxy data and indirect evidence that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain proxy data and indirect evidence using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply proxy data and indirect evidence to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on proxy data and indirect evidence against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply proxy data and indirect evidence to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain proxy data and indirect evidence using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply proxy data and indirect evidence to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for proxy data and indirect evidence that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for proxy data and indirect evidence (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for proxy data and indirect evidence (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for proxy data and indirect evidence (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for proxy data and indirect evidence (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for proxy data and indirect evidence (evidence type: application, require explanation: True, review timing: same-day).
...
- online: Locating datasets online can expose the learner to tracking or account prompts. — mitigation: An adult supplies the dataset file or exact source. No account creation, sign-in, or purchase is required.
- physical: Building a large scale model outdoors involves walking measured distances. — mitigation: Stay on the property or in a park away from roads and water, with an adult aware of the route.
...
**Disposal and clearing up:** Collect the string and markers and leave the site as it was found. Dataset files stay on the adult's device; nothing is uploaded and no location is stored. Nothing hazardous is produced.
...
_Package `swp-ma-hs12-earth-space-environmental-u01-l09` · built from `hs912-science-h3` at `e7551b9`._
