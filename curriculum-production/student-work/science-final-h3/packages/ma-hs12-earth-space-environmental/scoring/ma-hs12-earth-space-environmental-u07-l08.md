# Scoring authority — Reteach and varied practice: the greenhouse effect

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 7: Climate, Energy Flow, and the Carbon Cycle · Day 8 · lesson `ma-hs12-earth-space-environmental-u07-l08`
...
_Topic `ma-hs12-earth-space-environmental-u07::the greenhouse effect` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Greenhouse gases are largely transparent to incoming sunlight and absorb outgoing infrared radiation, re-emitting some of it back toward the surface.
- The natural greenhouse effect keeps Earth's surface far warmer than it would otherwise be and is necessary for life as it exists.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Water vapour, carbon dioxide, methane, and nitrous oxide absorb infrared radiation because of how their molecules vibrate.
- Adding greenhouse gases reduces outgoing infrared for a given surface temperature, so the surface warms until balance is restored.
- The concern is the enhancement of the natural effect by gases added through human activity, not the existence of the effect.

**Also correct.** Accept any of these framings — do not require the wording above:

- Describing the mechanism as absorption and re-emission of infrared, or as reduced outgoing longwave radiation, are both correct.
...
- Claiming greenhouse gases work by trapping heat like the glass of a greenhouse, which warms mainly by suppressing convection.
- Claiming greenhouse gases block incoming sunlight.
- Confusing the natural greenhouse effect with the enhanced effect from added gases, or treating the natural effect as the problem.

**Grade boundary.** Radiative forcing calculation and line-by-line absorption modelling are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain the greenhouse effect using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply the greenhouse effect to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on the greenhouse effect against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply the greenhouse effect to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain the greenhouse effect using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply the greenhouse effect to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain the greenhouse effect using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for the greenhouse effect (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for the greenhouse effect (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for the greenhouse effect (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for the greenhouse effect (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for the greenhouse effect (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u07-l08` · built from `hs912-science-h3` at `e7551b9`._
