# Scoring authority — Correction and reflection: solar system architecture

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 4: Orbital Motion and the Solar System · Day 12 · lesson `ma-hs12-earth-space-environmental-u04-l12`
...
_Topic `ma-hs12-earth-space-environmental-u04::solar system architecture` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The inner planets are small and rocky; the outer planets are large and gas- or ice-rich, and this pattern reflects conditions in the forming solar nebula.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The solar system formed from a rotating disc of gas and dust, which is why the planets orbit in nearly the same plane and direction.
- Temperature in the disc decreased with distance from the young Sun, so volatile materials could condense only in the outer regions.
- Asteroids, comets, and Kuiper belt objects are remnants of that formation process and carry evidence about it.
...
- Claiming the planets formed independently and were later captured by the Sun.
- Claiming the inner-outer composition difference is a coincidence rather than a consequence of formation conditions.
- Claiming the asteroid belt is the remains of a destroyed planet.

**Grade boundary.** Planetary migration models and exoplanet system comparison are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain solar system architecture using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply solar system architecture to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on solar system architecture against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply solar system architecture to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain solar system architecture using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply solar system architecture to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain solar system architecture using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for solar system architecture (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for solar system architecture (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for solar system architecture (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for solar system architecture (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for solar system architecture (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u04-l12` · built from `hs912-science-h3` at `e7551b9`._
