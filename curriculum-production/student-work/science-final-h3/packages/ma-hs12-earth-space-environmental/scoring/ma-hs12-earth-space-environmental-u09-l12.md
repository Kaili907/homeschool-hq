...
_Topic `ma-hs12-earth-space-environmental-u09::defending a design under uncertainty` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Defending a design means stating the evidence, the assumptions, the uncertainty, and what result would change the recommendation.
- Uncertainty is a reason to state the range of outcomes and to build in adaptability, not a reason to withhold a recommendation.
- The strongest objection is addressed directly rather than omitted.
...
- Presenting a recommendation with no acknowledgement of its uncertainties.
- Treating uncertainty as grounds for taking no position at all.
- Omitting the strongest objection to the proposed design.

**Grade boundary.** Decision theory under uncertainty and formal risk management frameworks are beyond this course.
...
_Package `swp-ma-hs12-earth-space-environmental-u09-l12` · built from `hs912-science-h3` at `e7551b9`._
