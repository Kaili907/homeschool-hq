# Scoring authority — Correction and reflection: reconstructing geologic history

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 5: Plate Tectonics and the History of Earth · Day 12 · lesson `ma-hs12-earth-space-environmental-u05-l12`
...
_Topic `ma-hs12-earth-space-environmental-u05::reconstructing geologic history` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- A geologic history is reconstructed by combining relative dating of structures with absolute ages and with evidence about depositional environment.
- Cross-cutting relationships, inclusions, and unconformities each constrain the sequence of events.
- A reconstruction states which parts are well constrained and which rest on fewer lines of evidence.

**Also correct.** Accept any of these framings — do not require the wording above:

- Naming the stratigraphic principles, or applying them without the names, are equally correct.
...
- Applying superposition to a sequence with evidence of overturning without accounting for it.
- Treating an unconformity as evidence that no time passed rather than that record is missing.
- Presenting a reconstruction as settled where only one line of evidence supports it.

**Grade boundary.** Basin analysis and quantitative subsidence modelling are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain reconstructing geologic history using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply reconstructing geologic history to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on reconstructing geologic history against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply reconstructing geologic history to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain reconstructing geologic history using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply reconstructing geologic history to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain reconstructing geologic history using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for reconstructing geologic history (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for reconstructing geologic history (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for reconstructing geologic history (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for reconstructing geologic history (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for reconstructing geologic history (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u05-l12` · built from `hs912-science-h3` at `e7551b9`._
