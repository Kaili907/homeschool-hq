# Scoring authority — Unit assessment: resource availability and human settlement

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 8: Coevolution of Earth and Life, Resources, and Natural Hazards · Day 11 · lesson `ma-hs12-earth-space-environmental-u08-l11`
...
_Topic `ma-hs12-earth-space-environmental-u08::resource availability and human settlement` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Where people settle has been shaped by access to water, arable soil, energy, materials, and transport routes.
- Settlement in hazard-prone locations often reflects the resources that make those locations valuable, such as fertile volcanic soils and river floodplains.
- Resource availability changes over time through depletion, technology, and climate, and settlement patterns respond with a lag.
...
- Claiming settlement patterns are unrelated to the physical distribution of resources.
- Claiming people settle in hazardous places only through ignorance of the hazard.
- Claiming resource availability at a location is fixed over time.

**Grade boundary.** Human geography theory and economic modelling are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for resource availability and human settlement on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain resource availability and human settlement using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply resource availability and human settlement to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on resource availability and human settlement against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply resource availability and human settlement to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain resource availability and human settlement using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply resource availability and human settlement to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for resource availability and human settlement on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for resource availability and human settlement (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for resource availability and human settlement (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for resource availability and human settlement (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for resource availability and human settlement (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for resource availability and human settlement (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u08-l11` · built from `hs912-science-h3` at `e7551b9`._
