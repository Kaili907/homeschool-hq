# Scoring authority — Correction and reflection: constructing an explanation from multiple evidence lines

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 3: Cosmology and the Evidence for the Big Bang · Day 12 · lesson `ma-hs12-earth-space-environmental-u03-l12`
...
_Topic `ma-hs12-earth-space-environmental-u03::constructing an explanation from multiple evidence lines` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- The strength of the Big Bang model rests on several independent lines agreeing: galaxy redshifts, the microwave background, and the light-element abundances.
- Independent lines of evidence converging carries far more weight than any single line, because they fail in different ways.
- An explanation states which observations it accounts for and which it does not.
...
- Presenting one line of evidence as establishing the model on its own.
- Omitting an observation the explanation does not account for.
- Treating independent confirmations as though they were repetitions of the same measurement.

**Grade boundary.** Formal model comparison and likelihood analysis are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain constructing an explanation from multiple evidence lines using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply constructing an explanation from multiple evidence lines to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on constructing an explanation from multiple evidence lines against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply constructing an explanation from multiple evidence lines to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain constructing an explanation from multiple evidence lines using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply constructing an explanation from multiple evidence lines to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain constructing an explanation from multiple evidence lines using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for constructing an explanation from multiple evidence lines (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for constructing an explanation from multiple evidence lines (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for constructing an explanation from multiple evidence lines (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for constructing an explanation from multiple evidence lines (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for constructing an explanation from multiple evidence lines (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u03-l12` · built from `hs912-science-h3` at `e7551b9`._
