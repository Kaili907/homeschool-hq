# Scoring authority — Performance task build: Hubble's relationship

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 3: Cosmology and the Evidence for the Big Bang · Day 9 · lesson `ma-hs12-earth-space-environmental-u03-l09`
...
_Topic `ma-hs12-earth-space-environmental-u03::Hubble's relationship` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- More distant galaxies recede faster, with recession speed approximately proportional to distance.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The proportionality between recession speed and distance is what is expected if space is expanding uniformly.
- The relationship allows distance to be estimated from redshift, once the constant of proportionality is calibrated.
- Extrapolating the expansion backward gives an estimate of the time since the expansion began.

**Also correct.** Accept any of these framings — do not require the wording above:

- Naming the constant of proportionality the Hubble constant, or describing the relationship without the name, are equally correct.
...
- Claiming all galaxies recede at the same speed regardless of distance.
- Claiming the relationship shows the Milky Way is at the centre of the expansion.
- Treating the calibration constant as exactly known when its measured value carries real uncertainty.

**Grade boundary.** Distance ladder methodology and tension between measured values of the constant are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for Hubble's relationship that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain Hubble's relationship using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply Hubble's relationship to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on Hubble's relationship against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply Hubble's relationship to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain Hubble's relationship using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply Hubble's relationship to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for Hubble's relationship that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for Hubble's relationship (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for Hubble's relationship (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for Hubble's relationship (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for Hubble's relationship (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for Hubble's relationship (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u03-l09` · built from `hs912-science-h3` at `e7551b9`._
