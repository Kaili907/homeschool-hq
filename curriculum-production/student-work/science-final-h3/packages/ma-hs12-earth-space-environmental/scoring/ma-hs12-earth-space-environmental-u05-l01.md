# Scoring authority — Launch and diagnostic: evidence for plate motion

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 5: Plate Tectonics and the History of Earth · Day 1 · lesson `ma-hs12-earth-space-environmental-u05-l01`
...
_Topic `ma-hs12-earth-space-environmental-u05::evidence for plate motion` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs12-earth-space-environmental.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Plate motion is directly measured today by satellite geodesy, at rates of centimetres per year.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Evidence includes matching coastlines and rock sequences across oceans, matching fossil distributions, seafloor age patterns, earthquake and volcano distributions, and direct measurement.
- The distribution of earthquakes and volcanoes in narrow bands marks plate boundaries.
- No single line of evidence established plate tectonics; the convergence of many did.
...
- Claiming continents plough through oceanic crust rather than moving as parts of plates.
- Claiming plate motion is too slow to be measured directly.
- Claiming earthquakes and volcanoes are distributed randomly across Earth's surface.

**Grade boundary.** Quantitative plate kinematics and GPS processing methods are beyond this course.
...
> Published seafloor age, radiometric age, and fossil distribution data. Spreading rates are calculated by the learner from the map.


## What a complete response contains, question by question

**Q1 — phenomenon question.** _Serves objective 1:_ Observe the anchoring phenomenon, surface initial thinking about evidence for plate motion without penalty, and pose a testable question.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain evidence for plate motion using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply evidence for plate motion to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on evidence for plate motion against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply evidence for plate motion to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain evidence for plate motion using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply evidence for plate motion to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Observe the anchoring phenomenon, surface initial thinking about evidence for plate motion without penalty, and pose a testable question.
...
- **prerequisite-gap** — Run the prerequisite reteach route for evidence for plate motion (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for evidence for plate motion (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for evidence for plate motion (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for evidence for plate motion (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for evidence for plate motion (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u05-l01` · built from `hs912-science-h3` at `e7551b9`._
