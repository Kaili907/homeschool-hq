# Scoring authority — Synthesis and review: fair tests

**Grade 3 Science** · Unit 1: Science and Engineering Practices · Day 10 · lesson `ma-g3-science-u01-l10`
...
_Topic `ma-g3-science-u01::fair tests` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g3-science.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- In a fair test only one thing is changed on purpose.

**Accepted relationships and models.** What this lesson's learning target asserts:

- In a fair test one thing is changed on purpose and everything else that could affect the result is kept the same.
- If more than one thing changes at once, you cannot tell which change caused the result.
- Repeating a test gives more confidence in the result than a single try does.
...
- Calling a test fair when two or more conditions were changed at the same time.
- Claiming a single trial proves a result.
- Treating a result that disagrees with the prediction as a failed test; a fair test that disagrees with the prediction is still a good test.

**Grade boundary.** Control groups by name, randomisation, and sample-size reasoning are later work.


## What a complete response contains, question by question

**Q1 — connect unit.** _Serves objective 1:_ Connect fair tests to the unit’s other ideas and select evidence that shows readiness for assessment.
...
**Q2 — communicate representation.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of fair tests.
...
**Q3 — check revise.** _Serves objective 3:_ Check, revise, or improve work about fair tests using stated success criteria.
...
**Q4 — essential question.** _Serves objective 1:_ Connect fair tests to the unit’s other ideas and select evidence that shows readiness for assessment.
...
**Q5 — evidence quality.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of fair tests.
...
**Q6 — limitation.** _Serves objective 1:_ Connect fair tests to the unit’s other ideas and select evidence that shows readiness for assessment.
...
**Q7 — provenance.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of fair tests.
...
- **prerequisite gap** — Return to the smallest prerequisite needed for fair tests, model it with a concrete or text-only representation, then retry one fresh item.
- **procedure without understanding** — Ask the learner to explain or represent why the process for fair tests works before continuing.
- **correct but low confidence** — Confirm the reasoning specifically, offer one varied example of fair tests, and avoid unnecessary remediation.
...
- **mastery evidence** — Require accurate independent application plus explanation on a later or different occasion before marking fair tests mastered.
- **requests an experiment** — Explain the science and the safety rule for fair tests; do not invent an investigation, procedure, result, or observation. Direct the learner to the adult-approved investigation in the lesson.
...
_Package `swp-ma-g3-science-u01-l10` · built from `g34-committed` at `4c6ca4e`._
