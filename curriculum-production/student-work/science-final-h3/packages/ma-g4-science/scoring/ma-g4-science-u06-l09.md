# Scoring authority — Performance task build: modeling light paths

**Grade 4 Science** · Unit 6: Light, Vision, and the Senses · Day 9 · lesson `ma-g4-science-u06-l09`
...
_Topic `ma-g4-science-u06::modeling light paths` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-g4-science.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Light paths are modelled with straight arrows that start at the source and point in the direction the light travels.
- A complete model of seeing shows the path source, object, eye, in that order.
- The arrows are a model of the path; they are not something visible in the room.

**Also correct.** Accept any of these framings — do not require the wording above:

- Ray diagrams drawn with lines or with arrows are both acceptable, provided direction is shown.
...
- Drawing arrows from the eye to the object.
- Drawing light travelling in curves around an obstacle.
- Omitting the source from the path and starting at the object.

**Grade boundary.** Scale ray diagrams, angles of incidence and reflection, and lens ray tracing are later work.
...
**Q1 — build product.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies modeling light paths and the unit standards.
...
**Q2 — communicate representation.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of modeling light paths.
...
**Q3 — check revise.** _Serves objective 3:_ Check, revise, or improve work about modeling light paths using stated success criteria.
...
**Q4 — essential question.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies modeling light paths and the unit standards.
...
**Q5 — evidence quality.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of modeling light paths.
...
**Q6 — limitation.** _Serves objective 1:_ Create, test, or revise a meaningful product that applies modeling light paths and the unit standards.
...
**Q7 — provenance.** _Serves objective 2:_ Use subject-appropriate evidence or representation to communicate understanding of modeling light paths.
...
- **prerequisite gap** — Return to the smallest prerequisite needed for modeling light paths, model it with a concrete or text-only representation, then retry one fresh item.
- **procedure without understanding** — Ask the learner to explain or represent why the process for modeling light paths works before continuing.
- **correct but low confidence** — Confirm the reasoning specifically, offer one varied example of modeling light paths, and avoid unnecessary remediation.
...
- **mastery evidence** — Require accurate independent application plus explanation on a later or different occasion before marking modeling light paths mastered.
- **requests an experiment** — Explain the science and the safety rule for modeling light paths; do not invent an investigation, procedure, result, or observation. Direct the learner to the adult-approved investigation in the lesson.
...
_Package `swp-ma-g4-science-u06-l09` · built from `g34-committed` at `4c6ca4e`._
