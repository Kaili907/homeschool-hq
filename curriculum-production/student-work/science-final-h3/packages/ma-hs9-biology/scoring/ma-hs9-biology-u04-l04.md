# Scoring authority — Independent application A: feedback failure and disease

**High School Biology (Grade 9)** · Unit 4: Homeostasis and Feedback Mechanisms · Day 4 · lesson `ma-hs9-biology-u04-l04`
...
_Topic `ma-hs9-biology-u04::feedback failure and disease` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- Disease can result when a sensor, a control centre, an effector, or the signal between them fails.
- The same symptom can arise from failures at different points in the loop, so the symptom alone does not identify the cause.
- Treatment that replaces an effector's output does not restore regulation unless the loop can still detect and respond.
...
- Inferring the point of failure from the symptom alone with no further evidence.
- Claiming a failure at any point in the loop produces the same physiological effect.
- Claiming replacing a hormone restores feedback regulation as well as the hormone level.

**Grade boundary.** Clinical diagnosis, pharmacology, and specific disease mechanisms in depth are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain feedback failure and disease using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply feedback failure and disease to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on feedback failure and disease against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply feedback failure and disease to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain feedback failure and disease using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply feedback failure and disease to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain feedback failure and disease using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for feedback failure and disease (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for feedback failure and disease (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for feedback failure and disease (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for feedback failure and disease (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for feedback failure and disease (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u04-l04` · built from `hs912-science-h3` at `e7551b9`._
