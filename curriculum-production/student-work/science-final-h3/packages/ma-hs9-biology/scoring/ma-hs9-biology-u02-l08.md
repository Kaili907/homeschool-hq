# Scoring authority — Reteach and varied practice: replication as information copying

**High School Biology (Grade 9)** · Unit 2: DNA, Proteins, and the Information Flow of the Cell · Day 8 · lesson `ma-hs9-biology-u02-l08`
...
_Topic `ma-hs9-biology-u02::replication as information copying` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- DNA replication is semiconservative: each new double helix contains one original strand and one newly made strand.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The double helix separates and each original strand acts as a template for a new complementary strand.
- Because replication is semiconservative, every daughter molecule retains one parental strand.
- Copying is highly accurate but not perfect, and uncorrected errors become mutations.

**Also correct.** Accept any of these framings — do not require the wording above:

- Describing the template as the parental strand, or as the original strand, are equally correct.
...
- Claiming replication is conservative, with the original helix intact and a wholly new one produced.
- Claiming the new strand has the same sequence as its template rather than the complementary sequence.
- Claiming replication is error-free.

**Grade boundary.** Replication fork enzymology, leading and lagging strand detail, and telomere biology are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain replication as information copying using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply replication as information copying to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on replication as information copying against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply replication as information copying to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain replication as information copying using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply replication as information copying to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain replication as information copying using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for replication as information copying (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for replication as information copying (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for replication as information copying (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for replication as information copying (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for replication as information copying (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u02-l08` · built from `hs912-science-h3` at `e7551b9`._
