# Scoring authority — Independent application A: nitrogen and nutrient cycling

**High School Biology (Grade 9)** · Unit 6: Ecosystems: Matter Cycling and Energy Flow · Day 4 · lesson `ma-hs9-biology-u06-l04`
...
_Topic `ma-hs9-biology-u06::nitrogen and nutrient cycling` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- Atmospheric nitrogen gas cannot be used directly by plants or animals; it must first be fixed into usable compounds.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Nitrogen-fixing bacteria, and industrial processes, convert atmospheric nitrogen into forms plants can take up.
- Nitrogen moves from soil to plants to consumers and back through excretion and decomposition.
- Adding excess nutrients to water bodies stimulates growth that, on decomposing, depletes dissolved oxygen.

**Also correct.** Accept any of these framings — do not require the wording above:

- Naming the steps fixation, nitrification, assimilation, ammonification, and denitrification is correct but not required.
...
- Claiming plants take nitrogen directly from the air.
- Claiming animals obtain nitrogen from the atmosphere.
- Claiming adding more nutrients to an ecosystem always improves it.

**Grade boundary.** Detailed microbial biochemistry and nutrient-loading models are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain nitrogen and nutrient cycling using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply nitrogen and nutrient cycling to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on nitrogen and nutrient cycling against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply nitrogen and nutrient cycling to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain nitrogen and nutrient cycling using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply nitrogen and nutrient cycling to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain nitrogen and nutrient cycling using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for nitrogen and nutrient cycling (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for nitrogen and nutrient cycling (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for nitrogen and nutrient cycling (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for nitrogen and nutrient cycling (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for nitrogen and nutrient cycling (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u06-l04` · built from `hs912-science-h3` at `e7551b9`._
