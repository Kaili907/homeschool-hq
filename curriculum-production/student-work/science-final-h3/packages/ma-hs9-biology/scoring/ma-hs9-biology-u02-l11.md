# Scoring authority — Unit assessment: protein folding and function

**High School Biology (Grade 9)** · Unit 2: DNA, Proteins, and the Information Flow of the Cell · Day 11 · lesson `ma-hs9-biology-u02-l11`
...
_Topic `ma-hs9-biology-u02::protein folding and function` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- A protein's amino acid sequence, together with its cellular environment, determines the three-dimensional structure it folds into, and that structure determines its function.

**Accepted relationships and models.** What this lesson's learning target asserts:

- The order of amino acids determines how the chain folds, because the side chains interact with each other and with water.
- A protein that loses its folded shape loses its function, which is what denaturation by heat or extreme pH does.
- Because structure follows from sequence, a change in the sequence can change or abolish the function.

**Also correct.** Accept any of these framings — do not require the wording above:

- Referring to primary, secondary, tertiary, and quaternary structure is correct but not required.
...
- Claiming a protein's function is determined by its amino acid composition regardless of order.
- Claiming a denatured protein retains its function because its sequence is unchanged.
- Claiming proteins fold into shapes that are chosen to suit the function required.

**Grade boundary.** Chaperone biology, folding thermodynamics, and structure prediction are beyond this course.
...
**Q1 — mastery fresh task.** _Serves objective 1:_ Demonstrate independent mastery evidence for protein folding and function on a fresh task.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain protein folding and function using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply protein folding and function to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on protein folding and function against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply protein folding and function to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain protein folding and function using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply protein folding and function to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Demonstrate independent mastery evidence for protein folding and function on a fresh task.
...
- **prerequisite-gap** — Run the prerequisite reteach route for protein folding and function (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for protein folding and function (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for protein folding and function (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for protein folding and function (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for protein folding and function (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u02-l11` · built from `hs912-science-h3` at `e7551b9`._
