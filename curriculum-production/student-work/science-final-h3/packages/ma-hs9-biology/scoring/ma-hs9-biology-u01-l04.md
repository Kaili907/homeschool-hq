# Scoring authority — Independent application A: structure-function relationships

**High School Biology (Grade 9)** · Unit 1: Scientific Practice, Measurement, and the Molecules of Life · Day 4 · lesson `ma-hs9-biology-u01-l04`
...
_Topic `ma-hs9-biology-u01::structure-function relationships` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- The three-dimensional shape and chemical properties of a biological molecule determine what it can interact with and therefore what it does.
- Changing the structure changes the function, which is why a small change in shape can abolish activity entirely.
- The relationship runs from structure to function; a molecule does not acquire a structure because a function is needed.
...
- Claiming a molecule changes its structure in order to perform a function it is required to perform.
- Claiming function can be predicted from composition without regard to structure.
- Claiming structure and function are independent.

**Grade boundary.** Structural biology methods and molecular dynamics are beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain structure-function relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply structure-function relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on structure-function relationships against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply structure-function relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain structure-function relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply structure-function relationships to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain structure-function relationships using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for structure-function relationships (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for structure-function relationships (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for structure-function relationships (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for structure-function relationships (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for structure-function relationships (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u01-l04` · built from `hs912-science-h3` at `e7551b9`._
