# Scoring authority — Independent application A: probability and Punnett models

**High School Biology (Grade 9)** · Unit 8: Inheritance and Variation of Traits · Day 4 · lesson `ma-hs9-biology-u08-l04`
...
_Topic `ma-hs9-biology-u08::probability and Punnett models` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- A Punnett square gives expected proportions across many offspring, not an outcome for any individual offspring.

**Accepted relationships and models.** What this lesson's learning target asserts:

- A Punnett square enumerates the possible gamete combinations and their expected frequencies.
- Each fertilisation is an independent event, so previous offspring do not change the probabilities for the next.
- Observed proportions in small samples deviate from expectation by chance, and that deviation is not evidence against the model.

**Also correct.** Accept any of these framings — do not require the wording above:

- Punnett squares, probability rules, and branch diagrams give the same predictions and are all acceptable.
...
- Reading a three-to-one expected ratio as a guarantee about four offspring.
- Claiming previous offspring change the probability for a subsequent one.
- Rejecting a genetic model because a small family departed from the expected ratio.

**Grade boundary.** Chi-squared goodness-of-fit testing is beyond this course.


## What a complete response contains, question by question

**Q1 — explain with evidence.** _Serves objective 1:_ Explain probability and Punnett models using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply probability and Punnett models to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on probability and Punnett models against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply probability and Punnett models to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain probability and Punnett models using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply probability and Punnett models to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain probability and Punnett models using evidence, a model, or a documented process rather than an unsupported answer.
...
- The learner completes the central task about probability and Punnett models.
...
- **prerequisite-gap** — Run the prerequisite reteach route for probability and Punnett models (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for probability and Punnett models (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for probability and Punnett models (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for probability and Punnett models (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for probability and Punnett models (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u08-l04` · built from `hs912-science-h3` at `e7551b9`._
