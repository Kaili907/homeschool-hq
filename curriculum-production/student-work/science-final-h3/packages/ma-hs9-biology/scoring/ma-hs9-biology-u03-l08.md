# Scoring authority — Reteach and varied practice: organelles as interacting subsystems

**High School Biology (Grade 9)** · Unit 3: Cells, Hierarchical Organization, and Cellular Division · Day 8 · lesson `ma-hs9-biology-u03-l08`
...
_Topic `ma-hs9-biology-u03::organelles as interacting subsystems` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The nucleus holds the genetic material, ribosomes assemble proteins, mitochondria carry out most ATP production, and chloroplasts carry out photosynthesis in plant cells.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Organelles are specialised compartments whose functions depend on one another, so the cell operates as an integrated system.
- Membranes create separate chemical environments, which is what allows incompatible processes to run in the same cell at once.
- A product of one organelle is frequently the input of another, so disrupting one disrupts others.
...
- Claiming plant cells have chloroplasts instead of mitochondria; plant cells have both.
- Claiming mitochondria produce energy rather than converting it into a usable chemical form.
- Claiming organelles function independently of one another.

**Grade boundary.** Vesicular trafficking detail and organelle proteomics are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain organelles as interacting subsystems using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply organelles as interacting subsystems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on organelles as interacting subsystems against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply organelles as interacting subsystems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain organelles as interacting subsystems using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply organelles as interacting subsystems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain organelles as interacting subsystems using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for organelles as interacting subsystems (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for organelles as interacting subsystems (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for organelles as interacting subsystems (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for organelles as interacting subsystems (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for organelles as interacting subsystems (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u03-l08` · built from `hs912-science-h3` at `e7551b9`._
