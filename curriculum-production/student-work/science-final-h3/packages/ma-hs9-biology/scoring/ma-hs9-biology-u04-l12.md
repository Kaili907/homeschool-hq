# Scoring authority — Correction and reflection: modelling feedback quantitatively

**High School Biology (Grade 9)** · Unit 4: Homeostasis and Feedback Mechanisms · Day 12 · lesson `ma-hs9-biology-u04-l12`
...
_Topic `ma-hs9-biology-u04::modelling feedback quantitatively` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Accepted relationships and models.** What this lesson's learning target asserts:

- A quantitative feedback model represents the variable over time, the setpoint, and the size and delay of the response.
- Delay in the response produces oscillation around the setpoint rather than a smooth return.
- A model is evaluated against measured time-course data, not against how reasonable it looks.

**Also correct.** Accept any of these framings — do not require the wording above:

- Graphical, tabular, and equation-based representations are all acceptable model forms here.
...
- Modelling a regulated variable as returning instantly to setpoint with no delay or overshoot.
- Presenting a model output as though it were measured data.
- Claiming a model is validated without comparison to data.

**Grade boundary.** Differential equation modelling and parameter estimation are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain modelling feedback quantitatively using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply modelling feedback quantitatively to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on modelling feedback quantitatively against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply modelling feedback quantitatively to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain modelling feedback quantitatively using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply modelling feedback quantitatively to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain modelling feedback quantitatively using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for modelling feedback quantitatively (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for modelling feedback quantitatively (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for modelling feedback quantitatively (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for modelling feedback quantitatively (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for modelling feedback quantitatively (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u04-l12` · built from `hs912-science-h3` at `e7551b9`._
