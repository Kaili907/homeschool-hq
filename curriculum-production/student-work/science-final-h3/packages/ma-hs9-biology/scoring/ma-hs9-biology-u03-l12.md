# Scoring authority — Correction and reflection: scale and surface-area-to-volume

**High School Biology (Grade 9)** · Unit 3: Cells, Hierarchical Organization, and Cellular Division · Day 12 · lesson `ma-hs9-biology-u03-l12`
...
_Topic `ma-hs9-biology-u03::scale and surface-area-to-volume` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `RUBRIC_CORRECTNESS_CONSTRAINT`, `SUPPLIED_DATA_ANSWER_AUTHORITY` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- As an object grows with its shape unchanged, its volume increases faster than its surface area, so the surface-area-to-volume ratio falls.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Exchange with the environment happens across the surface, while demand scales with volume, so cells are limited in the size they can reach.
- Multicellular organisms overcome the limit with many small cells and with structures such as folds, villi, and branching that increase surface area.
- The ratio explains why cells divide rather than simply growing larger.

**Also correct.** Accept any of these framings — do not require the wording above:

- Explaining the constraint through diffusion distance, or through the ratio of surface to volume, are both correct.
...
- Claiming surface area and volume increase in the same proportion as an object grows.
- Claiming a larger cell exchanges materials more efficiently than a smaller one.
- Claiming the ratio increases as a cell grows.

**Grade boundary.** Quantitative diffusion modelling and allometric scaling laws are beyond this course.
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain scale and surface-area-to-volume using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply scale and surface-area-to-volume to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on scale and surface-area-to-volume against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply scale and surface-area-to-volume to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain scale and surface-area-to-volume using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply scale and surface-area-to-volume to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain scale and surface-area-to-volume using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for scale and surface-area-to-volume (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for scale and surface-area-to-volume (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for scale and surface-area-to-volume (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for scale and surface-area-to-volume (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for scale and surface-area-to-volume (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs9-biology-u03-l12` · built from `hs912-science-h3` at `e7551b9`._
