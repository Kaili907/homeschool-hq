# Scoring authority — Performance task build: sources of genetic variation

**High School Biology (Grade 9)** · Unit 8: Inheritance and Variation of Traits · Day 9 · lesson `ma-hs9-biology-u08-l09`
...
_Topic `ma-hs9-biology-u08::sources of genetic variation` · forms in force: `ACCEPTED_RELATIONSHIPS`, `EXPECTED_REASONING_CRITERIA`, `FIXED_FACTUAL`, `INVESTIGATION_CRITERIA`, `RUBRIC_CORRECTNESS_CONSTRAINT` · authored in `policy/correctness/ma-hs9-biology.correctness.json`._

**Fixed for this topic.** These are settled, and work that contradicts one is `Not yet` on Scientific correctness:

- The sources of heritable variation are mutation, recombination during meiosis, independent assortment, and the random combination of gametes at fertilisation.

**Accepted relationships and models.** What this lesson's learning target asserts:

- Mutation is the ultimate source of new alleles; the other mechanisms rearrange existing alleles into new combinations.
- Sexual reproduction generates far more genetic variety among offspring than asexual reproduction does.
- Variation is present in a population before selection acts on it.
...
- Claiming recombination creates new alleles.
- Claiming variation arises in response to environmental need.
- Claiming asexual reproduction produces the same level of genetic variety as sexual reproduction.

**Grade boundary.** Population genetics and allele frequency mathematics are beyond this course.
...
**Q1 — performance evidence.** _Serves objective 1:_ Produce performance-task evidence for sources of genetic variation that a reader could check independently.
...
**Q2 — explain with evidence.** _Serves objective 2:_ Explain sources of genetic variation using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q3 — apply new task.** _Serves objective 3:_ Apply sources of genetic variation to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q4 — check revise next step.** _Serves objective 4:_ Check and revise the work on sources of genetic variation against the stated success criteria and name the next step.
...
**Q5 — essential question.** _Serves objective 3:_ Apply sources of genetic variation to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q6 — evidence quality.** _Serves objective 2:_ Explain sources of genetic variation using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q7 — limitation.** _Serves objective 3:_ Apply sources of genetic variation to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q8 — provenance.** _Serves objective 1:_ Produce performance-task evidence for sources of genetic variation that a reader could check independently.
...
- **prerequisite-gap** — Run the prerequisite reteach route for sources of genetic variation (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for sources of genetic variation (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for sources of genetic variation (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for sources of genetic variation (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for sources of genetic variation (evidence type: application, require explanation: True, review timing: same-day).
...
- physical: Collecting leaves without identifying the plant risks irritant sap, poison ivy, oak or sumac, and pesticide residue. — mitigation: Collect only from a plant an adult has identified as safe. Never from a plant with milky sap, never poison ivy, oak, or sumac, and never from a plant or verge that has been treated with pesticide. Wear gloves and wash hands afterwards.
- physical: Small counters and dried beans are a choking hazard around young children and are no longer food. — mitigation: Keep counters away from younger siblings, count them back into a labelled container, and never return them to food storage or eat them.
- privacy: Family trait or pedigree work can expose genetic and health information about relatives who never consented. — mitigation: This unit uses NON-HUMAN specimens and fictional pedigrees only. The learner is never asked to record family traits, family medical history, ancestry, or any relative's data.
- emotional: Heredity topics can touch on disability, adoption, or family history painfully. — mitigation: All examples are fictional or non-human; the learner may decline any example without penalty and no personal disclosure is ever requested.
...
_Package `swp-ma-hs9-biology-u08-l09` · built from `hs912-science-h3` at `e7551b9`._
