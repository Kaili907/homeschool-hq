...
const MUTATIONS = [
  ...
  {
    check: 'hs-safety-from-h3-only',
    description: 'paraphrase the H3 safety-review segment instead of carrying it verbatim',
    apply(state) {
      const pkg = find(
        state.packages,
        (entry) => entry.course_id.startsWith('ma-hs') && entry.safety_brief.h3_safety_segment_verbatim,
      )
      pkg.safety_brief = {
        ...pkg.safety_brief,
        h3_safety_segment_verbatim: 'Be careful with the materials and ask an adult if unsure.',
      }
    },
  },
  {
  ...
  },
  {
    ...
    description: 'soften a floor prohibition so it no longer matches the H3 policy set',
    apply(state) {
  ...
  },
  {
    ...
    description: 'leave a repinned correctness key on the superseded H2 commit the package no longer reads',
    apply(state) {
      const pkg = find(
        state.packages,
  ...
  },
  {
    ...
    description: 'strip the H3 SAFETY rule off a phenomenon that names finely divided iron',
    apply(state) {
      const pkg = find(
        state.packages,
