...
export const ELEM_TECH_TASK = {
  ...
  MODEL: (c) => `Follow the example about ${f(c)} one step at a time. Do not skip ahead. After each step, write what changed. Make a small table. Show what things looked like before and after each step. Now change one thing in the example. Guess what will happen. Then check. Was your guess right? Last, say the rule for ${f(c)} in your own words.`,
  MODEL_A: (c) => `Look at the first example of ${f(c)}. Find the rule it follows. Write that rule in one short sentence. Do not put an example inside the sentence. Now make up your own example that follows the rule. Then make one that breaks the rule. Try both. Show which one the rule accepts.`,
  MODEL_B: (c) => `Look at a second example of ${f(c)}. Put it next to the first one you studied. Make two lists. List what is the same. List what is different. Then say when you would use each one. Take one input the first example handled. Run it through the second one. Write what happened and why.`,
