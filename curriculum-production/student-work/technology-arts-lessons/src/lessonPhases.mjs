...
export const TECH_TASK = {
  ...
  GUIDED: (c) => `Complete the first two test cases printed in activity_setup for ${t(c).focus}. On the first, work step by step with the printed method available, and after each step write the one-line reason that step is correct. On the second, work without the method in view and only check it after you commit to an answer. Both problems must show the intermediate states — a trace table, printed values, or a labelled diagram — not just a final result. Then state which of the two felt less certain and name the specific step where your confidence dropped.`,
  GUIDED_A: (c) => `Work the first supported pass on ${t(c).focus}. Take the first two complete test cases printed in activity_setup and solve each with the method visible, writing after every step the reason that step is legal and what would have gone wrong without it. Record intermediate values or states for both cases. Then mark, on your own work, the single step you would most likely get wrong if the method were removed — that mark is what the second pass will test.`,
