...
const MUTATIONS = [
  ...
  {
    check: 'hs-safety-from-h2-only',
    description: 'paraphrase the H2 safety-review segment instead of carrying it verbatim',
    apply(state) {
      const pkg = find(
        state.packages,
        (entry) => entry.course_id.startsWith('ma-hs') && entry.safety_brief.h2_safety_segment_verbatim,
      )
      pkg.safety_brief = {
        ...pkg.safety_brief,
        h2_safety_segment_verbatim: 'Be careful with the materials and ask an adult if unsure.',
      }
    },
  },
  {
  ...
  },
  {
    ...
    description: 'soften a floor prohibition so it no longer matches the H2 policy set',
    apply(state) {
