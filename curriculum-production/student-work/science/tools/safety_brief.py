...
High School briefs are built from the H2 package (mac/hs912-science-h2 @
265ea3a). Where H2 already authored a student-visible `safety-review` segment,
...
def build_hs_brief(lesson: dict, floor: dict) -> dict:
    ...
    return {
        ...
        "h2_safety_segment_verbatim": segment,
        "h2_lab_safety_summary_verbatim": summary,
    }
...
def brief_lines(resolved: dict) -> list[str]:
    ...
    if resolved.get("h2_safety_segment_verbatim"):
        lines.append(resolved["h2_safety_segment_verbatim"])
