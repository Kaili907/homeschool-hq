# Scoring authority — Guided practice A: computational simulation of coupled systems

**High School Earth, Space, and Environmental Science (Grade 12)** · Unit 9: Sustainability Engineering and the Grade 12 Capstone · Day 3 · lesson `ma-hs12-earth-space-environmental-u09-l03`
...
**Q1 — explain with evidence.** _Serves objective 1:_ Explain computational simulation of coupled systems using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q2 — apply new task.** _Serves objective 2:_ Apply computational simulation of coupled systems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q3 — check revise next step.** _Serves objective 3:_ Check and revise the work on computational simulation of coupled systems against the stated success criteria and name the next step.
...
**Q4 — essential question.** _Serves objective 2:_ Apply computational simulation of coupled systems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q5 — evidence quality.** _Serves objective 1:_ Explain computational simulation of coupled systems using evidence, a model, or a documented process rather than an unsupported answer.
...
**Q6 — limitation.** _Serves objective 2:_ Apply computational simulation of coupled systems to a task the learner has not seen before, showing the reasoning that produced the result.
...
**Q7 — provenance.** _Serves objective 1:_ Explain computational simulation of coupled systems using evidence, a model, or a documented process rather than an unsupported answer.
...
- **prerequisite-gap** — Run the prerequisite reteach route for computational simulation of coupled systems (representation: concrete, require explanation: True, retry count: 1).
- **procedure-without-understanding** — Run the conceptual explanation route for computational simulation of coupled systems (representation: visual, require explanation: True).
- **correct-low-confidence** — Run the confidence calibration route for computational simulation of coupled systems (representation: text, require explanation: False, retry count: 0).
- **repeated-error-pattern** — Run the error pattern contrast route for computational simulation of coupled systems (representation: worked-example, retry count: 2, review timing: next-session).
- **mastery-evidence** — Run the mastery evidence collection route for computational simulation of coupled systems (evidence type: application, require explanation: True, review timing: same-day).
...
_Package `swp-ma-hs12-earth-space-environmental-u09-l03` · built from `hs912-science-h2` at `265ea3a`._
