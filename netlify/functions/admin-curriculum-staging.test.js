...
function setup(authorizationResult = {
  ...
  return { authorization, authoring, studio, handler }
}

describe('Admin curriculum release staging HTTP boundary', () => {
  it('requires a current draft assignment before exposing staging evidence', async () => {
    const { authoring, studio, handler } = setup()
    authoring.read.mockRejectedValue(Object.assign(new Error('private assignment'), { code: 'forbidden' }))
    const response = await handler(event('GET', `/api/admin/curriculum/drafts/${DRAFT}/staging`))
    expect(response.statusCode).toBe(403)
    expect(response.body).toContain('curriculum_collaboration_required')
    expect(studio.readStaging).not.toHaveBeenCalled()
  })
