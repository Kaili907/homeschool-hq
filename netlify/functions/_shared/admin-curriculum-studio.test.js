...
describe('Curriculum Studio materialization service', () => {
  ...
  it('binds validation to the exact draft revision and preserves Michigan PE human-review blockers', async () => {
    ...
    expect(result).toMatchObject({ baseReleaseVersion: '1.0.0', targetVersion: '2.0.0-draft.1' })
  ...
  it('carries one exact identity from standards resolution through validation, approval, staging, and mutation staleness', async () => {
    let draftRevision = 5
    let ledger = []
    let latestValidation = null
    let approved = false
    const authoring = {
      read: vi.fn(async () => detail([], draftRevision)),
      readEntity: vi.fn(),
    }
    const standardsReview = {
      list: vi.fn(async () => ({ schemaVersion: 1, decisions: ledger })),
    }
    const approval = {
      recordValidation: vi.fn(async (_actor, input) => {
        latestValidation = {
          validationSnapshotId: `40000000-0000-4000-8000-00000000000${draftRevision}`,
          draftRevision: input.draftRevision,
          engineVersion: input.engineVersion,
          resultDigest: input.resultDigest,
          status: input.status,
      ...
      read: vi.fn(async () => ({
        ...
        status: approved && latestValidation?.draftRevision === draftRevision ? 'approved' : 'stale',
        latestValidation,
        currentDecision: approved && latestValidation?.draftRevision === draftRevision ? {
          approvalId: '50000000-0000-4000-8000-000000000001',
          draftRevision,
        } : null,
        publishGate: {
          eligible: approved && latestValidation?.draftRevision === draftRevision,
          draftRevision,
          approvalId: approved ? '50000000-0000-4000-8000-000000000001' : null,
          validationSnapshotId: latestValidation?.draftRevision === draftRevision
            ? latestValidation.validationSnapshotId : null,
        },
      })),
    }
    const staging = {
      stage: vi.fn(async (_actor, candidate) => ({
        ...
        stagingId: '60000000-0000-4000-8000-000000000001',
        candidate,
      })),
    }
    const service = createAdminCurriculumStudioService({ authoring, approval, staging, standardsReview })

    const blocked = await service.validateDraft('actor', DRAFT_ID, 5)
    expect(blocked.run.publicationReady).toBe(false)
    expect(blocked.validationSnapshot.humanReviewBlockerCount).toBeGreaterThan(0)

    const review = await service.readStandardsReview('actor', DRAFT_ID, 5)
    const queue = buildCurriculumStandardsReviewQueue(
      review.occurrences,
      { kind: 'draft', ref: DRAFT_ID },
      [],
    )
    ledger = queue.map((item) => ({
      schemaVersion: 1,
      ...
      evidenceSource: 'State framework evidence',
      reviewerNote: 'Approved for this exact draft evidence set.',
      revision: 1,
      updatedAt: '2026-08-10T12:00:00Z',
    }))

    const validated = await service.validateDraft('actor', DRAFT_ID, 5)
    expect(validated.run.publicationReady).toBe(true)
    expect(validated.validationSnapshot).toMatchObject({
      draftRevision: 5,
      status: 'valid',
      humanReviewBlockerCount: 0,
    })
    approved = true
    const staged = await service.stageDraft('actor', DRAFT_ID, 5, '70000000-0000-4000-8000-000000000001')
    expect(staged).toMatchObject({ draftId: DRAFT_ID, draftRevision: 5 })
    expect(staging.stage).toHaveBeenCalledWith('actor', expect.objectContaining({
      draftId: DRAFT_ID,
      draftRevision: 5,
      validationSnapshotId: validated.validationSnapshot.validationSnapshotId,
      approvalId: '50000000-0000-4000-8000-000000000001',
      contentHash: expect.stringMatching(/^[0-9a-f]{64}$/),
    }), '70000000-0000-4000-8000-000000000001')

    draftRevision = 6
    await expect(service.stageDraft('actor', DRAFT_ID, 5, '70000000-0000-4000-8000-000000000002'))
      .rejects.toMatchObject({ code: 'gate-blocked' })
    await expect(service.readStandardsReview('actor', DRAFT_ID, 5))
