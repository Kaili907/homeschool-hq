...
describe('D1 composition: end-to-end worker run', () => {
  ...
  it('reports 200 no_work with claimed=0 when nothing is due', async () => {
    const estate = createDurableEstate({ jobs: [] })
    const response = await handlerFor(estate)(manualEvent())
    expect(response.statusCode).toBe(200)
    expect(JSON.parse(response.body)).toEqual({
      status: 'no_work', claimed: 0, delivered: 0, indeterminate: 0, failed: 0,
    })
  ...
  it('runs overlapping workers without delivering the same job twice', async () => {
    const estate = createDurableEstate()
    let claimed = false
    ...
    const first = handlerFor(estate)
    const second = handlerFor(estate)
    const responses = await Promise.all([first(manualEvent()), second(manualEvent())])
    expect(responses.map((response) => JSON.parse(response.body).status).sort())
      .toEqual(['no_work', 'processed'])
    expect(estate.countOf('academy_study_deliver_in_app_notification_v2')).toBe(1)
  })

  it('leaves a lease-lost claim durable for retry and reports a partial non-success', async () => {
    const jobs = [
    ...
    expect(estate.callsTo('academy_study_release_delivery_lease_v2'))
      .toHaveLength(1)
    expect(estate.callsTo('academy_study_release_delivery_lease_v2')[0].parameters.p_job_id)
      .toBe('job:0001')
  })
...
describe('D1 composition: invocation authorization', () => {
  ...
  it('treats a forged schedule header as inert on an authorized manual invocation', async () => {
    const estate = createDurableEstate()
    const response = await handlerFor(estate)(manualEvent({ 'x-nf-event': 'schedule' }))
    expect(response.statusCode).toBe(200)
    expect(estate.names()).toContain('academy_study_claim_delivery_jobs_v2')
  })
...
describe('D1 composition: lazy fail-closed readiness', () => {
  ...
  it.each(CASES)('returns 503 unavailable for %s', async (_name, patch) => {
    const estate = createDurableEstate()
    const env = { ...ENV, ...patch }
  ...
  ])('returns 503 unavailable for %s', async (_name, rpcName, handler) => {
    const estate = createDurableEstate()
  ...
  it('reports a systemic claim outage as unavailable without raw durable detail', async () => {
    const estate = createDurableEstate()
