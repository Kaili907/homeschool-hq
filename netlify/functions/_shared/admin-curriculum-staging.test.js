...
describe('curriculum release staging materialization', () => {
  ...
  it('materializes base + override + draft-created - tombstone into an isolated full snapshot', async () => {
    ...
    const summaries = [
      { entityType: 'course', entityRef: originalCourse.course_id, origin: 'base_override', revision: 1, position: 0, tombstoned: false },
      { entityType: 'media_resource', entityRef: created.resource_id, origin: 'draft_created', revision: 1, position: 99, tombstoned: false },
      { entityType: 'media_resource', entityRef: immutable.resources[0].resource_id, origin: 'base_override', revision: 1, position: 0, tombstoned: true },
    ]
    ...
    const authoring = {
      ...
      readEntity: vi.fn(async (_actor, _draft, entityType, entityRef) => ({
        entityType, entityRef, payload: details.get(`${entityType}:${entityRef}`),
      })),
    }
    let staged
    const staging = {
