...
describe('authorized Admin System Health endpoint', () => {
  ...
  it('reports trusted saved-off AI/TTS runtime as disabled even when deployment gates are on', async () => {
    const source = { read: vi.fn().mockResolvedValue(evidence()) }
    const effectiveConfigurationReader = {
      read: vi.fn(async () => ({
        status: 'available', runtime: { aiEnabled: false, ttsEnabled: false },
      })),
    }
    const handler = createAdminHealthHandler({
    ...
    expect(effectiveConfigurationReader.read).toHaveBeenCalledOnce()
  ...
  it('reports AI/TTS disabled when an injected runtime resolver throws', async () => {
    ...
    const handler = createAdminHealthHandler({
      ...
      effectiveConfigurationReader: {
        read: vi.fn(async () => { throw new Error('SECRET configuration detail') }),
      },
      now: () => NOW,
    })
    const response = await handler(request())
