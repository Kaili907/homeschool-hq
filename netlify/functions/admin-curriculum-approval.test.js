...
describe('curriculum human approval API', () => {
  it('requires a current draft editor or reviewer assignment in addition to global capability', async () => {
    const approval = { read: vi.fn(), decide: vi.fn(), recordValidation: vi.fn() }
    const handler = createHandler({
      approval,
      authoring: { read: vi.fn().mockRejectedValue(Object.assign(new Error('private assignment'), { code: 'forbidden' })) },
    ...
    expect(response.body).not.toContain('private assignment')
