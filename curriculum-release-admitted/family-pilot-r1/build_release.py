...
ELA_SOURCE_INPUTS = (
    (
        "mac-g34-ela-r1",
        "ef81511c2b582d003e397bb79daa8a26a41e3b10",
        "curriculum-authoring/full-family-grade34/subjects/english-language-arts",
    ),
    (
        "mac-ela-production-r1",
        "00374a8dc26eddfac2cf52aec5661deff760ddbb",
        "curriculum-content/manuel-academy/1.0.0/grades",
    ),
    (
        "mac-hs912-ela-r1",
        "42f2505bb04d831c4aefc195a7ce03edb2d7b1d9",
        "curriculum-authoring/full-family-highschool-9-12/subjects/english-language-arts",
    ),
)
...
def rerun_ela_h3() -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="manuel-academy-ela-h3-", dir=REPO) as temporary:
        temp = Path(temporary)
        # Keep the standalone gate run from inheriting an unrelated tsconfig
        # that may exist above the system temporary directory.
        (temp / "tsconfig.json").write_text("{}\n", encoding="utf-8")
        ela_archive = run(
            "git", "-c", "core.autocrlf=false", "archive", ela["commit"], "--",
            "curriculum-production/student-work/english-language-arts",
        )
        subprocess.run(["tar", "-x", "-f", "-", "-C", str(temp)], input=ela_archive, check=True)
        gate_archive = run(
            "git", "-c", "core.autocrlf=false", "archive", GATE_H3_SHA, "--", "src/curriculum/production-quality",
        )
        subprocess.run(["tar", "-x", "-f", "-", "-C", str(temp)], input=gate_archive, check=True)
        source_worktrees = temp / "source-worktrees"
        for checkout, commit, source_path in ELA_SOURCE_INPUTS:
            source_archive = run(
                "git",
                "-c",
                "core.autocrlf=false",
                "archive",
                f"--prefix=source-worktrees/{checkout}/",
                commit,
                "--",
                source_path,
            )
        ...
        ela_root = temp / "curriculum-production/student-work/english-language-arts"
        for test_name in ("gate.test.ts", "lib.test.ts", "content-quality.test.ts"):
            shutil.copyfile(
                REPO / "curriculum-production/student-work/english-language-arts/tests" / test_name,
                ela_root / "tests" / test_name,
            )
        config = temp / "curriculum-production/student-work/english-language-arts/tooling/vitest.config.mjs"
        vitest_name = "vitest.cmd" if os.name == "nt" else "vitest"
        vitest_candidates = [
            REPO / "node_modules/.bin" / vitest_name,
            *sorted(REPO.parent.glob(f"*/node_modules/.bin/{vitest_name}")),
        ]
        ...
        if os.name == "nt":
            subprocess.run(
                ["cmd", "/c", "mklink", "/J", str(node_modules), str(vitest.parents[1])],
                check=True,
                stdout=subprocess.PIPE,
            )
        else:
            node_modules.symlink_to(vitest.parents[1], target_is_directory=True)
        completed = subprocess.run(
            ...
            env={
                **os.environ,
                ...
                "TMP": str(temp),
                "TMPDIR": str(temp),
                "ELA_SOURCE_WORKTREES": str(source_worktrees),
            },
        )
...
def run_admission_adapter() -> None:
    admission = SOURCES["release-admission"]
    admission_root = REPO / admission["path"]
    extracted = False
    if not admission_root.exists():
        archive = run(
            "git",
            "-c",
            "core.autocrlf=false",
...
def build() -> None:
    ...
    ela_h3 = rerun_ela_h3()
    structural_manifest = json.loads((STRUCTURAL_ROOT / "MANIFEST.json").read_text(encoding="utf-8"))
    structural_assessments = json.loads((STRUCTURAL_ROOT / "assessment-index.json").read_text(encoding="utf-8"))
...
def main() -> None:
    ...
    if args.validate_only:
        ...
        validation = json.loads((ROOT / "validation/binding-validation.json").read_text(encoding="utf-8"))
        mutation = json.loads((ROOT / "validation/mutation-controls.json").read_text(encoding="utf-8"))
