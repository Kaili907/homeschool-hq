...
export function classifyMisconceptions(
  skillId: StableId,
  definitions: readonly MisconceptionDefinition[],
  evidence: readonly ClassificationEvidence[],
): MisconceptionClassification {
  const observedTags = evidence.flatMap((item) => item.tags);
  const hypotheses: MisconceptionHypothesis[] = definitions
    .filter((definition) => definition.skillId === skillId)
    .map((definition) => {
      let support = 0;
      let contradiction = 0;
      const supportingTags: StableId[] = [];
      const contradictingTags: StableId[] = [];
      for (const signal of definition.distinguishingEvidence) {
        if (!observedTags.includes(signal.tag)) continue;
      ...
      let status: MisconceptionHypothesis["status"] = "insufficient-evidence";
