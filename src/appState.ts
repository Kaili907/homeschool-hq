...
export function importBackup(current: AppState, text: string): ImportResult {
  ...
  // The file's learner PINs are non-authoritative: rebind to this device's
  // credentials BEFORE validating, so no imported pin is ever trusted. Old
  // backups that still carry a pin stay importable — the pin is simply ignored.
  ...
  if (asV1) {
    const merged = {
      ...
      profiles: {
        ...current.profiles,
        // A legacy file establishes no credential either: p1 keeps whatever
        // this device already trusts (migrateV1ToV2 seeds an empty pin, which
        // would otherwise blank an existing girl's PIN).
        p1: {
          ...asV1.profiles.p1,
