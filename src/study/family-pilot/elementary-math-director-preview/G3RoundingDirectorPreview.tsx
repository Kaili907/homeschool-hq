import { useEffect, useMemo, useState } from 'react'
import { loadFinalFamilyPilotCatalog, type FinalLearnerProductionMaterial } from '../../../curriculum/final-app-data'
import { ElementaryMathSamplePlayer } from '../elementary-math-sample-player'
import {
  BrowserLearnerResponseStore,
  LearnerResponseRuntime,
} from '../final-app/learner-response'

export const G3_ROUNDING_LESSON_REF = 'ma-g3-mathematics-u01-l02' as const
export const G3_ROUNDING_CHILD_DISPLAY_TITLE = 'Round Numbers to the Nearest 100' as const
export const G3_ROUNDING_CANONICAL_TITLE = 'Concept build A: the place-value structure of three-digit numbers' as const

export function G3RoundingDirectorPreview() {
  const [material, setMaterial] = useState<FinalLearnerProductionMaterial | null>(null)
  const [error, setError] = useState('')
  const store = useMemo(() => new BrowserLearnerResponseStore({
    // Review responses remain isolated from production Family Pilot records and migration.
    databaseName: 'manuel-academy.director-review.g3-rounding-r1',
    legacyStorage: { getItem: () => null },
  }), [])

  useEffect(() => {
    let live = true
    void loadFinalFamilyPilotCatalog()
      .then((catalog) => catalog.getMaterial(G3_ROUNDING_LESSON_REF))
      .then((loaded) => {
        if (!live) return
        if (!loaded || loaded.lessonRef !== G3_ROUNDING_LESSON_REF) {
          throw new Error('The canonical Grade 3 rounding lesson is unavailable.')
        }
        if (loaded.title !== G3_ROUNDING_CANONICAL_TITLE) {
          throw new Error('The Grade 3 rounding lesson provenance did not match this review.')
        }
        setMaterial(loaded)
      })
      .catch((cause: unknown) => {
        if (live) setError(cause instanceof Error ? cause.message : 'The lesson could not be opened.')
      })
    return () => { live = false }
  }, [])

  if (error) return <main><h1>Preview unavailable</h1><p role="alert">{error}</p></main>
  if (!material) return <main aria-busy="true"><p role="status">Opening the Grade 3 rounding lesson…</p></main>

  const runtime = new LearnerResponseRuntime(material, {
    lessonRef: G3_ROUNDING_LESSON_REF,
    studentRef: 'director-review-r1',
    assignmentRef: 'g3-rounding-director-review-r1',
    attemptRef: 'g3-rounding-director-review-r1',
  }, store)

  return (
    <ElementaryMathSamplePlayer
      runtime={runtime}
      material={material}
      displayTitle={G3_ROUNDING_CHILD_DISPLAY_TITLE}
      onNeedHelp={() => { /* Jarvis R1 review callback placeholder. */ }}
    />
  )
}
