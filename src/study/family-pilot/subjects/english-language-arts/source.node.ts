...
import { classifyElaLessonKind } from './lessonKind'
import { ELA_SUBJECT, type ElaCatalog, type ElaCourseRef, type ElaLessonRef, type ElaUnitRef } from './types'

/**
 * FAMILY-PILOT-SUBJECT-ELA — Node-only loader for the ELA course tree under
 * curriculum-content/manuel-academy/<release>/grades/grade-{5,7,8}/courses/
 * english-language-arts/. Mirrors src/curriculum/family-pilot/source.node.ts's
 * loadCourse validation exactly (course_day sequencing, unit<->lesson cross
 * checks, release-version gate) against the same curriculum-content tree —
 * there is no second copy of curriculum content here, only a second reader of
 * the one that already exists. Throws ElaContentError instead of returning a
 * partial or guessed catalog.
 */
export class ElaContentError extends Error {
  constructor(message: string) {
    super(message)
    this.name = 'ElaContentError'
  }
}

const ELA_GRADES: readonly AcademyGrade[] = ['5', '7', '8']
...
function readJson(path: string, label: string): unknown {
  ...
  try {
    ...
    throw new ElaContentError(`${label} is unavailable at ${path}`)
  }
  try {
    return JSON.parse(raw)
  } catch {
    throw new ElaContentError(`${label} is not valid JSON at ${path}`)
  }
}
...
function readLessons(path: string, label: string): readonly RawLesson[] {
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    try {
      ...
      throw new ElaContentError(`${label} contains an invalid line`)
    }
    ...
    if (
      ...
      throw new ElaContentError(`${label} has a lesson missing required fields`)
    }
    lessons.push({
...
function readReleaseVersion(): string {
  ...
  if (typeof registry.currentRelease !== 'string' || !registry.currentRelease) {
    throw new ElaContentError('production release registry has no currentRelease')
  }
  ...
  if (!active || active.version !== registry.currentRelease) {
    throw new ElaContentError('production release registry has no matching active release')
  }
  return registry.currentRelease
}

function loadCourse(releaseVersion: string, grade: AcademyGrade): ElaCourseRef {
  const base = join(CONTENT_ROOT, releaseVersion, 'grades', `grade-${grade}`, 'courses', ELA_SUBJECT)
  const label = `grade-${grade} ${ELA_SUBJECT}`

  const units = readJson(join(base, 'units.json'), `${label} units.json`)
  if (!Array.isArray(units) || units.length === 0) {
    throw new ElaContentError(`${label} units.json is empty or malformed`)
  }
  const assessments = readJson(join(base, 'assessments.json'), `${label} assessments.json`)
  if (!Array.isArray(assessments)) {
    throw new ElaContentError(`${label} assessments.json is malformed`)
  }
  ...
  if (rawLessons.length === 0) throw new ElaContentError(`${label} lessons.jsonl is empty`)

  const courseId = rawLessons[0].course_id
  const byId = new Map<string, RawLesson>()
  for (const lesson of rawLessons) {
    if (lesson.course_id !== courseId) {
      throw new ElaContentError(`${label} has mixed course_id across lessons.jsonl`)
    }
    if (byId.has(lesson.lesson_id)) {
      throw new ElaContentError(`${label} lessons.jsonl has duplicate lesson_id ${lesson.lesson_id}`)
    }
  ...
  const unitRefs: ElaUnitRef[] = units
    .slice()
    ...
    .map((raw) => {
      ...
      if (
        ...
        throw new ElaContentError(`${label} unit ${String(unit.unit_id)} is missing required fields`)
      }
      if (unit.course_id !== courseId) {
        throw new ElaContentError(`${label} has mixed course_id between units.json and lessons.jsonl`)
      }
      for (const lessonId of unit.lesson_ids) {
        if (typeof lessonId !== 'string') {
          throw new ElaContentError(`${label} unit ${unit.unit_id} has a non-string lesson id`)
        }
        const lesson = byId.get(lessonId)
        if (!lesson) {
          throw new ElaContentError(`${label} unit ${unit.unit_id} references unknown lesson ${lessonId}`)
        }
        ...
        if (seenLessonIds.has(lessonId)) {
          throw new ElaContentError(`${label} lesson ${lessonId} is listed in more than one unit`)
        }
      ...
      if (assessmentId !== null && !assessmentIds.has(assessmentId)) {
        throw new ElaContentError(`${label} unit ${unit.unit_id} references unknown assessment ${assessmentId}`)
      }
  ...
  if (seenLessonIds.size !== byId.size) {
    throw new ElaContentError(`${label} units.json does not cover every lesson in lessons.jsonl`)
  }

  const unitIdByNumber = new Map(unitRefs.map((unit) => [unit.unitNumber, unit.unitId]))
  const lessons: ElaLessonRef[] = sortedByDay.map((lesson) => {
    const unitId = unitIdByNumber.get(lesson.unit_number)
    if (!unitId) {
      throw new ElaContentError(`${label} lesson ${lesson.lesson_id} references unit ${lesson.unit_number}, which has no units.json entry`)
    }
    return Object.freeze({
      ...
      title: lesson.title,
      phase: lesson.phase,
      studyKind: classifyElaLessonKind(lesson.phase),
    })
  })
...
let cached: ElaCatalog | undefined

/** Loads (and caches) the ELA catalog from the frozen curriculum-content
 * source. Throws ElaContentError instead of returning a partial or guessed
 * catalog. */
export function loadElaCatalog(): ElaCatalog {
  if (cached) return cached
  const releaseVersion = readReleaseVersion()
  const courses = ELA_GRADES.map((grade) => loadCourse(releaseVersion, grade))
