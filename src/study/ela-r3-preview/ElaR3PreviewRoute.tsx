...
import { ELA_R3_PREVIEW_PATH } from './route'

import grade3Unit7Lesson8 from '../../../curriculum-production/final/english-language-arts/r3/lessons/grade-03/ma-g3-english-language-arts-u07-l08.lesson.json'

/**
 * Ephemeral Director preview for English Language Arts Production R3.
 *
 * Same posture as the frozen R2 gallery and the Mathematics R3 preview: it mounts
 * the real Rich Study Player over a throwaway in-memory session. It writes no
 * learner progress, assignment, or cloud data, and it introduces no engine,
 * runtime, or lesson model of its own. src/study/director-review/ is untouched.
 */

interface ElaR3Lesson {
...
const LESSONS: readonly ElaR3Lesson[] = [
  grade3Unit7Lesson8 as unknown as ElaR3Lesson,
]

function selectedLesson(): ElaR3Lesson | null {
...
function LessonPreview({ lesson }: { readonly lesson: ElaR3Lesson }) {
  ...
  const snapshot: FamilyPilotStudySnapshot = {
    session: {
      householdRef: 'ela-r3-preview:ephemeral', learnerRef: 'ela-r3-preview:reviewer',
      blockRef: `ela-r3-preview:${lessonRef}`, sessionRef: `ela-r3-preview:${lessonRef}`,
    },
    lessonRef,
  ...
  const segmentContent = {
    ...
    // Constructed writing is never machine-scored: everything the reviewer saves
    // stays REVIEW_REQUIRED, which is what Parent Review means here.
  ...
  return (
    <div className="ela-r3-preview">
      <header>
        <p>English Language Arts Production R3 · ephemeral preview · {meta.status}</p>
        <h1>Grade {meta.grade} English Language Arts · Unit {meta.unitNumber}, Day {meta.dayInUnit}</h1>
        <p>{meta.lessonRef} · {material.title}</p>
        <p>{meta.unitTitle} · course day {meta.courseDay} · {meta.focus}</p>
      </header>
      <p role="note">
        ...
        and no written response is given an automatic score.
      </p>
      <section aria-label={`Grade ${meta.grade} English Language Arts Rich Study Player`}>
...
export default function ElaR3PreviewRoute() {
  const lesson = selectedLesson()
  if (!lesson) {
    return (
      <main>
        <h1>English Language Arts Production R3 preview</h1>
        <p>No R3 lesson files were found under <code>curriculum-production/final/english-language-arts/r3/lessons</code>.</p>
      </main>
    )
  }
