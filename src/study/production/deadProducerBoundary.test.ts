...
import { code, RC1_SENTINEL, walkImportClosure } from '../testing/importClosure'
...
/** The safe production host surface. The production route remains dark. */
const PRODUCTION_CONTAINER = 'src/components/study/ProductionStudySessionContainer.tsx'
/** The RC1-backed preview wrapper the dark browser route still reaches. */
const PREVIEW_CONTAINER = 'src/components/study/StudySessionContainer.tsx'
...
// ---------------------------------------------------------------------------
// Import-closure walk. The mechanism lives in ../testing/importClosure.ts and
// is shared with the safe-container guard. This file only normalizes its report
// into the historical dead-producer assertion shape.
// ---------------------------------------------------------------------------

function walk(entries: readonly string[]) {
  const closure = walkImportClosure(repoRoot, entries)
  return {
    files: closure.files,
    packages: [...new Set(closure.packages.map((edge) => edge.specifier))].sort(),
    unresolved: [...new Set(closure.unresolved.map((edge) =>
      `${relative(repoRoot, edge.from).replaceAll('\\', '/')} -> ${edge.specifier}`))].sort(),
    unanalysable: [...closure.unanalysable].sort(),
  }
}

describe('STUDY-A1 dead-producer closure walker controls', () => {
  it('follows literal dynamic import and require edges through a three-hop fixture', () => {
    const fixtureRoot = mkdtempSync(resolve(tmpdir(), 'study-dead-producer-closure-'))
    try {
      writeFileSync(resolve(fixtureRoot, 'entry.ts'), "void import('./dynamic')\n")
      writeFileSync(resolve(fixtureRoot, 'dynamic.ts'), "require('./required')\n")
      writeFileSync(resolve(fixtureRoot, 'required.ts'), "export * from './target'\n")
      writeFileSync(resolve(fixtureRoot, 'target.ts'), 'export const reached = true\n')

      const closure = walkImportClosure(fixtureRoot, ['entry.ts'])
      expect(closure.unanalysable).toEqual([])
      expect(closure.unresolved).toEqual([])
      expect(closure.files).toEqual(['dynamic.ts', 'entry.ts', 'required.ts', 'target.ts'])
    } finally {
...
describe('STUDY-A1 dead production producers — nothing dead reaches the browser', () => {
  ...
  it('reaches the surfaces it is supposed to reach', () => {
    ...
    expect(closure.files).toContain(PREVIEW_CONTAINER)
...
// ---------------------------------------------------------------------------
// Closed: the safe production container is clean; preview remains the control.
// ---------------------------------------------------------------------------

const CLOSED_PRODUCTION_CONTAINER_BOUNDARY = Object.freeze({
  status: 'closed',
  surface: PRODUCTION_CONTAINER,
  forbiddenModuleFragments: Object.freeze([
    'src/study/runtimeFacade',
    'src/study/localDevelopmentPorts',
    'adaptive-tutor/study-engine/runtime/src/tutor-bridge',
    'adaptive-tutor/study-engine/runtime/src/subject-registry',
...
function markerCarriers(files: readonly string[], marker: string): readonly string[] {
  return files.filter((file) => code(read(file)).includes(marker))
}

describe('STUDY-A1 dead-producer production container handoff — closed', () => {
  const production = walk([CLOSED_PRODUCTION_CONTAINER_BOUNDARY.surface])
  const preview = walk([PREVIEW_CONTAINER])

  it('walks the production closure completely with the shared safe-container walker', () => {
    expect(production.unanalysable).toEqual([])
    expect(production.unresolved).toEqual([])
    expect(production.packages).toEqual(['react'])
    expect(production.files).toContain(PRODUCTION_CONTAINER)
    expect(production.files).toContain('src/components/study/studySessionSurface.tsx')
  })

  it('keeps runtimeFacade, RC1/local runtime, Tutor bridge, registry, and frozen content unreachable', () => {
    const moduleViolations = production.files.flatMap((file) =>
      CLOSED_PRODUCTION_CONTAINER_BOUNDARY.forbiddenModuleFragments
        .filter((fragment) => file.includes(fragment))
        .map((fragment) => `${file} reaches forbidden ${fragment}`))
    const markerViolations = CLOSED_PRODUCTION_CONTAINER_BOUNDARY.forbiddenMarkers.flatMap((marker) =>
      markerCarriers(production.files, marker).map((file) => `${file} carries forbidden ${marker}`))

    expect(moduleViolations).toEqual([])
    expect(markerViolations).toEqual([])

    // The CLOSED datum is measured. Restoring the old OPEN record, switching
    // the surface back to preview, or adding a direct or transitive edge makes
    // this disagree and fail rather than turning the record into prose.
    const measured = moduleViolations.length === 0 && markerViolations.length === 0
      ? 'closed'
      : 'open'
    expect(measured).toBe(CLOSED_PRODUCTION_CONTAINER_BOUNDARY.status)
  })

  it('CONTROL: preview still reaches RC1/local runtime and frozen Tutor content', () => {
    expect(preview.files).toContain(PREVIEW_CONTAINER)
    expect(preview.files).toContain('src/study/runtimeFacade.ts')
    expect(preview.files).toContain('adaptive-tutor/study-engine/runtime/src/tutor-bridge.ts')
    expect(preview.files).toContain('adaptive-tutor/study-engine/runtime/src/subject-registry.ts')
    expect(preview.files.some((file) => file.startsWith('adaptive-tutor/subjects/'))).toBe(true)
    expect(preview.files.some((file) => file.startsWith('adaptive-tutor/core/'))).toBe(true)

    for (const marker of ['AcceptedRc1HostRuntime', RC1_SENTINEL, 'LOCAL DEVELOPMENT ONLY']) {
      expect(markerCarriers(preview.files, marker).length, marker).toBeGreaterThan(0)
    }
  })
})
