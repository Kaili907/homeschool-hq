import { SUPPORTED_ACADEMY_GRADES, gradeLabel } from '../../curriculum/grade-authority'
...
export function CurriculumStandardsReviewWorkspace({
  ...
  if (readState.status === 'missing-context') return <section role="status"><h2>Select an exact draft revision</h2><p>Open a draft in Curriculum Studio, then continue to Standards Review. Published-release evidence is not substituted for a missing draft identity.</p></section>
  if (readState.status === 'stale') return <section role="alert"><h2>Draft revision is stale</h2><p>The requested standards queue no longer matches the current draft revision. Return to Studio and continue from the current revision.</p></section>
  ...
  return <ReadyWorkspace {...{
    decisions: readState.decisions,
    occurrences: readState.occurrences ?? occurrences,
    context: readState.context ?? context,
    identity: readState.identity,
    canManage,
    canApprove,
    savingReviewKey,
    saveError,
...
function ReadyWorkspace({
  ...
  readonly occurrences: readonly CurriculumStandardsReviewOccurrence[]
  readonly context: { readonly kind: CurriculumStandardsReviewContextKind; readonly ref: string }
  readonly identity?: { readonly draftRevision: number; readonly baseReleaseVersion: string; readonly targetVersion: string }
  readonly canManage: boolean
  readonly canApprove: boolean
  readonly savingReviewKey: string | null
  ...
  return (
    <section aria-labelledby="standards-review-title" className="min-w-0 space-y-5 text-slate-100">
      <header className="rounded-2xl border border-slate-700 bg-slate-950 p-5 sm:p-7">
        ...
        <div className="mt-2 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            {identity && <p className="mt-2 text-xs text-slate-400">Draft {context.ref} · revision {identity.draftRevision} · base {identity.baseReleaseVersion} · target {identity.targetVersion}</p>}
          </div>
      ...
      <div className="grid gap-3 rounded-2xl border border-slate-700 bg-slate-900 p-4 sm:grid-cols-2 xl:grid-cols-5" aria-label="Standards review filters">
        ...
        <label className="text-sm" htmlFor="standards-review-grade">Grade<select id="standards-review-grade" value={grade} onChange={(event) => setGrade(event.target.value === 'all' ? 'all' : Number(event.target.value))} className="mt-1 w-full rounded-lg border border-slate-600 bg-slate-950 px-3 py-2"><option value="all">All grades</option>{SUPPORTED_ACADEMY_GRADES.map((value) => <option key={value} value={value}>{gradeLabel(value)}</option>)}</select></label>
