...
import { makeFinLitQuestion, money, pick, type CurriculumGenerator, type FinLitQuestion } from './grade8FinLitCore'
...
export function generateInflationAdjustedPriceQuestion(difficulty:Difficulty):Grade8FinLitUnit8Question {const p=pick([{priceCents:2_000n,increaseBps:500n},{priceCents:3_500n,increaseBps:800n},{priceCents:4_000n,increaseBps:1_250n}] as const);const answer=p.priceCents*(10_000n+p.increaseBps)/10_000n;return makeFinLitQuestion({itemType:'inflation-adjusted-price',difficulty,prompt:`A sample item's price is ${money(p.priceCents)} and it increases by ${(Number(p.increaseBps)/100).toFixed(2)}%. What is the new price?`,correctAnswer:money(answer),distractors:[money(p.priceCents),money(p.priceCents+p.priceCents*p.increaseBps/10_000n+100n),money(p.priceCents*p.increaseBps/10_000n),money(answer+200n)],parameters:p,definition})}
