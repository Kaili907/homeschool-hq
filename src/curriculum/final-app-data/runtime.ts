...
export async function loadFinalFamilyPilotCatalog(
  ...
  const manifestGrades = new Set(manifest.runtime.courses.map((course) => course.grade))
  const manifestLessons = manifest.runtime.courses.reduce(
    (count, course) => count + course.lessonCount,
    0,
  )
  const courseRefs = new Set(manifest.runtime.courses.map((course) => course.courseRef))
  const indexedLessonRefs = manifest.runtime.units.flatMap((unit) => unit.lessonRefs)
  const indexedLessonsByCourse = new Map<string, number>()
  for (const unit of manifest.runtime.units) {
    indexedLessonsByCourse.set(
      unit.courseRef,
      (indexedLessonsByCourse.get(unit.courseRef) ?? 0) + unit.lessonRefs.length,
    )
  }
  if (
    manifest.admissionStatus !== 'ADMITTED' ||
    manifest.classification !== 'ADMITTED_PRODUCTION_BOUND_FAMILY_PILOT_R1' ||
    manifest.counts.grades !== 9 ||
    manifest.counts.courses !== 90 ||
    manifest.counts.units !== 698 ||
    manifest.counts.lessons !== 8292 ||
    manifest.counts.assessments !== 699 ||
    manifest.runtime.courses.length !== 90 ||
    courseRefs.size !== 90 ||
    manifest.runtime.units.length !== 698 ||
    manifestLessons !== 8292 ||
    indexedLessonRefs.length !== 8292 ||
    new Set(indexedLessonRefs).size !== 8292 ||
    manifest.runtime.courses.some(
      (course) => indexedLessonsByCourse.get(course.courseRef) !== course.lessonCount,
    ) ||
    FINAL_CURRICULUM_GRADES.some((grade) => !manifestGrades.has(grade)) ||
    manifest.productionBindings !== 8292 ||
    manifest.assessmentBindings !== 699 ||
    manifest.assessments.length !== 699 ||
    new Set(manifest.assessments.map((item) => item.assessmentRef)).size !== 699 ||
    manifest.assessments.some((item) => !courseRefs.has(item.courseRef))
