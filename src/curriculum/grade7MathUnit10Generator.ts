...
/**
 * An hourly-rate term in canonical algebra notation. A coefficient of one is
 * implicit, so a $1-per-hour rate is written `x`, never `1x`. The swapped-rate
 * distractor below puts the base fee in the coefficient slot, and that fee is
 * drawn from 1 upward, so it is the slot that reaches the unit coefficient.
 */
const term = (coefficient: number) => (coefficient === 1 ? 'x' : `${coefficient}x`)
export function generateProblemFormulationQuestion(difficulty: Difficulty) { const a = ri(2, difficulty + 5); const x = ri(2, difficulty * 4 + 5); const b = ri(1, difficulty * 5 + 6); const c = a * x + b; const answer = `${term(a)} + ${b} = ${c}; x = ${x}`; return q('problem-formulation', difficulty, `A rental company charges a $${b} base fee plus $${a} per hour. A customer's total bill was $${c}. Which equation models this situation, and how many hours were rented?`, answer, { a, b, c, solution: x }, [`${term(a)} + ${b} = ${c}; x = ${x - 1}`, `${term(a)} - ${b} = ${c}; x = ${x + 1}`, `${term(a)} + ${b} = ${c}; x = ${x + 1}`, `${term(b)} + ${a} = ${c}; x = ${x + 2}`]) }
