...
export function generateBudgetBalanceQuestion(difficulty: Difficulty): Grade8FinLitUnit1Question {
  const incomeCents = BigInt(ri(8 * difficulty, 20 * difficulty) * 1_000)
  const needsCents = BigInt(ri(2 * difficulty, 5 * difficulty) * 1_000)
  const savingsCents = BigInt(ri(5 * difficulty, 15 * difficulty) * 100)
