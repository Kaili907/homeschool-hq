...
import { createCatalogProvider } from '../../study/family-pilot/catalog-runtime/provider'
import type {
  CatalogCourse,
  CatalogUnit,
} from '../../study/family-pilot/catalog-runtime/types'
import type { GeneratedLessonRow } from '../../study/family-pilot/catalog-runtime/rows'
import { COURSES, UNITS } from '../../study/family-pilot/catalog-runtime/generated/index'
import GRADE_5_MATHEMATICS from '../../study/family-pilot/catalog-runtime/generated/courses/ma-g5-mathematics'
...
describe('buildBrowserCatalogProjection', () => {
  it('emits the field set the shipped catalog runtime already uses', () => {
    const projection = buildBrowserCatalogProjection(admitted(buildCandidateFixture()))
    expect(Object.keys(projection.courses[0]).sort()).toEqual(Object.keys(COURSES[0]).sort())
    expect(Object.keys(projection.units[0]).sort()).toEqual(Object.keys(UNITS[0]).sort())
    const rows = projection.lessonRowsByCourse['ma-g5-mathematics']
    expect(Object.keys(rows[0]).sort()).toEqual(Object.keys(GRADE_5_MATHEMATICS[0]).sort())
  })

  it('drives the real provider, lazy lesson loading included', async () => {
    ...
    // Grades 5/7/8 are the whole of AcademyGrade today, so this release types
    // against the runtime as-is. Wider canonical grades project identically as
    // data and land in the runtime when that union widens.
    const provider = createCatalogProvider({
      releaseVersion: projection.releaseVersion,
      courses: projection.courses as readonly CatalogCourse[],
      units: projection.units as readonly CatalogUnit[],
      loaders: Object.fromEntries(
        Object.entries(projection.lessonRowsByCourse).map(([courseRef, rows]) => [
          courseRef,
          () => Promise.resolve({ default: rows as readonly GeneratedLessonRow[] }),
        ]),
      ),
    })

    expect(provider.releaseVersion).toBe('2.0.0')
    expect(provider.listGrades()).toEqual(['5', '7', '8'])
    expect(provider.listSubjects('5')).toEqual(['mathematics'])
    expect(provider.listCourses('7').map((course) => course.courseRef)).toEqual(['ma-g7-mathematics'])

    const lessons = await provider.listLessons('ma-g5-mathematics')
    expect(lessons.map((lesson) => lesson.courseDay)).toEqual([1, 2, 3])
    expect(lessons[0].unitRef).toBe('ma-g5-mathematics-u01')

    const lesson = await provider.getLesson('ma-g8-mathematics-u01-l02')
    expect(lesson?.courseRef).toBe('ma-g8-mathematics')
