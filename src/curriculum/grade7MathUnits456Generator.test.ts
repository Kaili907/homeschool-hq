...
function oracle(itemType: string, prompt: string, answer: string): string {
  switch (itemType) {
    ...
    case 'two-step-equation': { const m = match(prompt, /Solve (\d+)x \+ (\d+) = (\d+)\./); return String((Number(m[3]) - Number(m[2])) / Number(m[1])) }
    ...
    case 'solution-set': { const m = match(prompt, /of (\d+)x \+ (\d+) ≥ (\d+)\?/); return Number(answer) >= (Number(m[3]) - Number(m[2])) / Number(m[1]) ? answer : 'INVALID' }
    ...
    case 'scale-drawing': { const m = match(prompt, /1 cm : (\d+) m\. What actual distance does (\d+) cm/); return `${Number(m[1]) * Number(m[2])} m` }
    ...
    case 'unique-triangle-conditions': return 'three side lengths'
...
describe('Grade 7 Math Units 4-6 source contracts and prompt-only property oracles', () => {
  ...
  for (const unit of units) {
    ...
    for (const [index, itemType] of unit.types.entries()) it(`${unit.name} ${itemType}: 200 prompt-parsed items per difficulty`, () => { setRng(seededRng(0x740_000 + index + units.indexOf(unit) * 100)); for (const difficulty of [1, 2, 3] as const) for (let run = 0; run < RUNS_PER_DIFFICULTY; run++) { const question = (unit.generate as (type: string, d: 1 | 2 | 3) => any)(itemType, difficulty); assertWellFormed(question); expect(curriculumAnswer(question)).toBe(oracle(question.itemType, question.prompt, curriculumAnswer(question))) } })
  }
