 ...
 * Those two digest values changed because the choice sets those two item types
 * generate changed. That is the whole of the intended change, but it does not mean
 * the prompts recorded under these two digests are byte-identical to the old ones,
 * and this baseline cannot be read as claiming they are: each digest replays 60
 * items per difficulty from a single seeded stream, and `finishDistinctChoices`
 * shuffles the deduplicated distractor pool, so a pool that deduplicates to a
 * different size consumes a different number of random values and realigns the
 * stream for every item drawn after it. The generator's prompt and correct-answer
 * logic is invariant under this fix, and that is established independently of this
 ...
 * so rendered `1x + 2 = 11; x = 7` whenever that fee was $1.
 *
 * Nine of the 48 digests have therefore moved: the six `u3:` values, two `u4:`
 * values and one `u10:` value. Unit 7's canonical-rendering correction moved none
 * of them, and could not have -- `UNITS` covers Units 1-6, 9 and 10, so Unit 7 has
 * no row here at all, and its own invariance is pinned by
 * `grade7MathUnit7Generator.test.ts` instead. The remaining 39 digests are the
 * originals, and they are what proves no other Grade 7 unit moved with any of
 * these corrections.
...
describe('Grade 7 generated questions move only where Units 3, 4 and 10 were corrected', () => {
