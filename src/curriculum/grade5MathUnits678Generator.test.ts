...
function oracle8(q:Grade5MathUnit8Question):string {const p=q.prompt
 ...
 if(q.itemType==='line-plot-fraction-data'){const m=p.match(/has (\d+) (marks?) at 1\/4 meter, (\d+) (marks?) at 1\/2 meter, and (\d+) (marks?) at 3\/4/)!
  // The noun must agree with its own count: "1 mark", "2 marks".
  for(const [count,noun] of [[m[1],m[2]],[m[3],m[4]],[m[5],m[6]]])expect(noun).toBe(count==='1'?'mark':'marks')
  return `${rational(BigInt(m[1])+2n*BigInt(m[3])+3n*BigInt(m[5]),4n)} meters`}
