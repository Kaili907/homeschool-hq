...
import { GRADE7_MATH_UNIT10_GENERATORS, GRADE7_MATH_UNIT10_ITEM_DEFINITIONS, GRADE7_MATH_UNIT10_ITEM_TYPES, generateGrade7MathUnit10Question } from './grade7MathUnit10Generator'
...
describe('Grade 7 Math Unit 10 generator contract', () => {
  it('registers every source-derived focus', () => {
    expect(Object.keys(GRADE7_MATH_UNIT10_ITEM_DEFINITIONS)).toEqual([...GRADE7_MATH_UNIT10_ITEM_TYPES])
    expect(Object.keys(GRADE7_MATH_UNIT10_GENERATORS)).toEqual([...GRADE7_MATH_UNIT10_ITEM_TYPES])
  })

  it('produces 20 desk samples with valid choices', () => {
    setRng(seededRng(0xa00_000))
    for (let index = 0; index < 20; index++) {
      const type = GRADE7_MATH_UNIT10_ITEM_TYPES[index % GRADE7_MATH_UNIT10_ITEM_TYPES.length]
      const question = generateGrade7MathUnit10Question(type, ([1, 2, 3] as const)[index % 3])
      assertUsable(question)
      expect(question.itemType).toBe(type)
      expect(question.standard).toBe(GRADE7_MATH_UNIT10_ITEM_DEFINITIONS[type].standard)
      expect(question.lessonFocus).toBe(GRADE7_MATH_UNIT10_ITEM_DEFINITIONS[type].lessonFocus)
    }
  })

  it('makes each item reachable at every difficulty', () => {
    setRng(seededRng(0xa00_900))
    for (const type of GRADE7_MATH_UNIT10_ITEM_TYPES) for (const difficulty of [1, 2, 3] as const) assertUsable(generateGrade7MathUnit10Question(type, difficulty))
  })
