...
/**
 * Unit 4 distractor format policy.
 *
 * Every answer this unit asks a student to produce is an exact integer, an exact
 * simplest-form fraction, a whole-dollar amount or an inequality statement, so a
 * distractor is only ever rendered from an exact value of that same kind. A raw
 * JavaScript quotient such as 11.333333333333334 is not a number any student
 * writes down, and offering it teaches nothing about the mistake it came from,
 * so a "divided too early" error becomes a choice only when the division comes
 * out exact (`exactQuotient`); otherwise that slot falls through to the next
 * candidate in the pool. Fractions are rendered in simplest form because the
 * item asking for them says so, and integers stay integers. Nothing here changes
 * how any other unit formats its numbers.
 */
const rationalText = (numerator: number, denominator: number) => { const divisor = gcd(numerator, denominator) || 1; const n = numerator / divisor, d = denominator / divisor; return d === 1 ? integerText(n) : `${n}/${d}` }
const exactQuotient = (numerator: number, divisor: number) => Number.isInteger(numerator / divisor) ? [numerator / divisor] : []
/**
 * Takes the first four candidates that are distinct from each other and from the
 * correct answer, so the choice builder is never handed a collapsed pool and never
 * has to retry. Order is priority: the conceptual errors come first and the
 * near-miss arithmetic slips at the tail only fill slots the earlier ones lost to
 * a collision. Every caller ends its pool with four distinct offsets of the answer,
 * which is what makes four distinct distractors reachable on every draw.
 */
const firstDistinct = (answer: string, candidates: readonly string[]) => { const seen = new Set([answer]); const kept: string[] = []; for (const candidate of candidates) { if (kept.length === 4) break; if (seen.has(candidate)) continue; seen.add(candidate); kept.push(candidate) } return kept }
...
export function generateTwoStepEquationQuestion(difficulty: Difficulty) { if (difficulty === 3) { const a = pick([-6, -4, 4, 6] as const); const denominator = Math.abs(a) === 4 ? pick([2, 4] as const) : pick([2, 3] as const); const numerator = denominator === 3 ? pick([-7, -5, -4, -2, 1, 2, 4, 5, 7] as const) : denominator === 4 ? pick([-7, -5, -3, 1, 3, 5, 7] as const) : pick([-7, -5, -3, 1, 5, 7] as const); const b = pick([-12, -6, 3, 9] as const); const c = b + a / denominator * numerator; const answer = `${numerator}/${denominator}`; return q('two-step-equation', difficulty, `Solve ${a}x ${signedTerm(b)} = ${c}. Write the answer in simplest form.`, answer, { a, b, c, solution: numerator, denominator }, firstDistinct(answer, [rationalText(numerator + 2, denominator), rationalText(numerator - 2, denominator), integerText(c - b), rationalText(numerator, denominator === 2 ? 3 : 2), rationalText(numerator + 4, denominator), rationalText(numerator - 4, denominator)])) } const p = facts(difficulty); const b = difficulty === 2 ? -ri(1, 14) : p.b; const solution = difficulty === 2 ? -ri(1, 10) : p.x; const c = p.a * solution + b; return q('two-step-equation', difficulty, `Solve ${p.a}x ${signedTerm(b)} = ${c}.`, String(solution), { ...p, b, c, solution }, firstDistinct(String(solution), [...exactQuotient(c, p.a).map(quotient => integerText(quotient - b)), integerText(c - b), integerText(-solution), integerText(solution + 1), integerText(solution - 1), integerText(solution + 2), integerText(solution - 2)])) }
...
export function generateCheckAndInterpretSolutionQuestion(difficulty: Difficulty) { const p = facts(difficulty); return q('check-and-interpret-solution', difficulty, `A club charges $${p.b} plus $${p.a} per session for a total of $${p.c}. How many sessions were purchased?`, String(p.x), { ...p, solution: p.x }, firstDistinct(String(p.x), [...exactQuotient(p.c, p.a).map(integerText), integerText(p.c - p.b), integerText(p.x + 1), integerText(p.x - 1), integerText(p.x + 2), integerText(p.x + 3)])) }
