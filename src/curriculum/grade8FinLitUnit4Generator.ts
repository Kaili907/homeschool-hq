...
const definition={standard:'FL.8.C',lessonFocus:'computing neutral credit cost',workedExample:{prompt:'A $300.00 balance has a 12% annual rate. What is one month of simple interest?',answer:'$3.00',steps:['Monthly rate is 12% ÷ 12 = 1%.','One percent of $300.00 is $3.00.']}} as const
export function generateCreditInterestQuestion(difficulty:Difficulty):Grade8FinLitUnit4Question {const balanceCents=BigInt(ri(200,900)*100),annualRateBps=BigInt(ri(6,24)*100),answer=roundHalfUp(balanceCents*annualRateBps,120_000n);return makeFinLitQuestion({itemType:'credit-interest',difficulty,prompt:`A loan balance is ${money(balanceCents)} with a nominal annual rate of ${(Number(annualRateBps)/100).toFixed(2)}%. What is one month's interest, calculated as balance × annual rate ÷ 12 and rounded to the nearest cent (half-cent up)?`,correctAnswer:money(answer),distractors:[money(answer+100n),money(answer-100n),money(roundHalfUp(balanceCents*annualRateBps,10_000n)),money(balanceCents-answer)],parameters:{balanceCents,annualRateBps},definition})}
