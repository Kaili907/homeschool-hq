...
export function createCurriculumDraftAuthoringHttpSource(
  ...
  return Object.freeze({
    ...
    readPreview: async (draftId: string, revision: number) => {
      const value = await request<unknown>(`${basePath}/${encodeURIComponent(draftId)}/preview/${revision}`)
...
const PREVIEW_FIELD_ROOTS = new Set([
  ...
  'formative_check', 'scoring_guidance', 'mastery', 'tutor_routes', 'extension_activity',
  'accessibility', 'safety_privacy', 'resource_refs', 'guardian_visibility_note', 'home_connection',
