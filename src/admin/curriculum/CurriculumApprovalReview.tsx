...
export function CurriculumApprovalReview({
  ...
  const approvalIdentityCurrent = approval?.draftId === draftId && approval.draftRevision === draftRevision
  const currentValidation = validation?.draftId === draftId && validation.draftRevision === draftRevision ? validation : null
  const validationSummary = currentValidation?.validationSnapshot
    ?? (approvalIdentityCurrent ? approval.latestValidation : null)
  const blockingFindings = currentValidation?.run.findings.filter((finding) => finding.blocking) ?? []
  const status = approval && !approvalIdentityCurrent ? 'stale' : approval?.status ?? 'pending_review'
  const approvalAllowed = canApprove
    && !hasUnsavedChanges
    && approval !== null
    && approvalIdentityCurrent
  ...
  const changesAllowed = canApprove && !hasUnsavedChanges && approval !== null && approvalIdentityCurrent && status !== 'changes_requested'
  ...
  return (
    <section className="curriculum-approval-review" aria-labelledby="curriculum-approval-title">
      ...
      {approval && !approvalIdentityCurrent && (
        <div className="curriculum-approval-stale" role="alert">
          <strong>Approval evidence is stale.</strong> The loaded decision belongs to revision {approval.draftRevision}; this workspace is revision {draftRevision}.
        </div>
      )}
