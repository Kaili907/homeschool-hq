 ...
 * fixtures deliberately carry legacy credential-shaped material.
 */
...
/** A state whose p1 carries legacy PIN material and recognisable school data. */
...
describe('the cloud mutation payload carries no learner credential', () => {
  ...
  it('the captured RPC row carries school data but never the learner pin', async () => {
    ...
    expect(sent.p_profiles[0].data).not.toHaveProperty('pin')
