...
import { consumeParentCredentialAuthorizationEvidence } from './parentAuthorizationEvidence'
...
export async function markParentCredentialResetRequiredAuthorized(
  ...
  return withParentCredentialMutationLock(binding, options, async (lockedBinding) => {
    ...
    const tombstone = await createCredentialRecord(
      lockedBinding,
      nextGeneration(authorityBefore),
      'reset-required',
      null,
    ...
    await consumeParentCredentialAuthorizationEvidence(
      authorization,
      'parent-pin:reset-required',
      reference,
      authorityBefore.generation,
      await recordCommitment(tombstone, options),
      options,
    )
    if (previous) {
      await requireStableSnapshot(
        ...
        previous,
        options,
        'Parent credential changed while recovery evidence was being consumed.',
      )
    }
    const authority = await readGeneration(reference, options)
    if (!parentGenerationSnapshotsEqual(authorityBefore, authority)) {
      throw new ParentCredentialVaultError(
        'generation-conflict',
        'Parent credential authority changed while recovery evidence was being consumed.',
      )
    }
...
export async function recoverParentPinAuthorized(
  ...
  return withParentCredentialMutationLock(binding, options, async (lockedBinding) => {
    ...
    await consumeParentCredentialAuthorizationEvidence(
      ...
      await recordCommitment(replacement, options),
      options,
    )
    await requireStableSnapshot(
      lockedBinding,
      previous,
      options,
      'Parent credential changed while its recovery replacement and evidence were being verified.',
    )
    return mutationResult(await reserveAndPersistActiveRecord(
      lockedBinding,
      previous.authority,
