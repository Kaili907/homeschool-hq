...
context. Reset and recovery do not accept a caller verification boolean. They
require issuer-signed evidence loaded from a rollback-resistant authority,
bound to the exact operation, installation, household, prior generation, and
validity window. The evidence is atomically consumed once and read back with
the exact proposed credential-record commitment before credential authority
may advance. Missing, forged, stale, replayed, partially persisted, or
wrong-binding evidence fails closed. Neither API creates a recovery PIN or
elevates Parent authority into installation-manager, Study, Admin, learner, or
hosted authentication authority.
