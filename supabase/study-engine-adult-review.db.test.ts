...
const G1_MIGRATION = './migrations/20260810152000_academy_study_in_app_receipt_timestamp.sql'
const C2_MIGRATION = './migrations/20260810152100_academy_study_worker_operations_contract.sql'

// Natural migration version order. 20260810152000 (receipt timestamp
// normalization) sorts before 20260810152100 (worker operations contract), so a
...
describe.sequential('STUDY-C2 v2 operations contract', () => {
  ...
  //
  // The 20260810152000 correction is below 20260810152100 in the same
...
//
// The operations migration originally asserted its predecessor state with an exact
...
// broke: the timestamp correction sorts first, applies first, appends its own name,
...
describe.sequential('STUDY-C2 and G1 migration order', () => {
  ...
  it('records both markers when the natural order is applied', async () => {
    ...
    try {
      ...
      expect(row.names).toContain('20260810152000_academy_study_in_app_receipt_timestamp')
      expect(row.names).toContain('20260810152100_academy_study_worker_operations_contract')
      // G1 first, C2 second — the order the names were appended.
      expect(row.names.indexOf('20260810152000_academy_study_in_app_receipt_timestamp'))
        .toBeLessThan(row.names.indexOf('20260810152100_academy_study_worker_operations_contract'))
  ...
  it('fails closed on every incomplete or unknown predecessor marker state', async () => {
    ...
    const mutations = [
      ...
      },
      {
        ...
        sql: `update academy_private.study_persistence_metadata
              set migration_names = array_remove(
                migration_names, '20260810152000_academy_study_in_app_receipt_timestamp')
              where singleton`,
      },
      {
  ...
  it('asserts predecessors by containment, never by exact array equality', async () => {
    ...
    expect(source).toContain('20260810152000_academy_study_in_app_receipt_timestamp')
