...
test('lesson Tutor saves an unlimited text conversation for Parent-only review', async ({ page }) => {
  await setupFamily(page, [{ name: 'Tutor Learner', grade: '5', subjects: ['Mathematics'] }])
  await page.getByRole('button', { name: 'Assignments', exact: true }).click()
  await page.getByRole('button', { name: 'Assign lesson', exact: true }).first().click()
  await page.getByRole('button', { name: 'Student', exact: true }).click()
  await page.getByRole('listitem', { name: /Continue as Tutor Learner/ }).click()
  await page.getByRole('button', { name: 'Continue', exact: true }).click()
  await page.getByRole('button', { name: /Start lesson|Continue lesson/ }).click()
  await page.getByRole('button', { name: 'Ask the Tutor for help' }).click()

  const tutor = page.getByTestId('family-pilot-tutor')
  await expect(tutor).toContainText('Ask questions for as long as you need')
  await tutor.getByLabel('Your question', { exact: true }).fill('Why do grouping symbols matter?')
  await tutor.getByRole('button', { name: 'Send', exact: true }).click()
  await expect(tutor).toContainText('Ask a parent to open Parent Hub → Tutor chats')

  await page.getByRole('button', { name: 'Save and exit' }).click()
  await page.getByRole('button', { name: 'Lock', exact: true }).click()
  await page.getByRole('button', { name: 'Parent', exact: true }).click()
  await page.getByLabel('Unlock parent PIN').fill(PARENT_PIN)
  await page.getByRole('button', { name: 'Unlock Parent Hub' }).click()
  await page.getByRole('button', { name: 'Tutor chats', exact: true }).click()
  const transcript = page.locator('details').filter({ hasText: 'Launch and diagnostic: problem-solving routines' })
  await transcript.locator('summary').click()
  await expect(transcript).toContainText('Why do grouping symbols matter?')
  await expect(transcript).toContainText('Academy')
})
...
test('complete family workflow survives a real browser-process reopen and stays isolated', async ({}, testInfo) => {
  ...
  try {
    ...
    const bCard = overview.locator(`[data-learner-ref="${bRef}"]`)
    const cCard = overview.locator(`[data-learner-ref="${cRef}"]`)
    await expect(aCard).toContainText('Avery Synthetic')
    await expect(aCard).toContainText(LESSON.a.title)
    await expect(aCard).not.toContainText(LESSON.b.title)
    await expect(bCard).toContainText('Blake Synthetic')
    await expect(bCard).toContainText(LESSON.b.title)
    await expect(bCard).not.toContainText(LESSON.a.title)
    await expect(cCard).toContainText('Casey Synthetic')
    await expect(cCard).toContainText(LESSON.c.title)
    ...
    await expect(page.getByText(/^(?:Part|Step) 1 of 3$/, { exact: true }).first()).toBeVisible()
    await page.getByRole('button', { name: 'Ask the Tutor for help' }).click()
    await expect(page.getByTestId('family-pilot-tutor')).toContainText('Ask questions for as long as you need')
    await expect(page.getByTestId('family-pilot-tutor').getByLabel('Your question', { exact: true })).toBeVisible()
    ...
    const refreshedBlake = refreshedOverview.locator(`[data-learner-ref="${bRef}"]`)
    const refreshedCasey = refreshedOverview.locator(`[data-learner-ref="${cRef}"]`)
    await expect(refreshedAvery.locator('dt').filter({ hasText: /^Completed$/ }).locator('..')).toContainText('2')
    await expect(refreshedAvery.locator('dt').filter({ hasText: /^Remaining$/ }).locator('..')).toContainText('1')
    await expect(refreshedBlake.getByText('Done for today', { exact: true })).toBeVisible()
