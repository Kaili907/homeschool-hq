...
describe('hosted Study sync R2 — 37 scenarios through the converged adapter', () => {
  ...
  it('35. keeps hosted activation default-off after installing the privacy serializer', async () => { const h = await linked(); await h.hydrate(); expect(HOSTED_SYNC_PRODUCTION_ACTIVATION).toEqual({ enabled: false, reason: 'HOSTED_SYNC_R2_DEFAULT_OFF' }); expect(requireHostedSyncProductionPrivacySerializer()).toBe(true) })
