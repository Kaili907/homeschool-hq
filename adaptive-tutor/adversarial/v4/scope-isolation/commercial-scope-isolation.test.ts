...
function successfulInput(): {
  readonly input: CommercialTutorExecutionInput;
  readonly transport: ScriptedCommercialTransport;
} {
...
function expectContained(
  ...
  transport: ScriptedCommercialTransport,
): void {
  const result = executeCommercialTutorInvocation(input);
  assert.equal(
    transport.requests.length,
    0,
    `${label}: foreign binding reached provider transport with status ${result.status}`,
  );
  assert.equal(
    result.providerCalls,
    0,
    `${label}: foreign binding was charged as a provider call`,
  );
  assert.equal(
    result.telemetry.length,
    0,
    `${label}: foreign binding produced attributable telemetry`,
  );
}
...
test("deterministic combinations of household, learner, session, and interaction substitutions fail closed", async (t) => {
  ...
  for (let mask = 1; mask < 1 << dimensions.length; mask += 1) {
    ...
    await t.test(selected.map(([label]) => label).join("+"), () => {
      const { input, transport } = successfulInput();
      const invocation = clone(input.invocation) as MutableRecord;
      for (const [, key, value] of selected) invocation[key] = value;
      expectContained(
...
test("A session plus a fully valid sibling curriculum cannot influence a provider request", () => {
  ...
  const metadata = clone(input.curriculumMetadata) as MutableRecord;
  ...
  const capability = clone(input.capabilityDeclaration) as MutableRecord;
  ...
  expectContained("A session + B curriculum", {
    ...
    capabilityDeclaration: capability as CommercialTutorExecutionInput["capabilityDeclaration"],
  }, transport);
});
...
test("sibling physical-attempt refs cannot receive A's cost or telemetry attribution", async (t) => {
  const run = () => {
    ...
    assert.equal(result.status, "advisory");
    if (result.status !== "advisory") throw new Error("mixed-scope fixture did not execute");
    return result;
  };
...
test("a sibling concept or opportunity cannot influence A", async (t) => {
  ...
  for (const [label, key, value] of mutations) {
    await t.test(label, () => {
      const { input, transport } = successfulInput();
      const invocation = clone(input.invocation) as MutableRecord;
      (invocation.curriculum as MutableRecord)[key] = value;
...
test("a valid sibling learner-stage binding cannot select A's provider route", () => {
  ...
  const siblingStageModels = input.routing.modelProfiles.map((profile) => ({
    ...profile,
    learnerStages: ["SECONDARY" as const],
  }));
...
test("a stale prior-session grounding scope cannot reach transport", () => {
  const { input, transport } = successfulInput();
  const invocation = clone(input.invocation) as MutableRecord;
  const groundedContext = invocation.groundedContext as MutableRecord;
  groundedContext.scopeRef = "interaction:prior-session";
  groundedContext.items = (groundedContext.items as MutableRecord[]).map((item) => ({
    ...item,
    scopeRef: "interaction:prior-session",
  }));
  invocation.groundingRequirements = (
    invocation.groundingRequirements as MutableRecord[]
  ).map((requirement) => ({ ...requirement, scopeRef: "interaction:prior-session" }));
...
test("sibling reviewed presentation refs cannot influence A", () => {
  const { input, transport } = successfulInput();
  const invocation = clone(input.invocation) as MutableRecord;
  const requestedPresentation = invocation.requestedPresentation as MutableRecord;
...
test("foreign concept, opportunity, stage, and presentation refs never appear in A advisory/request", async (t) => {
  await t.test("provider request concept", () => {
    const { input, transport } = successfulInput();
    const invocation = clone(input.invocation) as MutableRecord;
    (invocation.curriculum as MutableRecord).conceptRef = "concept:sibling-valid";
    executeCommercialTutorInvocation({ ...input, invocation });
    const request = transport.requests[0] as { academicScope?: { conceptRef?: string } } | undefined;
  ...
  await t.test("advisory opportunity", () => {
    const { input } = successfulInput();
    const invocation = clone(input.invocation) as MutableRecord;
    (invocation.curriculum as MutableRecord).opportunityRef = "opportunity:sibling-valid";
  ...
  await t.test("advisory fallback presentation", () => {
    ...
    if (result.status === "advisory") {
      assert.notEqual(
        result.advisory.presentationIntent?.fallbackPresentation?.presentationRef,
        "presentation-fallback:sibling-valid",
      );
    }
  });
});
