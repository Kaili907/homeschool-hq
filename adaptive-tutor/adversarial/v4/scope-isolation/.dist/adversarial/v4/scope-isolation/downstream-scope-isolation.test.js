...
import { StudyCommercialEffectReceiptSchema, } from "../../../core/v3/contracts/index.js";
import { InstructionalMemoryProjectionStore, InstructionalMemoryScopeSchema, createInstructionalMemoryDelta, } from "../../../core/v3/memory/index.js";
import { MinimizedAcceptedStudyEffectEventSchema, RecoverableInstructionalOperationCoordinator, } from "../../../core/v3/recovery/index.js";
import { TrustedPresentationAcceptanceSchema, mapTrustedAcceptedIntentToW306PresentationPieces, } from "../../../core/v3/presentation/index.js";
...
import { executionInput, } from "../../../tests/tutor-v3-convergence/fixtures.js";
const DIGEST = `sha256:${"e".repeat(64)}`;
function clone(value) {
    return structuredClone(value);
}
function schemaCarriesScopeField(properties, field) {
    if (Object.hasOwn(properties, field))
        return true;
    const nestedScope = properties.scope;
    if (typeof nestedScope !== "object" || nestedScope === null)
        return false;
    const nestedProperties = nestedScope.properties;
    return typeof nestedProperties === "object" && nestedProperties !== null &&
        Object.hasOwn(nestedProperties, field);
}
function baselineAdvisory() {
    const result = executeCommercialTutorInvocation(executionInput());
    assert.equal(result.status, "advisory");
    if (result.status !== "advisory")
        throw new Error("commercial fixture did not produce advisory");
    return result.advisory;
}
function effectReceipt(advisory) {
    return {
...
function memoryDelta(receipt, conceptRef = "concept:fractions-one") {
...
class InMemoryAcceptedEffectGateway {
    accepted = null;
    lookup(_identity) {
    ...
    accept(command) {
        ...
        return { status: "accepted", event: clone(command.acceptedEvent) };
    }
}
...
test("A memory scope plus B accepted-effect scope is rejected without mutation", () => {
    ...
    const receipt = effectReceipt(advisory);
    const delta = memoryDelta(receipt);
    const foreignDelta = clone(delta);
    foreignDelta.scope.learnerScopeRef = "learner-scope:learner-b";
    const { coordinator, memory } = recoveryHarness();
...
test("accepted commercial effect cannot inject a sibling concept into A memory", () => {
    ...
    const { coordinator, memory } = recoveryHarness();
    const result = recoverAcceptedCommercialEffect(receipt, advisory, delta, coordinator);
    assert.notEqual(result.status, "complete", "foreign concept was accepted into learner A instructional memory");
...
test("household scope is carried across effect receipt, memory, and accepted-event contracts", () => {
    const effectProperties = StudyCommercialEffectReceiptSchema.properties;
    const memoryProperties = InstructionalMemoryScopeSchema.properties;
    const acceptedProperties = MinimizedAcceptedStudyEffectEventSchema.properties;
    const contracts = [
    ...
    const missing = contracts.filter(([, properties]) => !schemaCarriesScopeField(properties, "householdScopeRef"))
        .map(([name]) => name);
...
test("telemetry carries enough trusted scope to prevent cross-child attribution", () => {
    const properties = TutorCommercialTelemetryEventSchema.properties;
    const required = [
        "householdScopeRef",
        "learnerScopeRef",
        "sessionRef",
...
test("presentation acceptance is scope-bound before sibling refs can be rendered", () => {
    const properties = TrustedPresentationAcceptanceSchema.properties;
    const required = [
        "householdScopeRef",
        "learnerScopeRef",
        "sessionRef",
...
test("scope-free sibling presentation acceptance is rejected", () => {
    ...
    assert.notEqual(mapTrustedAcceptedIntentToW306PresentationPieces(foreignAcceptance).status, "accepted", "scope-free sibling presentation acceptance was rendered");
});
...
test("guardian, household, learner, session, and evidence substitutions cannot produce a Parent report", async (t) => {
    const attacks = [
        ["A guardian + B learner authorization", (request) => {
                request.guardianAuthorization.learnerScopeRef = "learner-scope:learner-b";
            }],
        ["foreign guardian", (request) => {
                request.guardianAuthorization.guardianRef = "guardian:family-two";
            }],
        ["foreign household", (request) => {
                request.guardianAuthorization.householdScopeRef = "household-scope:family-two";
            }],
        ["same IDs under foreign request household", (request) => {
                request.scope.householdScopeRef = "household-scope:family-two";
            }],
        ["stale session authorization", (request) => {
                request.guardianAuthorization.sessionRef = "session:prior";
            }],
        ["foreign evidence learner", (request) => {
                request.evidence[0].learnerRef = "learner-scope:learner-b";
            }],
        ["foreign evidence household", (request) => {
                const evidence = request.evidence[0];
                const provenance = evidence.provenance;
                provenance.scope.householdScopeRef = "household-scope:family-two";
            }],
        ["foreign evidence session", (request) => {
                const evidence = request.evidence[0];
                const provenance = evidence.provenance;
                provenance.scope.sessionRef = "session:prior";
            }],
    ];
