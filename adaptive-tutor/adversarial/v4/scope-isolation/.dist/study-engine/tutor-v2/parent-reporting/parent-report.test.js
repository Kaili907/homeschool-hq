...
import { PARENT_REPORT_DISCLAIMER, PARENT_REPORT_VERSION, ParentReportConsentSchema, ParentReportGuardianAuthorizationSchema, ParentReportRequestSchema, ParentReportResultSchema, REVIEWED_PARENT_REPORT_COPY, buildMinimizedParentHubReport as buildReportWithTrustedAuthority, } from "./parent-report.js";
...
function trustedTransition(decisionStatus, reasonCode, sourceEventRef) {
...
function trustedAuthorityFor(value) {
    const request = value;
    const authorization = typeof request?.guardianAuthorization === "object" &&
        request.guardianAuthorization !== null
        ? structuredClone(request.guardianAuthorization)
        : sessionAuthorization();
    const consent = authorization
        .consent;
    return {
        ...
        policy: {
            ...
            currentPolicyRevisionRef: request?.policyRevisionRef ??
                "policy-revision:parent-reporting-v1-r4",
            policyRevisionStatus: "current",
            policyIssuedEventRef: "event:parent-reporting-policy-r4",
            consentRequirement: consent?.policyRequirement === "required" ? "required" : "not-required",
        },
        ...
        evidenceReceipts: Array.isArray(request?.evidence)
            ? request.evidence.map((candidate) => {
                ...
                return {
                    ...
                    transition: trustedTransition(evidence.decisionStatus, evidence.reasonCode, evidence.provenance.sourceEventRef),
                };
            })
            : [],
    };
}
function buildMinimizedParentHubReport(value, trustedAuthority = trustedAuthorityFor(value)) {
