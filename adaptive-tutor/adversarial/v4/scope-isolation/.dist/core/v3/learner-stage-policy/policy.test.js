...
import { createLearnerStagePolicyRegistry, } from "./index.js";
const fallback = {
...
function createRegistry(profiles = [boundedProfile, transitionalProfile, expansiveProfile]) {
...
test("break suggestions obey disabled, early, cooldown, count, and eligible states", () => {
    const disabledProfile = structuredClone(boundedProfile);
...
test("unknown, malformed, and mismatched stage bindings use reviewed static fallback", () => {
    ...
    if (unknown.status !== "static-fallback" ||
...
test("rejects incoherent or duplicate profiles and an invalid fallback", () => {
    const duplicate = createLearnerStagePolicyRegistry([boundedProfile, structuredClone(boundedProfile)], fallback);
    assert.deepEqual(duplicate, {
        status: "rejected",
        code: "DUPLICATE_LEARNER_STAGE_POLICY_PROFILE",
        profileIndex: 1,
    });
    const incoherent = structuredClone(boundedProfile);
    ...
    const visualMismatch = structuredClone(boundedProfile);
    ...
    assert.deepEqual(createLearnerStagePolicyRegistry([boundedProfile], {
        ...fallback,
