...
import { DurableMultimodalEvidenceSchema, MULTIMODAL_CONTRACT_VERSION, MultimodalPresentationSchema, } from "./contracts.js";
function reviewedVisualFrom(source) {
 ...
 */
export function projectDurableMultimodalEvidence(source) {
    if (!Value.Check(MultimodalPresentationSchema, source.presentation)) {
        return {
            status: "rejected",
            code: "INVALID_MULTIMODAL_EVIDENCE_SOURCE",
            issues: [...Value.Errors(MultimodalPresentationSchema, source.presentation)].map((error) => `${error.path.length > 0 ? error.path : "$"}: ${error.message}`),
        };
    }
    ...
    if (!Value.Check(DurableMultimodalEvidenceSchema, candidate)) {
        return {
            ...
            issues: [...Value.Errors(DurableMultimodalEvidenceSchema, candidate)].map((error) => `${error.path.length > 0 ? error.path : "$"}: ${error.message}`),
        };
    }
