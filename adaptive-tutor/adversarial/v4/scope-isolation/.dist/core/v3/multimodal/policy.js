import { MultimodalPresentationSchema, TrustedMultimodalPolicyContextSchema, } from "./contracts.js";
...
function advisoryMatchesPresentation(presentation, advisory, commercialScope) {
...
function trustedVisualMatches(visual, context) {
    return context.reviewedVisualBindings.some((binding) => sameScope(binding.scope, context.scope)
 ...
 */
export function enforceMultimodalPresentationPolicy(candidate, trustedContextCandidate) {
    const presentationValidation = validateExactSnapshot(MultimodalPresentationSchema, candidate);
    if (presentationValidation.status === "rejected") {
        return {
            status: "rejected",
            code: "INVALID_MULTIMODAL_PRESENTATION",
            issues: presentationValidation.issues.map((issue) => `${issue.path.length > 0 ? issue.path : "$"}: ${issue.message}`),
        };
    }
    const contextValidation = validateExactSnapshot(TrustedMultimodalPolicyContextSchema, trustedContextCandidate);
    ...
    if (!sameScope(presentation.scope, context.scope)
    ...
    if (caption.captionRef !== captionBinding.captionRef
    ...
    if (!advisoryMatchesPresentation(presentation, context.studyAdvisory, context.commercialExecutionScope)) {
        return {
            status: "rejected",
            code: presentation.assessmentDisclosure.phase === "active-assessment"
                ? "ACTIVE_ASSESSMENT_ANSWER_BLOCKED"
