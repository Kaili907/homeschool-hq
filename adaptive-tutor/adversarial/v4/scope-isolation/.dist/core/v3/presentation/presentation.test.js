...
import { COMMERCIAL_RESPONSE_CONTRACT_VERSION, PRESENTATION_CONTRACT_VERSION, constrainPresentationByLearnerStageAllowance, mapTrustedAcceptedIntentToW306PresentationPieces, mapValidatedModelOutputToCommercialResponse, validateCommercialModelResponse, validatePresentationIntent, } from "./index.js";
...
function acceptedProposal(instructionalDisplayMode, reviewedContentRefs) {
    const context = {
...
function mappedProposal(displayMode, refs, context = mappingContext) {
    const result = mapValidatedModelOutputToCommercialResponse(acceptedProposal(displayMode, refs), context);
...
test("preserves distinct text and visual pieces for reviewed-text-and-visual", () => {
    ...
    if (result.status !== "accepted")
        return;
    assert.deepEqual(result.presentation.pieces.map((piece) => piece.pieceKind), [
    ...
    assert.deepEqual(mapValidatedModelOutputToCommercialResponse(acceptedProposal("reviewed-text", [textRef, visualRef]), mappingContext), { status: "rejected", code: "DISPLAY_CONTENT_ARITY_MISMATCH" });
});
...
test("caption remains accessibility metadata and cannot become instructional text", () => {
    ...
    if (result.status !== "accepted")
        return;
    const caption = result.presentation.pieces.find((piece) => piece.pieceKind === "accessibility-caption-metadata");
    ...
    assert.equal(result.presentation.pieces.some((piece) => piece.pieceKind === "reviewed-text-reference"), false);
...
test("speech is emitted only as a post-acceptance delivery piece", () => {
    ...
    const beforeAcceptance = mapTrustedAcceptedIntentToW306PresentationPieces(response.presentationIntent);
...
test("resolved learner-stage allowance can deny but cannot infer modality or content", () => {
    const response = mappedProposal("reviewed-visual", [visualRef]);
    const denied = constrainPresentationByLearnerStageAllowance(response.presentationIntent, { allowedModalities: ["text"], maximumModalitiesPerResponse: 1 });
...
test("routing receives only the derived requirement and fails when modality is absent", () => {
    const response = mappedProposal("reviewed-visual", [visualRef]);
    const constrained = constrainPresentationByLearnerStageAllowance(response.presentationIntent, { allowedModalities: ["image"], maximumModalitiesPerResponse: 1 });
    assert.equal(constrained.status, "allowed");
    if (constrained.status !== "allowed")
        return;
...
test("raw audio, raw image bytes, and provider raw media are rejected", () => {
    ...
    assert.equal(mapValidatedModelOutputToCommercialResponse(acceptedProposal("reviewed-text", [textRef]), { reviewedVisuals: [], requestSpeechAfterAcceptance: false, providerRawMedia: true }).status, "rejected");
});
