...
function expectedBaseChannels(intent) {
    const channels = [];
...
function normalizedIntent(intent) {
    ...
    if (normalized.reviewedVisual !== undefined)
        Object.freeze(normalized.reviewedVisual);
...
export function validatePresentationIntent(candidate) {
    ...
    if (speechRequested)
        expected.push("speech-after-acceptance");
...
function visualFor(contentRef, context) {
    return context.reviewedVisuals.find((visual) => visual.contentRef === contentRef);
}
function mappedIntent(proposal, context) {
    ...
    const withSpeech = (channels) => [
        ...channels,
        ...(context.requestSpeechAfterAcceptance
            ? ["speech-after-acceptance"]
            : []),
    ];
    switch (proposal.instructionalDisplayMode) {
        ...
        case "reviewed-visual": {
            ...
            if (visual === undefined || context.requestSpeechAfterAcceptance)
                return null;
            return {
                ...common,
        ...
        case "reviewed-text-and-visual": {
            if (refs.length !== 2 || refs[0] === undefined || refs[1] === undefined)
                return null;
...
export function mapValidatedModelOutputToCommercialResponse(result, contextCandidate) {
    if (result.status === "refused") {
        ...
        const groundingClaim = {
            claimKind: "reference-only-grounding-claim",
            claimScope: "references-only",
            groundingRefs: [],
        };
    ...
    if (result.proposal.instructionalDisplayMode === "reviewed-visual"
        && context.requestSpeechAfterAcceptance) {
        return { status: "rejected", code: "SPEECH_SOURCE_REQUIRED" };
    }
...
function proposalMatchesIntent(response) {
...
export function validateCommercialModelResponse(candidate) {
    ...
    if (!equalStringArrays(response.groundingClaim.groundingRefs, response.validatedOutput.groundingRefs)) {
        return { status: "rejected", code: "INVALID_COMMERCIAL_MODEL_RESPONSE" };
    }
...
function deliveryModalities(intent) {
    const modalities = [];
    if (intent.requestedDeliveryChannels.includes("text"))
        modalities.push("text");
    if (intent.reviewedVisual !== undefined)
        modalities.push(intent.reviewedVisual.kind);
 ...
 */
export function constrainPresentationByLearnerStageAllowance(intentCandidate, allowanceCandidate) {
    const intentValidation = validatePresentationIntent(intentCandidate);
    if (intentValidation.status === "rejected") {
        return { status: "rejected", code: "INVALID_PRESENTATION_INTENT" };
    }
    ...
    if (new Set(allowance.allowedModalities).size !== allowance.allowedModalities.length
    ...
    const denied = modalities.filter((modality) => !allowance.allowedModalities.includes(modality));
    if (denied.length > 0) {
        return {
            status: "denied",
