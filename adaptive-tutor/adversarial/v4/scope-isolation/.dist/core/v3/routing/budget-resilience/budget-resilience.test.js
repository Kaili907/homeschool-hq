...
import { COMMERCIAL_ATTEMPT_USAGE_RECEIPT_VERSION, COMMERCIAL_ROUTE_ATTEMPT_PLAN_VERSION, } from "../../commercial-operation/index.js";
import { BUDGET_RESILIENCE_VERSION, MAX_INTEGER_MICROS, decideFallback, evaluateCircuitBreaker, reserveExecutionBudget, reserveCommercialRouteAttemptPlan, settleBudgetReservation, } from "./index.js";
...
function attempt(attemptIndex, overrides = {}) {
    return {
        ...
        modelRevisionRef: attemptIndex === 0 ? "model-revision:primary-v1" : "model-revision:failover-v1",
        ...
        providerPolicyEvidenceRef: attemptIndex === 0
...
function reserve(budget = executionBudget(), attempts = [attempt(0), attempt(1)]) {
...
function receipt(reservation, attemptIndex, actualCostMicros, overrides = {}) {
...
function latencyBudget(endToEndDeadlineMs = 500) {
    return {
        contractVersion: BUDGET_RESILIENCE_VERSION,
        endToEndDeadlineMs,
...
test("admits the exact deadline boundary and rejects one millisecond over", () => {
    ...
    const over = decideFallback(decisionInput({ latencyBudget: latencyBudget(499) }));
    assert.equal(over.decision, "reviewed-static-fallback");
...
test("provider timeout retains its full reservation before failover", () => {
    ...
    assert.deepEqual(settleBudgetReservation(reservation, [], [reservation.attempts[0]?.physicalAttemptRef]), {
        contractVersion: BUDGET_RESILIENCE_VERSION,
        reservationRef: reservation.reservationRef,
...
test("provider outage fails over only to an equally eligible route", () => {
    ...
    assert.equal(hardConstraintFallback.reason, "failover-not-equally-eligible");
});
...
test("circuit half-open admits one dedicated probe and never a learner probe", () => {
    const open = { state: "open", openedAtMs: 10, reopenAtMs: 110 };
    ...
    assert.equal(learnerCannotReportProbeSuccess.reason, "circuit-half-open-learner-denied");
...
test("cost over reservation becomes an anomaly without fabricated release", () => {
    const reservation = reserve();
    const settlement = settleBudgetReservation(reservation, [receipt(reservation, 0, "101")]);
    ...
    assert.equal(settleBudgetReservation(inconsistent, [receipt(reservation, 0, "100")]), null);
});
test("settles each executed attempt independently and exactly", () => {
    const reservation = reserve();
    const primaryOnly = settleBudgetReservation(reservation, [receipt(reservation, 0, "40")]);
...
test("rejects malformed, mismatched, and duplicate attempt receipts", () => {
    ...
    for (const invalidMoney of ["-1", "1.5", "1e2", "01"]) {
        assert.equal(settleBudgetReservation(reservation, [receipt(reservation, 0, invalidMoney)]), null, invalidMoney);
    }
    assert.equal(settleBudgetReservation(reservation, [receipt(reservation, 0, "10", {
