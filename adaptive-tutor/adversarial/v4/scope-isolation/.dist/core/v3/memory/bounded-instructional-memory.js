...
import { AssistanceLevelSchema, HintLevelSchema, ISODateTimeSchema, OpaqueReferenceSchema, PolicyCodeSchema, } from "../../v2/contracts/primitives.js";
...
const RememberInstructionalMemoryRequestSchema = Type.Object({
    ...
    scope: InstructionalMemoryScopeSchema,
    instructionalState: InstructionalMemoryContentSchema,
    ttlMs: Type.Optional(Type.Integer({ minimum: 1, maximum: MAXIMUM_INSTRUCTIONAL_MEMORY_TTL_MS })),
...
function isSemanticallyValidContent(content) {
    return (hasUniqueItems(content.conceptRefs) &&
        hasUniqueItems(content.reviewedContentRefs) &&
        hasUniqueItems(content.recentActionReasonCodes));
}
function isSemanticallyValidEntry(entry) {
    return (isSemanticallyValidContent(entry.instructionalState) &&
...
export class BoundedInstructionalMemoryStore {
    ...
    constructor(options = {}) {
        ...
        if (!Number.isInteger(maximumEntries) ||
            ...
            throw new RangeError(`maximumEntries must be an integer from 1 to ${MAXIMUM_INSTRUCTIONAL_MEMORY_ENTRIES}`);
        }
        if (!Number.isInteger(defaultTtlMs) ||
            defaultTtlMs < 1 ||
            defaultTtlMs > MAXIMUM_INSTRUCTIONAL_MEMORY_TTL_MS) {
            throw new RangeError(`defaultTtlMs must be an integer from 1 to ${MAXIMUM_INSTRUCTIONAL_MEMORY_TTL_MS}`);
        }
    ...
    remember(requestCandidate) {
        ...
        if (validateExact(InstructionalMemoryEntrySchema, state).status === "rejected" ||
    ...
    expireOpportunity(scopeCandidate) {
        const validation = validateExact(InstructionalMemoryScopeSchema, scopeCandidate);
        if (validation.status === "rejected")
            return rejected("INVALID_MEMORY_REQUEST");
        return {
    ...
    #resolveAccess(requestCandidate) {
