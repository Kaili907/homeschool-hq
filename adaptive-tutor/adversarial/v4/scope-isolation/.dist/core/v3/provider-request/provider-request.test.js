...
import { BoundedCommercialProviderRequestSchema, COMMERCIAL_PROVIDER_RAW_ATTEMPT_DISCLOSURE_ALLOWED, projectCommercialProviderRequest, validateBoundedCommercialProviderRequest, } from "./index.js";
...
test("projects a closed provider request containing only structured Study evidence", () => {
    ...
    if (result.status !== "accepted-commercial-provider-request")
        return;
    assert.equal(validateExact(BoundedCommercialProviderRequestSchema, result.request).status, "accepted");
...
test("rejects attacks at the direct commercial provider boundary", () => {
    ...
    if (projected.status !== "accepted-commercial-provider-request")
        return;
    for (const field of [
        "learnerResponse",
        "rawAttemptText",
...
test("rejects nested raw-attempt and answer fields instead of stripping them", () => {
    for (const contaminatedAttempt of [
        ...
        assert.equal(projectCommercialProviderRequest({
            ...facts(),
...
test("rejects hostile prototypes and accessors without invoking getters", () => {
    ...
    assert.equal(projectCommercialProviderRequest({ ...facts(), groundingEvidence: hostileArray }).status, "provider-request-rejected");
    ...
    const providerGetter = structuredClone(projected.request);
    ...
    assert.equal(validateBoundedCommercialProviderRequest(providerGetter).status, "provider-request-rejected");
    assert.equal(providerGetterReads, 0);
});
test("rejects inconsistent or duplicate structured evidence with bounded reasons", () => {
    assert.deepEqual(projectCommercialProviderRequest({
        ...facts(),
        attemptEvidence: {
    ...
    if (projected.status !== "accepted-commercial-provider-request")
        return;
    assert.deepEqual(validateBoundedCommercialProviderRequest({
        ...projected.request,
        attemptEvidence: {
