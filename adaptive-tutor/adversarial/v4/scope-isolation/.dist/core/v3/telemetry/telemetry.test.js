...
function reservation(reservedAttempt = attempt(), overrides = {}) {
    return {
...
function requireEvent(input, commercialLineage = lineage()) {
...
test("rejects physical-attempt and logical-operation lineage mismatches", () => {
    ...
    assert.deepEqual(projectTutorCommercialTelemetry(source(), lineage(wrongPhysical, reservation(reserved))), { status: "rejected", reasonCode: "INVALID_COMMERCIAL_LINEAGE" });
    const wrongLogical = reservation(reserved, {
        logicalOperationRef: "logical-operation:different",
    });
    assert.deepEqual(projectTutorCommercialTelemetry(source(), lineage(reserved, wrongLogical)), { status: "rejected", reasonCode: "INVALID_COMMERCIAL_LINEAGE" });
});
