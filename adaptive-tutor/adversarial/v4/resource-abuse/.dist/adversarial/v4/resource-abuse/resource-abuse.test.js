...
import { BUDGET_RESILIENCE_VERSION, MAX_INTEGER_MICROS, decideFallback, reserveExecutionBudget, } from "../../../core/v3/routing/budget-resilience/index.js";
import { MODEL_CAPABILITY_PROFILE_VERSION, PROVIDER_AVAILABILITY_STATE_VERSION, PROVIDER_CAPABILITY_PROFILE_VERSION, ROUTING_REQUEST_VERSION, createEligibleRouteCatalog, routeProviderModel, } from "../../../core/v3/routing/provider-routing/index.js";
...
function provider(index = 0, overrides = {}) {
...
function model(index = 0, overrides = {}) {
    const id = suffix(index);
    return {
...
function policyProfile(index = 0, eligible = true) {
    const id = suffix(index);
    return {
        providerRef: `provider-profile:${id}`,
...
function policyRequirement(index = 0) {
    return {
...
function catalog(providers, models, profiles = providers.map((_, index) => policyProfile(index)), requirements = providers.map((_, index) => policyRequirement(index))) {
...
function executionBudget(ceiling = "2", maximumPhysicalAttempts = 2) {
    return {
...
function fallbackInput(overrides = {}) {
    return {
        executionBudget: executionBudget(),
...
test("cost grammar and signed-int64 arithmetic fail closed", () => {
    ...
    for (const micros of [
        ...
        assert.deepEqual(["decision" in result ? result.decision : null, "reason" in result ? result.reason : null], ["fixed-stop", "invalid-budget"], micros);
    }
...
test("catalog work is deterministic at the 64-profile boundary", () => {
    ...
    assert.ok(large);
    assert.deepEqual([large.eligibleProviderCount, large.eligibleRouteCount], [64, 64]);
    const decision = routeProviderModel(routingRequest({ providerAvailability: availabilityStates }), large);
...
test("empty, one-provider, ineligible, tie, and duplicate catalogs stay bounded", () => {
    ...
    const ineligibleProfiles = Array.from({ length: 64 }, (_, index) => policyProfile(index, false));
    const largeIneligible = catalog(Array.from({ length: 64 }, (_, index) => provider(index)), Array.from({ length: 64 }, (_, index) => model(index)), ineligibleProfiles, Array.from({ length: 64 }, (_, index) => policyRequirement(index)));
    ...
    assert.ok(tied);
    const tiedDecision = routeProviderModel(routingRequest({ providerAvailability: [availability(1), availability(0)] }), tied);
    ...
    assert.equal(catalog([provider(0), provider(1)], [model(0), model(1, { routeRef: model(0).routeRef })]), null);
});
test("token and context requests stop at exact structural ceilings", () => {
    const trusted = catalog([provider()], [model()]);
    assert.ok(trusted);
    const zeroInput = routeProviderModel(routingRequest({ contextSizeRequirement: { inputTokens: 0, requiredOutputTokens: 1 } }), trusted);
    assert.equal(zeroInput.status, "ROUTE_SELECTED");
    const exact = routeProviderModel(routingRequest({ contextSizeRequirement: { inputTokens: 900, requiredOutputTokens: 100 } }), trusted);
    ...
    for (const contextSizeRequirement of [
        ...
        assert.equal(routeProviderModel(routingRequest({ contextSizeRequirement }), trusted).status, "NO_ELIGIBLE_PROVIDER_ROUTE", JSON.stringify(contextSizeRequirement));
    }
});
test("attempt expansion, duplicate references, and same-route failover are rejected", () => {
    ...
    assert.equal(routeProviderModel(routingRequest({ physicalAttemptRefs: [duplicate, duplicate] }), trusted).status, "NO_ELIGIBLE_PROVIDER_ROUTE");
    assert.equal(routeProviderModel(routingRequest({
        physicalAttemptRefs: [
            "physical-attempt:first",
            "physical-attempt:second",
            "physical-attempt:third",
        ],
    }), trusted).status, "NO_ELIGIBLE_PROVIDER_ROUTE");
    ...
    assert.equal("decision" in duplicateReservationAttempt ? duplicateReservationAttempt.decision : null, "fixed-stop");
...
test("[BLOCKER RA-01] provider-policy requirement cardinality is not bounded", () => {
    const requirements = Array.from({ length: 4_096 }, (_, index) => policyRequirement(index));
    ...
    const nestedRequirement = {
        ...policyRequirement(),
        allowedRetentionClasses: Array.from({ length: 4_096 }, () => "none"),
    };
