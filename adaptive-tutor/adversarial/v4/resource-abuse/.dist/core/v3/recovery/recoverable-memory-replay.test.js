...
class FakeEffectGateway {
    ...
    lookup(identity) {
        ...
        if (event.effectRef !== identity.effectRef ||
    ...
    accept(command) {
        ...
        this.#accepted.set(command.logicalOperationRef, structuredClone(command.acceptedEvent));
...
class FlakyMemoryPort {
    store;
    failuresRemaining;
    applyCount = 0;
    constructor(store, failuresRemaining) {
        this.store = store;
...
test("crash before effect acceptance permits a new distinct physical attempt", () => {
    ...
    assert.notEqual(gateway.acceptCommands[0]?.physicalAttemptRef, gateway.acceptCommands[1]?.physicalAttemptRef);
...
test("foreign learner, session, interaction, and opportunity are quarantined before effects", () => {
    ...
    for (const trustedScope of foreignScopes) {
        ...
        if (result.status === "quarantined")
            assert.equal(result.code, "FOREIGN_SCOPE");
        assert.equal(gateway.acceptCommands.length, 0);
...
test("raw transcript and unknown prose carriers are rejected by delta and event schemas", () => {
    const operation = makeOperation();
    assert.equal(validateExact(InstructionalMemoryDeltaSchema, {
        ...operation.memoryDelta,
        rawLearnerText: "verbatim private answer",
    }).status, "rejected");
    assert.equal(validateExact(MinimizedAcceptedStudyEffectEventSchema, {
...
test("authority mutations fail closed and projections remain non-authoritative", () => {
    const operation = makeOperation();
    const memory = new InstructionalMemoryProjectionStore();
    assert.deepEqual(memory.apply({ ...operation.memoryDelta, officialMasteryAuthority: true }), { status: "rejected", code: "INVALID_MEMORY_DELTA", tutorMayUseMemory: false });
    assert.deepEqual(memory.apply({
        ...operation.memoryDelta,
        operations: [
