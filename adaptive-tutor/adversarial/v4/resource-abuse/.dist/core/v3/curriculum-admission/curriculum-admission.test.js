...
import { CURRICULUM_ADMISSION_VERSION, CURRICULUM_METADATA_VERSION, buildCurriculumCapabilityRegistry, evaluateCurriculumAdmission, evaluateCurriculumAdmissionWithRegistry, } from "./index.js";
function metadata() {
    return {
...
function assertDecision(input, status, reason) {
...
test("fails closed without a reviewed capability declaration", () => {
    ...
    assertDecision(nullDeclaration, "refused", "missing-reviewed-capability-declaration");
    const missingDeclaration = validInput();
    delete record(missingDeclaration).capabilityDeclaration;
    assertDecision(missingDeclaration, "refused", "missing-reviewed-capability-declaration");
...
test("routes unsupported reviewed capabilities according to their declaration", () => {
    ...
    assertDecision(staticFallback, "static-only", "unsupported-tutor-capability-static-only");
...
test("allows no free-form instructional Tutor during active assessment", () => {
    ...
    input.capabilityDeclaration.allowedAssessmentPhases.push("active-graded-or-mastery-check");
...
test("refuses a capability or curriculum release explicitly not admitted", () => {
    ...
    assert.equal(evaluateCurriculumAdmission(curriculum, validInput()).reason, "curriculum-release-not-admitted");
});
...
test("rejects ambiguous or malformed curriculum and capability metadata", () => {
    ...
    assert.equal(evaluateCurriculumAdmission(duplicateCourse, validInput()).reason, "invalid-curriculum-metadata");
    const duplicateCapabilityScope = validInput();
    duplicateCapabilityScope.capabilityDeclaration.supportedSubjectRefs = [
        "technology",
        "technology",
    ];
    assertDecision(duplicateCapabilityScope, "refused", "invalid-capability-declaration");
    ...
    assert.equal(evaluateCurriculumAdmissionWithRegistry(forgedRegistry, validInput()).reason, "invalid-curriculum-metadata");
});
...
test("coverage matches the current admitted canonical curriculum metadata", () => {
    const releaseRoot = resolve(process.cwd(), "../curriculum-release-admitted/family-pilot-r1");
    const manifest = JSON.parse(readFileSync(resolve(releaseRoot, "runtime/runtime-manifest.json"), "utf8"));
    const rowsByCourse = JSON.parse(readFileSync(resolve(releaseRoot, "runtime/lesson-rows-by-course.json"), "utf8"));
    ...
    assert.equal(result.registry.lessonUnit("ma-g9-science", "ma-hs9-biology-u01-l01"), "ma-hs9-biology-u01");
});
