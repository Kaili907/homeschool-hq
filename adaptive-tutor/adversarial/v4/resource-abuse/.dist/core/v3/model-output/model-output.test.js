...
import { validateProviderModelOutput, } from "./index.js";
const reviewedRef = "reviewed-content:fraction-hint";
const groundingRef = "grounding:fraction-model";
function contextFixture(assessmentPhase = "instruction-or-practice") {
    return {
        assessmentPhase,
...
function withInjectedField(field, value) {
...
test("rejects malformed, inconsistent, and over-bounded envelopes", () => {
    const malformed = [
        ...
        {
            ...proposalFixture(),
            reviewedContentRefs: Array.from({ length: 13 }, (_, index) => `reviewed-content:item-${index}`),
        },
        {
            responseKind: "refusal",
            reviewedContentRefs: [reviewedRef],
...
test("provider-requested actions and display modes remain subordinate to trusted policy", () => {
    ...
    const disallowedMode = {
        ...proposalFixture(),
        instructionalDisplayMode: "reviewed-visual",
    };
...
test("authority-field injection cannot change Study-owned state", () => {
    ...
    for (const field of injectedAuthorityFields) {
        const result = validateProviderModelOutput(withInjectedField(field, { authorize: true, value: "provider-selected" }), contextFixture());
        assert.deepEqual(result, {
            status: "static-fallback-required",
...
test("active assessment rejects answer-bearing and unrestricted tutoring fields", () => {
    ...
    for (const field of injectedAnswerFields) {
        assert.deepEqual(validateProviderModelOutput(withInjectedField(field, "The correct answer is 42."), active), {
            status: "static-fallback-required",
...
test("active assessment also rejects answer disclosure hidden under an unknown field", () => {
    ...
    assert.deepEqual(validateProviderModelOutput(output, contextFixture("active-graded-or-mastery-check")), {
        status: "static-fallback-required",
        reasonCode: "ACTIVE_ASSESSMENT_ANSWER_BEARING_OUTPUT",
        staticFallbackRequired: true,
    });
...
test("active assessment can accept the closed reference-only proposal", () => {
    const result = validateProviderModelOutput(proposalFixture(), contextFixture("active-graded-or-mastery-check"));
    assert.equal(result.status, "accepted-proposal");
    if (result.status !== "accepted-proposal")
        return;
...
test("free-form provider content is still rejected outside assessment by the closed schema", () => {
    assert.equal(validateProviderModelOutput(withInjectedField("content", "A provider-authored explanation."), contextFixture()).status, "malformed");
});
test("hazard inspection does not invoke provider-defined getters", () => {
    let getterCalled = false;
    const output = proposalFixture();
