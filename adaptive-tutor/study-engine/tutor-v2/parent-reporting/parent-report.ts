...
export const PARENT_REPORT_VERSION = "tutor-parent-report/v2" as const;
...
export type ParentReportDecisionStatus =
  (typeof PARENT_REPORT_DECISION_STATUSES)[number];
...
export type ParentReportReasonCode =
  (typeof PARENT_REPORT_REASON_CODES)[number];

export interface ReviewedParentReportCopy {
  readonly decisionStatus: ParentReportDecisionStatus | null;
  readonly reasonCode: ParentReportReasonCode;
...
] as const satisfies readonly ReviewedParentReportCopy[];

export const PARENT_REPORT_DISCLAIMER =
  "This report explains Study-approved records. Tutor may propose a step, but only Study can approve or apply it. This report does not make or change a learning decision." as const;
...
export type ParentReportRequestScope = Static<
  typeof ParentReportRequestScopeSchema
>;
...
const GuardianAuthorizationCommonProperties = {
  guardianAuthorizationKind: Type.Literal(
    "study-parent-report-guardian-authorization",
  ),
...
export type ParentReportGuardianAuthorization = Static<
  typeof ParentReportGuardianAuthorizationSchema
>;
...
export type ParentReportEvidenceScope = Static<
  typeof ParentReportEvidenceScopeSchema
>;

const ParentReportDecisionStatusSchema = Type.Union(
  PARENT_REPORT_DECISION_STATUSES.map((status) => Type.Literal(status)),
);
...
export type ParentReportRequest = Static<typeof ParentReportRequestSchema>;
...
export const ParentReportSummarySchema = Type.Union(
  REVIEWED_PARENT_REPORT_COPY.map((copy) =>
    Type.Object(
      {
  ...
  { $id: "TutorV2ParentReportSummary" },
);
export type ParentReportSummary = Static<typeof ParentReportSummarySchema>;
...
export type ParentReport = Static<typeof ParentReportSchema>;

export const ParentReportResultSchema = Type.Union(
  [
    Type.Object(
      { status: Type.Literal("accepted"), value: ParentReportSchema },
      { additionalProperties: false },
    ),
    Type.Object(
      {
...
export type ParentReportResult = Static<typeof ParentReportResultSchema>;
...
function isValidPeriod(
  scope: Extract<ParentReportRequestScope, { scopeKind: "reporting-period" }>,
): boolean {
  return (
    isValidDateTime(scope.startsAt) &&
...
function evidenceMatchesScope(
  ...
  if (
    evidence.learnerRef !== reportScope.selectedLearnerRef ||
  ...
  if (
    ...
    return (
      evidenceScope.reportingPeriodRef === reportScope.reportingPeriodRef &&
...
function authorizationMatchesScope(
  ...
  if (
    authorization.guardianRef !== reportScope.guardianRef ||
...
export function buildMinimizedParentHubReport(
  ...
  if (
    request.scope.scopeKind === "reporting-period" &&
  ...
  for (const evidence of request.evidence) {
    ...
    if (
      Date.parse(evidence.provenance.recordedAt) >
    ...
    const reasonExists = REVIEWED_PARENT_REPORT_COPY.some(
      (copy) => copy.reasonCode === evidence.reasonCode,
    );
    if (!reasonExists) {
      return { status: "rejected", code: "PARENT_REPORT_UNKNOWN_REASON" };
    }
    const reviewedCopy = REVIEWED_PARENT_REPORT_COPY.find(
      (copy) =>
        copy.reasonCode === evidence.reasonCode &&
