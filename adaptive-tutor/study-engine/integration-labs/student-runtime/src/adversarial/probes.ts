...
import {
  ...
  TEMPORARY_SESSION6_BRIDGE_VERSION,
} from "../bridges/session6Bridge.v1";
...
function duplicateEventProbe(): AdversarialProbeResult {
  ...
  for (const [segment, response] of acceptedResponses) {
    ...
    completed = submitCurrentResponse(completed, probeAt(step + 1));
  ...
  completed = submitCurrentResponse(completed, probeAt(step + 3));
...
function unsupportedVersionProbe(): AdversarialProbeResult {
  const rejected = runTemporarySession6Bridge({
    schemaVersion: "student-runtime.session6-bridge.v999" as typeof TEMPORARY_SESSION6_BRIDGE_VERSION,
    requestId: "bridge:unsupported:1",
    sessionId: asBrandedId<SessionId>(
      "session:student-runtime:math:demo-001",
    ),
    taskRef: canonicalSegmentId("math", "warm-up"),
    responseRef:
      "draft:session:student-runtime:math:demo-001:segment:g5:math:eq-frac:warm.up",
  ...
  return result(
    ...
    !rejected.ok && rejected.code === "unsupported-version",
...
export function runAdversarialProbes(): readonly AdversarialProbeResult[] {
  return [
