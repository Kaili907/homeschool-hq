...
export const RESUME_DRAFT_SIDECAR_PREFIX =
  "manuel-academy:student-runtime:protected-draft:v2:" as const;
...
interface ProtectedDraftSidecar {
  readonly responses: RuntimeWorkspaceV1["responses"];
  readonly transcript: RuntimeWorkspaceV1["transcript"];
}

function getDraftSidecarStorageKey(locator: ResumeLocator): string {
  return `${RESUME_DRAFT_SIDECAR_PREFIX}${encodeKeyPart(
...
async function checkpointPayload(
  ...
  readonly payload: RuntimeWorkspaceV1;
  readonly sidecar: string;
}> {
  const sidecar = canonicalSerialize({
    responses: workspace.responses,
    transcript: workspace.transcript,
  } satisfies ProtectedDraftSidecar);
  const protectedDraftReference = await sha256(sidecar);
  const payload = {
    ...workspace,
    responses: {},
    transcript: [],
    protectedDraftReference,
  } as RuntimeWorkspaceV1;
  return { payload, sidecar };
}

async function restoreProtectedDraft(
  payload: RuntimeWorkspaceV1,
  locator: ResumeLocator,
  storage: ResumeStorage,
): Promise<RuntimeWorkspaceV1 | undefined> {
  const reference = (payload as RuntimeWorkspaceV1 & {
    readonly protectedDraftReference?: unknown;
  }).protectedDraftReference;
  if (!isNonEmptyString(reference) || !SHA_256_HEX_PATTERN.test(reference)) {
    return undefined;
  }
  const raw = storageGet(storage, getDraftSidecarStorageKey(locator));
  if (raw === null || !constantTimeEqual(reference, await sha256(raw))) {
    return undefined;
  }
  const sidecar = safeJsonParse(raw);
  if (
    !isRecord(sidecar) ||
    !isRecord(sidecar.responses) ||
    !Array.isArray(sidecar.transcript)
  ) {
    return undefined;
  }
  return {
    ...payload,
    responses: sidecar.responses,
...
export function createResumeStore(options: ResumeStoreOptions): ResumeStore {
  ...
  const quarantine = (
    ...
    storageRemove(storage, getDraftSidecarStorageKey(locator));
  };
  ...
  const loadRaw = async (
    ...
    const restoredWorkspace = await restoreProtectedDraft(
      candidate.payload,
      request,
      storage,
    );
  ...
  const save = async (
    ...
    withResumeLock(async () => {
      ...
      const protectedCheckpoint = await checkpointPayload(snapshot);
      ...
      const unsigned: Omit<ResumeEnvelopeV1, "integrity"> = {
        ...
        payload: protectedCheckpoint.payload,
      };
      ...
      storageSet(
        storage,
        getDraftSidecarStorageKey(locator),
        protectedCheckpoint.sidecar,
      );
      return {
        status: "saved",
        envelope,
    ...
    withResumeLock(async () => {
      const keys = locator
        ...
        : storageKeys(storage).filter(
            (key) =>
              ...
              key.startsWith(RESUME_DRAFT_SIDECAR_PREFIX) ||
      ...
      for (const key of keys) {
        if (storageGet(storage, key) !== null) {
          ...
          if (!key.startsWith(RESUME_DRAFT_SIDECAR_PREFIX)) removed += 1;
        }
      }
      return removed;
    });
