...
} from "../bridges/session6Bridge.v2";
import type {
...
export function submitCurrentResponse(
  workspace: RuntimeWorkspaceV1,
  at = new Date().toISOString(),
): RuntimeWorkspaceV1 {
  const session = activeSession(workspace);
  ...
  const result = runTemporarySession6Bridge({
    schemaVersion: TEMPORARY_SESSION6_BRIDGE_VERSION,
    ...
    ...(segment === "self-check"
      ...
      : definition.expectedAnswer
        ...
        : { minimumLength: definition.minLength ?? 1 }),
    occurredAt: at,
  });
  if (!result.ok) {
    return bump(workspace, at, {
      ...
      transcript(
        ...
        ready
          ? lowConfidence
            ...
            : "temporary-core-continue"
          : "temporary-core-reteach",
      ),
    ],
  });
}
...
function bridgeReceiptBindingIsValid(
  ...
  const reasonMatchesDirective =
    (receipt.directive === "continue" &&
      receipt.reasonCode === "demo-rubric-accepted") ||
    (receipt.directive === "reteach" &&
      receipt.reasonCode === "demo-rubric-needs-support");
  return (
    receipt.bridgeVersion === TEMPORARY_SESSION6_BRIDGE_VERSION &&
    ...
    occurredAt <= savedAt &&
    receipt.masteryAuthority === "withheld-pending-tutor-core" &&
    receipt.misconceptionAuthority === "withheld-pending-tutor-core" &&
...
export function completeCurrentSegment(
  ...
  const reviewRecommendation = createReviewRecommendation(nextWorkspace, at);
