...
/** Manuel Academy sample only; production callers must supply household data. */
export const LEARNER_TIME_ZONE = "America/New_York";

const IANA_TIME_ZONE_SHAPE = /^[A-Za-z_]+(?:\/[A-Za-z0-9_+\-]+)+$/;

export function isSupportedIanaTimeZone(value: unknown): value is string {
  if (typeof value !== "string" || !IANA_TIME_ZONE_SHAPE.test(value)) {
    return false;
  }
  try {
    return (
...
export function requireIanaTimeZone(value: unknown): string {
  if (!isSupportedIanaTimeZone(value)) {
    throw new RangeError("A supported household IANA timezone is required.");
  }
  return value;
}
