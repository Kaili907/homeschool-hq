...
import { classifyMisconceptions, type ClassificationEvidence } from "./misconception-classifier.js";
...
export interface TutorEngineSnapshot {
  ...
  transcript: Array<{ speaker: "jarvis" | "learner" | "system"; text: string }>;
}

const defaultHooks: TutorRuntimeHooks = {
...
function nowId(prefix: string, count: number): StableId {
  return `${prefix}-${String(count).padStart(3, "0")}` as StableId;
}

function spoken(id: StableId, text: string, requireLearnerResponse: boolean) {
  return {
    id: `${id}-spoken` as StableId,
    text,
    locale: "en-US",
    ...
    fallbackText: text,
    captionsRequired: true as const,
    transcriptRequired: true as const,
    claimsHumanIdentity: false as const,
  };
}
...
function sourceForPhase(phase: TutorPhase): ConfidenceObservation["source"] {
  if (phase === "assessment") return "diagnostic";
  if (phase === "guided-practice") return "guided-practice";
  if (phase === "independent-attempt") return "independent-attempt";
  return "reassessment";
}

function weightForPhase(phase: TutorPhase): number {
  if (phase === "guided-practice") return 0.45;
  if (phase === "assessment") return 0.65;
  if (phase === "independent-attempt") return 1;
  return 1;
}

export class AdaptiveTutorEngine {
  readonly program: TutorProgram;
  private readonly hooks: TutorRuntimeHooks;
  private phase: TutorPhase = "assessment";
  private cycleCount = 0;
  private currentItemIndex = 0;
  private teachingTurnIndex = 0;
  private selectedMisconceptionId: StableId | null = null;
  private observations: ConfidenceObservation[] = [];
  private misconceptionEvidence: ClassificationEvidence[] = [];
  private responseCount = 0;
  private attemptsByItem = new Map<StableId, number>();
  private transcript: TutorEngineSnapshot["transcript"] = [];
  private scores = {
    diagnostic: [] as number[],
    guided: [] as number[],
    independent: [] as number[],
    reassessment: [] as number[],
  };

  constructor(program: TutorProgram, hooks: Partial<TutorRuntimeHooks> = {}) {
    this.program = assertValid<TutorProgram>(TutorProgramSchema, program, "Tutor program");
  ...
  submit(input: LearnerInput): TutorResponse {
    ...
    if (safety.triggers.length > 0) {
      ...
      if (safety.mustStopAcademicFlow) this.phase = "escalated";
      return baseResponse({
    ...
    if (this.phase === "assessment") return this.afterDiagnostic(evaluation);
    if (this.phase === "guided-practice") return this.afterGuided(item, input, evaluation, attempt);
    if (this.phase === "independent-attempt") return this.afterIndependent(evaluation);
  ...
  continue(): TutorResponse {
    ...
    if (this.phase === "teach-visually") {
      ...
      this.currentItemIndex = 0;
      return this.presentCurrentItem(
        "guided-practice",
        "Now let’s do one together. You can use a hint, and mistakes are part of the practice.",
      );
    }
  ...
  requestAlternateExplanation(): TutorResponse {
    const sequences = this.program.teachingSequences.filter(
      (sequence) =>
        sequence.misconceptionId === this.selectedMisconceptionId || sequence.misconceptionId === "default",
    );
    const sequence = sequences[1] ?? sequences[0] ?? this.program.teachingSequences[0];
    if (!sequence) throw new Error("No teaching sequence is available.");
    const turn = sequence.turns[0];
    if (!turn) throw new Error("Alternate teaching sequence is empty.");
    this.phase = "teach-visually";
    this.teachingTurnIndex = 0;
  ...
  private recordEvidence(
    ...
    const observation: ConfidenceObservation = {
      id: `evidence-${String(this.observations.length + 1).padStart(3, "0")}` as StableId,
    ...
    if (source === "diagnostic") this.scores.diagnostic.push(evaluation.score);
    else if (source === "guided-practice") this.scores.guided.push(evaluation.score);
    else if (source === "independent-attempt") this.scores.independent.push(evaluation.score);
    else this.scores.reassessment.push(evaluation.score);
  }

  private afterDiagnostic(evaluation: EvaluationResult): TutorResponse {
    ...
    const definition = this.program.misconceptions.find(
      (item) => item.id === this.selectedMisconceptionId,
    );
  ...
  private afterGuided(
    ...
    return this.presentCurrentItem(
      "independent-attempt",
      "Now try independently. I will not give hints during these items, and one answer will not be treated as mastery.",
    );
  }

  private afterIndependent(evaluation: EvaluationResult): TutorResponse {
    ...
    return this.presentCurrentItem(
      "reassess",
      "Let’s reassess with a fresh item. This checks whether the idea transfers after teaching.",
    );
  }

  private afterReassessment(_evaluation: EvaluationResult): TutorResponse {
    ...
    const independentAndReassessmentCount =
      this.scores.independent.length + this.scores.reassessment.length;
    const independentAndReassessmentMean =
      independentAndReassessmentCount === 0
        ? 0
        : [...this.scores.independent, ...this.scores.reassessment].reduce(
            (sum, score) => sum + score,
            0,
          ) / independentAndReassessmentCount;
    const advance =
      independentAndReassessmentCount >= contract.minimumEvidenceCount &&
  ...
  private presentCurrentItem(phase: TutorPhase, introduction: string): TutorResponse {
    this.phase = phase;
    const item = this.currentItem();
  ...
  private currentTeachingTurn(): TutorResponse {
    const sequence = this.selectedTeachingSequence();
    const turn = sequence.turns[this.teachingTurnIndex];
    if (!turn) throw new Error("Teaching turn index is out of range.");
    const response = {
      ...turn,
  ...
  private selectedTeachingSequence(): TeachingSequence {
    const selected =
      this.program.teachingSequences.find(
        (sequence) => sequence.misconceptionId === this.selectedMisconceptionId,
      ) ??
      this.program.teachingSequences.find((sequence) => sequence.misconceptionId === "default") ??
      this.program.teachingSequences[0];
    if (!selected) throw new Error("No teaching sequence is available.");
    return selected;
  }

  private classification(): MisconceptionClassification {
    return classifyMisconceptions(
      this.program.targetSkillId,
      this.program.misconceptions,
      this.misconceptionEvidence,
    );
  }

  private currentItem(): AssessmentItem {
    const item = this.safeCurrentItem();
    if (!item) throw new Error(`No item is available for phase ${this.phase}.`);
    return item;
  }

  private safeCurrentItem(): AssessmentItem | undefined {
    if (this.phase === "assessment") return this.program.diagnosticItems[this.currentItemIndex];
    if (this.phase === "guided-practice") return this.program.guidedPractice.items[this.currentItemIndex];
    if (this.phase === "independent-attempt") return this.program.independentMastery.items[this.currentItemIndex];
    if (this.phase === "reassess") return this.program.reassessmentItems[this.currentItemIndex];
    return undefined;
  }
}
