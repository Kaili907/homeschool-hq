...
Under accepted Session 5-R2 `DEC-004`, the canonical `SegmentId` is also the task identity.
...
- Event IDs are opaque and unique. Consumers never derive or normalize them.
...
- Tutor inputs first pass the Session 6-R2 safety gateway and a one-time,
  event-bound permit. Completion can consume only a validated Tutor result
  accepted by the bridge ledger and bound to the active session, canonical
  segment/task, draft reference, revision, and occurrence time.
...
## Bridge events, ledger, and outbox

Tutor event kinds are validated bridge inputs, not additional
`StudySessionEvent` values. The supported Tutor kinds are
`assessment-result`, `confidence-estimate`, `misconception-result`,
`prerequisite-result`, `tutor-response`, `parent-teacher-review`,
`safety-decision`, and `study-completion`.

The mandatory order is safety classification, one-time permit, Tutor Core,
authority adapter, event ledger, minimized Study projection, then outbox
proposals. No supported path projects or enqueues before ledger acceptance.
An identical duplicate is ignored. An event-ID collision with changed content
is quarantined. Unsupported versions and unknown Tutor kinds are quarantined
without canonical mutation. Adult-review hooks and downstream work remain
