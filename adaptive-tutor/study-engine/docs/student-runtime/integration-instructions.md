...
Frozen Tutor Core v0.2 and the accepted Session 6-R2 bridge are verified and
vendored read-only inside the lab. `src/bridges/acceptedStudyCoreBridge.ts`
uses bridge package `1.0.1`, contract `1`, the accepted orchestrator, event
ledger idempotency, and Tutor Core-owned assessment scoring. The retired
Session 7 temporary bridge modules are absent.
