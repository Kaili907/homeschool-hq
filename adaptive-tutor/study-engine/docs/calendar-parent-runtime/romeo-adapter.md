...
Laptop correction: DEC-018 is crosswalked to the accepted R2 credential-free
sidecar mapping with `ACCEPTED_R2_PARITY`. Production and final assembly remain
unauthorized, but Tutor Core is not a direct Session 8 input and does not block
this local adapter. This statement supersedes historical Card 5 status wording
retained below.
...
decision crosswalk, `ACCEPTED_R2_PARITY` state, production limitation, and negative
...
| untyped support reference | Require `linkedManuelAcademyTutoring.supportStudyPlanId: VersionedReference<StudyPlanId>` |
...
The focused suite reads accepted R2 mappings and the reconciliation manifest
and asserts the DEC-018 crosswalk. It covers version gating, metadata normalization,
