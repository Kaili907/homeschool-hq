...
describe("persistence and duplicate-event agent regressions", () => {
  it("keeps a caller-supplied opaque session ID immutable across canonical events", async () => {
  ...
  it("binds ready feedback to one exact v2 session/SegmentId/draft receipt", async () => {
  ...
  it("rejects same command ID with a different canonical payload fingerprint", async () => {
    let workspace = beginLearning("math");
    workspace = setResponseDraft(
      workspace,
      "warm-up",
  ...
  it("guards support transcript revisions and no-audio fallback centrally", async () => {
