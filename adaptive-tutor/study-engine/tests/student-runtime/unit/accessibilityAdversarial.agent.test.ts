...
function acceptedWarmUpWorkspace(): RuntimeWorkspaceV1 {
...
function completeMathCycle(): RuntimeWorkspaceV1 {
  let workspace = beginLearning("math");
...
describe("accessibility/adversarial agent boundaries", () => {
  it("rejects cross-session, cross-segment, response-ref, and reused receipt forgeries", () => {
    const workspace = acceptedWarmUpWorkspace();
  ...
  it("does not grant completion to forged ready feedback without its bound continue receipt", () => {
  ...
  it("replaying the exit-ticket action cannot duplicate completion, evidence, or review output", () => {
    const completed = completeMathCycle();
    const before = {
  ...
  it("keeps names, contact data, raw work, and injected instructions out of canonical output", () => {
    const secrets = [
      "Alexandra Example",
  ...
  it("keeps all seven learner-visible probe outcomes deterministic and reason-coded", () => {
    const first = runAdversarialProbes();
    const second = runAdversarialProbes();
