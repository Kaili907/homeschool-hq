...
describe("policy, timer, idempotency, and adversarial boundaries", () => {
  it("applies the provisional Card 5 precedence and all duration caps", () => {
    const preferences = {
      ...defaultParentPreferences(),
  ...
  it("keeps visible, minimal, and hidden timer modes state-equivalent", () => {
  ...
  it("enforces the parent hard maximum and saves at the exact segment", () => {
    const preferences = {
      ...defaultParentPreferences(),
  ...
  it("triggers a required break after guided practice without marking failure", () => {
    const preferences = {
      ...defaultParentPreferences(),
      requiredBreaks: true,
    };
  ...
  it("does not grant progress for random answers and ignores a replayed completion", () => {
    let workspace = beginLearning("math");
    ...
    expect(workspace.feedback["warm-up"]).toBe("support");
    const unchanged = completeCurrentSegment(workspace, traceAt(5));
    expect(unchanged.completedSegments).toEqual([]);
    ...
    workspace = submitCurrentResponse(workspace, traceAt(7));
    workspace = completeCurrentSegment(workspace, traceAt(8));
  ...
  it("passes all seven adversarial demonstrations with safe language", () => {
  ...
  it("withholds pacing increases until five comparable reliable sessions exist", () => {
  ...
  it("keeps forged canonical histories invalid after break activity", () => {
    let workspace = beginLearning("math");
