...
const ACCEPTED_R2_MAPPINGS = JSON.parse(
  readFileSync(
    new URL(
      "../../reconciliation/exact-mappings.json",
      import.meta.url,
    ),
    "utf8",
  ),
...
const ACCEPTED_R2_MANIFEST = JSON.parse(
  readFileSync(
    new URL(
      "../../reconciliation/reconciliation-manifest.json",
      import.meta.url,
    ),
    "utf8",
  ),
  ...
  readonly package: {
    ...
    readonly finalAssembly: boolean;
  };
};
...
describe("Card 5 DEC-018 reconciliation parity", () => {
  it("crosswalks DEC-018 to the accepted R2 credential-free Romeo sidecar", () => {
    const mapping = ACCEPTED_R2_MAPPINGS.fieldMappings.find(
      ({ source }) => source === "S4.RomeoVirtualAcademyAssignment",
    );

    expect(mapping).toEqual({
      source: "S4.RomeoVirtualAcademyAssignment",
      target: "Credential-free external-assignment sidecar",
      conversion: "namespaced external ref; completion remains Romeo-owned",
      disposition: "sidecar-authority",
    });
    expect(ACCEPTED_R2_MANIFEST.package).toEqual(
      expect.objectContaining({
        status: "reconciled_with_evidence_supported_blockers",
        finalAssembly: false,
      }),
    );
