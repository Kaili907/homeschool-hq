...
import {
  ...
  COMMERCIAL_PROVIDER_REQUEST_VERSION,
  TrustedStudyCommercialInvocationFactsSchema,
  type BoundedCommercialProviderRequest,
  type CommercialProviderRequestRejectionReasonCode,
  type CommercialProviderRequestResult,
 ...
 */
function isSafeClosedJsonBoundary(candidate: unknown): boolean {
  const ancestors = new Set<object>();
  let nodeCount = 0;

  const inspect = (value: unknown, depth: number): boolean => {
    nodeCount += 1;
    if (nodeCount > MAXIMUM_BOUNDARY_NODES || depth > MAXIMUM_BOUNDARY_DEPTH) return false;
    if (value === null || typeof value === "boolean") return true;
    if (typeof value === "string") return value.length <= MAXIMUM_BOUNDARY_STRING_LENGTH;
...
function rejected(
  reasonCode: CommercialProviderRequestRejectionReasonCode,
): CommercialProviderRequestResult {
  return {
...
function semanticReason(
  facts: Pick<
    TrustedStudyCommercialInvocationFacts,
    "attemptEvidence" | "reviewedContentEvidence" | "groundingEvidence"
  >,
): CommercialProviderRequestRejectionReasonCode | null {
...
function copyAcademicScope(
  facts: TrustedStudyCommercialInvocationFacts,
): BoundedCommercialProviderRequest["academicScope"] {
  return {
...
function projectValidatedFacts(
  facts: TrustedStudyCommercialInvocationFacts,
 ...
 */
export function validateBoundedCommercialProviderRequest(
  candidate: unknown,
): CommercialProviderRequestResult {
 ...
 */
export function projectCommercialProviderRequest(
  candidate: unknown,
): CommercialProviderRequestResult {
  if (!isSafeClosedJsonBoundary(candidate)) {
