...
function suffixOf(reference: string): "alpha" | "beta" {
...
function model(
  ...
  return {
    ...
    capabilityProfileDigest:
      suffix === "alpha" ? PROFILE_DIGEST_ALPHA : PROFILE_DIGEST_BETA,
...
function policyProfile(providerRef: string): TrustedProviderProfile {
  const suffix = suffixOf(providerRef);
  return {
    providerRef,
...
function policyRequirements(providerRef: string): ProviderEligibilityRequirements {
  return {
    providerRef,
...
function trustedCatalog(
  ...
  requirements = providers.map((item) => policyRequirements(item.providerRef)),
  profiles = providers.map((item) => policyProfile(item.providerRef)),
) {
...
function route(
  input: unknown,
  providers: readonly ProviderCapabilityProfile[] = [provider()],
  models: readonly ModelCapabilityProfile[] = [model()],
) {
...
test("provider-policy must be evaluated per provider and revision", () => {
  ...
  const mismatchCatalog = trustedCatalog(
    [provider()],
    [model()],
    [{ ...mismatched, requiredContractPolicyRevision: "provider-policy-revision:2026-08-r2" }],
  );
...
test("preplans one primary and one separately costed failover", () => {
  ...
  if (decision.status !== "ROUTE_SELECTED") return;
  assert.deepEqual(
    decision.routeAttemptPlan.attempts.map((attempt) => [attempt.role, attempt.reservedCostMicros]),
    [["primary", "1200"], ["failover", "1700"]],
  );
...
test("catalog and alias drift cannot rewrite an existing plan", () => {
  ...
  if (first.status !== "ROUTE_SELECTED") return;

  (mutableModel as { modelRevisionRef: string }).modelRevisionRef = "model-revision:alpha-2026-08-r2";
  (mutableModel as { worstCaseCostMicros: string }).worstCaseCostMicros = "9999";
  ...
  if (revised.status !== "ROUTE_SELECTED") return;
...
test("supports exact integer micros beyond Number.MAX_SAFE_INTEGER", () => {
  ...
  const decision = route(
    request({ costCeilingMicros: value }),
    [provider()],
    [model("model-profile:alpha", { worstCaseCostMicros: value })],
  );
  assert.equal(decision.status, "ROUTE_SELECTED");
...
test("rejects duplicate physical attempt references", () => {
  ...
  assertNoRoute(
    route(request({ physicalAttemptRefs: [duplicate, duplicate] })),
