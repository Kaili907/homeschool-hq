...
} as const satisfies Record<(typeof MULTIMODAL_MODES)[number], MultimodalContent>;

function presentationFor(mode: (typeof MULTIMODAL_MODES)[number]): MultimodalPresentation {
  return {
...
test("keeps raw audio and learner image descriptors transient and inference-free", () => {
  ...
  for (const field of [
    ...
    const invalid = clone(audio) as unknown as Record<string, unknown>;
...
test("disables transcript persistence structurally", () => {
  const invalid = clone(presentationFor("speech")) as unknown as Record<string, unknown>;
  const content = invalid.content as Record<string, unknown>;
  const transcript = content.transcript as Record<string, unknown>;
...
test("admits learner images only through a transient review request and approved ref", () => {
  ...
  const unreviewed = clone(presentationFor("reviewed-image")) as unknown as Record<string, unknown>;
  const visual = (unreviewed.content as Record<string, unknown>).visual as Record<string, unknown>;
...
test("makes media failure nonblocking with a captioned continuation fallback", () => {
  ...
  const blocking = clone(failure) as unknown as Record<string, unknown>;
  (blocking.fallback as Record<string, unknown>).blocking = true;
...
test("minimization projection cannot carry raw media, transcript, caption, or content", () => {
  ...
  if (speech.content.mode !== "speech") return;
...
test("durable evidence schema rejects raw fields and raw-like ref smuggling", () => {
  ...
  const unreviewedPresentation = clone(source.presentation) as unknown as Record<
    string,
    unknown
  >;
  const unreviewedContent = unreviewedPresentation.content as Record<string, unknown>;
  const unreviewedVisual = unreviewedContent.visual as Record<string, unknown>;
  unreviewedVisual.reviewStatus = "pending";
  const unreviewedProjection = projectDurableMultimodalEvidence({
    ...source,
    presentation: unreviewedPresentation as MultimodalPresentation,
  });
