...
function fallbackItem(scopeRef = "scope:lesson-a"): GroundedContextItem {
  return {
...
function bundle(
  item: GroundedContextItem = contextItem(),
...
function issueCodes(
  decision: ReturnType<typeof evaluateGrounding>,
): readonly string[] {
  return decision.assessment.issues.map((issue) => issue.code);
}

function assertNormativeRefusal(
  decision: ReturnType<typeof evaluateGrounding>,
): asserts decision is Extract<
  ReturnType<typeof evaluateGrounding>,
  { status: "refused" }
> {
...
for (const shape of ["math", "language", "science", "humanities"] as const) {
  test(`uses the same opaque grounding rule for ${shape}-shaped context`, () => {
    const item = contextItem(
      `content:${shape}-unit-a`,
      `scope:${shape}-unit-a`,
      DIGEST_A,
    );
    const decision = evaluateGrounding(
      bundle(item),
      [requirement(item, `claim:${shape}-material-a`)],
      [claim(item, `claim:${shape}-material-a`)],
    );
...
for (const validity of ["stale", "invalid"] as const) {
...
test("refuses context that Study has not reviewed", () => {
  const item: GroundedContextItem = {
    ...contextItem(),
...
test("numeric model confidence is outside the closed claim contract", () => {
  const item = contextItem();
  assert.equal(
    validateGroundedClaims([{ ...claim(item), confidence: 0.999 }]),
    false,
  );
});
...
for (const mutation of [
  ...
  { materialKind: "instructional" as const },
...
test("uses partial only when at least one material claim is grounded", () => {
  ...
  const decision = evaluateGrounding(
    inputBundle,
    [requirement(first), secondRequirement],
    [claim(first), claim(second, "claim:material-b")],
  );
  assertNormativeRefusal(decision);
