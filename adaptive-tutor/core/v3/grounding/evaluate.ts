import {
  GroundedClaimsSchema,
  GroundedContextBundleSchema,
...
function sortedIssues(issues: readonly GroundingIssue[]): readonly GroundingIssue[] {
  const unique = new Map<string, GroundingIssue>();
...
function resolveContext(
  ...
  contextsByRef: ReadonlyMap<string, readonly GroundedContextItem[]>,
): readonly GroundingIssue[] {
  ...
  if (!item) return [{ code: "unknown-content-ref", ref: contextRef }];

  const issues: GroundingIssue[] = [];
  if (
    requirement.scopeRef !== bundle.scopeRef ||
...
function fallbackFor(
  bundle: GroundedContextBundle,
  contextsByRef: ReadonlyMap<string, readonly GroundedContextItem[]>,
): ReviewedStaticFallback | null {
  const fallbackRef = bundle.fallbackContextRef;
  if (fallbackRef === null) return null;
...
function refusal(
  ...
  fallback: ReviewedStaticFallback | null,
): RefusalDecision {
  return {
    status: "refused",
...
function evaluateValidated(
  bundle: GroundedContextBundle,
  requirements: readonly GroundingRequirement[],
  claims: readonly GroundedClaim[],
): GroundingDecision {
  const issues: GroundingIssue[] = [];
  const groundedClaimRefs: string[] = [];
  const unsupportedClaimRefs: string[] = [];
  ...
  for (const requirement of requirements) {
    ...
    const claimIssues: GroundingIssue[] = [];
...
export function evaluateGrounding(
  ...
  let bundleResult: ReturnType<
    typeof validateExact<typeof GroundedContextBundleSchema>
  >;
  try {
  ...
  const contextsByRef = indexByRef(
    bundleResult.value.items,
    (item) => item.contextRef,
  );

  let requirementResult: ReturnType<
    typeof validateExact<typeof GroundingRequirementsSchema>
  >;
  try {
    requirementResult = validateExact(
      GroundingRequirementsSchema,
      requirements,
    );
  } catch {
    return refusal(
      malformedAssessment("malformed-grounding-requirements"),
      fallbackFor(bundleResult.value, contextsByRef),
    );
  }
  ...
  let claimResult: ReturnType<
    typeof validateExact<typeof GroundedClaimsSchema>
  >;
  try {
    claimResult = validateExact(GroundedClaimsSchema, claims);
  } catch {
    return refusal(
      malformedAssessment("malformed-grounding-claims"),
      fallbackFor(bundleResult.value, contextsByRef),
    );
  }
