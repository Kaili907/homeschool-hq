...
import {
  InstructionalMemoryContentSchema,
  InstructionalMemoryScopeSchema,
  StudyApprovedInstructionalStateSchema,
  type InstructionalMemoryContent,
  type InstructionalMemoryScope,
  type StudyApprovedInstructionalState,
} from "./bounded-instructional-memory.js";
...
export type InstructionalMemoryDeltaOperation = Static<
  typeof InstructionalMemoryDeltaOperationSchema
>;
...
export type InstructionalMemoryDelta = Static<typeof InstructionalMemoryDeltaSchema>;
...
export type InstructionalMemoryProjection = Static<typeof InstructionalMemoryProjectionSchema>;
...
export type InstructionalMemoryDeltaFailureCode =
  ...
  | "CONFLICTING_MEMORY_DELTA"
  | "RESULT_MISMATCH";

export type ApplyInstructionalMemoryDeltaResult =
  | {
  ...
  | {
      ...
      readonly code: InstructionalMemoryDeltaFailureCode;
...
export interface CreateInstructionalMemoryDeltaInput {
  ...
  readonly prior: InstructionalMemoryProjection | null;
  readonly operations: readonly InstructionalMemoryDeltaOperation[];
}
...
function memoryDigest(
  memoryRef: string,
  scope: InstructionalMemoryScope,
  instructionalState: InstructionalMemoryContent,
): string {
...
function validContent(content: InstructionalMemoryContent): boolean {
  return (
    validateExact(InstructionalMemoryContentSchema, content).status === "accepted" &&
...
function isArrayField(
  field: string,
): field is "conceptRefs" | "reviewedContentRefs" | "recentActionReasonCodes" {
  return (
    field === "conceptRefs" ||
...
function validArrayValue(field: string, value: unknown): value is string {
...
function validReplaceValue(field: string, value: unknown): boolean {
...
function applyOperations(
  prior: InstructionalMemoryContent | null,
  operations: readonly InstructionalMemoryDeltaOperation[],
): InstructionalMemoryContent | null {
  const state: InstructionalMemoryContent = structuredClone(
    prior ?? {
      conceptRefs: [],
  ...
  for (const operation of operations) {
    if (operation.operationKind === "add" || operation.operationKind === "remove") {
      ...
      const values = state[operation.field] as string[];
      if (operation.operationKind === "add") {
        if (!values.includes(operation.value)) values.push(operation.value);
      } else {
    ...
    if (!validReplaceValue(operation.field, operation.value)) return null;
    if (isArrayField(operation.field)) {
      (state[operation.field] as string[]) = structuredClone(operation.value) as string[];
    } else if (operation.field === "lastAssistanceLevel") {
      state.lastAssistanceLevel = operation.value as InstructionalMemoryContent["lastAssistanceLevel"];
    } else if (operation.field === "lastHintLevel") {
      state.lastHintLevel = operation.value as InstructionalMemoryContent["lastHintLevel"];
    } else {
      state.studyApprovedState = structuredClone(
        operation.value,
      ) as StudyApprovedInstructionalState | null;
    }
  }
...
function deriveResult(
  input: Omit<CreateInstructionalMemoryDeltaInput, "prior"> & {
    ...
    readonly priorState: InstructionalMemoryContent | null;
  },
): { readonly state: InstructionalMemoryContent; readonly digest: string; readonly revision: string } | null {
...
export function createInstructionalMemoryDelta(
  input: CreateInstructionalMemoryDeltaInput,
  ...
  if (!result) throw new RangeError("memory delta operations do not produce valid bounded state");
...
function rejected(code: InstructionalMemoryDeltaFailureCode): ApplyInstructionalMemoryDeltaResult {
  return { status: "rejected", code, tutorMayUseMemory: false };
}

export class InstructionalMemoryProjectionStore {
  readonly #projections = new Map<string, InstructionalMemoryProjection>();
  readonly #logicalOperations = new Map<string, AppliedIdentity>();
  readonly #deltas = new Map<string, AppliedIdentity>();

  get projectionCount(): number {
    return this.#projections.size;
  }

  read(memoryRef: string, scope: InstructionalMemoryScope): InstructionalMemoryProjection | null {
    const projection = this.#projections.get(memoryRef);
    if (!projection || !scopesMatch(projection.scope, scope)) return null;
  ...
  apply(candidate: unknown): ApplyInstructionalMemoryDeltaResult {
    ...
    if (
      (prior?.revisionRef ?? null) !== delta.expectedPriorRevisionRef ||
    ...
    const projection: InstructionalMemoryProjection = {
