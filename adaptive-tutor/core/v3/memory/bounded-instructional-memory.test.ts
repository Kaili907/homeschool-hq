...
import {
  ...
  InstructionalMemoryEntrySchema,
  MAXIMUM_INSTRUCTIONAL_MEMORY_ENTRIES,
  MAXIMUM_INSTRUCTIONAL_MEMORY_TTL_MS,
  MAXIMUM_MEMORY_CONCEPT_REFERENCES,
  MAXIMUM_RECENT_ACTION_REASON_CODES,
  MAXIMUM_REVIEWED_CONTENT_REFERENCES,
...
function rememberRequest(
  memoryRef = "memory:instruction-a",
  scope: InstructionalMemoryScope = baseScope,
  instructionalState: object = baseInstructionalState,
) {
...
test("rejects every forbidden raw, sensitive, provider, and judgment field", () => {
  ...
  for (const [field, value] of forbiddenFields) {
    ...
    const result = store.remember(
      rememberRequest("memory:forbidden", baseScope, {
...
test("rejects official mastery and grade or working-level mutation claims", () => {
  for (const forbiddenState of [
    ...
    const store = new BoundedInstructionalMemoryStore();
    assert.equal(
      store.remember(rememberRequest("memory:authority", baseScope, forbiddenState)).status,
      "rejected",
    );
  }
});
...
test("fails closed for cross-child and every other scope mismatch", () => {
  ...
  for (const scope of mismatchedScopes) {
    ...
    assert.deepEqual(
      store.remember(rememberRequest("memory:instruction-a", scope)),
      {
        status: "rejected",
        code: "MEMORY_SCOPE_MISMATCH",
        tutorMayUseMemory: false,
...
test("enforces hard entry and structured-list bounds", () => {
  ...
  const tooManyConcepts = Array.from(
    { length: MAXIMUM_MEMORY_CONCEPT_REFERENCES + 1 },
    (_, index) => `concept:c-${index}`,
  );
  const tooManyReviewed = Array.from(
    { length: MAXIMUM_REVIEWED_CONTENT_REFERENCES + 1 },
    (_, index) => `content:r-${index}`,
  );
  const tooManyReasons = Array.from(
    { length: MAXIMUM_RECENT_ACTION_REASON_CODES + 1 },
  ...
  for (const instructionalState of [
    ...
    const store = new BoundedInstructionalMemoryStore();
    assert.deepEqual(
      store.remember(rememberRequest("memory:too-many", baseScope, instructionalState)),
      {
        status: "rejected",
        code: "INVALID_MEMORY_REQUEST",
        tutorMayUseMemory: false,
...
test("rejects duplicate references and reason codes", () => {
  for (const instructionalState of [
    ...
    const store = new BoundedInstructionalMemoryStore();
    assert.deepEqual(
      store.remember(rememberRequest("memory:duplicate", baseScope, instructionalState)),
      {
        status: "rejected",
...
test("evicts the least-recent successful write deterministically", () => {
  ...
  const scopeTwo = { ...baseScope, opportunityRef: "opportunity:practice-two" } as const;
  const scopeThree = { ...baseScope, opportunityRef: "opportunity:practice-three" } as const;
  ...
  if (inserted.status === "accepted") assert.equal(inserted.evictionOccurred, true);
...
test("opportunity expiration removes only the exact learner/session/context/opportunity scope", () => {
  ...
  assert.equal(
    store.remember(rememberRequest("memory:b-one", siblingOpportunity)).status,
    "accepted",
  );
  ...
  assert.equal(
    store.read(accessRequest("memory:b-one", siblingOpportunity)).status,
    "accepted",
  );
});
...
test("clear and scope reset remove memory without touching adjacent scopes", () => {
  const store = new BoundedInstructionalMemoryStore();
  const adjacentScope = { ...baseScope, contextRef: "context:adjacent" } as const;
  ...
  assert.equal(
    store.remember(rememberRequest("memory:adjacent", adjacentScope)).status,
    "accepted",
  );
...
test("produces a minimized non-persistable snapshot and defensive copies", () => {
  ...
  if (result.status !== "snapshotted") return;
  assert.equal(
    validateExact(MinimizedInstructionalMemorySnapshotSchema, result.snapshot).status,
    "accepted",
  );
