...
type OutputHazard = "authority" | "active-assessment-answer";

function normalizedFieldName(field: string): string {
  return field.toLowerCase().replace(/[^a-z0-9]/g, "");
}

function fieldClaimsAuthority(field: string): boolean {
...
function fieldCanCarryFreeFormAnswer(field: string): boolean {
  const normalized = normalizedFieldName(field);
 ...
 */
function detectOutputHazard(
  root: unknown,
  activeAssessment: boolean,
): OutputHazard | undefined {
  const ancestors = new Set<object>();
  let visited = 0;
  let hazard: OutputHazard | undefined;

  const visit = (value: unknown, owningField: string | undefined, depth: number): void => {
    if (hazard || visited >= 10_000 || depth > 24) return;
    visited += 1;
    ...
    for (const [field, descriptor] of Object.entries(descriptors)) {
      ...
      if (descriptor.get || descriptor.set || !("value" in descriptor)) continue;
 ...
 */
function snapshotJsonBoundary(root: unknown): JsonBoundarySnapshot {
  const ancestors = new Set<object>();
  let visited = 0;

  const visit = (value: unknown, depth: number): JsonBoundarySnapshot => {
    visited += 1;
    if (visited > 10_000 || depth > 24) return { status: "rejected" };
    ...
    if (ownKeys.some((key) => typeof key === "symbol")) return { status: "rejected" };
...
function malformedResult(): NormalizedModelOutputResult {
  return {
    status: "malformed",
...
function staticFallback(reasonCode: StaticFallbackReasonCode): NormalizedModelOutputResult {
  return {
...
function normalizedProposal(
  proposal: ProviderModelProposalEnvelope,
): NormalizedModelProposal {
...
function normalizedRefusal(
  refusal: ProviderModelRefusalEnvelope,
): NormalizedModelRefusal {
  return Object.freeze({
 ...
 */
export function validateProviderModelOutput(
  output: unknown,
  context: ModelOutputValidationContext,
): NormalizedModelOutputResult {
  try {
    const hazard = detectOutputHazard(
      output,
      context.assessmentPhase === "active-graded-or-mastery-check",
    );
    if (hazard === "authority") {
    ...
    if (snapshot.status === "rejected") return malformedResult();

    const validated = validateExact(ProviderModelOutputEnvelopeSchema, snapshot.value);
    if (validated.status === "rejected") return malformedResult();
