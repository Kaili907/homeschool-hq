...
import {
  APPROVED_LEARNER_STAGE_ROUTING_MAPPING,
  LEARNER_ROUTING_STAGE_CLASSES,
  LEARNER_STAGE_CATALOG_APPROVAL_REF,
  LEARNER_STAGE_CATALOG_BINDING_VERSION,
  LEARNER_STAGE_CATALOG_VERSION,
  LEARNER_STAGE_POLICY_REVISION_REF,
  SUPPORTED_LEARNER_STAGE_REFS,
  createCommercialLearnerStagePolicyCatalog,
  listApprovedLearnerStageCatalogProfiles,
  validateApprovedLearnerStageCatalogProfile,
  validateApprovedLearnerStageRoutingMapping,
  type SupportedLearnerStageRef,
  type TrustedStudyLearnerStageCatalogBinding,
} from "./index.js";

function bindingFor(
  learnerStageRef: string,
  overrides: Partial<TrustedStudyLearnerStageCatalogBinding> = {},
): TrustedStudyLearnerStageCatalogBinding {
  return {
...
test("every committed catalog entry validates with every required policy dimension", () => {
  ...
  for (const profile of profiles) {
    ...
    assert.ok(
      profile.adultReviewThreshold.maximumConsecutiveTutorTurnsWithoutResolution > 0,
    );
  }
...
test("unknown learner stage fails closed to reviewed static content", () => {
  const result = createCommercialLearnerStagePolicyCatalog().resolve(
    bindingFor("learner-stage:not-approved"),
  );
  assert.equal(result.status, "static-fallback");
  if (result.status !== "static-fallback") return;
...
test("catalog and policy revision mismatches fail closed", () => {
  const catalog = createCommercialLearnerStagePolicyCatalog();
  const catalogMismatch = catalog.resolve(
    bindingFor(SUPPORTED_LEARNER_STAGE_REFS[0], {
      catalogVersion: "study-tutor-v2.learner-stage-catalog.v0",
    }),
  );
  const revisionMismatch = catalog.resolve(
    bindingFor(SUPPORTED_LEARNER_STAGE_REFS[0], {
...
test("reviewed learner-stage routing mapping is complete, unique, and deterministic", () => {
  assert.equal(
    APPROVED_LEARNER_STAGE_ROUTING_MAPPING.length,
    SUPPORTED_LEARNER_STAGE_REFS.length,
  );
  assert.equal(
  ...
  assert.deepEqual(
    APPROVED_LEARNER_STAGE_ROUTING_MAPPING.map((entry) => entry.learnerStageRef),
    SUPPORTED_LEARNER_STAGE_REFS,
  );
  assert.deepEqual(
    APPROVED_LEARNER_STAGE_ROUTING_MAPPING.map((entry) => entry.routingStageClass),
    LEARNER_ROUTING_STAGE_CLASSES,
  );
  assert.equal(
    new Set(APPROVED_LEARNER_STAGE_ROUTING_MAPPING.map((entry) => entry.learnerStageRef))
      .size,
    SUPPORTED_LEARNER_STAGE_REFS.length,
  );
...
test("commercial evaluation uses committed W3-11 bounds", () => {
  const result = createCommercialLearnerStagePolicyCatalog().evaluate(
    bindingFor("learner-stage:early-elementary" satisfies SupportedLearnerStageRef),
    {
