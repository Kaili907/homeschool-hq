...
function commandMarkup(command) {
    ...
    if (command.kind === "draw-fraction") {
        const cells = Array.from({ length: command.denominator }, (_, index) => `<span class="fraction-cell ${index < command.numerator ? "fraction-shaded" : ""}"></span>`).join("");
...
function applyBoardCommands(commands) {
    if (commands.some((command) => command.kind === "clear-board"))
        boardCommands = [];
...
function renderAssessment(item) {
    ...
    if (item.kind === "multiple-choice") {
        return `<div class="answer-grid">${item.options
            .map((option) => `<button class="answer-button" data-option-id="${escapeHtml(option.id)}" data-option-text="${escapeHtml(option.text)}">${escapeHtml(option.text)}</button>`)
            .join("")}</div>`;
    }
...
function render() {
    ...
    const phaseIndex = response
        ? Math.max(0, ["assessment", "identify-missing-concept", "teach-visually", "guided-practice", "independent-attempt", "reassess", "advance", "reteach", "escalated"].indexOf(response.phase))
        : 0;
...
function speak(text, pace) {
    ...
    if (!voiceEnabled) {
        if (status)
            status.textContent = "Voice disabled. Read the visible caption or transcript.";
        return;
    }
    if (!("speechSynthesis" in window) || typeof SpeechSynthesisUtterance === "undefined") {
        if (status)
            status.textContent = "Browser voice unavailable. The visible caption and transcript are the full fallback.";
        return;
    }
    try {
        ...
        utterance.onstart = () => {
            ...
            if (status)
                status.textContent = "Jarvis is speaking. The same words remain visible as captions.";
        };
        utterance.onend = () => {
            orb?.setAttribute("data-speaking", "false");
            if (status)
                status.textContent = "Voice complete. Captions and transcript remain available.";
        };
        utterance.onerror = () => {
            orb?.setAttribute("data-speaking", "false");
            if (status)
                status.textContent = "Voice failed. The visible caption and transcript are the full fallback.";
        };
...
function resetEngine(programKey) {
...
function bindEvents() {
    document.querySelector("#program-select")?.addEventListener("change", (event) => {
        const value = event.target.value;
        resetEngine(value);
    });
    document.querySelector("#restart-button")?.addEventListener("click", () => resetEngine(selectedProgram));
    document.querySelector("#start-button")?.addEventListener("click", () => setResponse(engine.start()));
    document.querySelector("#continue-button")?.addEventListener("click", () => setResponse(engine.continue()));
    document.querySelector("#alternate-button")?.addEventListener("click", () => setResponse(engine.requestAlternateExplanation()));
    ...
    document.querySelector("#short-answer-form")?.addEventListener("submit", (event) => {
