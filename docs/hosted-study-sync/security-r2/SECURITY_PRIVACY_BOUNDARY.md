...
- `BrowserLearnerResponseStore` persists the full device record in IndexedDB. Family cross-device data R1 synchronizes only a bounded instructional-input projection: attempt/section/item/segment identity, the learner's selected choice reference or entered text, pending/assessed state, and a minimized trusted receipt. Published prompts/choices, answer authority, Tutor state, audio and inference remain excluded. See `docs/family-pilot/FAMILY_CROSS_DEVICE_DATA_COMPLETENESS_R1.md`.
- Parent Download Backup remains a separate emergency-recovery artifact and its current schema does not embed the IndexedDB learner-response store. Hosted hydrate does not alter or obsolete a downloaded backup.
