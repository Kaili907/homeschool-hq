...
- `FAMILY_PILOT_LEARNER_RESPONSES_KEY` separately persists raw choice/text/numeric/constructed/activity responses and assessment receipts in localStorage. Raw response values remain device-local and are excluded from portable backup and Hosted Sync. The later Family Services convergence may project only an exact minimized trusted receipt into the already-allowlisted `assessmentStates[].outcome` fields; it never synchronizes the surrounding response record.
