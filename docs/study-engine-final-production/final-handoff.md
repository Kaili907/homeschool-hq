...
- Manifest: exact nine-file order, dependencies, markers, unique names/versions, classifications, checksums, and supersession validate locally.
- Hosted baseline: record the three equivalent historical versions through a future supported history procedure only after read-only equivalence confirmation and explicit authorization; never replay their SQL.
- Hosted re-preflight: **HOSTED RE-PREFLIGHT NOT PERFORMED — VERIFIED ACCESS WAS NOT AVAILABLE**.
