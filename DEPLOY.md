...
## Controlled learner pilot branch (R2)

The Family Pilot learner route is enabled only for the Netlify custom branch
context `mac/learner-web-release-r2`. The global build environment does not set
`VITE_FAMILY_PILOT_ENABLED`, so production, Deploy Previews, and unrelated
branch deploys remain off. In Netlify, allow only this exact branch under
**Project configuration → Build & deploy → Continuous Deployment → Branches
and deploy contexts → Configure → Let me add individual branches**. Do not make
this branch the production branch and do not promote its deploy.

The `production-item-assessment` function explicitly bundles the admitted
release manifest/bindings plus its trusted production package and scoring files.
Those server-only authorities are outside `dist` and are never browser assets.
